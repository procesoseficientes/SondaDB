...
-- Ejemplo de Ejecucion:
				-- 
				EXEC [SWIFT_INTERFACES_QA].[diprocom].[BULK_DATA_SP_IMPORT_CUSTOMER]
*/
-- =============================================
CREATE PROCEDURE [DIPROCOM].[BULK_DATA_SP_IMPORT_CUSTOMER]
AS
BEGIN
  SET NOCOUNT ON;
  --
	TRUNCATE TABLE [SWIFT_INTERFACES_QA].[diprocom].[SWIFT_ERP_CUSTOMERS];

	MERGE [SWIFT_INTERFACES_QA].[diprocom].[SWIFT_ERP_CUSTOMERS] [TRG]
	USING
		(SELECT
				*
			FROM
				[SWIFT_INTERFACES_ONLINE_QA].[diprocom].[ERP_VIEW_COSTUMER])
		AS [SRC]
	ON [TRG].[CODE_CUSTOMER] = [SRC].[CODE_CUSTOMER]
	WHEN MATCHED THEN
		UPDATE SET
				[TRG].[CODE_CUSTOMER] = [SRC].[CODE_CUSTOMER]
				...
				,[TRG].[SPECIAL_MARKER] = [SRC].[SPECIAL_MARKER]
	WHEN NOT MATCHED THEN
		...
		VALUES	(
					[SRC].[CODE_CUSTOMER]
					,RTRIM(LTRIM([SRC].[NAME_CUSTOMER]))
					,[SRC].[PHONE_CUSTOMER]
					,[SRC].[ADRESS_CUSTOMER]
					,[SRC].[CLASSIFICATION_CUSTOMER]
					,[SRC].[CONTACT_CUSTOMER]
					,[SRC].[CODE_ROUTE]
					,[SRC].[LAST_UPDATE]
					,[SRC].[LAST_UPDATE_BY]
					,[SRC].[SELLER_DEFAULT_CODE]
					,[SRC].[CREDIT_LIMIT]
					,[SRC].[FROM_ERP]
					,[SRC].[NAME_ROUTE]
					...
					,ISNULL([SRC].[LATITUDE] + ','
							+ [SRC].[LONGITUDE], '0,0')
					,ISNULL([SRC].[LATITUDE], '0')
					,ISNULL([SRC].[LONGITUDE], '0')
					,ISNULL([SRC].[FREQUENCY], '1')
					,ISNULL([SRC].[SUNDAY], '0')
					,ISNULL([SRC].[MONDAY], '0')
					,ISNULL([SRC].[TUESDAY], '0')
					,ISNULL([SRC].[WEDNESDAY], '0')
					,ISNULL([SRC].[THURSDAY], '0')
					,ISNULL([SRC].[FRIDAY], '0')
					,ISNULL([SRC].[SATURDAY], '0')
					,[SRC].[SCOUTING_ROUTE]
					,[SRC].[EXTRA_MONT]
					,[SRC].[EXTRA_DAYS]
					,[SRC].[DISCOUNT]
