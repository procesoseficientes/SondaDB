...
-- Ejemplo de Ejecucion:
				-- 
				EXEC [SWIFT_INTERFACES_QA].[SONDA].[BULK_DATA_SP_IMPORT_WAREHOUSE]
*/
-- =============================================
CREATE PROCEDURE [SONDA].[BULK_DATA_SP_IMPORT_WAREHOUSE]
AS
BEGIN
	SET NOCOUNT ON;
	--
	MERGE [SWIFT_EXPRESS_QA].[SONDA].[SWIFT_WAREHOUSES] SWI 
	USING ( SELECT * FROM  [SWIFT_INTERFACES_ONLINE_QA].[SONDA].[ERP_VIEW_WAREHOUSE]) WVH 
	...
	VALUES (
	 ...
	 --Location
	 MERGE [SWIFT_EXPRESS_QA].[SONDA].[SWIFT_LOCATIONS] SWI
	USING (SELECT * FROM [SWIFT_INTERFACES_ONLINE_QA].[SONDA].[ERP_VIEW_WAREHOUSE]) WVH 
	ON SWI.[CODE_WAREHOUSE] = WVH.[CODE_WAREHOUSE] 
	WHEN MATCHED THEN 
	UPDATE 
