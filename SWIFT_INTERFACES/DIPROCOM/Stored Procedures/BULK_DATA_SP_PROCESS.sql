...
-- Ejemplo de Ejecucion:
				-- 
				EXEC [DIPROCOM].[BULK_DATA_SP_PROCESS]
				--
				SELECT TOP 25 * FROM DIPROCOM.[BULK_DATA_CONFIGURATION_LOG] ORDER BY 1 DESC
*/
-- =============================================
CREATE PROCEDURE [DIPROCOM].[BULK_DATA_SP_PROCESS]
AS
BEGIN
	SET NOCOUNT ON;
	--
	...
	SELECT 
		...
		,C.[ORDER]
	INTO #CONFIG
	FROM DIPROCOM.[BULK_DATA_CONFIGURATION] C
	WHERE C.ACTIVE = 1
	ORDER BY 
		C.[ORDER]
		,C.CONFIGURATION
	...
	BEGIN
		...
		SET @END_RUN = GETDATE()
		--
		EXEC DIPROCOM.[BULK_DATA_SP_INSERT_LOG]
			@SP = @SP
			,@START_RUN = @START_RUN
			,@END_RUN = @END_RUN
			,@MESSAGE = @MESSAGE
