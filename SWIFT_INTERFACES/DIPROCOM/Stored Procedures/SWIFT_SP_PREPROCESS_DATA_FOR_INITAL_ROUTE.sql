...
-- Ejemplo de Ejecucion:
			EXEC  DIPROCOM.[SWIFT_SP_PREPROCESS_DATA_FOR_INITAL_ROUTE]
*/
-- =============================================
CREATE PROCEDURE [DIPROCOM].[SWIFT_SP_PREPROCESS_DATA_FOR_INITAL_ROUTE]
AS
BEGIN
  SET NOCOUNT ON;
  --
  ...
  INSERT INTO @ROUTES ([CODE_ROUTE])
    SELECT DISTINCT
      [SR].[CODE_ROUTE]
    FROM [SWIFT_EXPRESS].[DIPROCOM].[SWIFT_ROUTES] [SR]
  ...
  BEGIN TRY
    ...
    EXEC [SWIFT_EXPRESS].[DIPROCOM].[SONDA_SP_GET_READY_TO_START_ROUTE] @CODE_ROUTE = @CODE_ROUTE
  ...
  BEGIN CATCH
    ...
    EXEC [SWIFT_EXPRESS].[DIPROCOM].[SONDA_SP_INSERT_SONDA_SERVER_ERROR_LOG] @CODE_ROUTE = @CODE_ROUTE
                                                         ,@LOGIN = 'Preprocess'
