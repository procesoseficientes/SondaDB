...
-- Ejemplo de Ejecucion:
				-- 
				EXEC [SWIFT_INTERFACES_QA].[DIPROCOM].[BULK_DATA_SP_IMPORT_PACK_CONVERSION]
*/
-- =============================================
CREATE PROCEDURE [DIPROCOM].[BULK_DATA_SP_IMPORT_PACK_CONVERSION]
AS
BEGIN
	SET NOCOUNT ON;
	--
	MERGE SWIFT_EXPRESS_QA.DIPROCOM.SONDA_PACK_UNIT AS TRG
	  USING (SELECT
			  epu.CODE_PACK_UNIT
			 ...
			 ,epu.LAST_UPDATE_BY
			 ,epu.UM_ENTRY
			FROM SWIFT_INTERFACES_ONLINE_QA.DIPROCOM.ERP_PACK_UNIT epu) AS SRC
	  ON TRG.CODE_PACK_UNIT COLLATE database_default = SRC.CODE_PACK_UNIT
	  WHEN MATCHED
		THEN UPDATE
		  SET DESCRIPTION_PACK_UNIT = SUBSTRING(SRC.DESCRIPTION_PACK_UNIT,1,25)
	  ...
	  WHEN NOT MATCHED
		THEN INSERT (CODE_PACK_UNIT
			  ...
			  , LAST_UPDATE_BY
			  , UM_ENTRY)
			VALUES (SRC.CODE_PACK_UNIT, SUBSTRING(SRC.DESCRIPTION_PACK_UNIT,1,25), SRC.LAST_UPDATE, SRC.LAST_UPDATE_BY, SRC.UM_ENTRY);
  --
  	TRUNCATE TABLE [SWIFT_EXPRESS_QA].[DIPROCOM].[SONDA_PACK_CONVERSION]

	MERGE SWIFT_EXPRESS_QA.DIPROCOM.SONDA_PACK_CONVERSION AS TRG
	USING (SELECT 
			epc.CODE_SKU
			...
			,epc.LAST_UPDATE_BY 
			,epc.[ORDER]
		FROM SWIFT_INTERFACES_ONLINE_QA.DIPROCOM.ERP_PACK_CONVERSION epc) AS SRC 
	ON 
		TRG.CODE_SKU  COLLATE database_default = SRC.CODE_SKU 
