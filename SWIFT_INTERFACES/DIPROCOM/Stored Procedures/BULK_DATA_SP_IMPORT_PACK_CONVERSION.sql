...
-- Ejemplo de Ejecucion:
				-- 
				EXEC [SWIFT_INTERFACES_QA].[SONDA].[BULK_DATA_SP_IMPORT_PACK_CONVERSION]
*/
-- =============================================
...
BEGIN
	SET NOCOUNT ON;
	--
	MERGE SWIFT_EXPRESS_QA.[SONDA].SONDA_PACK_UNIT AS TRG
	  USING (SELECT
			  epu.CODE_PACK_UNIT
			 ...
			 ,epu.LAST_UPDATE_BY
			 ,epu.UM_ENTRY
			FROM SWIFT_INTERFACES_ONLINE_QA.[SONDA].ERP_PACK_UNIT epu) AS SRC
  ...
  --
  	TRUNCATE TABLE [SWIFT_EXPRESS_QA].[SONDA].[SONDA_PACK_CONVERSION]

	MERGE SWIFT_EXPRESS_QA.[SONDA].SONDA_PACK_CONVERSION AS TRG
	USING (SELECT 
			epc.CODE_SKU
			...
			,epc.LAST_UPDATE_BY 
			,epc.[ORDER]
		FROM SWIFT_INTERFACES_ONLINE_QA.[SONDA].ERP_PACK_CONVERSION epc) AS SRC 
	ON 
