...
-- Ejemplo de Ejecucion:
				-- 
				EXEC [diprocom].[BULK_DATA_SP_IMPORT_RESOLUTION]
*/
-- =============================================
CREATE PROCEDURE [diprocom].[BULK_DATA_SP_IMPORT_RESOLUTION]
AS
BEGIN
  SET NOCOUNT ON;
  --	
	MERGE [SWIFT_EXPRESS].[diprocom].[SONDA_POS_RES_SAT] [TRG]
	USING
		(SELECT [R].*, [U].[SELLER_ROUTE] 
			FROM [SWIFT_INTERFACES_ONLINE].[diprocom].[ERP_VIEW_RESOLUTION] R
			INNER JOIN [SWIFT_EXPRESS].[DIPROCOM].[USERS] U ON [R].[Vendedor_Asignado] = [U].[RELATED_SELLER]) AS [SRC]
