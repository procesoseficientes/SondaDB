...
/*
--DROP PROCEDURE [SONDA].[BULK_DATA_SP_IMPORT_ROUTE_FRECUENCY]
-- Ejemplo de Ejecucion:
				-- 
				EXEC [SONDA].[BULK_DATA_SP_IMPORT_ROUTE_FRECUENCY]
				SELECT * FROM SWIFT_EXPRESS.[SONDA].SWIFT_FREQUENCY_X_CUSTOMER
				SELECT * FROM SWIFT_EXPRESS.[SONDA].SWIFT_FREQUENCY
				SELECT * FROM SWIFT_EXPRESS.[SONDA].SWIFT_TASKS
*/
-- =============================================
CREATE PROCEDURE [SONDA].[BULK_DATA_SP_IMPORT_ROUTE_FRECUENCY]
AS
BEGIN
  SET NOCOUNT ON;
  --
  ...
  TRUNCATE TABLE SWIFT_EXPRESS.[SONDA].SWIFT_FREQUENCY_X_CUSTOMER 
  	
  DELETE FROM SWIFT_EXPRESS.[SONDA].SWIFT_FREQUENCY 
	WHERE REFERENCE_SOURCE = 'BULK_DATA'

	INSERT INTO SWIFT_EXPRESS.[SONDA].[SWIFT_FREQUENCY]
	        ( [CODE_FREQUENCY] ,
	          [SUNDAY] ,
	          [MONDAY] ,
	          [TUESDAY] ,
	...
	INSERT INTO SWIFT_EXPRESS.[SONDA].SWIFT_FREQUENCY_X_CUSTOMER 
	(
		[ID_FREQUENCY]
		,[CODE_CUSTOMER]
		,[PRIORITY]	
	...
	SELECT 
		...
		,[C].[CODE_CUSTOMER]
		,1 AS [PRIORITY]
	 FROM [SWIFT_INTERFACES_ONLINE].[SONDA].[ERP_VIEW_COSTUMER] C
		INNER JOIN SWIFT_EXPRESS.[SONDA].SWIFT_FREQUENCY F ON C.[SELLER_DEFAULT_CODE] = F.CODE_ROUTE
