...
-- Ejemplo de Ejecucion:
			EXEC [SWIFT_INTERFACES_QA].[DIPROCOM].[BULK_DATA_SP_GENERATE_PRICE_LIST_FOR_ROUTE]
*/
-- =============================================
CREATE PROCEDURE [DIPROCOM].[BULK_DATA_SP_GENERATE_PRICE_LIST_FOR_ROUTE]
AS
BEGIN
  SET NOCOUNT ON;
  --
  ...
  IF EXISTS (SELECT
        *
      FROM [SWIFT_EXPRESS_QA].[DIPROCOM].[SWIFT_PARAMETER] [SP]
  ...
  BEGIN
    ...
    -- ------------------------------------------------------------------------------------
    TRUNCATE TABLE [SWIFT_EXPRESS_QA].[DIPROCOM].[SWIFT_PRICE_LIST_BY_CUSTOMER_FOR_ROUTE]
    ...
    INSERT INTO @ROUTES ([CODE_ROUTE])
      SELECT DISTINCT
        [SR].[CODE_ROUTE]
      FROM [SWIFT_EXPRESS_QA].[DIPROCOM].[SWIFT_ROUTES] [SR]
      INNER JOIN [SWIFT_EXPRESS_QA].[DIPROCOM].[USERS] [U]
    ...
    BEGIN
      ...
      EXEC [SWIFT_EXPRESS_QA].[DIPROCOM].[SWIFT_SP_GENERATE_PRICE_LIST_FOR_ROUTE] @CODE_ROUTE = @CODE_ROUTE
