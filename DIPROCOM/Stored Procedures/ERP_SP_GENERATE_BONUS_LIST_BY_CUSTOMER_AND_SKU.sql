...
-- Ejemplo de Ejecucion:
				EXEC [DIPROCOM].[ERP_SP_GENERATE_BONUS_LIST_BY_CUSTOMER_AND_SKU]
					@BONUS_TYPE = 3
					,@ORDER = 1
*/
-- =============================================
CREATE PROCEDURE [DIPROCOM].[ERP_SP_GENERATE_BONUS_LIST_BY_CUSTOMER_AND_SKU](
	@BONUS_TYPE INT
	,@ORDER INT
)
AS
BEGIN
	...
	IF @ORDER = 1
	BEGIN
		TRUNCATE TABLE [DIPROCOM].[ERP_TB_BONUS]
		--
		PRINT '----> Limpio tabla'
	END
	...
	BEGIN
		...
		-- ------------------------------------------------------------------------------------
		-- Obtiene las bonificaciones por cliente y sku
		-- ------------------------------------------------------------------------------------
		INSERT INTO #BONUS
		SELECT DISTINCT
			DM.Z_RUTA
		...
		LEFT JOIN MDA.ERP_TB_BONUS B ON (
		...
		ORDER BY
			...
			,DM.MATNR
		--
		PRINT 'Obtubo bonificacion por cliente y sku'
