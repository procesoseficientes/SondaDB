...
-- Ejemplo de Ejecucion:
		...
		EXEC [acsa].[SWIFT_SP_VALIDATE_IF_ROUTE_GENERATE_SPECIAL_PRICES_BY_SCALE]
		@CODE_ROUTE = '136',
...
*/
-- ==============================================================================================================
CREATE PROCEDURE [acsa].[SWIFT_SP_VALIDATE_IF_ROUTE_GENERATE_SPECIAL_PRICES_BY_SCALE]
	(
...
AS
  ...
  --
		...
		FROM
			[acsa].[SWIFT_TRADE_AGREEMENT_WATERMARK] [TAW]
		WHERE
		...
		FROM
			[acsa].[SWIFT_PROMO_SPECIAL_PRICE_LIST_BY_SCALE] [DBS]
		WHERE
		...
		IF NOT EXISTS ( SELECT
							[WATER_MARK]
						FROM
							[acsa].[SWIFT_TRADE_AGREEMENT_WATERMARK]
						WHERE
							[CODE_ROUTE] = @CODE_ROUTE )
		BEGIN

			INSERT	INTO [acsa].[SWIFT_TRADE_AGREEMENT_WATERMARK]
					(
						[WATER_MARK]
						,[CODE_ROUTE]
						,[TIMES_REQUIRE]
