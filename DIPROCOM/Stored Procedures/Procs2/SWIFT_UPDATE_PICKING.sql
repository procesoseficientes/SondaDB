...
CREATE PROCEDURE [DIPROCOM].[SWIFT_UPDATE_PICKING]
@OBSERVATION VARCHAR(MAX),
@PICKING_HEADER INT
AS
UPDATE DIPROCOM.SWIFT_PICKING_HEADER SET COMMENTS=@OBSERVATION, STATUS='CLOSED', FF_STATUS='CLOSED', [IS_POSTED_ERP] = 0
WHERE PICKING_HEADER = @PICKING_HEADER AND (IS_POSTED_ERP <> 1 OR [IS_POSTED_ERP] IS NULL)

UPDATE DIPROCOM.SWIFT_PICKING_HEADER SET FF_STATUS='CLOSED'
WHERE PICKING_HEADER = @PICKING_HEADER AND FF = '1'

UPDATE DIPROCOM.SWIFT_TASKS SET ACTION='CLOSED'
WHERE PICKING_NUMBER=@PICKING_HEADER

--Elimnia las facturas draft que pudo haber generado
EXEC DIPROCOM.SWIFT_SP_DELETE_INVOICE_DRAFT @PICKING_HEADER
