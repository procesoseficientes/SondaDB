...
*/
-- =============================================
CREATE PROCEDURE [acsa].SWIFT_SP_MOVE_TAGS_BY_ACCEPTED_SCOUTING @CODE_CUSTOMER VARCHAR(50)
AS
BEGIN TRY

  DELETE [acsa].SWIFT_TAG_X_CUSTOMER
    WHERE CUSTOMER = @CODE_CUSTOMER;

  INSERT INTO [acsa].SWIFT_TAG_X_CUSTOMER (TAG_COLOR, CUSTOMER)
    SELECT
      stxcn.TAG_COLOR
     ,stxcn.CUSTOMER
    FROM [acsa].[SWIFT_VIEW_ALL_TAG_X_CUSTOMER_NEW] stxcn
