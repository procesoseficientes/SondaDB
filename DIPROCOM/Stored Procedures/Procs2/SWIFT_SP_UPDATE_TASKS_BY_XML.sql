...
-- Ejemplo de Ejecucion:
				EXEC [DIPROCOM].[SWIFT_SP_UPDATE_TASKS_BY_XML]
				@XML = '
...
<Data>
    ...
    <routeid>46</routeid>
    <loginId>hector@DIPROCOM</loginId>
</Data>'

*/
-- =============================================
CREATE PROCEDURE [DIPROCOM].[SWIFT_SP_UPDATE_TASKS_BY_XML] (@XML XML)
AS
BEGIN
  BEGIN TRY
    DECLARE @ID INT
    ...
    -- ----------------------------------------------------------------------------------------------------------
	----SI LA TAREA NO TIENE UN GPS OBTENEMOS EL ÚLTIMO GPS REGISTRADO EN LAS FACTURAS, SI NO SE HA CREADO AUN UNA FACTURA CON UN GPS VÁLIDO OBTENEMOS EL GPS DE LA VISTA DE COSTUMERS
	--IF NOT EXISTS(SELECT TOP 1 1 FROM @TASKS WHERE POSTED_GPS IS NOT NULL AND POSTED_GPS <> '0,0')
	--BEGIN
	--	SELECT @ALTERNATIVEGPS = (
	--		SELECT TOP 1 CASE PIH.GPS_URL
	--						WHEN '0,0' THEN VAC.GPS
	--						WHEN NULL THEN VAC.GPS
	--						ELSE PIH.GPS_URL
	--						END
	--					FROM DIPROCOM.SONDA_POS_INVOICE_HEADER PIH
	--		LEFT JOIN DIPROCOM.SWIFT_VIEW_ALL_COSTUMER VAC ON PIH.CLIENT_ID = VAC.CODE_CUSTOMER
	--		WHERE VAC.CODE_CUSTOMER = ( SELECT TOP 1 CUSTOMER_CODE FROM @TASKS) AND PIH.GPS_URL IS NOT NULL AND PIH.GPS_URL <> '0,0' ORDER BY PIH.INVOICE_ID DESC
	--	)
	--END
    ...
    BEGIN
	  ...
	  SELECT @ALTERNATIVEGPS = (
		SELECT TOP 1 PIH.GPS_URL FROM DIPROCOM.SONDA_POS_INVOICE_HEADER PIH 
			INNER JOIN @TASKS T 
				ON PIH.CLIENT_ID = T.CUSTOMER_CODE 
			WHERE PIH.CLIENT_ID = T.CUSTOMER_CODE 
				AND PIH.GPS_URL IS NOT NULL 
				AND PIH.GPS_URL <> '0,0' 
				ORDER BY PIH.INVOICE_ID DESC
	  )

	  IF (@ALTERNATIVEGPS IS NULL)
	  BEGIN
		SELECT @ALTERNATIVEGPS = (
				SELECT TOP 1 VAC.GPS FROM DIPROCOM.SWIFT_VIEW_ALL_COSTUMER VAC INNER JOIN @TASKS T ON VAC.CODE_CUSTOMER = T.CUSTOMER_CODE WHERE VAC.CODE_CUSTOMER = T.CUSTOMER_CODE
		)
	  END
	  ---------------------------------------------------------------------------------------------------------------------------------------------------------------
      ...
      SELECT TOP 1
       ...
       ,@POSTED_GPS = CASE [T].[POSTED_GPS]
							WHEN '0,0' THEN @ALTERNATIVEGPS
							WHEN null THEN @ALTERNATIVEGPS
							ELSE [T].[POSTED_GPS]
							END
       ,@TASK_SEQ = [T].[TASK_SEQ]
    ...
    BEGIN CATCH
      ...
      EXEC [DIPROCOM].[SONDA_SP_INSERT_SONDA_SERVER_ERROR_LOG] @CODE_ROUTE = NULL
                                                           ,@LOGIN = NULL
