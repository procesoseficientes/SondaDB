...
-- Ejemplo de Ejecucion:
        ...
        GO
        --
        EXEC [DIPROCOM].[SWIFT_SP_VALIDATE_STOCK_INVENTORY_BY_WS_AND_SKUS_FOR_SALES_ORDERS]
			    @CODE_WAREHOUSE = 'BODEGA_CENTRAL'
			    ,@SALES_ORDERS = '831|833'
			    
*/
-- =============================================
CREATE PROCEDURE [DIPROCOM].[SWIFT_SP_VALIDATE_STOCK_INVENTORY_BY_WS_AND_SKUS_FOR_SALES_ORDERS]
...
BEGIN
	...
	SELECT *
	INTO #RESERVED
	FROM [DIPROCOM].[SWIFT_FN_GET_INVENTORY_RESERVED](@CODE_WAREHOUSE) R	
	...
	SELECT
		...
		,S.VALUE SALES_ORDER
	INTO #SALES_ORDER
	FROM [DIPROCOM].[SWIFT_FN_SPLIT](@SALES_ORDERS,@DELIMITER) S
	...
	INTO #INVENTORY
	FROM #SALES_ORDER SO
	INNER JOIN [DIPROCOM].SONDA_SALES_ORDER_DETAIL SOD ON (
    SOD.SALES_ORDER_ID = SO.SALES_ORDER
  )
	INNER JOIN [DIPROCOM].[SWIFT_VIEW_ALL_SKU] VS ON (
    VS.CODE_SKU = SOD.SKU
  )
	LEFT JOIN [DIPROCOM].SWIFT_INVENTORY I ON (
		I.SKU = SOD.SKU
