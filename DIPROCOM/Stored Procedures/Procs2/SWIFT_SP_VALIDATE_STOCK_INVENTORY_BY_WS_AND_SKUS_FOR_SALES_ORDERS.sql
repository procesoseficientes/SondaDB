...
-- Ejemplo de Ejecucion:
        ...
        GO
        --
        EXEC [PACASA].[SWIFT_SP_VALIDATE_STOCK_INVENTORY_BY_WS_AND_SKUS_FOR_SALES_ORDERS]
			    @CODE_WAREHOUSE = 'BODEGA_CENTRAL'
			    ,@SALES_ORDERS = '831|833'
			    
*/
-- =============================================
CREATE PROCEDURE [PACASA].[SWIFT_SP_VALIDATE_STOCK_INVENTORY_BY_WS_AND_SKUS_FOR_SALES_ORDERS]
...
BEGIN
	...
	SELECT
		...
		,S.VALUE SALES_ORDER
	INTO #SALES_ORDER
	FROM [PACASA].[SWIFT_FN_SPLIT](@SALES_ORDERS,@DELIMITER) S
	...
	INTO #INVENTORY
	FROM #SALES_ORDER SO
	INNER JOIN [PACASA].SONDA_SALES_ORDER_DETAIL SOD ON (
