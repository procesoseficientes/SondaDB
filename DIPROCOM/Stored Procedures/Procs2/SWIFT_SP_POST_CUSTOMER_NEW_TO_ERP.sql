...
=========================================================*/
CREATE PROCEDURE [acsa].[SWIFT_SP_POST_CUSTOMER_NEW_TO_ERP]
AS
BEGIN
	...
	SELECT 
		...
		,[CODE_CUSTOMER]
	INTO #NEW_CUSTOMERS
	FROM [acsa].[SWIFT_VIEW_ALL_CUSTOMER_NEW]
	...
	BEGIN
		...
		FROM [acsa].[SWIFT_VIEW_ALL_CUSTOMER_NEW] [vacn]
		INNER JOIN [acsa].[SWIFT_VIEW_ALL_TAG_X_CUSTOMER_NEW] [tcn] ON [tcn].[CUSTOMER] = [vacn].[CODE_CUSTOMER]
		INNER JOIN [acsa].[SWIFT_TAGS] [tag] ON [tag].[TAG_COLOR] = [tcn].[TAG_COLOR] 
		...
		BEGIN TRY
			...
			INSERT INTO [DIPROCOM_SERVER].[acsa].[dbo].[REPLICA_CLIENTE]
					( [CODIGO_SONDA] ,
						[CATEGORIA_CLIENTE] ,
						[NOMBRE_CLIENTE] ,
						[NOMBRE_COMERCIAL] ,
			...
			SELECT
				[vacn].[CODE_CUSTOMER]
				,ISNULL(@CATEGORY,(SELECT TOP 1 [TAG_COMMENTS] FROM [acsa].[SWIFT_TAGS] 
				...
				,ISNULL(@GIRO,(SELECT TOP 1 [TAG_COMMENTS] FROM [acsa].[SWIFT_TAGS] 
				...
				,[vacn].[LONGITUDE]
			FROM [acsa].[SWIFT_VIEW_ALL_CUSTOMER_NEW] [vacn]
				INNER JOIN [acsa].[USERS] [us] ON [us].[LOGIN] = [vacn].[POSTED_BY]
			WHERE [vacn].[CUSTOMER] = @CUSTOMER
		...
		BEGIN CATCH
			...
			DELETE FROM [DIPROCOM_SERVER].[acsa].[dbo].[REPLICA_CLIENTE]
