...
-- Ejemplo de Ejecucion:
		...
		EXECUTE @RC = [DIPROCOM].SWIFT_SP_STATUS_SEND_CUSTOMER_TO_SAP 
			@CODE_CUSTOMER
			,@POSTED_RESPONSE
			,@CODE_CUSTOMER_BO
GO
*/
-- =============================================
CREATE PROCEDURE [DIPROCOM].[SWIFT_SP_STATUS_SEND_CUSTOMER_TO_SAP] (
	@CODE_CUSTOMER VARCHAR(50)
	,@POSTED_RESPONSE VARCHAR(150)
	,@CODE_CUSTOMER_BO VARCHAR(50)
) AS
BEGIN
	...
	SELECT @FROM = IS_FROM FROM [DIPROCOM].[SWIFT_VIEW_ALL_CUSTOMER_NEW] WHERE [CODE_CUSTOMER] = @CODE_CUSTOMER

	BEGIN TRAN [TransSend];
		BEGIN TRY
			EXECUTE @RC = [DIPROCOM].[SWIFT_SP_MOVE_TAGS_BY_ACCEPTED_SCOUTING] @CODE_CUSTOMER;

			IF(@FROM = 'SONDA_CORE')
			BEGIN
				EXECUTE @SP = [DIPROCOM].[SWIFT_SP_MOVE_FREQUENCY_BY_ACCEPTED_SCOUTING] @CODE_CUSTOMER;

				UPDATE [DIPROCOM].[SWIFT_CUSTOMERS_NEW]
				SET	
					[IS_POSTED_ERP] = 1
					,[POSTED_ERP] = GETDATE()
