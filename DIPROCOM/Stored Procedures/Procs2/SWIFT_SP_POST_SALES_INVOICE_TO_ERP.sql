...
-- EJEMPLO DE EJECUCION: 
		EXEC [PACASA].[SWIFT_SP_POST_SALES_INVOICE_TO_ERP]

=========================================================*/
CREATE PROCEDURE [PACASA].[SWIFT_SP_POST_SALES_INVOICE_TO_ERP]
AS
BEGIN
	SET NOCOUNT ON;
	...
	BEGIN
		...
		BEGIN TRY
			...
			INSERT INTO [DIPROCOM_SERVER].[PACASA].[dbo].[REPLICA_FACTURA_ENC]
			(	
				[SERIE] ,
				[NUMERO_FACTURA] ,
				[FECHA_DE_FACTURA] ,
			...
			SELECT 
				...
				NULL
			FROM [PACASA].SONDA_POS_INVOICE_HEADER SPIH
			INNER JOIN [PACASA].USERS US ON SPIH.POSTED_BY = US.LOGIN
			...
			INNER JOIN [PACASA].[SONDA_POS_INVOICE_HEADER] spih ON [spih].[ID] = [spid].[ID]
			LEFT JOIN [PACASA].[SWIFT_PRICE_LIST_BY_SKU] plbs 
				ON [plbs].[CODE_SKU] = [spid].[SKU] AND [plbs].[CODE_PRICE_LIST] = [spih].[CLIENT_ID]
			WHERE [spid].[ID]=@SALES_INVOICE_ID
				
			EXEC [PACASA].SWIFT_SP_MARK_INVOICE_AS_SEND_TO_ERP @INVOICE_ID = @FACTURA, -- int
		...
		BEGIN CATCH
			...
			DELETE FROM [DIPROCOM_SERVER].[PACASA].[dbo].[REPLICA_FACTURA_DET]
			WHERE SERIE = @SERIE AND NUMERO_FACTURA = @FACTURA

			DELETE FROM [DIPROCOM_SERVER].[PACASA].[dbo].[REPLICA_FACTURA_ENC]
			WHERE SERIE = @SERIE AND NUMERO_FACTURA = @FACTURA

			EXEC [PACASA].SWIFT_SP_MARK_INVOICE_AS_FAILED_TO_ERP @INVOICE_ID = @FACTURA, -- int
			    @CDF_SERIE = @SERIE, -- varchar(50)
			    @CDF_RESOLUCION = @RESOLUCION, -- nvarchar(50)
			    @IS_CREDIT_NOTE = 0, -- int
