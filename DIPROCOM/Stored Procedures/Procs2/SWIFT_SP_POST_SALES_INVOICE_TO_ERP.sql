...
-- EJEMPLO DE EJECUCION: 
		EXEC [diprocom].[SWIFT_SP_POST_SALES_INVOICE_TO_ERP]

=========================================================*/
CREATE PROCEDURE [DIPROCOM].[SWIFT_SP_POST_SALES_INVOICE_TO_ERP]
AS
BEGIN
	SET NOCOUNT ON;
	...
	BEGIN
		...
		BEGIN TRY
			...
			SELECT 
				...
				NULL
			FROM DIPROCOM.SONDA_POS_INVOICE_HEADER SPIH
			INNER JOIN DIPROCOM.USERS US ON SPIH.POSTED_BY = US.LOGIN
			...
			INNER JOIN [DIPROCOM].[SONDA_POS_INVOICE_HEADER] spih ON [spih].[ID] = [spid].[ID]
			LEFT JOIN [DIPROCOM].[SWIFT_PRICE_LIST_BY_SKU] plbs 
				ON [plbs].[CODE_SKU] = [spid].[SKU] AND [plbs].[CODE_PRICE_LIST] = [spih].[CLIENT_ID]
			WHERE [spid].[ID]=@SALES_INVOICE_ID
				
			EXEC [diprocom].SWIFT_SP_MARK_INVOICE_AS_SEND_TO_ERP @INVOICE_ID = @FACTURA, -- int
		...
		BEGIN CATCH
			...
			EXEC [diprocom].SWIFT_SP_MARK_INVOICE_AS_FAILED_TO_ERP @INVOICE_ID = @FACTURA, -- int
			    @CDF_SERIE = @SERIE, -- varchar(50)
			    @CDF_RESOLUCION = @RESOLUCION, -- nvarchar(50)
			    @IS_CREDIT_NOTE = 0, -- int
