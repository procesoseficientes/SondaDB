...
-- Ejemplo de Ejecucion:				
				--
EXECUTE  [PACASA].[SWIFT_SP_VALIDATE_SKU_UBICATION] 
    @LOCATION = 2
   ,@CODE_SKU = '20GE'
   ,@PALLET_ID = 1279
				--				
*/
-- =============================================
CREATE PROCEDURE [PACASA].[SWIFT_SP_VALIDATE_SKU_UBICATION]
...
BEGIN 
  ...
  INNER JOIN [PACASA].[SWIFT_BATCH] AS BC ON (SU.[CODE_SKU] = BC.[SKU])
  INNER JOIN [PACASA].[SWIFT_PALLET] AS PA ON (PA.[BATCH_ID] = BC.[BATCH_ID])
  INNER JOIN [PACASA].[SWIFT_LOCATIONS] AS LO ON (LO.[CODE_LOCATION] = PA.[LOCATION])
