...
-- Ejemplo de Ejecucion:
				EXEC [DIPROCOM].[SWIFT_SP_SET_CUSTOMER_IN_MULTPOLIGON]
					@LOGIN = 'GERENTE@DIPROCOM'
					,@SECTOR_POLYGON_ID = 5168
*/
-- =============================================
CREATE PROCEDURE [DIPROCOM].[SWIFT_SP_SET_CUSTOMER_IN_MULTPOLIGON](
	@LOGIN VARCHAR(50)
...
BEGIN
	...
	SELECT
		...
		,[P].[POLYGON_ID]
		--,[TP].[TASK_TYPE]
	FROM [DIPROCOM].[SWIFT_POLYGON_BY_ROUTE] [P]
	INNER JOIN [DIPROCOM].[SWIFT_POLYGON] [SP] ON (
		[SP].[POLYGON_ID] = [P].[POLYGON_ID]
	)
	--INNER JOIN [DIPROCOM].[SWIFT_TASK_BY_POLYGON] [TP] ON (
	...
	LEFT JOIN [DIPROCOM].[SWIFT_POLYGON_X_CUSTOMER] [PXC] ON (
	...
	BEGIN
		...
		SELECT TOP 1
			...
			,@GEOMETRY_POLYGON = [DIPROCOM].[SWIFT_GET_GEOMETRY_POLYGON_BY_POLIGON_ID]([M].[POLYGON_ID])
		FROM @MULTIPOLYGON [M]
	...
	BEGIN TRAN
		...
		-- ------------------------------------------------------------------------------------
		DELETE [FXC]
		FROM [DIPROCOM].[SWIFT_FREQUENCY_X_CUSTOMER] [FXC]
		INNER JOIN [#POLYGON_BY_ROUTE] [PBR] ON (
		...
		-- ------------------------------------------------------------------------------------
		MERGE [DIPROCOM].[SWIFT_CUSTOMER_FREQUENCY] AS [CF]
		USING (
			SELECT DISTINCT
				[PBR].[CODE_CUSTOMER]
				,[F].[SUNDAY]
			...
			INNER JOIN [DIPROCOM].[SWIFT_FREQUENCY] [F] ON (
		...
		-- ------------------------------------------------------------------------------------
		INSERT INTO [DIPROCOM].[SWIFT_POLYGON_X_CUSTOMER]
				(
					[POLYGON_ID]
					,[CODE_CUSTOMER]
					,[IS_NEW]
