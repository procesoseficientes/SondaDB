CREATE PROC [PACASA].SWIFT_SP_INSERTROUTE
...
SET @UserName = (SELECT TOP 1 [NAME_USER] FROM [PACASA].[USERS] WHERE [LOGIN] = @LAST_UPDATE_BY)
...
SET @EschemaName = (SELECT TOP 1 ENTERPRISE FROM [PACASA].[USERS] WHERE [LOGIN] = @LAST_UPDATE_BY)
SET @EnterpriseAddress = (SELECT TOP 1 ADDRESS_ENTERPRISE FROM [dbo].[SWIFT_ENTERPRISE] WHERE CODE_ENTERPRISE = @EschemaName)

	INSERT INTO [PACASA].[SONDA_POS_RES_SAT]
           ([AUTH_ID]
