CREATE PROCEDURE [acsa].[SWIFT_SP_VALIDATE_SERIE]
(              
	@pSERIE			VARCHAR(50),
...
AS
	...
	IF @pSERVICE_ID = 'VALIDATE_QTY' BEGIN
			DECLARE @RESULT INT 
			SELECT @RESULT = SUM(ON_HAND) FROM [acsa].SWIFT_INVENTORY  WHERE SKU = UPPER(@pSKU) AND [LOCATION] = @pLOCATION
	...
	IF @pSERVICE_ID = 'VALIDATE_SERIE_DEVOLUTION' BEGIN
	        SELECT * FROM [acsa].[SWIFT_TXNS] WHERE
