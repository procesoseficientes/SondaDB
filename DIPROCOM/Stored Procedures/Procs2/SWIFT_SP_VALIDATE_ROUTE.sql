...
-- Ejemplo de Ejecucion:				
				--
				exec [PACASA].[SWIFT_SP_VALIDATE_ROUTE] @CODE_ROUTE = 'R011'
				exec [PACASA].[SWIFT_SP_VALIDATE_ROUTE] @CODE_ROUTE = 'JOSE@DIPROCOM'
				exec [PACASA].[SWIFT_SP_VALIDATE_ROUTE] @CODE_ROUTE = 'JOEL@DIPROCOM'
				exec [PACASA].[SWIFT_SP_VALIDATE_ROUTE] @CODE_ROUTE = 'ALBERTO@DIPROCOM'
				exec [PACASA].[SWIFT_SP_VALIDATE_ROUTE] @CODE_ROUTE = 'RUDI@DIPROCOM'				
				exec [PACASA].[SWIFT_SP_VALIDATE_ROUTE] @CODE_ROUTE = '001'
*/
-- =============================================
CREATE PROCEDURE [PACASA].[SWIFT_SP_VALIDATE_ROUTE] @CODE_ROUTE VARCHAR(50)
AS
BEGIN
    SET NOCOUNT ON;
    --
    ...
    FROM [PACASA].[SWIFT_ASIGNED_QUIZ]
    ...
    BEGIN
        ...
        FROM [PACASA].[SWIFT_PORTFOLIO_BY_SKU] [PS]
    ...
    FROM [PACASA].[SWIFT_EVENT] [E]
        INNER JOIN [PACASA].[SWIFT_RULE_X_EVENT] [RE]
            ON ([RE].[EVENT_ID] = [E].[EVENT_ID])
        INNER JOIN [PACASA].[SWIFT_RULE] [R]
            ON ([R].[RULE_ID] = [RE].[RULE_ID])
        INNER JOIN [PACASA].[SWIFT_RULE_X_ROUTE] [RR]
    ...
    FROM [PACASA].[SWIFT_DOCUMENT_SEQUENCE]
    ...
    IF (@SALE_QTY > 0)
    BEGIN
        SELECT @PDOC = [PACASA].[SWIFT_FUNC_VALIDATE_DOCUMENT_SALE](@CODE_ROUTE);
        --
        IF (@PDOC = 0)
        BEGIN
        ...
        SELECT @PWH = [PACASA].[SWIFT_FUNC_VALIDATE_ROUTE_WH_SALE](@CODE_ROUTE);
        --
        IF (@PWH = 0)
        BEGIN
            IF (@RESULT != '')
    ...
    IF (@PRESALE_QTY > 0)
    BEGIN
        SELECT @PDOC = [PACASA].[SWIFT_FUNC_VALIDATE_DOCUMENT_PRESALE](@CODE_ROUTE);
        --
        IF (@PDOC = 0)
        BEGIN
            IF (@RESULT != '')
            ...
            END;
        END;
        SELECT @PWH = [PACASA].[SWIFT_FUNC_VALIDATE_ROUTE_WH_PRESALE](@CODE_ROUTE);
        --
        IF (@PWH = 0)
        BEGIN
            IF (@RESULT != '')
    ...
    BEGIN
        SELECT @HAVE_INVENTORY_DOCS = [PACASA].[SWIFT_FUNC_VALIDATE_DOCUMENT_TAKE_INVENTORY](@CODE_ROUTE);
        --
    ...
    -- -----------------------------------------------
    SELECT @EXIST_PRICE_LIST_DEFAULT = [PACASA].[SWIFT_FN_GET_PRICE_LIST](NULL);
    ...
    FROM [PACASA].[SWIFT_EVENT] AS [E]
        INNER JOIN [PACASA].[SWIFT_RULE_X_EVENT] AS [RE]
            ON ([RE].[EVENT_ID] = [E].[EVENT_ID])
        INNER JOIN [PACASA].[SWIFT_RULE] AS [R]
            ON ([R].[RULE_ID] = [RE].[RULE_ID])
        INNER JOIN [PACASA].[SWIFT_RULE_X_ROUTE] AS [RR]
    ...
    BEGIN
        ...
        FROM [PACASA].[SWIFT_DOCUMENT_SEQUENCE] AS [DS]
    ...
    -- ------------------------------------------------------------------------------------
    SELECT @HAS_TRADE_AGREEMENT = [PACASA].[SWIFT_FN_VALIDATE_TRADE_AGREEMENT_BY_ROUTE](@CODE_ROUTE);
    --
    ...
    -- ------------------------------------------------------------------------------------
    SELECT @EXISTS_LABEL_GROUP = [PACASA].[SWIFT_FN_VALIDATE_EXISTX_GROUP_IN_PARAMETER]('LABEL');
    --
    ...
    SELECT @HAVE_DOCUMENT_SEQUENCE_OF_HISTORY_PROMO
        = [PACASA].[SWIFT_FN_VALIDATE_IF_ROUTE_HAVE_DOCUMENT_SEQUENCE_OF_HISTORY_PROMO](@CODE_ROUTE);
    --
    ...
    BEGIN
        ...
        BEGIN
            ...
            EXEC [PACASA].[SONDA_SP_GET_PHRASES_AND_SCENARIOS_FEL_BY_ROUTE] @CODE_ROUTE = @CODE_ROUTE; -- varchar(50)
