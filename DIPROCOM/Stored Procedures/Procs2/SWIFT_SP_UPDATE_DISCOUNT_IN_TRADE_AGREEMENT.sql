...
-- Ejemplo de Ejecucion:
				EXEC [acsa].SWIFT_SP_UPDATE_DISCOUNT_IN_TRADE_AGREEMENT
					@TRADE_AGREEMENT_ID  = 2
	        ,@CODE_SKU = '100002'
          ,@DISCOUNT = 8
				-- 
				SELECT * FROM [acsa].SWIFT_TRADE_AGREEMENT_DISCOUNT WHERE TRADE_AGREEMENT_ID = 21
*/
-- =============================================
CREATE PROCEDURE [acsa].SWIFT_SP_UPDATE_DISCOUNT_IN_TRADE_AGREEMENT(
...
BEGIN
	BEGIN TRY		
		...
		/*EXEC [acsa].[SWIFT_SP_VALIDATED_DISCOUNT_SCALe] 
			...
			*/
		--
		UPDATE [acsa].SWIFT_TRADE_AGREEMENT_DISCOUNT SET
