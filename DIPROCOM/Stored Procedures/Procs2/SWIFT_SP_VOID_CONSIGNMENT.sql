...
-- Ejemplo de Ejecucion:
				EXEC [PACASA].[SWIFT_SP_VOID_CONSIGNMENT]
					@DOC_SERIE = 'Serie Re-Consignación'
					,@DOC_NUM = 18
					,@REASON = 'Ya no queria el producto'
					,@DEFAULT_WAREHOUSE = 'C001'
				--
				SELECT * FROM [PACASA].[SWIFT_CONSIGNMENT_HEADER] 
				WHERE [DOC_SERIE] = 'Serie Re-Consignación'
					AND [DOC_NUM] = 18
*/
-- =============================================
CREATE PROCEDURE [PACASA].[SWIFT_SP_VOID_CONSIGNMENT](
...
BEGIN
	...
	-- --------------------------------------------------------------------------------
	-- ACTUALIZA [PACASA].SONDA_POS_SKUS
	-- --------------------------------------------------------------------------------
	UPDATE POS_SKUS
	SET
		POS_SKUS.ON_HAND = (POS_SKUS.ON_HAND + CONS_DETAIL.QTY)
	FROM
		[PACASA].[SONDA_POS_SKUS] AS POS_SKUS
		INNER JOIN [PACASA].[SWIFT_CONSIGNMENT_DETAIL] AS CONS_DETAIL
			ON(POS_SKUS.SKU = CONS_DETAIL.SKU)
	WHERE CONS_DETAIL.CONSIGNMENT_ID = (
			SELECT TOP 1 CONSIGNMENT_ID FROM [PACASA].[SWIFT_CONSIGNMENT_HEADER] AS CH 
			WHERE CH.DOC_SERIE = @DOC_SERIE AND CH.DOC_NUM = @DOC_NUM
	) AND POS_SKUS.ROUTE_ID = @DEFAULT_WAREHOUSE
	
	-- --------------------------------------------------------------------------------
	-- ACTUALIZA [PACASA].[SWIFT_INVENTORY]
	-- --------------------------------------------------------------------------------
	UPDATE INVENTORY
	SET
		INVENTORY.ON_HAND = (INVENTORY.ON_HAND + CONS_DETAIL.QTY)
	FROM
		[PACASA].[SWIFT_INVENTORY] AS INVENTORY
		INNER JOIN [PACASA].[SWIFT_CONSIGNMENT_DETAIL] AS CONS_DETAIL
