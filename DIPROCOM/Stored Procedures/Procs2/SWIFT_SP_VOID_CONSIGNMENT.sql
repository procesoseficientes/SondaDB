...
-- Ejemplo de Ejecucion:
				EXEC [DIPROCOM].[SWIFT_SP_VOID_CONSIGNMENT]
					@DOC_SERIE = 'Serie Re-Consignación'
					,@DOC_NUM = 18
					,@REASON = 'Ya no queria el producto'
					,@DEFAULT_WAREHOUSE = 'C001'
				--
				SELECT * FROM [DIPROCOM].[SWIFT_CONSIGNMENT_HEADER] 
				WHERE [DOC_SERIE] = 'Serie Re-Consignación'
					AND [DOC_NUM] = 18
*/
-- =============================================
CREATE PROCEDURE [DIPROCOM].[SWIFT_SP_VOID_CONSIGNMENT](
...
BEGIN
	...
	-- --------------------------------------------------------------------------------
	-- ACTUALIZA [DIPROCOM].SONDA_POS_SKUS
	-- --------------------------------------------------------------------------------
	UPDATE POS_SKUS
	SET
		POS_SKUS.ON_HAND = (POS_SKUS.ON_HAND + CONS_DETAIL.QTY)
	FROM
		[DIPROCOM].[SONDA_POS_SKUS] AS POS_SKUS
		INNER JOIN [DIPROCOM].[SWIFT_CONSIGNMENT_DETAIL] AS CONS_DETAIL
			ON(POS_SKUS.SKU = CONS_DETAIL.SKU)
	WHERE CONS_DETAIL.CONSIGNMENT_ID = (
			SELECT TOP 1 CONSIGNMENT_ID FROM [DIPROCOM].[SWIFT_CONSIGNMENT_HEADER] AS CH 
			WHERE CH.DOC_SERIE = @DOC_SERIE AND CH.DOC_NUM = @DOC_NUM
	) AND POS_SKUS.ROUTE_ID = @DEFAULT_WAREHOUSE
	
	-- --------------------------------------------------------------------------------
	-- ACTUALIZA [DIPROCOM].[SWIFT_INVENTORY]
	-- --------------------------------------------------------------------------------
	UPDATE INVENTORY
	SET
		INVENTORY.ON_HAND = (INVENTORY.ON_HAND + CONS_DETAIL.QTY)
	FROM
		[DIPROCOM].[SWIFT_INVENTORY] AS INVENTORY
		INNER JOIN [DIPROCOM].[SWIFT_CONSIGNMENT_DETAIL] AS CONS_DETAIL
