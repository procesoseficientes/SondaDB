...
-- Ejemplo de Ejecucion:
				exec [SONDA].[SWIFT_SP_UPDATE_CUSTOMER_SEND_TO_SAP]
*/
-- =============================================
CREATE PROCEDURE [SONDA].[SWIFT_SP_UPDATE_CUSTOMER_SEND_TO_SAP]
AS
...
select top 1 @USUARIO = SELLER_DEFAULT_CODE from [SONDA].SWIFT_CUSTOMERS_NEW
where IS_POSTED_ERP <> 1 --and @WH != (select  DEFAULT_WAREHOUSE from [SONDA].USERS where DEFAULT_WAREHOUSE != @WH)
select @USUARIO as seller


select @DF = DEFAULT_WAREHOUSE from [SONDA].USERS
