...
-- Ejemplo de Ejecucion:
				exec [PACASA].[SWIFT_SP_UPDATE_CUSTOMER_SEND_TO_SAP]
*/
-- =============================================
CREATE PROCEDURE [PACASA].[SWIFT_SP_UPDATE_CUSTOMER_SEND_TO_SAP]
AS
...
select top 1 @USUARIO = SELLER_DEFAULT_CODE from [PACASA].SWIFT_CUSTOMERS_NEW
where IS_POSTED_ERP <> 1 --and @WH != (select  DEFAULT_WAREHOUSE from [PACASA].USERS where DEFAULT_WAREHOUSE != @WH)
select @USUARIO as seller


select @DF = DEFAULT_WAREHOUSE from [PACASA].USERS
