...
-- MODIFICADO: 09-03-2016
	...
	EJEMPLO DE EJECUCION:
		...
		EXEC [acsa].[SWIFT_SP_PROCESS_PICKING]
			@pTASK_ID = 16574
			,@pBARCODE_LOC = 'A3'
			,@pSKU = '100017'
			,@pSERIE = '1000000007'
...
CREATE PROCEDURE [acsa].[SWIFT_SP_PROCESS_PICKING]
(
@pTASK_ID INT
    ...
    DECLARE @lSKU VARCHAR(75)
		...
		,@NAME_USER VARCHAR(50)
            ...
            BEGIN TRY
                  ...
                  SELECT @lSKU = ISNULL((SELECT 1 FROM [acsa].SWIFT_VIEW_SKU  WHERE CODE_SKU = @pSKU OR BARCODE_SKU = @pSKU),'NF')
				   IF(@lSKU = 'NF') BEGIN
					SELECT @pResult = 'ERROR, SKU: ' +@pSKU+ ', NO EXISTE';
                    --RETURN -6
				  END

				  SELECT @lPICKING_HEADER = (SELECT [PICKING_NUMBER] FROM [acsa].[SWIFT_TASKS] WHERE [TASK_ID] = @pTASK_ID)

				  IF NOT EXISTS(SELECT 1 FROM [acsa].[SWIFT_PICKING_DETAIL] WHERE [PICKING_HEADER] = @lPICKING_HEADER AND [CODE_SKU] = @lSKU) BEGIN
				  ...
				  --SI MANEJA SERIE
				  IF(@pSERIE != '')  BEGIN
					IF NOT EXISTS(SELECT 1 FROM [acsa].[SWIFT_INVENTORY] WHERE [SKU] = @pSKU AND [SERIAL_NUMBER] = @pSERIE AND [LOCATION] = @pBARCODE_LOC)BEGIN
						SELECT @pResult = 'ERROR, SKU: ' +@pSKU+ '/ SERIE: ' + @pSERIE + ' UBICACION NO EXISTE EN INVETARIO';
						--RETURN -7
					END
				  END

                  IF NOT EXISTS(SELECT TOP 1 CODE_WAREHOUSE FROM [acsa].SWIFT_LOCATIONS WHERE CODE_LOCATION = @pBARCODE_LOC) BEGIN
                    SELECT @pResult = 'ERROR, UBICACION: ' +@pBARCODE_LOC+ ', NO EXISTE';
                    --RETURN -8
                  END
                  
                  SELECT @lSAP_REFERENCE = ISNULL((SELECT SAP_REFERENCE FROM [acsa].SWIFT_TASKS WHERE TASK_ID = @pTASK_ID),'N/F')
                  IF(@lSAP_REFERENCE = 'N/F') BEGIN
                    SELECT @pResult = 'ERROR, TAREA: ' +convert(varchar(10), @pTASK_ID)+ ', NO TIENE DOCUMENTO ERP';
                    --RETURN -9
                  END

                  SELECT @lHEADER_REFERENCE = ISNULL((SELECT PICKING_NUMBER FROM [acsa].SWIFT_TASKS WHERE TASK_ID = @pTASK_ID),-9999)
                  IF(@lHEADER_REFERENCE = -9999) BEGIN
                    SELECT @pResult = 'ERROR, TAREA: ' +convert(varchar(10), @pTASK_ID)+ ', NO TIENE DOCUMENTO ERP';
                    --RETURN -10
                  END
				 
                  SELECT @lERP_INTERFACE_TYPE = ISNULL((SELECT MPC01 FROM [acsa].SWIFT_CLASSIFICATION 
														WHERE CLASSIFICATION = (SELECT CLASSIFICATION_PICKING 
														FROM [acsa].SWIFT_PICKING_HEADER 
                  ...
                  --SELECT @lTASK_COMMENTS = (SELECT TASK_COMMENTS FROM [acsa].SWIFT_TASKS WHERE TASK_ID = @pTASK_ID)
                  
                  SELECT @lWAREHOUSE = (SELECT CODE_WAREHOUSE FROM [acsa].SWIFT_LOCATIONS WHERE CODE_LOCATION = @pBARCODE_LOC)
                      
				IF EXISTS( SELECT CODE_SKU FROM [acsa].SWIFT_PICKING_DETAIL 
							WHERE PICKING_HEADER = @lHEADER_REFERENCE 
							AND CODE_SKU = @pSKU) BEGIN

					UPDATE [acsa].SWIFT_PICKING_HEADER 
				...
				END
				 ...
				 --SI MANEJA SERIE
                 IF(@pSERIE != '') BEGIN
						DELETE FROM [acsa].[SWIFT_TXNS_SERIES]
						WHERE [TXN_CODE_SKU] = @pSKU AND [TXN_SERIE] = @pSERIE
                        
						DELETE FROM [acsa].[SWIFT_INVENTORY]
				...
				ELSE
					BEGIN
						...
						SELECT INVENTORY, ON_HAND FROM [acsa].SWIFT_INVENTORY  WHERE SKU = @pSKU AND [LOCATION] = @pBARCODE_LOC
					...
					SELECT ST.SAP_REFERENCE
						...
						,@pQTY
						,@pSERIE
					FROM [acsa].SWIFT_TASKS AS ST
					INNER JOIN  [acsa].SWIFT_VIEW_SKU AS VS ON (
						VS.CODE_SKU = @pSKU
					)
