...
-- =============================================
CREATE PROCEDURE [PACASA].[SWIFT_SP_PROCESS_DEVOLUTION]
...
BEGIN
	...
	SELECT @SKU_DESCRIPTION = (SELECT [DESCRIPTION_SKU] FROM [PACASA].[SWIFT_VIEW_ALL_SKU] WHERE [CODE_SKU] = @SKU_CODE)
	SELECT @HEADER_REFERENCE = [PICKING_NUMBER] FROM [PACASA].[SWIFT_TASKS] WHERE [TASK_ID] = @TASK_ID
    ...
    BEGIN TRAN t1
		BEGIN
			...
			VALUES(
				(SELECT [SAP_REFERENCE] FROM [PACASA].[SWIFT_TASKS] WHERE [TASK_ID] = @TASK_ID)
				, @TASK_ID
				, 'REALLOC'
				...
				, GetDate()
				, @OPERATOR_ID
				, (SELECT [NAME_USER] FROM [PACASA].[USERS] WHERE [LOGIN] = @OPERATOR_ID)
				, @SKU_CODE
				, @SKU_DESCRIPTION
				, @QTY
			...
			VALUES(
				(SELECT [CODE_WAREHOUSE] FROM [PACASA].[SWIFT_LOCATIONS] WHERE [CODE_LOCATION] = @CODE_LOCATION)
				, @CODE_LOCATION
				, @SKU_CODE
				, @SKU_DESCRIPTION
				, @QTY
