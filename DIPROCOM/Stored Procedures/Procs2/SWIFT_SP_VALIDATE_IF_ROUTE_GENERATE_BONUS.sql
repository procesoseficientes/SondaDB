...
-- Ejemplo de Ejecucion:
			EXEC  [DIPROCOM].[SWIFT_SP_VALIDATE_IF_ROUTE_GENERATE_DISCOUNT] @CODE_ROUTE = '136'
*/
-- =============================================
CREATE PROCEDURE [DIPROCOM].[SWIFT_SP_VALIDATE_IF_ROUTE_GENERATE_BONUS] (@CODE_ROUTE VARCHAR(50), @GENERATE_BONUS INT OUTPUT)
AS
BEGIN
  SET NOCOUNT ON;
  --
  ...
  FROM [DIPROCOM].[SWIFT_TRADE_AGREEMENT_WATERMARK] [TAW]
  ...
  FROM [DIPROCOM].[SWIFT_PROMO_BONUS_BY_GENERAL_AMOUNT] [BBGA]
  ...
  FROM [DIPROCOM].[SWIFT_PROMO_BONUS_BY_SCALE] [BBS]
  ...
  FROM [DIPROCOM].[SWIFT_PROMO_BONUS_BY_MULTIPLE] [BBM]
  ...
  FROM [DIPROCOM].[SWIFT_PROMO_BY_BONUS_RULE] [PBR]
  ...
  FROM [DIPROCOM].[SWIFT_PROMO_SKU_BY_PROMO_RULE] [PSPR]
  ...
  FROM [DIPROCOM].[SWIFT_SKU_BY_COMBO] [SC]
  ...
  IF NOT EXISTS (SELECT
        WATER_MARK
      FROM [DIPROCOM].[SWIFT_TRADE_AGREEMENT_WATERMARK]
      WHERE [CODE_ROUTE] = @CODE_ROUTE)
  BEGIN

    INSERT INTO [DIPROCOM].[SWIFT_TRADE_AGREEMENT_WATERMARK] ([WATER_MARK], [CODE_ROUTE], [TIMES_REQUIRE], [LAST_REQUIRE], [MARKED_TABLE])
      VALUES (@LAST_WATER_MARK, @CODE_ROUTE, 1, GETDATE(), @MARKED_TABLE);

  END
  ELSE
  BEGIN
    UPDATE [DIPROCOM].[SWIFT_TRADE_AGREEMENT_WATERMARK]
