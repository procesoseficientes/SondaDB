...
-- Ejemplo de Ejecucion:
			EXEC  [PACASA].[SWIFT_SP_VALIDATE_IF_ROUTE_GENERATE_DISCOUNT] @CODE_ROUTE = '136'
*/
-- =============================================
CREATE PROCEDURE [PACASA].[SWIFT_SP_VALIDATE_IF_ROUTE_GENERATE_BONUS] (@CODE_ROUTE VARCHAR(50), @GENERATE_BONUS INT OUTPUT)
AS
BEGIN
  SET NOCOUNT ON;
  --
  ...
  FROM [PACASA].[SWIFT_TRADE_AGREEMENT_WATERMARK] [TAW]
  ...
  FROM [PACASA].[SWIFT_PROMO_BONUS_BY_GENERAL_AMOUNT] [BBGA]
  ...
  FROM [PACASA].[SWIFT_PROMO_BONUS_BY_SCALE] [BBS]
  ...
  FROM [PACASA].[SWIFT_PROMO_BONUS_BY_MULTIPLE] [BBM]
  ...
  FROM [PACASA].[SWIFT_PROMO_BY_BONUS_RULE] [PBR]
  ...
  FROM [PACASA].[SWIFT_PROMO_BY_COMBO_PROMO_RULE] [PCPR]
  ...
  FROM [PACASA].[SWIFT_PROMO_SKU_BY_PROMO_RULE] [PSPR]
  ...
  FROM [PACASA].[SWIFT_SKU_BY_COMBO] [SC]
  ...
  IF NOT EXISTS (SELECT
        WATER_MARK
      FROM [PACASA].[SWIFT_TRADE_AGREEMENT_WATERMARK]
      WHERE [CODE_ROUTE] = @CODE_ROUTE)
  BEGIN

    INSERT INTO [PACASA].[SWIFT_TRADE_AGREEMENT_WATERMARK] ([WATER_MARK], [CODE_ROUTE], [TIMES_REQUIRE], [LAST_REQUIRE], [MARKED_TABLE])
      VALUES (@LAST_WATER_MARK, @CODE_ROUTE, 1, GETDATE(), @MARKED_TABLE);

  END
  ELSE
  BEGIN
    UPDATE [PACASA].[SWIFT_TRADE_AGREEMENT_WATERMARK]
