...
*/
-- =============================================
CREATE PROCEDURE [PACASA].[SWIFT_SP_VALIDATE_IF_ROUTE_GENERATE_TRADE_AGREEMENT] (@CODE_ROUTE VARCHAR(50), @GENERATE_TRADE_AGREEMENT INT OUTPUT)
AS
BEGIN
  SET NOCOUNT ON;
  --
  ...
  FROM [PACASA].[SWIFT_TRADE_AGREEMENT_BY_CHANNEL] [TAC]
  ...
  SELECT TOP 1
    @CURRENT_WATER_MARK = [TAC].[LAST_UPDATE]
  FROM [PACASA].[SWIFT_TRADE_AGREEMENT_BY_CUSTOMER] [TAC]
  WHERE [TAC].[LAST_UPDATE] > 0
  ORDER BY [TAC].[LAST_UPDATE] DESC
 ...
 -- ------------------------------------------------------------------------------------
  ...
  FROM [PACASA].[SWIFT_CHANNEL_X_CUSTOMER] [CXC]
