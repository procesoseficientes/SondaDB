...
*/
-- =============================================
CREATE PROCEDURE [DIPROCOM].[SWIFT_SP_VALIDATE_IF_ROUTE_GENERATE_TRADE_AGREEMENT] (@CODE_ROUTE VARCHAR(50), @GENERATE_TRADE_AGREEMENT INT OUTPUT)
AS
BEGIN
  SET NOCOUNT ON;
  --
  ...
  FROM [DIPROCOM].[SWIFT_TRADE_AGREEMENT_BY_PROMO] [TAP]
  ...
  FROM [DIPROCOM].[SWIFT_TRADE_AGREEMENT_BY_CHANNEL] [TAC]
  ...
  SELECT TOP 1
    @CURRENT_WATER_MARK = [TAC].[LAST_UPDATE]
  FROM [DIPROCOM].[SWIFT_TRADE_AGREEMENT_BY_CUSTOMER] [TAC]
  WHERE [TAC].[LAST_UPDATE] > 0
  ORDER BY [TAC].[LAST_UPDATE] DESC
 ...
 -- ------------------------------------------------------------------------------------
  ...
  FROM [DIPROCOM].[SWIFT_CHANNEL_X_CUSTOMER] [CXC]
