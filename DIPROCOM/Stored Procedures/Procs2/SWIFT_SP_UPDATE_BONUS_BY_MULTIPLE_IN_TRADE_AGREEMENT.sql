-- =============================================
-- Autor:				alberto.ruiz
-- Fecha de Creacion: 	21-Nov-16 @ A-TEAM Sprint 5 
-- Description:			SP que actualiza la bonificacion por multiplo

/*
-- Ejemplo de Ejecucion:
				EXEC [PACASA].[SWIFT_SP_UPDATE_BONUS_BY_MULTIPLE_IN_TRADE_AGREEMENT]
					@TRADE_AGREEMENT_BONUS_MULTIPLE_ID = 2
					,@TRADE_AGREEMENT_ID = 20
					,@CODE_SKU = '100002'
					,@PACK_UNIT = 7
					,@MULTIPLE = 5
					,@CODE_SKU_BONUS = '100002'
					,@PACK_UNIT_BONUS = 7
					,@BONUS_QTY = 2
				-- 
				SELECT * FROM [PACASA].[SWIFT_TRADE_AGREEMENT_BONUS_BY_MULTIPLE]
*/
-- =============================================
CREATE PROCEDURE [PACASA].[SWIFT_SP_UPDATE_BONUS_BY_MULTIPLE_IN_TRADE_AGREEMENT](
	@TRADE_AGREEMENT_BONUS_MULTIPLE_ID INT
	,@TRADE_AGREEMENT_ID INT
	,@CODE_SKU VARCHAR(50)
	,@PACK_UNIT INT
	,@MULTIPLE NUMERIC(18,0)
	,@CODE_SKU_BONUS VARCHAR(50)
	,@PACK_UNIT_BONUS INT
	,@BONUS_QTY NUMERIC(18,0)
)
AS
BEGIN
	BEGIN TRY
		UPDATE [PACASA].[SWIFT_TRADE_AGREEMENT_BONUS_BY_MULTIPLE]
		SET 
			TRADE_AGREEMENT_ID = @TRADE_AGREEMENT_ID
			,CODE_SKU = @CODE_SKU
			,PACK_UNIT = @PACK_UNIT
			,MULTIPLE = @MULTIPLE
			,CODE_SKU_BONUS = @CODE_SKU_BONUS
			,PACK_UNIT_BONUS = @PACK_UNIT_BONUS
			,BONUS_QTY = @BONUS_QTY
		WHERE [TRADE_AGREEMENT_BONUS_MULTIPLE_ID] = @TRADE_AGREEMENT_BONUS_MULTIPLE_ID
		--
		SELECT  1 as Resultado , 'Proceso Exitoso' Mensaje ,  0 Codigo, '' DbData
	END TRY
	BEGIN CATCH
		SELECT  -1 as Resultado
		,CASE CAST(@@ERROR AS VARCHAR)
			WHEN '2627' THEN 'Error: Ya existe la bonificacion por multiplo'
			ELSE ERROR_MESSAGE() 
		END Mensaje 
		,@@ERROR Codigo 
	END CATCH
END
