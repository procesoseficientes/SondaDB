...
-- Ejemplo de Ejecucion:				
				--
EXECUTE  [PACASA].[SWIFT_SP_RELOCATE_PALLET] 
   @PALLET_ID = 1277
  ...
  ,@LAST_UPDATE_BY = 'gerente@DIPROCOM'
				...
				--				
*/
CREATE PROCEDURE [PACASA].[SWIFT_SP_RELOCATE_PALLET]
    @PALLET_ID AS INT
...
BEGIN TRANSACTION
    ...
    SELECT @CODE_SKU =[SKU] FROM [PACASA].[SWIFT_BATCH] WHERE @BATCHID = [BATCH_ID] AND @TASK_ID = [TASK_ID]

    SELECT @NEW_WAREHOUSE = [CODE_WAREHOUSE] FROM  [PACASA].[SWIFT_LOCATIONS] WHERE [CODE_LOCATION] =@CODE_LOCATION;

	SELECT @NAME_USER = [NAME_USER] FROM [PACASA].[USERS] WHERE [LOGIN] = @LAST_UPDATE_BY;
	
	SELECT @DESCRIPTION_SKU = [DESCRIPTION_SKU],  @TXN_SKU_BARCODE= [BARCODE_SKU] FROM [PACASA].[SWIFT_VIEW_ALL_SKU] 
		WHERE @CODE_SKU = [CODE_SKU];

	SELECT @SOURCE_WAREHOUSE = [WAREHOUSE] ,@SOURCE_LOCATION = [LOCATION], @SOURCE_PALLET = [PALLET_ID] FROM [PACASA].[SWIFT_INVENTORY]
	WHERE [PALLET_ID] = @PALLET_ID



	SELECT @VALIDATE_PALLET = [PACASA].[SWIFT_FUNC_VALIDATE_PALLET_LOCATED] (@PALLET_ID)
		...
		IF (@VALIDATE_PALLET>=1)
			...
			EXEC [PACASA].[SWIFT_SP_LOCATE_PALLET] @LAST_UPDATE_BY,@CODE_LOCATION ,@PALLET_ID,@NEW_WAREHOUSE 
