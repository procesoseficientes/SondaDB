...
-- Ejemplo de Ejecucion:
				exec [acsa].[SWIFT_SP_PROCESS_REALLOC]
*/
-- =============================================
CREATE PROCEDURE [acsa].[SWIFT_SP_PROCESS_REALLOC]
...
AS
	  ...
	  SET @lDebugLine = 0;
            ...
            BEGIN TRY
              SELECT @pSKU = CODE_SKU FROM [acsa].SWIFT_VIEW_SKU AS SVS WHERE SVS.BARCODE_SKU = @pBARCODE_SKU OR SVS.CODE_SKU = @pBARCODE_SKU
						...
						SELECT 0
							...
							,(CASE SVS.HANDLE_SERIAL_NUMBER  
								...
								WHEN 1 THEN
									1
							END)
						FROM [acsa].SWIFT_VIEW_SKU AS SVS
