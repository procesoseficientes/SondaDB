...
-- Ejemplo de Ejecucion:
	EXEC [PACASA].[SWIFT_SP_SET_ERP_POSTING_STATUS_ON_INVOICE]
  
*/
-- =============================================

CREATE PROCEDURE [PACASA].[SWIFT_SP_SET_ERP_POSTING_STATUS_ON_INVOICE]
(
    @INVOICE_ID INT,
    @POSTING_STATUS INT,
...
BEGIN
    ...
    FROM [PACASA].[SONDA_POS_INVOICE_HEADER] AS [IH]
    ...
    IF (@POSTING_STATUS = 1)
    BEGIN
        EXEC [PACASA].[SWIFT_SP_MARK_INVOICE_AS_SEND_TO_ERP] @INVOICE_ID = @INVOICE_NUM,           -- int
    ...
    ELSE
    BEGIN
        EXEC [PACASA].[SWIFT_SP_MARK_INVOICE_AS_FAILED_TO_ERP] @INVOICE_ID = @INVOICE_NUM,           -- int
                                                              @CDF_SERIE = @CDF_SERIE,              -- varchar(50)
                                                              @CDF_RESOLUCION = @CDF_RESOLUCION,    -- nvarchar(50)
                                                              @IS_CREDIT_NOTE = @IS_CREDIT_NOTE,    -- int
                                                              @POSTED_RESPONSE = @POSTING_RESPONSE; -- varchar(150)
