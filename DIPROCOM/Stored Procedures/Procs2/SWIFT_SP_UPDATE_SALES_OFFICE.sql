...
-- Ejemplo de Ejecucion:
				EXEC [DIPROCOM].[SWIFT_SP_UPDATE_SALES_OFFICE]
				@SALES_OFFICE_ID = 2
				,@SALES_ORGANIZATION_ID = 1
				,@NAME_SALES_OFFICE = 'prueba oficina de ventas'
				,@DESCRIPTION_SALES_OFFICE = 'descripcion oficina de ventas'
				-- 
				SELECT * FROM [DIPROCOM].[SWIFT_SALES_OFFICE] WHERE SALES_OFFICE_ID = 2
*/
-- =============================================
CREATE PROCEDURE [DIPROCOM].[SWIFT_SP_UPDATE_SALES_OFFICE](
	@SALES_OFFICE_ID INT
...
BEGIN
	BEGIN TRY
		UPDATE [DIPROCOM].[SWIFT_SALES_OFFICE]
		SET	
