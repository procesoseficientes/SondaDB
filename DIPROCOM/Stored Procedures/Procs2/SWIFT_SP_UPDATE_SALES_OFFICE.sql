...
-- Ejemplo de Ejecucion:
				EXEC [PACASA].[SWIFT_SP_UPDATE_SALES_OFFICE]
				@SALES_OFFICE_ID = 2
				,@SALES_ORGANIZATION_ID = 1
				,@NAME_SALES_OFFICE = 'prueba oficina de ventas'
				,@DESCRIPTION_SALES_OFFICE = 'descripcion oficina de ventas'
				-- 
				SELECT * FROM [PACASA].[SWIFT_SALES_OFFICE] WHERE SALES_OFFICE_ID = 2
*/
-- =============================================
CREATE PROCEDURE [PACASA].[SWIFT_SP_UPDATE_SALES_OFFICE](
	@SALES_OFFICE_ID INT
...
BEGIN
	BEGIN TRY
		UPDATE [PACASA].[SWIFT_SALES_OFFICE]
		SET	
