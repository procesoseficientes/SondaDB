...
-- Ejemplo de Ejecucion:
				...
				EXEC [acsa].SWIFT_SP_UPDATE_TRADE_AGREEMENT
...
*/
-- =============================================
CREATE PROCEDURE [acsa].[SWIFT_SP_UPDATE_TRADE_AGREEMENT](
...
BEGIN
	...
	BEGIN TRY
		...
		FROM [acsa].SWIFT_TRADE_AGREEMENT
		...
		BEGIN
			SELECT @QUERY = N'' + CASE CAST(@ORIGINAL_LINKED_TO AS NVARCHAR)
				WHEN 'CUSTOMER' THEN 'SELECT @QTY = COUNT(*) FROM [acsa].SWIFT_TRADE_AGREEMENT_BY_CUSTOMER WHERE TRADE_AGREEMENT_ID = ' + CAST(@TRADE_AGREEMENT_ID AS varchar)
				ELSE 'SELECT @QTY = COUNT(*) FROM [acsa].SWIFT_TRADE_AGREEMENT_BY_CHANNEL WHERE TRADE_AGREEMENT_ID = ' + CAST(@TRADE_AGREEMENT_ID AS varchar)
			END			
			--
