...
-- Ejemplo de Ejecucion:
				...
				EXEC [DIPROCOM].SWIFT_SP_UPDATE_TRADE_AGREEMENT
          ...
          ,@VALID_END_DATETIME = '20160909 00:05:00.000'
					,@STATUS = 0
					,@LAST_UPDATE_BY = 'prueba@DIPROCOM'
					,@LINKED_TO = 'CUSTOMER'
				-- 
				SELECT * FROM [DIPROCOM].SWIFT_TRADE_AGREEMENT
*/
-- =============================================
CREATE PROCEDURE [DIPROCOM].[SWIFT_SP_UPDATE_TRADE_AGREEMENT](
...
BEGIN
	...
	BEGIN TRY
		...
		FROM [DIPROCOM].SWIFT_TRADE_AGREEMENT
		...
		BEGIN
			SELECT @QUERY = N'' + CASE CAST(@ORIGINAL_LINKED_TO AS NVARCHAR)
				WHEN 'CUSTOMER' THEN 'SELECT @QTY = COUNT(*) FROM [DIPROCOM].SWIFT_TRADE_AGREEMENT_BY_CUSTOMER WHERE TRADE_AGREEMENT_ID = ' + CAST(@TRADE_AGREEMENT_ID AS varchar)
				ELSE 'SELECT @QTY = COUNT(*) FROM [DIPROCOM].SWIFT_TRADE_AGREEMENT_BY_CHANNEL WHERE TRADE_AGREEMENT_ID = ' + CAST(@TRADE_AGREEMENT_ID AS varchar)
			END			
			--
