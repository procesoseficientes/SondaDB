...
-- Ejemplo de Ejecucion:
				-- 
				EXEC [PACASA].[SWIFT_SP_POLYGON_OPTIMIZE]
*/
-- =============================================
CREATE PROCEDURE [PACASA].[SWIFT_SP_POLYGON_OPTIMIZE]
AS
BEGIN
	SET NOCOUNT ON;
	--
	...
	BEGIN TRY
		...
		-- ------------------------------------------------------------
		SELECT 
			@TYPE_POLYGON_WITH_WAREHOUSE = [PACASA].SWIFT_FN_GET_PARAMETER('POLYGON','TYPE_POLYGON_WITH_WAREHOUSE')
			,@TYPE_POLYGON_ROUTE = [PACASA].SWIFT_FN_GET_PARAMETER('POLYGON','TYPE_POLYGON_ROUTE')
		...
		INNER JOIN [PACASA].[SWIFT_FREQUENCY_BY_POLYGON] [PBR] ON (
		...
		INNER JOIN [PACASA].[SWIFT_WAREHOUSES] [W] ON (
			[PS].[CODE_WAREHOUSE] = [W].[CODE_WAREHOUSE]
		)
		INNER JOIN [PACASA].[USERS] [U] ON ([U].[SELLER_ROUTE] = [F].[CODE_ROUTE])
		INNER JOIN [PACASA].[SWIFT_SELLER] [S] ON ([U].[RELATED_SELLER] = [S].[SELLER_CODE])
		...
		SELECT
			...
			,[F].[CODE_ROUTE]
		FROM   [PACASA].[SWIFT_FREQUENCY] [F]
		LEFT JOIN [PACASA].[SWIFT_FREQUENCY_BY_POLYGON] [PBR] ON (
			[PBR].[ID_FREQUENCY] = [F].[ID_FREQUENCY]
		)
		LEFT JOIN [PACASA].[SWIFT_POLYGON] [PR] ON (
			[PR].[POLYGON_ID] = [F].[POLYGON_ID]
		)
		LEFT JOIN [PACASA].[SWIFT_POLYGON] [PS] ON (
			[PR].[POLYGON_ID_PARENT] = [PS].[POLYGON_ID]
			AND [PR].[POLYGON_TYPE] = @TYPE_POLYGON_ROUTE
			AND [PS].[POLYGON_TYPE] = @TYPE_POLYGON_WITH_WAREHOUSE
		)
		LEFT JOIN [PACASA].[SWIFT_WAREHOUSES] [W] ON (
			[PS].[CODE_WAREHOUSE] = [W].[CODE_WAREHOUSE]
		)
		INNER JOIN [PACASA].[USERS] [U] ON ([U].[SELLER_ROUTE] = [F].[CODE_ROUTE])
		INNER JOIN [PACASA].[SWIFT_SELLER] [S] ON ([U].[RELATED_SELLER] = [S].[SELLER_CODE])
		...
		BEGIN
			...
			-- ------------------------------------------------------------
			EXEC [PACASA].[SWIFT_SP_SET_CUSTOMER_PRIORITY_IN_FREQUENCY]
