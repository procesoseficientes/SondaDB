CREATE PROC [DIPROCOM].SWIFT_UPDATE_INSERT_TASK_PRESALE
...
SET @USER_ROLE = (SELECT [USER_TYPE] FROM [DIPROCOM].USERS WHERE RELATED_SELLER = (SELECT SELLER_CODE FROM  [DIPROCOM].[SWIFT_VIEW_SELLER_LOGIN] WHERE [LOGIN] = @ASSIGNED_TO))--(SELECT SELLER_CODE FROM [DIPROCOM].SWIFT_VIEW_ALL_SELLERS WHERE SELLER_NAME = (SELECT DISTINCT SELLER_DEFAULT_CODE FROM [DIPROCOM].SWIFT_VIEW_ALL_COSTUMER WHERE CODE_CUSTOMER = @CODE_CUSTOMER)))
MERGE [DIPROCOM].SWIFT_TASKS TRG
