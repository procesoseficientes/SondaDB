CREATE PROC [acsa].SWIFT_UPDATE_INSERT_TASK_PRESALE
...
SET @USER_ROLE = (SELECT [USER_TYPE] FROM [acsa].USERS WHERE RELATED_SELLER = (SELECT SELLER_CODE FROM  [acsa].[SWIFT_VIEW_SELLER_LOGIN] WHERE [LOGIN] = @ASSIGNED_TO))--(SELECT SELLER_CODE FROM [acsa].SWIFT_VIEW_ALL_SELLERS WHERE SELLER_NAME = (SELECT DISTINCT SELLER_DEFAULT_CODE FROM [acsa].SWIFT_VIEW_ALL_COSTUMER WHERE CODE_CUSTOMER = @CODE_CUSTOMER)))
MERGE [acsa].SWIFT_TASKS TRG
