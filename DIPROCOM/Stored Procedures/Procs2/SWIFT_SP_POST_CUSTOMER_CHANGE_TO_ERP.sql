...
-- EJEMPLO DE EJECUCION: 
		EXEC [PACASA].[SWIFT_SP_POST_CUSTOMER_CHANGE_TO_ERP]

=========================================================*/
CREATE PROCEDURE [PACASA].[SWIFT_SP_POST_CUSTOMER_CHANGE_TO_ERP]
AS
BEGIN
	SET NOCOUNT OFF;
	...
	SELECT
		...
		,[GPS]
		INTO #CHANGES
	FROM [PACASA].[SWIFT_CUSTOMER_CHANGE]
	...
	BEGIN
		...
		BEGIN TRY
			...
			IF (SELECT COUNT(*) FROM [DIPROCOM_SERVER].[PACASA].[dbo].[REPLICA_CLIENTE] WHERE [CODIGO_SONDA]=@CODE_CUSTOMER)>0
			BEGIN
				UPDATE [DIPROCOM_SERVER].[PACASA].[dbo].[REPLICA_CLIENTE]
			...
			EXEC [PACASA].[SWIFT_SP_SET_STATUS_SEND_CUSTOMER_CHANGE_TO_ERP] @CUSTOMER = @CUSTOMER, -- int
