...
-- Ejemplo de Ejecucion:
				-- 
				EXEC [DIPROCOM].[ERP_SP_GENERATE_DISCOUNT_LIST_BY_CUSTOMER]
					@DISCOUNT_TYPE = 2
					,@ORDER = 3
*/
-- =============================================
CREATE PROCEDURE [DIPROCOM].[ERP_SP_GENERATE_DISCOUNT_LIST_BY_CUSTOMER] (
	@DISCOUNT_TYPE INT
	,@ORDER INT
)
AS
BEGIN
	...
	IF @ORDER = 1
	BEGIN
		TRUNCATE TABLE MDA.ERP_TB_DISCOUNT
		--
		PRINT '----> Limpio tabla'
	END
	...
	BEGIN
		...
		BEGIN
			...
			SELECT
				...
				,S.CODE_SKU SKU	
				,@DISCOUNT DISCOUNT
			FROM MDA.ERP_VIEW_SKU S
			LEFT JOIN MDA.ERP_TB_DISCOUNT D ON (
	...
	-- ------------------------------------------------------------------------------------
	INSERT INTO MDA.ERP_TB_DISCOUNT
	SELECT
		[CODE_ROUTE]
		,[CODE_CUSTOMER]
		,[CODE_SKU]
