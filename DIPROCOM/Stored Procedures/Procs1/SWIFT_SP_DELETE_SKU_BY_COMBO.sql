...
-- Ejemplo de Ejecucion:
	      SELECT * FROM [DIPROCOM].SWIFT_COMBO 
  			SELECT * FROM [DIPROCOM].[SWIFT_SKU_BY_COMBO]
				--
				EXEC [DIPROCOM].[SWIFT_SP_DELETE_SKU_BY_COMBO]
					@COMBO_ID = 5
					,@CODE_SKU = '100002'
					,@PACK_UNIT = 8
				-- 
...
*/
-- =============================================
CREATE PROCEDURE [DIPROCOM].SWIFT_SP_DELETE_SKU_BY_COMBO (@COMBO_ID INT
, @CODE_SKU VARCHAR(50)
, @PACK_UNIT INT)
AS
BEGIN
  ...
  BEGIN TRY
    ...
    DELETE FROM [DIPROCOM].[SWIFT_SKU_BY_COMBO]
    WHERE [COMBO_ID] = @COMBO_ID
      AND [CODE_SKU] = @CODE_SKU
      AND [PACK_UNIT] = @PACK_UNIT
    --
    UPDATE [DIPROCOM].[SWIFT_COMBO]
