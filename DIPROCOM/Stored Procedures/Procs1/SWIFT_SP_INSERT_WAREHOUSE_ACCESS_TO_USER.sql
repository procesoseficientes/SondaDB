...
-- Ejemplo de Ejecucion:
				EXEC [DIPROCOM].[SWIFT_SP_INSERT_WAREHOUSE_ACCESS_TO_USER]
					@USER_CORRELATIVE = 1
					, @CODE_WAREHOUSE = 'BODEGA_CENTRAL'
				-- 
				SELECT * FROM [DIPROCOM].[SWIFT_WAREHOUSE_BY_USER_WITH_ACCESS] 
*/
-- =============================================
CREATE PROCEDURE [DIPROCOM].[SWIFT_SP_INSERT_WAREHOUSE_ACCESS_TO_USER](
...
BEGIN
	BEGIN TRY
		DECLARE @ID INT
		--
		INSERT INTO [DIPROCOM].[SWIFT_WAREHOUSE_BY_USER_WITH_ACCESS]
				(
					[USER_CORRELATIVE]
					,[CODE_WAREHOUSE]
				)
