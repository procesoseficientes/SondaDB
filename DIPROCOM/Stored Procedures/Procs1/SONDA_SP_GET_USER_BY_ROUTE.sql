...
-- Ejemplo de Ejecucion:
	EXECUTE[DIPROCOM].[SONDA_SP_GET_USER_BY_ROUTE] @CODE_ROUTE = 'ES000003'
GO
*/
-- =============================================
CREATE PROCEDURE [DIPROCOM].[SONDA_SP_GET_USER_BY_ROUTE]
...
BEGIN
	...
	FROM [DIPROCOM].[USERS] AS USR 
		LEFT JOIN [DIPROCOM].[SWIFT_WAREHOUSES] AS WRH ON (USR.[DEFAULT_WAREHOUSE] = WRH.[CODE_WAREHOUSE])
		LEFT JOIN [DIPROCOM].[SWIFT_WAREHOUSES] AS WRHT ON (USR.[PRESALE_WAREHOUSE] = WRHT.[CODE_WAREHOUSE])
		LEFT JOIN [DIPROCOM].[SWIFT_SELLER] AS SLLR ON ([SLLR].[SELLER_CODE] = [USR].[RELATED_SELLER])
		LEFT JOIN [DIPROCOM].[SWIFT_COMPANY] AS CMPN ON ([CMPN].[COMPANY_NAME] = [SLLR].[OWNER])
		LEFT JOIN [DIPROCOM].[SWIFT_COMPANY] AS CMP ON ([SLLR].[OWNER] = [CMP].[COMPANY_NAME])
