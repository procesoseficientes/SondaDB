...
-- Ejemplo de Ejecucion:
				-- 
				EXEC [DIPROCOM].[SWIFT_SP_GENERATE_BONUS_BY_TRADE_AGREEMENT]
					@CODE_ROUTE = '44'
				--
				SELECT * FROM [DIPROCOM].[SWIFT_BONUS_LIST_BY_SKU]
				SELECT * FROM [DIPROCOM].[SWIFT_BONUS_LIST_BY_SKU_MULTIPLE]
				SELECT * FROM [DIPROCOM].[SWIFT_BONUS_LIST_BY_COMBO]
				SELECT * FROM [DIPROCOM].[SWIFT_BONUS_LIST_BY_COMBO_SKU]
				SELECT * FROM [DIPROCOM].[SWIFT_BONUS_LIST_BY_GENERAL_AMOUNT]
*/
-- =============================================
CREATE PROCEDURE [DIPROCOM].[SWIFT_SP_GENERATE_BONUS_BY_TRADE_AGREEMENT] (@CODE_ROUTE VARCHAR(250)) 
AS
BEGIN
  SET NOCOUNT ON;
  -- ------------------------------------------------------------------------------------
  ...
  , [NAME_BONUS_LIST])
    ...
    LEFT JOIN [DIPROCOM].[SWIFT_TRADE_AGREEMENT] [TA]
      ON (
      [TAC].[TRADE_AGREEMENT_ID] = [TA].[TRADE_AGREEMENT_ID]
      )
  ...
  -- ------------------------------------------------------------------------------------
  INSERT INTO DIPROCOM.[SWIFT_BONUS_LIST_BY_CUSTOMER] ([BONUS_LIST_ID]
  , [CODE_CUSTOMER])
    SELECT DISTINCT
      [BL].[BONUS_LIST_ID]
     ,[TA].[CODE_CUSTOMER]
    FROM @TRADE_AGREEMENT_BY_CUSTOMER [TA]
    INNER JOIN [DIPROCOM].[SWIFT_BONUS_LIST] [BL]
      ON (
  ...
  BEGIN
    ...
    BEGIN
      SELECT TOP 1
        @IS_ALREADY = 1
      FROM [DIPROCOM].[SWIFT_BONUS_LIST_BY_CUSTOMER] [BLC]
      INNER JOIN [DIPROCOM].[SWIFT_BONUS_LIST] [BL]
        ON (
        [BL].[BONUS_LIST_ID] = [BLC].[BONUS_LIST_ID]
        )
      WHERE [BL].[CODE_ROUTE] = @CODE_ROUTE
      AND [BL].[NAME_BONUS_LIST] = (@CODE_ROUTE + '|' + @CODE_TRADE_AGREEMENT)
    END
    --
    IF @IS_ALREADY = 0
    BEGIN
  ...
  -- ------------------------------------------------------------------------------------
  -- Genera SKUs de la lista de bonificaciones
  -- ------------------------------------------------------------------------------------
  INSERT INTO DIPROCOM.[SWIFT_BONUS_LIST_BY_SKU] ([BONUS_LIST_ID]
  , [CODE_SKU]
  , [CODE_PACK_UNIT]
  , [LOW_LIMIT]
  , [HIGH_LIMIT]
  ...
  , [FREQUENCY])
    ...
    INNER JOIN [DIPROCOM].[SWIFT_PROMO_BONUS_BY_SCALE] [TAB]
      ON (
    ...
    INNER JOIN [DIPROCOM].[SONDA_PACK_UNIT] [SPU1]
      ON (
      [SPU1].[PACK_UNIT] = [TAB].[PACK_UNIT]
      )
    INNER JOIN [DIPROCOM].[SONDA_PACK_UNIT] [SPU2]
      ON (
  ...
  -- ------------------------------------------------------------------------------------
  INSERT [DIPROCOM].[SWIFT_BONUS_LIST_BY_SKU_MULTIPLE] ([BONUS_LIST_ID]
  , [CODE_SKU]
  , [CODE_PACK_UNIT]
  , [MULTIPLE]
  , [CODE_SKU_BONUS]
  ...
  , [FREQUENCY])
    ...
    INNER JOIN [DIPROCOM].[SWIFT_PROMO_BONUS_BY_MULTIPLE] [TAM]
      ON (
  ...
  -- ------------------------------------------------------------------------------------
  INSERT INTO [DIPROCOM].[SWIFT_BONUS_LIST_BY_COMBO] ([BONUS_LIST_ID]
  , [COMBO_ID]
  , [BONUS_TYPE]
  , [BONUS_SUB_TYPE]
  ...
  , [FREQUENCY])
    ...
    INNER JOIN [DIPROCOM].[SWIFT_PROMO_BY_BONUS_RULE] [TAB]
      ON (
      [TAB].[PROMO_ID] = [P].[PROMO_ID]
      )
    INNER JOIN [DIPROCOM].[SWIFT_PROMO_BY_COMBO_PROMO_RULE] [TAR]
      ON (
  ...
  -- Se agrega la configuracion de bonificaciones por combo
  -- ------------------------------------------------------------------------------------
  INSERT INTO [DIPROCOM].[SWIFT_BONUS_LIST_BY_COMBO_SKU] ([BONUS_LIST_ID]
  , [COMBO_ID]
  , [CODE_SKU]
  , [CODE_PACK_UNIT]
  , [QTY]
  , [IS_MULTIPLE])
    ...
    INNER JOIN [DIPROCOM].[SWIFT_PROMO_SKU_BY_PROMO_RULE] [TAS]
      ON (
    ...
    INNER JOIN [DIPROCOM].[SONDA_PACK_UNIT] [SPU]
      ON (
  ...
  -- ------------------------------------------------------------------------------------
  INSERT INTO [DIPROCOM].[SWIFT_BONUS_LIST_BY_GENERAL_AMOUNT] ([BONUS_LIST_ID]
  , [LOW_LIMIT]
  , [HIGH_LIMIT]
  , [CODE_SKU_BONUS]
  ...
  , [FREQUENCY])
    ...
    INNER JOIN DIPROCOM.[SWIFT_PROMO_BONUS_BY_GENERAL_AMOUNT] [BGA]
      ON (
