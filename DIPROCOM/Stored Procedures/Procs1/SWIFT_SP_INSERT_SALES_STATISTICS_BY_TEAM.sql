...
-- Ejemplo de Ejecucion:
        EXEC DIPROCOM.SWIFT_SP_INSERT_SALES_STATISTICS_BY_TEAM
        SELECT * FROM [DIPROCOM].[SWIFT_STATISTICS_GOALS_BY_SALES] 
*/
-- =============================================

CREATE PROCEDURE [DIPROCOM].[SWIFT_SP_INSERT_SALES_STATISTICS_BY_TEAM]
AS
BEGIN
  SET NOCOUNT ON;
  -- ---------------------------
  ...
  INSERT INTO @GOAL_HEADER_IN_PROGRESS ([GOAL_HEADER_ID], [GOAL_NAME], [TEAM_ID], [SUPERVISOR_ID], [GOAL_AMOUNT], [GOAL_DATE_FROM], [GOAL_DATE_TO], [INCLUDE_SATURDAY], [SALE_TYPE])
    SELECT
     ...
     ,[GH].[INCLUDE_SATURDAY]
     ,[GH].[SALE_TYPE]
    FROM [DIPROCOM].[SWIFT_GOAL_HEADER] [GH]
  ...
  INSERT INTO @GOAL_DETAIL_IN_PROGRESS ([GOAL_DETAIL_ID], [GOAL_HEADER_ID], [SELLER_ID], [GOAL_BY_SELLER], [DAILY_GOAL_BY_SELLER])
    ...
    FROM [DIPROCOM].[SWIFT_GOAL_DETAIL] [GD]
  ...
  BEGIN
    ...
    INSERT INTO @STATISTICS_GOALS_BY_SALES ([STATISTICS_GOAL_BY_SALE_ID], [GOAL_HEADER_ID], [USER_ID], [RANKING], [DAILY_GOAL], [ACCUMULATED_BY_PERIOD], [PERCENTAGE_GOAL_DAILY], [DAYS_OF_SALE], [REMAINING_DAYS], [PERCENTAGE_OF_DAYS], [GENERAL_GOAL], [DIFFERENCE_FROM_THE_GOAL], [NEXT_SALE_GOAL], [PERCENTAGE_OF_GENERAL_GOAL], [SALE_OF_THE_DAY], [SALES_DATE])
      SELECT
       ...
       ,[SGS].[SALE_OF_THE_DAY]
       ,[SGS].[SALES_DATE]
      FROM [DIPROCOM].[SWIFT_STATISTICS_GOALS_BY_SALES] [SGS]
    ...
    BEGIN
      ...
      BEGIN
        ...
        BEGIN
          PRINT 'RD-01'
          DELETE FROM [DIPROCOM].[SWIFT_STATISTICS_GOALS_BY_SALES]
          WHERE [GOAL_HEADER_ID] = @GOAL_HEADER_ID
        ...
        BEGIN
          ...
          INSERT INTO @SALES_ORDER ([RANKING], [USER_ID], [SELLER_CODE], [SELLER_NAME], [CODE_ROUTE], [NAME_ROUTE], [TOTAL_AMOUNT], [COUNT_SALES_ORDERS])
            SELECT
              ROW_NUMBER() OVER (ORDER BY SUM([DIPROCOM].[SWIFT_FN_GET_SALES_ORDER_TOTAL]([SH].[SALES_ORDER_ID])) DESC) AS RANKING
             ...
             ,ISNULL(SUM([DIPROCOM].[SWIFT_FN_GET_SALES_ORDER_TOTAL]([SH].[SALES_ORDER_ID])), 0) AS [TOTAL_AMOUNT]
             ,COUNT([SH].[SALES_ORDER_ID]) AS [COUNT_SALES_ORDERS]
            FROM [DIPROCOM].[SWIFT_GOAL_DETAIL] [DT]
            INNER JOIN [DIPROCOM].[USERS] [U]
              ON (
              [DT].[SELLER_ID] = [U].[CORRELATIVE]
              )
            INNER JOIN [DIPROCOM].[SWIFT_USER_BY_TEAM] [UT]
              ON (
            ...
            LEFT JOIN [DIPROCOM].[SWIFT_ROUTES] [R]
              ON (
              [U].[SELLER_ROUTE] = [R].[CODE_ROUTE]
              )
            LEFT JOIN [DIPROCOM].[SONDA_SALES_ORDER_HEADER] [SH]
              ON (
        ...
        BEGIN
          INSERT INTO @SALES_ORDER ([RANKING], [USER_ID], [SELLER_CODE], [SELLER_NAME], [CODE_ROUTE], [NAME_ROUTE], [TOTAL_AMOUNT], [COUNT_SALES_ORDERS])
            ...
            LEFT JOIN [DIPROCOM].[SONDA_POS_INVOICE_HEADER] [IH]
              ON (
        ...
        -- ---------------------------
        INSERT INTO [DIPROCOM].[SWIFT_STATISTICS_GOALS_BY_SALES] ([GOAL_HEADER_ID]
        , [TEAM_ID]
        , [USER_ID]
        , [SELLER_CODE]
        , [SELLER_NAME]
        ...
        , [LAST_CREATED], [SALE_TYPE])
          SELECT
           ...
           ,CASE
              WHEN [SGS].[REMAINING_DAYS] IS NULL THEN ([DIPROCOM].[SWIFT_FN_GET_LABOR_DAYS](@GOAL_DATE_FROM, @GOAL_DATE_TO, @INCLUDE_SATURDAY)) - 1
              ELSE [SGS].[REMAINING_DAYS] - 1
            END
           ,(100 / [DIPROCOM].[SWIFT_FN_GET_LABOR_DAYS](@GOAL_DATE_FROM, @GOAL_DATE_TO, @INCLUDE_SATURDAY) * (CASE
              WHEN [SGS].[REMAINING_DAYS] IS NULL THEN ([DIPROCOM].[SWIFT_FN_GET_LABOR_DAYS](@GOAL_DATE_FROM, @GOAL_DATE_TO, @INCLUDE_SATURDAY)) - 1
              ELSE [SGS].[REMAINING_DAYS] - 1
            END))
           ,CASE
