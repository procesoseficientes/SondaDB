...
-- Ejemplo de Ejecucion:
				EXEC [DIPROCOM].SWIFT_SP_DELETE_COMPLETE_PROMO_OF_BONUS_BY_SACALE
				@PROMO_ID = 2128
*/
-- =============================================
CREATE PROCEDURE [DIPROCOM].[SWIFT_SP_DELETE_COMPLETE_PROMO_OF_BONUS_BY_SACALE] (@PROMO_ID INT)
AS
BEGIN
  SET NOCOUNT ON;
  -- -----------------------------------------------------------------
  ...
  BEGIN TRY
    ...
    BEGIN
      ...
      -- --------------------------------------------------------------------------------
      -- Se eliminan los detalles de la promo por escala
      -- --------------------------------------------------------------------------------
      DELETE FROM [DIPROCOM].[SWIFT_PROMO_BONUS_BY_SCALE]
      WHERE [PROMO_ID] = @PROMOTION_ID
