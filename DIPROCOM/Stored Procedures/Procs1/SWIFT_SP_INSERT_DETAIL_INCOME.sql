CREATE PROCEDURE [PACASA].[SWIFT_SP_INSERT_DETAIL_INCOME]
@INCOMEHEADER INT,
@CODE_SKU VARCHAR(50),
@EXPECTED FLOAT,
...
IF EXISTS (SELECT CODE_SKU FROM [PACASA].SWIFT_TEMP_RECEPTION_DETAIL WHERE CODE_SKU = @CODE_SKU AND RECEPTION_HEADER = @INCOMEHEADER)
            BEGIN
