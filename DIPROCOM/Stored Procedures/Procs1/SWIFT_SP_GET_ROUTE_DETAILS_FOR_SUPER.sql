...
-- Ejemplo de Ejecucion:
				EXEC [DIPROCOM].[SWIFT_SP_GET_ROUTE_DETAILS_FOR_SUPER]
					@LOGIN = 'adolfo@DIPROCOM',
					@TASK_TYPE = 'sale'
*/
-- =============================================
CREATE PROCEDURE [DIPROCOM].[SWIFT_SP_GET_ROUTE_DETAILS_FOR_SUPER]
    (
     @LOGIN VARCHAR(50)
    ...
    SET NOCOUNT ON;
       ...
       BEGIN
              SELECT
                 ...
                 ,[T].[TASK_STATUS]
                 ,[H].[DOC_NUM]
                 ,[DIPROCOM].[SWIFT_FN_GET_SALES_ORDER_TOTAL]([H].[SALES_ORDER_ID]) [DOC_TOTAL]
              ...
              INNER JOIN [DIPROCOM].[SWIFT_TASKS] [T] ON [RP].[TASK_ID] = [T].[TASK_ID]
              LEFT JOIN [DIPROCOM].[SONDA_SALES_ORDER_HEADER] [H] ON [H].[TASK_ID] = [T].[TASK_ID]
              WHERE
       ...
       BEGIN
              ...
              FROM
                     [DIPROCOM].[SONDA_ROUTE_PLAN] [RP]
              INNER JOIN [DIPROCOM].[SWIFT_TASKS] [T] ON [RP].[TASK_ID] = [T].[TASK_ID]
              LEFT JOIN [DIPROCOM].[SONDA_POS_INVOICE_HEADER] [H] ON [H].[TASK_ID] = [T].[TASK_ID]
              WHERE
                     [RP].[ASSIGEND_TO] = @LOGIN
                     AND [RP].[TASK_TYPE] = @TASK_TYPE
              ORDER BY
