CREATE PROCEDURE [DIPROCOM].[SWIFT_GET_INCOME_PANEL_RECEPTION]
@INCOMEHEADER INT
AS
SELECT DISTINCT
	...
	, D.TASK_ID
FROM
	DIPROCOM.SWIFT_RECEPTION_HEADER AS A
	left JOIN DIPROCOM.SWIFT_VIEW_ALL_PROVIDERS B on(A.CODE_PROVIDER = B.CODE_PROVIDER)
	left JOIN DIPROCOM.SWIFT_VIEW_ALL_COSTUMER C on(A.CODE_PROVIDER = C.CODE_CUSTOMER)
	left JOIN DIPROCOM.SWIFT_TASKS D on(A.RECEPTION_HEADER = D.RECEPTION_NUMBER)
...
z.SCANNED 
FROM DIPROCOM.SWIFT_RECEPTION_DETAIL Z
