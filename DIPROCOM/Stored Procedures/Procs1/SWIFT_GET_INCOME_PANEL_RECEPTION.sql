CREATE PROCEDURE [PACASA].[SWIFT_GET_INCOME_PANEL_RECEPTION]
@INCOMEHEADER INT
AS
SELECT DISTINCT
	...
	, D.TASK_ID
FROM
	[PACASA].SWIFT_RECEPTION_HEADER AS A
	left JOIN [PACASA].SWIFT_VIEW_ALL_PROVIDERS B on(A.CODE_PROVIDER = B.CODE_PROVIDER)
	left JOIN [PACASA].SWIFT_VIEW_ALL_COSTUMER C on(A.CODE_PROVIDER = C.CODE_CUSTOMER)
	left JOIN [PACASA].SWIFT_TASKS D on(A.RECEPTION_HEADER = D.RECEPTION_NUMBER)
...
z.SCANNED 
FROM [PACASA].SWIFT_RECEPTION_DETAIL Z
