...
Ejemplo de Ejecución:
	...
	EXEC [DIPROCOM].[SWIFT_SP_GET_CUSTOMER_NOT_IN_TRADE_AGREEMENT] @TRADE_AGREEMENT_ID = 30 , @CODE_CUSTOMER = 'V1'

==================================================*/

CREATE PROCEDURE [DIPROCOM].[SWIFT_SP_GET_CUSTOMER_NOT_IN_TRADE_AGREEMENT] (
...
BEGIN
	SELECT
		...
		,CASE ISNULL([TA].[TRADE_AGREEMENT_ID], 1)
			...
			END AS [ASOCIADO]
	FROM
		[DIPROCOM].[SWIFT_VIEW_ALL_COSTUMER] AS [VAC]
	LEFT JOIN [DIPROCOM].[SWIFT_TRADE_AGREEMENT_BY_CUSTOMER] AS [TA] ON ([VAC].[CODE_CUSTOMER] = [TA].[CODE_CUSTOMER])
	LEFT JOIN [DIPROCOM].[SWIFT_SELLER] [svas] ON ([svas].[SELLER_CODE] = [VAC].[SELLER_DEFAULT_CODE])
	LEFT JOIN [DIPROCOM].[SWIFT_CHANNEL_X_CUSTOMER] [scxc] ON ([VAC].[CODE_CUSTOMER] = [scxc].[CODE_CUSTOMER])
	LEFT JOIN [DIPROCOM].[SWIFT_CHANNEL] [sc] ON (
