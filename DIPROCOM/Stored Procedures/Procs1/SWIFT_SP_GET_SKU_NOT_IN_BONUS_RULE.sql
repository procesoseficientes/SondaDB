...
-- Ejemplo de Ejecucion:
				EXEC [PACASA].[SWIFT_SP_GET_SKU_NOT_IN_BONUS_RULE]
					@TRADE_AGREEMENT_ID = 21
					,@COMBO_ID = 5
*/
-- =============================================
CREATE PROCEDURE [PACASA].[SWIFT_SP_GET_SKU_NOT_IN_BONUS_RULE](
...
BEGIN
	...
	LEFT JOIN [PACASA].[SWIFT_TRADE_AGREEMENT_SKU_BY_BONUS_RULE] [CRS] ON (
		[CRS].[CODE_SKU] = [S].[CODE_SKU]
		AND [CRS].[PACK_UNIT] = [PU].[PACK_UNIT]
	)	
	LEFT JOIN [PACASA].[SWIFT_TRADE_AGREEMENT_BY_COMBO_BONUS_RULE] [CR] ON(
		[CR].[TRADE_AGREEMENT_BONUS_RULE_BY_COMBO_ID] = [CRS].[TRADE_AGREEMENT_BONUS_RULE_BY_COMBO_ID]
		AND [CR].[COMBO_ID] = @COMBO_ID
	)
	LEFT JOIN [PACASA].[SWIFT_TRADE_AGREEMENT_BY_BONUS_RULE] [TAR] ON (
