...
-- Ejemplo de Ejecucion:
        EXEC [DIPROCOM].[SWIFT_SP_GET_SAP_RECEPTION] @pERP_DOC = 8
*/
-- =============================================
CREATE PROCEDURE [DIPROCOM].[SWIFT_SP_GET_SAP_RECEPTION]
	@pERP_DOC VARCHAR(50)
AS
BEGIN
	...
	SELECT @SQL = 'select * from openquery (ERPSERVER,''SELECT 
		...
		END AS COMMENTS
	FROM
	[BD_Prueba_Viscosa].dbo.por1 POD
	inner join [BD_Prueba_Viscosa].DBO.OPOR PO ON (po.DocEntry = pod.DocEntry)
	where 
		po.DocStatus=''''O''''
		AND po.DocType=''''I''''
		AND pod.OpenQty > 0
