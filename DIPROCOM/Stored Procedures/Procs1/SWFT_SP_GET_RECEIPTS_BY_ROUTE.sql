...
--exec [PACASA].[SWFT_SP_GET_RECEIPTS_BY_ROUTE] '2017-05-31 17:13:17.000','2019-05-31 17:13:17.000','gerente@DIPROCOM'
*/
-- =============================================
CREATE PROCEDURE [PACASA].[SWFT_SP_GET_RECEIPTS_BY_ROUTE]
...
AS
	SELECT
		...
		,[ST].[NAME_TEAM]
	FROM
		[PACASA].[SONDA_OVERDUE_INVOICE_PAYMENT_HEADER] [SIH]
	INNER JOIN [PACASA].[USERS] [US]
	ON	[US].[SELLER_ROUTE] = [SIH].[CODE_ROUTE]
	INNER JOIN [PACASA].[SWIFT_VIEW_ALL_COSTUMER] [SC]
	ON	[SC].[CODE_CUSTOMER] = [SIH].[CODE_CUSTOMER]
	INNER JOIN [PACASA].[SWIFT_USER_BY_TEAM] [SUT]
	ON	[US].[CORRELATIVE] = [SUT].[USER_ID]
	INNER JOIN [PACASA].[SWIFT_TEAM] [ST]
	ON	[ST].[TEAM_ID] = [SUT].[TEAM_ID]
	INNER JOIN [PACASA].[USERS] [SUP]
