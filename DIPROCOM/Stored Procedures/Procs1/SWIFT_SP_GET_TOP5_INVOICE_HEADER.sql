...
-- Ejemplo de Ejecucion:
      ...
      EXECUTE @RC = [DIPROCOM].SWIFT_SP_GET_TOP5_INVOICE_HEADER
      GO
*/
CREATE PROCEDURE [DIPROCOM].[SWIFT_SP_GET_TOP5_INVOICE_HEADER]
AS
BEGIN
	--
	...
	SELECT
		@INTERFACE_PAYMENT_TYPE = [DIPROCOM].[SWIFT_FN_GET_PARAMETER]('INVOICE' , 'INTERFACE_PAYMENT_TYPE')
		,@SHIPPING_ATTEMPTS = [DIPROCOM].[SWIFT_FN_GET_PARAMETER]('INVOICE' , 'SHIPPING_ATTEMPTS');
	...
	SELECT TOP 5
		[SPIH].[ID]
	FROM [DIPROCOM].[SONDA_POS_INVOICE_HEADER] [SPIH]
	INNER JOIN [DIPROCOM].[SWIFT_VIEW_ALL_COSTUMER] [SVAC] ON ([SPIH].[CLIENT_ID] = [SVAC].[CODE_CUSTOMER])
	WHERE ISNULL([IS_POSTED_ERP], 0) = 0
		AND ISNULL([SPIH].[VOIDED_INVOICE], 0) = 0
		AND ISNULL([SPIH].[IS_DRAFT], 0) = 0
		AND [SPIH].[IS_READY_TO_SEND] = 1
		AND SPIH.TOTAL_AMOUNT= (SELECT SUM([OD].[TOTAL_LINE]) FROM [DIPROCOM].[SONDA_POS_INVOICE_DETAIL] [OD] WHERE [OD].[ID]=[SPIH].[ID])
	...
	FROM [DIPROCOM].[SONDA_POS_INVOICE_HEADER] [SIH]
	...
	FROM
		[DIPROCOM].[SONDA_POS_INVOICE_HEADER] [spih] 
		INNER JOIN @INVOICE [IV] ON [IV].[INVOICE_ID] = [spih].[ID]
		INNER JOIN [DIPROCOM].[SWIFT_ROUTES] [sr] ON	([spih].[POS_TERMINAL] = [sr].[CODE_ROUTE])
		INNER JOIN [DIPROCOM].[USERS] [u] ON	([u].[SELLER_ROUTE] = [sr].[CODE_ROUTE])
		INNER JOIN [DIPROCOM].[SWIFT_WAREHOUSES] [sw] ON	([u].[DEFAULT_WAREHOUSE] = [sw].[CODE_WAREHOUSE])
		INNER JOIN [DIPROCOM].[SWIFT_VIEW_ALL_COSTUMER] [svac] ON	([spih].[CLIENT_ID] = [svac].[CODE_CUSTOMER])
		LEFT JOIN [DIPROCOM].[SWIFT_VIEW_COST_CENTER_BY_WAREHOUSE] [CCW] ON	([sw].[CODE_WAREHOUSE] = [CCW].[Descr] COLLATE SQL_Latin1_General_CP1_CI_AS)
	ORDER BY
		([spih].[ATTEMPTED_WITH_ERROR]) ASC
		,[spih].[POSTED_DATETIME] ASC
END;
