...
-- Ejemplo de Ejecucion:
				EXEC [DIPROCOM].[SWIFT_SP_GENERATE_DELIVERY_PLAN_FROM_MANIFEST_3PL] @MANIFEST_HADER_ID = 2145, @LOGIN_ID = 'ADOLFO@DIPROCOM', @CURRENT_GPS_USER = '14.64986000,-90.53980000'
*/
-- =============================================
CREATE PROCEDURE [DIPROCOM].[SWIFT_SP_GENERATE_DELIVERY_PLAN_FROM_MANIFEST_3PL]
(
        ...
        SELECT @GPS_WAREHOUSE = CASE
            ...
            (CONVERT(VARCHAR, [DC].[LATITUDE]) + ',' + CONVERT(VARCHAR, [DC].[LONGITUDE]))
                                    ELSE
                                        [SS].[GPS]
                                END
        FROM [DIPROCOM].[SWIFT_SELLER] AS [SS]
            INNER JOIN [DIPROCOM].[USERS] AS [U]
                ON ([U].[RELATED_SELLER] = [SS].[SELLER_CODE])
            INNER JOIN [DIPROCOM].[SWIFT_DISTRIBUTION_CENTER] AS [DC]
                ON ([U].[DISTRIBUTION_CENTER_ID] = [DC].[DISTRIBUTION_CENTER_ID])
            INNER JOIN [DIPROCOM].[SWIFT_WAREHOUSE_X_DISTRIBUTION_CENTER] AS [WXD]
                ON (
        ...
        BEGIN
            ...
            BEGIN
                ...
                FROM [#CLIENTS_FILTERED] [T]
                    INNER JOIN [DIPROCOM].[SWIFT_VIEW_ALL_COSTUMER] [VC]
