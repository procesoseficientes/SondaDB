...
-- Ejemplo de Ejecucion:
        EXEC [acsa].[SWIFT_SP_INSERT_TRANSFER_DETAIL]
			@TRANSFER_ID = 55
			,@SKU_CODE = '100003'
			,@QTY = 2
			,@SERIE = 'ASDB12315'
		--
		SELECT * FROM [acsa].[SWIFT_TRANSFER_DETAIL] WHERE [TRANSFER_ID] = 55
*/
-- =============================================
CREATE PROCEDURE [acsa].[SWIFT_SP_INSERT_TRANSFER_DETAIL] (
...
BEGIN
	BEGIN TRY
		BEGIN TRAN [t1];
		--
		DELETE [acsa].[SWIFT_TRANSFER_DETAIL]
		WHERE [TRANSFER_ID] = @TRANSFER_ID
			AND [SKU_CODE] = @SKU_CODE
      AND ISNULL([SERIE],'NA') = ISNULL(@SERIE,'NA')
		--		
		INSERT	INTO [acsa].[SWIFT_TRANSFER_DETAIL]
				(
					[TRANSFER_ID]
					,[SKU_CODE]
					,[QTY]
