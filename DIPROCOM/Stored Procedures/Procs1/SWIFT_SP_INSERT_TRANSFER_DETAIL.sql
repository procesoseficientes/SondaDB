...
-- Ejemplo de Ejecucion:
        EXEC [DIPROCOM].[SWIFT_SP_INSERT_TRANSFER_DETAIL]
			@TRANSFER_ID = 55
			,@SKU_CODE = '100003'
			,@QTY = 2
			,@SERIE = 'ASDB12315'
		--
		SELECT * FROM [DIPROCOM].[SWIFT_TRANSFER_DETAIL] WHERE [TRANSFER_ID] = 55
*/
-- =============================================
CREATE PROCEDURE [DIPROCOM].[SWIFT_SP_INSERT_TRANSFER_DETAIL] (
...
BEGIN
	BEGIN TRY
		BEGIN TRAN [t1];
		--
		DELETE [DIPROCOM].[SWIFT_TRANSFER_DETAIL]
		WHERE [TRANSFER_ID] = @TRANSFER_ID
			AND [SKU_CODE] = @SKU_CODE
      AND ISNULL([SERIE],'NA') = ISNULL(@SERIE,'NA')
		--		
		INSERT	INTO [DIPROCOM].[SWIFT_TRANSFER_DETAIL]
				(
					[TRANSFER_ID]
					,[SKU_CODE]
					,[QTY]
