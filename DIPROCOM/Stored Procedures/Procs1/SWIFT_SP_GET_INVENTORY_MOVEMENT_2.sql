CREATE PROCEDURE [DIPROCOM].[SWIFT_SP_GET_INVENTORY_MOVEMENT_2](
...
BEGIN
	...
	LEFT JOIN [DIPROCOM].[SWIFT_PALLET] [PS] ON (
		[PS].[PALLET_ID] = [TH].[TXN_SOURCE_PALLET_ID]
	)
	LEFT JOIN [DIPROCOM].[SWIFT_BATCH] [BS] ON (
		[BS].[BATCH_ID] = [PS].[BATCH_ID]
	)
	LEFT JOIN [DIPROCOM].[SWIFT_PALLET] [PT] ON (
		[PT].[PALLET_ID] = [TH].[TXN_TARGET_PALLET_ID]
	)
	LEFT JOIN [DIPROCOM].[SWIFT_BATCH] [BT] ON (
