...
-- Ejemplo de Ejecucion:
				EXEC [acsa].[SWIFT_SP_GET_PROMO_SPECIAL_PRICE_LIST_BY_SCALE]
				@PROMO_ID = 3360
*/
-- =============================================
CREATE PROCEDURE [acsa].[SWIFT_SP_GET_PROMO_SPECIAL_PRICE_LIST_BY_SCALE](
	@PROMO_ID INT = NULL
)
AS
BEGIN
	...
	FROM [acsa].[SONDA_PACK_UNIT] [PU1] 
		INNER JOIN [acsa].[SWIFT_PROMO_SPECIAL_PRICE_LIST_BY_SCALE] [PDS] ON [PU1].[PACK_UNIT] = [PDS].[PACK_UNIT]
		INNER JOIN [acsa].[SWIFT_VIEW_ALL_SKU] [VAS1] ON [VAS1].[CODE_SKU] = [PDS].[CODE_SKU]	
		LEFT JOIN [acsa].[SWIFT_FAMILY_SKU] FS ON [FS].[CODE_FAMILY_SKU] = [VAS1].[CODE_FAMILY_SKU]
