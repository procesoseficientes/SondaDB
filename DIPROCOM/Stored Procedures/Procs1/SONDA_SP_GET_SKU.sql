...
BEGIN
	...
	INTO #RESULT
	FROM [#PRESALE_SKU] [I]
	INNER JOIN [SONDA].[SWIFT_PRICE_LIST_BY_SKU_PACK_SCALE_FOR_ROUTE] [splbs] ON ([I].[SKU] = [splbs].[CODE_SKU])
	...
	SELECT 
		...
		,[PU].[CODE_PACK_UNIT]
	FROM [#RESULT] [R]
	INNER JOIN [SONDA].[SONDA_PACK_CONVERSION] [PC] ON ([PC].[CODE_SKU] = [R].[SKU])
	INNER JOIN [SONDA].[SONDA_PACK_UNIT] [PU] ON ([PU].[CODE_PACK_UNIT] = [PC].[CODE_PACK_UNIT_FROM])
	LEFT JOIN [SONDA].[SWIFT_FAMILY_SKU] AS F ON [F].[CODE_FAMILY_SKU] = [R].[CODE_FAMILY_SKU]
	WHERE [PC].[ORDER] = 1
