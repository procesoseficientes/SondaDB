...
-- Ejemplo de Ejecucion:
		EXEC [PACASA].[SONDA_SP_GET_SKU]
			@CODE_ROUTE ='RP-27';
*/
-- =============================================
CREATE PROCEDURE [PACASA].[SONDA_SP_GET_SKU] (
...
BEGIN
	...
	INNER JOIN [PACASA].[SWIFT_PRICE_LIST_BY_CUSTOMER_FOR_ROUTE] [splbc] ON ([srp].[RELATED_CLIENT_CODE] = [splbc].[CODE_CUSTOMER] AND [splbc].[CODE_ROUTE] = [srp].[CODE_ROUTE])
	WHERE [srp].[CODE_ROUTE] = @CODE_ROUTE;

	INSERT INTO [#PRICE_LIST] ([CODE_PRICE_LIST])
	 SELECT DISTINCT [PLBC].[CODE_PRICE_LIST]
    FROM [PACASA].[SWIFT_VIEW_ALL_COSTUMER] [SVAC]
	INNER JOIN [PACASA].[USERS] [US]
	  ON [US].[RELATED_SELLER] = [SVAC].[SELLER_DEFAULT_CODE] COLLATE DATABASE_DEFAULT
	INNER JOIN [PACASA].[SWIFT_PRICE_LIST_BY_CUSTOMER_FOR_ROUTE] [PLBC]
	...
	--
	INSERT INTO [#PRICE_LIST] ([CODE_PRICE_LIST])
	SELECT '-1'--[PACASA].[SWIFT_FN_GET_PARAMETER]('ERP_HARDCODE_VALUES','PRICE_LIST');
	...
	INTO #RESULT
	FROM [#PRESALE_SKU] [I]
	INNER JOIN [PACASA].[SWIFT_PRICE_LIST_BY_SKU_PACK_SCALE_FOR_ROUTE] [splbs] ON ([I].[SKU] = [splbs].[CODE_SKU] COLLATE DATABASE_DEFAULT)
	INNER JOIN [#PRICE_LIST] [pl] ON ([pl].[CODE_PRICE_LIST] = [splbs].[CODE_PRICE_LIST]COLLATE DATABASE_DEFAULT)
	INNER JOIN [PACASA].[SWIFT_VIEW_ALL_SKU] [S] ON ([I].[SKU] = [S].[CODE_SKU]COLLATE DATABASE_DEFAULT)
	LEFT JOIN [PACASA].[SWIFT_PORTFOLIO_BY_SKU] [PS] ON ([PS].[CODE_SKU] = [I].[SKU]COLLATE DATABASE_DEFAULT)
	...
	SELECT 
		...
		,[PU].[CODE_PACK_UNIT]
	FROM [#RESULT] [R]
	INNER JOIN [PACASA].[SONDA_PACK_CONVERSION] [PC] ON ([PC].[CODE_SKU] = [R].[SKU]COLLATE DATABASE_DEFAULT)
	INNER JOIN [PACASA].[SONDA_PACK_UNIT] [PU] ON ([PU].[CODE_PACK_UNIT] = [PC].[CODE_PACK_UNIT_FROM]COLLATE DATABASE_DEFAULT)
	LEFT JOIN [PACASA].[SWIFT_FAMILY_SKU] AS F ON [F].[CODE_FAMILY_SKU] = [R].[CODE_FAMILY_SKU]COLLATE DATABASE_DEFAULT
	WHERE [PC].[ORDER] = 1
