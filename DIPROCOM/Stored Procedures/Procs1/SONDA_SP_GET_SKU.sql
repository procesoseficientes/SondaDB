...
-- Ejemplo de Ejecucion:
		EXEC [DIPROCOM].[SONDA_SP_GET_SKU]
			@CODE_ROUTE = 'GUA0032@ARIUM';
*/
-- =============================================
CREATE PROCEDURE [DIPROCOM].[SONDA_SP_GET_SKU] (
...
BEGIN
	...
	INNER JOIN [DIPROCOM].[SWIFT_PRICE_LIST_BY_CUSTOMER_FOR_ROUTE] [splbc] ON ([srp].[RELATED_CLIENT_CODE] = [splbc].[CODE_CUSTOMER] AND [splbc].[CODE_ROUTE] = [srp].[CODE_ROUTE])
	WHERE [srp].[CODE_ROUTE] = @CODE_ROUTE;

	INSERT INTO [#PRICE_LIST] ([CODE_PRICE_LIST])
	 SELECT DISTINCT [PLBC].[CODE_PRICE_LIST]
    FROM [DIPROCOM].[SWIFT_VIEW_ALL_COSTUMER] [SVAC]
	INNER JOIN [DIPROCOM].[USERS] [US]
	  ON [US].[RELATED_SELLER] = [SVAC].[SELLER_DEFAULT_CODE]
	INNER JOIN [DIPROCOM].[SWIFT_PRICE_LIST_BY_CUSTOMER_FOR_ROUTE] [PLBC]
	...
	--
	INSERT INTO [#PRICE_LIST] ([CODE_PRICE_LIST])
	SELECT '-1'--[DIPROCOM].[SWIFT_FN_GET_PARAMETER]('ERP_HARDCODE_VALUES','PRICE_LIST');
	...
	FROM [DIPROCOM].[SWIFT_EVENT] [se]
	INNER JOIN [DIPROCOM].[SWIFT_RULE_X_EVENT] [srxe] ON [se].[EVENT_ID] = [srxe].[EVENT_ID]
	INNER JOIN [DIPROCOM].[SWIFT_RULE_X_ROUTE] [srxr] ON [srxe].[RULE_ID] = [srxr].[RULE_ID]
	...
	INTO [#PRESALE_SKU]
	FROM [DIPROCOM].[SONDA_IS_COMITED_BY_WAREHOUSE] [CW]
		INNER JOIN [DIPROCOM].[SWIFT_INVENTORY] [I] ON (
			[CW].[CODE_WAREHOUSE] = [I].[WAREHOUSE]
			AND [I].[SKU] = [CW].[CODE_SKU]
		)
	WHERE [CW].[CODE_WAREHOUSE] = '04'
	AND [I].[ON_HAND] > 0
	GROUP BY
		[I].[WAREHOUSE]
		,[I].[SKU]
	...
	INTO #RESULT
	FROM [#PRESALE_SKU] [I]
	INNER JOIN [DIPROCOM].[SWIFT_PRICE_LIST_BY_SKU_PACK_SCALE_FOR_ROUTE] [splbs] ON ([I].[SKU] = [splbs].[CODE_SKU])
	INNER JOIN [#PRICE_LIST] [pl] ON ([pl].[CODE_PRICE_LIST] = [splbs].[CODE_PRICE_LIST])
	INNER JOIN [DIPROCOM].[SWIFT_VIEW_ALL_SKU] [S] ON ([I].[SKU] = [S].[CODE_SKU])
	LEFT JOIN [DIPROCOM].[SWIFT_PORTFOLIO_BY_SKU] [PS] ON ([PS].[CODE_SKU] = [I].[SKU])
	...
	BEGIN
		...
		SELECT
			...
			,[S].[OWNER_ID]
			,[S].[HANDLE_SERIAL_NUMBER]
		FROM [DIPROCOM].[SONDA_POS_SKUS] [I]
		INNER JOIN [DIPROCOM].[SWIFT_VIEW_ALL_SKU] [S] ON ([I].[SKU] = [S].[CODE_SKU])
		LEFT JOIN [DIPROCOM].[SWIFT_PORTFOLIO_BY_SKU] [PS] ON ([PS].[CODE_SKU] = [I].[SKU])
	...
	SELECT 
		...
		,[PU].[CODE_PACK_UNIT]
	FROM [#RESULT] [R]
	INNER JOIN [DIPROCOM].[SONDA_PACK_CONVERSION] [PC] ON ([PC].[CODE_SKU] = [R].[SKU])
	INNER JOIN [DIPROCOM].[SONDA_PACK_UNIT] [PU] ON ([PU].[CODE_PACK_UNIT] = [PC].[CODE_PACK_UNIT_FROM])
	LEFT JOIN [DIPROCOM].[SWIFT_FAMILY_SKU] AS F ON [F].[CODE_FAMILY_SKU] = [R].[CODE_FAMILY_SKU]
	WHERE [PC].[ORDER] = 1
