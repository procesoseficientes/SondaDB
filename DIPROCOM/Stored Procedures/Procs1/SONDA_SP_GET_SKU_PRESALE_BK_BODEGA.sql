...
-- Ejemplo de Ejecucion:
				exec [PACASA].[SONDA_SP_GET_SKU_PRESALE] @WAREHOUSES = 'C002'
				  CODIGO BODEGA
...
*/
-- =============================================
CREATE PROCEDURE [PACASA].[SONDA_SP_GET_SKU_PRESALE_BK_BODEGA]
...
BEGIN
	...
	--
	--SELECT @ListaPrecios = [PACASA].[SWIFT_FN_GET_PRICE_LIST](DEFAULT)
	SELECT @ListaPrecios = [PACASA].[SWIFT_FN_GET_PRICE_LIST_BY_WH] (@WAREHOUSES)--(DEFAULT)
	--	
	SELECT 
		[WAREHOUSE]
		,I.[SKU]
	...
	LEFT JOIN [PACASA].SWIFT_VIEW_ALL_SKU S ON (I.SKU = S.CODE_SKU AND P.CODE_SKU = S.CODE_SKU)
