...
-- Ejemplo de Ejecucion:
				EXEC [DIPROCOM].[SWIFT_SP_GET_DISCOUNT_FROM_PROMO_OF_DISCOUNT_BY_SCALE]
				@PROMO_ID = 2121
*/
-- =============================================
CREATE PROCEDURE [DIPROCOM].[SWIFT_SP_GET_DISCOUNT_FROM_PROMO_OF_DISCOUNT_BY_SCALE](
	@PROMO_ID INT = NULL
)
AS
BEGIN
	...
	SELECT [PDS].[PROMO_DISCOUNT_ID]
			...
			[PDS].IS_UNIQUE
	FROM [DIPROCOM].[SONDA_PACK_UNIT] [PU1] 
		INNER JOIN [DIPROCOM].[SWIFT_PROMO_DISCOUNT_BY_SCALE] [PDS] ON [PU1].[PACK_UNIT] = [PDS].[PACK_UNIT]
		INNER JOIN [DIPROCOM].[SWIFT_VIEW_ALL_SKU] [VAS1] ON [VAS1].[CODE_SKU] = [PDS].[CODE_SKU]	
		LEFT JOIN DIPROCOM.[SWIFT_FAMILY_SKU] FS ON [FS].[CODE_FAMILY_SKU] = [VAS1].[CODE_FAMILY_SKU]
