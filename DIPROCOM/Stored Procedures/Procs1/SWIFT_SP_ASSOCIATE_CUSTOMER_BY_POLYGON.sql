...
-- Ejemplo de Ejecucion:
				EXEC [acsa].SWIFT_SP_ASSOCIATE_CUSTOMER_BY_POLYGON @POLYGON_ID = 1
*/
-- =============================================


CREATE PROCEDURE [acsa].[SWIFT_SP_ASSOCIATE_CUSTOMER_BY_POLYGON](
  @POLYGON_ID INT 
)
AS
...
SET @GEOMETRY_POLYGON = [acsa].[SWIFT_GET_GEOMETRY_POLYGON_BY_POLIGON_ID](@POLYGON_ID);
...
BEGIN
  ...
  INSERT INTO @NEW_CUSTOMERS ([CODE_CUSTOMER])
   ...
   FROM  @NEW_CUSTOMERS NC
    ...
    DELETE CAP 
    FROM [acsa].[SWIFT_CUSTOMER_GPS_ASSOCIATE_TO_POLYGON] CAP 
    INNER JOIN  @DELETED_CUSTOMERS DC ON (
      [DC].[CODE_CUSTOMER] = [CAP].[CODE_CUSTOMER]
    )
END
ELSE IF (@POLYGON_TYPE = 'SECTOR') BEGIN
    ...
    SELECT
      [CAP].[CODE_CUSTOMER]
    FROM [acsa].[SWIFT_POLYGON] [P]
    INNER JOIN [acsa].[SWIFT_CUSTOMER_ASSOCIATE_TO_POLYGON] [CAP]
      ON (
    ...
    INSERT INTO [acsa].[SWIFT_CUSTOMER_ASSOCIATE_TO_POLYGON]
					(
						[POLYGON_ID]
						,[POLYGON_NAME]
						,[POLYGON_TYPE]
