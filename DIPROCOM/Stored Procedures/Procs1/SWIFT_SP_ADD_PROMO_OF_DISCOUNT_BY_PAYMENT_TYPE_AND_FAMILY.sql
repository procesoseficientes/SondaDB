...
-- Ejemplo de Ejecucion:
				EXEC [acsa].[SWIFT_SP_GET_PROMO_ASSOCIATED_TO_TRADE_AGREEMENT] 
					@TRADE_AGREEMENT_ID = 1
*/
-- =============================================
CREATE PROCEDURE [acsa].[SWIFT_SP_ADD_PROMO_OF_DISCOUNT_BY_PAYMENT_TYPE_AND_FAMILY] 
(@PROMO_ID INT
, @XML XML
, @LOGIN_ID VARCHAR)
AS
BEGIN
	...
	FROM @TABLE_FAMILY F 
	INNER JOIN [acsa].[SWIFT_PROMO_DISCOUNT_BY_PAYMENT_TYPE_AND_FAMILY] DPF ON(
	...
	IF NOT EXISTS (SELECT TOP 1 1 FROM @TABLE_RESULT)BEGIN
      ...
      INSERT INTO [acsa].[SWIFT_PROMO_DISCOUNT_BY_PAYMENT_TYPE_AND_FAMILY] 
	  ([PROMO_ID]
      , [CODE_FAMILY_SKU]
	  , [PAYMENT_TYPE]
      , [DISCOUNT]
