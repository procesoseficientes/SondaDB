...
-- Ejemplo de Ejecucion:
				EXEC [PACASA].[SONDA_SP_GET_SALE_STATISTIC_BY_SELLER]
				@CODE_ROUTE = '46'
*/
-- =============================================
CREATE PROCEDURE [PACASA].[SONDA_SP_GET_SALE_STATISTIC_BY_SELLER]
	(
...
AS
	...
	-- --------------------------------------------------------------------------------------------------------------------------------------------
		...
		FROM
			[PACASA].[SWIFT_STATISTICS_GOALS_BY_SALES] [S]
		WHERE
	...
	-- --------------------------------------------------------------------------------------------------------------------------------------------
			SELECT
				...
				,(
					SELECT TOP 1
						[GH].[GOAL_NAME]
					FROM
						[PACASA].[SWIFT_GOAL_HEADER] AS [GH]
					WHERE
				...
				,(CAST([RANKING] AS VARCHAR) + ' / '
					+ (CAST((
								SELECT
									COUNT(*)
								FROM
									[PACASA].[SWIFT_USER_BY_TEAM] AS [T]
								WHERE
									[T].[TEAM_ID] = [S].[TEAM_ID]
				...
				,(
					SELECT TOP 1
						[GD].[GOAL_BY_SELLER]
					FROM
						[PACASA].[SWIFT_GOAL_DETAIL] AS [GD]
					WHERE
			...
			FROM
				[PACASA].[SWIFT_STATISTICS_GOALS_BY_SALES] AS [S]
			WHERE
				[S].[CODE_ROUTE] = @CODE_ROUTE
				AND [S].[LAST_CREATED] = 1
				AND [S].[SALE_TYPE] = @SELLER_TYPE;
		...
		BEGIN
			...
			IF (@GOAL_HEADER_ID > 0) BEGIN
				...
				SELECT
					...
					,[PACASA].[SWIFT_FN_GET_LABOR_DAYS]([GH].[GOAL_DATE_FROM] ,
														[GH].[GOAL_DATE_TO] ,
														[GH].[INCLUDE_SATURDAY]) AS [REMAINING_DAYS]
					,@DAILY_GOAL_OF_SELLER AS [GOAL_AMOUNT_OF_DAY]
				FROM
					[PACASA].[SWIFT_GOAL_HEADER] AS [GH]
				INNER JOIN [PACASA].[SWIFT_TEAM] AS [T]
