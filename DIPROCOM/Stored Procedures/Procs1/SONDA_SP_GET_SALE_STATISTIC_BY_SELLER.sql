...
-- Ejemplo de Ejecucion:
				EXEC [DIPROCOM].[SONDA_SP_GET_SALE_STATISTIC_BY_SELLER]
				@CODE_ROUTE = '46'
*/
-- =============================================
CREATE PROCEDURE [DIPROCOM].[SONDA_SP_GET_SALE_STATISTIC_BY_SELLER]
	(
...
AS
	...
	-- --------------------------------------------------------------------------------------------------------------------------------------------
		...
		FROM
			[DIPROCOM].[SWIFT_STATISTICS_GOALS_BY_SALES] [S]
		WHERE
	...
	-- --------------------------------------------------------------------------------------------------------------------------------------------
			SELECT
				...
				,(
					SELECT TOP 1
						[GH].[GOAL_NAME]
					FROM
						[DIPROCOM].[SWIFT_GOAL_HEADER] AS [GH]
					WHERE
				...
				,(CAST([RANKING] AS VARCHAR) + ' / '
					+ (CAST((
								SELECT
									COUNT(*)
								FROM
									[DIPROCOM].[SWIFT_USER_BY_TEAM] AS [T]
								WHERE
									[T].[TEAM_ID] = [S].[TEAM_ID]
				...
				,(
					SELECT TOP 1
						[GD].[GOAL_BY_SELLER]
					FROM
						[DIPROCOM].[SWIFT_GOAL_DETAIL] AS [GD]
					WHERE
			...
			FROM
				[DIPROCOM].[SWIFT_STATISTICS_GOALS_BY_SALES] AS [S]
			WHERE
				[S].[CODE_ROUTE] = @CODE_ROUTE
				AND [S].[LAST_CREATED] = 1
				AND [S].[SALE_TYPE] = @SELLER_TYPE;
		...
		BEGIN
			...
			FROM
				[DIPROCOM].[SWIFT_GOAL_DETAIL] AS [GD]
			INNER JOIN [DIPROCOM].[SWIFT_GOAL_HEADER] AS [GH]
			ON	[GH].[GOAL_HEADER_ID] = [GD].[GOAL_HEADER_ID]
			INNER JOIN [DIPROCOM].[USERS] AS [U]
			...
			IF (@GOAL_HEADER_ID > 0) BEGIN
				...
				SELECT
					...
					,[DIPROCOM].[SWIFT_FN_GET_LABOR_DAYS]([GH].[GOAL_DATE_FROM] ,
														[GH].[GOAL_DATE_TO] ,
														[GH].[INCLUDE_SATURDAY]) AS [REMAINING_DAYS]
					,@DAILY_GOAL_OF_SELLER AS [GOAL_AMOUNT_OF_DAY]
				FROM
					[DIPROCOM].[SWIFT_GOAL_HEADER] AS [GH]
				INNER JOIN [DIPROCOM].[SWIFT_TEAM] AS [T]
