...
--Yo Pedro Pablo Loukota cambio el procedimiento almacenado [DIPROCOM].[SONDA_SP_GET_SALE_X_DATE] el 15 de Diciembre del 2015.
...
-- Ejemplo de Ejecucion:
	exec [DIPROCOM].[SONDA_SP_GET_SALE_X_DATE] @POSTED_DATETIME='20160511 00:00:00.000', @CLOSED_ROUTE_DATETIME='20160822 00:00:00.000', @LOGIN='gerente@DIPROCOM', @IS_AUTHORIZED = 0

*/
-- =============================================
CREATE PROCEDURE [DIPROCOM].SONDA_SP_GET_SALE_X_DATE
...
SET NOCOUNT ON;
	...
	SELECT 
		...
		,REFERENCE_ID
		,CONVERT(DECIMAL(18,' + CAST(@DEFAULT_DISPLAY_DECIMALS AS VARCHAR) + '),[DIPROCOM].SWIFT_FN_GET_DISPLAY_NUMBER(E.TOTAL_AMOUNT)) [TOTAL_AMOUNT]
		,W.CODE_WAREHOUSE
		...
		,CASE E.SALES_ORDER_TYPE
			WHEN ''CASH'' THEN CONVERT(DECIMAL(18,' + CAST(@DEFAULT_DISPLAY_DECIMALS AS VARCHAR) + '),[DIPROCOM].SWIFT_FN_GET_DISPLAY_NUMBER(E.TOTAL_AMOUNT))
			ELSE 0
		END [CASH_AMOUNT]
		,CASE E.SALES_ORDER_TYPE
			WHEN ''CREDIT'' THEN CONVERT(DECIMAL(18,' + CAST(@DEFAULT_DISPLAY_DECIMALS AS VARCHAR) + '),[DIPROCOM].SWIFT_FN_GET_DISPLAY_NUMBER(E.TOTAL_AMOUNT))
			ELSE 0
		END [CREDIT_AMOUNT]
	  ,CONVERT(DECIMAL(18,' + CAST(@DEFAULT_DISPLAY_DECIMALS AS VARCHAR) + '),[DIPROCOM].SWIFT_FN_GET_DISPLAY_NUMBER(E.DISCOUNT)) [DISCOUNT]
	  ,CONVERT(DECIMAL(18,' + CAST(@DEFAULT_DISPLAY_DECIMALS AS VARCHAR) + '),[DIPROCOM].SWIFT_FN_GET_DISPLAY_NUMBER(E.TOTAL_AMOUNT-(E.TOTAL_AMOUNT*(E.DISCOUNT/100)))) [TOTAL_CD]	  
		, E.IS_POSTED_ERP
	...
	from [DIPROCOM].[SONDA_SALES_ORDER_HEADER] as E
		INNER JOIN [DIPROCOM].[SWIFT_VIEW_ALL_COSTUMER] VC ON (E.CLIENT_ID = VC.CODE_CUSTOMER)
		INNER JOIN [DIPROCOM].[SWIFT_VIEW_ALL_ROUTE] VR ON (VR.CODE_ROUTE = E.POS_TERMINAL)
		INNER JOIN [DIPROCOM].[SWIFT_ROUTE_BY_USER] RUS ON (RUS.CODE_ROUTE = E.POS_TERMINAL)
		LEFT JOIN [DIPROCOM].[SWIFT_VIEW_WAREHOUSES] W ON (W.CODE_WAREHOUSE = E.WAREHOUSE)
