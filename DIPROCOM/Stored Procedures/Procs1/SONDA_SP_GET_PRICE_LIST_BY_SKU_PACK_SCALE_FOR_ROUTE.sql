...
CREATE PROCEDURE [diprocom].[SONDA_SP_GET_PRICE_LIST_BY_SKU_PACK_SCALE_FOR_ROUTE]
	(
...
AS
  ...
  --
	...
	-- ------------------------------------------------------------------------
		...
		FROM
			[diprocom].[SWIFT_PRICE_LIST_BY_CUSTOMER_FOR_ROUTE] [PBCFR]
		WHERE
	...
	-- ------------------------------------------------------------------------
		...
		SELECT DISTINCT
			...
			,[PBSPS].[PRICE]
		FROM
			[diprocom].[SWIFT_PRICE_LIST_BY_SKU_PACK_SCALE_FOR_ROUTE] AS [PBSPS]
		INNER JOIN @PRICE_LISTS AS [PL]
		ON	([PL].[CODE_PRICE_LIST] = [PBSPS].[CODE_PRICE_LIST])
		INNER JOIN [diprocom].[USERS] [FU]
		ON	([PBSPS].[CODE_ROUTE] = [FU].[SELLER_ROUTE])
		INNER JOIN [diprocom].[SONDA_POS_SKUS] AS [PS]
		ON	(
