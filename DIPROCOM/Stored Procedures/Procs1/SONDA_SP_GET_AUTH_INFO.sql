...
-- Ejemplo de Ejecucion:
		-- 
		EXEC [PACASA].[SONDA_SP_GET_AUTH_INFO]
			@AUTH_ASSIGNED_TO = '46'
			,@AUTH_DOC_TYPE = 'FACTURA'
			,@AUTH_STATUS = '1'
*/
-- =============================================
CREATE PROCEDURE [PACASA].[SONDA_SP_GET_AUTH_INFO]
(	
...
BEGIN
	...
	SELECT @INVOICE_IN_ROUTE = [PACASA].[SWIFT_FN_GET_PARAMETER]('DELIVERY','INVOICE_IN_ROUTE');
	...
	IF(@INVOICE_IN_ROUTE = 0) BEGIN
		EXEC [PACASA].[SONDA_SP_GET_BRANCH_INFO]
				@AUTH_ASSIGNED_TO = @AUTH_ASSIGNED_TO
				,@INVOICE_IN_ROUTE = @INVOICE_IN_ROUTE
	END
