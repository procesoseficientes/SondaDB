...
-- Ejemplo de Ejecucion:
				EXEC [acsa].[SWIFT_SP_GET_CONSIGMENT_DETAIL]  @CONSIGNMENT_ID = 1006
*/
-- =============================================
CREATE PROCEDURE [acsa].SWIFT_SP_GET_CONSIGMENT_DETAIL (@CONSIGNMENT_ID INT)
AS
BEGIN
  ...
  SET @QUERY = N'
	SELECT
		...
		,[scd].[LINE_NUM]
		,[scd].[QTY]
		, CONVERT(DECIMAL(18,' + CAST(@DEFAULT_DISPLAY_DECIMALS AS VARCHAR) + '),[acsa].SWIFT_FN_GET_DISPLAY_NUMBER([scd].[PRICE])) [PRICE]   
		, CONVERT(DECIMAL(18,' + CAST(@DEFAULT_DISPLAY_DECIMALS AS VARCHAR) + '),[acsa].SWIFT_FN_GET_DISPLAY_NUMBER([scd].[DISCOUNT])) [DISCOUNT]   		
		, CONVERT(DECIMAL(18,' + CAST(@DEFAULT_DISPLAY_DECIMALS AS VARCHAR) + '),[acsa].SWIFT_FN_GET_DISPLAY_NUMBER([scd].[TOTAL_LINE])) [TOTAL_LINE]		
		,[scd].[POSTED_DATETIME]
		,[scd].[PAYMENT_ID]    
	...
	FROM [acsa].[SWIFT_CONSIGNMENT_DETAIL] [scd]
	INNER JOIN [acsa].[SWIFT_VIEW_ALL_SKU] AS VS ON(
		VS.CODE_SKU = scd.SKU
	)
