...
*/
-- =============================================
CREATE PROCEDURE [DIPROCOM].SWIFT_SP_GET_PRICE_LIST_BY_SKU_PACK_SCALE (@CODE_ROUTE VARCHAR(50))
AS
BEGIN
  SET NOCOUNT ON;
  --
  ...
  , [CODE_PRICE_LIST])
    SELECT DISTINCT
      @CODE_ROUTE
     ,PL.[CODE_PRICE_LIST]
    FROM [DIPROCOM].[SWIFT_PRICE_LIST_BY_SKU_PACK_SCALE_FOR_ROUTE] AS PL
    INNER JOIN [DIPROCOM].[SWIFT_PRICE_LIST_BY_CUSTOMER_FOR_ROUTE] CR
