...
BEGIN
  ...
  --
	...
	FROM [SONDA].[SONDA_INVENTORY_ONLINE] [SI]
	INNER JOIN [SONDA].[SWIFT_WAREHOUSES] [W] ON ([SI].[CODE_WAREHOUSE] = [W].[CODE_WAREHOUSE])
	INNER JOIN [SONDA].[SWIFT_WAREHOUSE_X_ZONE] [WZ] ON ([W].[CODE_WAREHOUSE] = [WZ].[CODE_WAREHOUSE])
	INNER JOIN [SONDA].[SWIFT_VIEW_ALL_SKU] [S] ON ([S].[CODE_SKU] = [SI].[CODE_SKU])
