...
-- Description:			Valida que se encuentre sincronizada la orden de venta


-- Modificación: pablo.aguilar
-- Fecha de Creacion: 	2017-03-16 Team ERGON - Sprint ERGON V
-- Description:	 Se modifica  copiando la funcionalidad de [SONDA_SP_VALIDATED_IF_EXISTS_SALES_ORDER_2]


-- Modificación: pablo.aguilar
-- Fecha de Modificación: 2017-07-05 Nexus@AgeOfEmpires
-- Description:	  Se modifica para que no valide el saleOrder mas y menos 1 para validar duplicidad. 
...
-- Ejemplo de Ejecucion:
				-- 
  SELECT * FROM [DIPROCOM].[SONDA_SALES_ORDER_LOG_EXISTS]
				EXEC [DIPROCOM].[SONDA_SP_VALIDATE_SALE_ORDER_IS_POSTED]
					@SALES_ORDER_ID = 897
					,@SALES_ORDER_ID_HH = -2
					,@DOC_SERIE = 'C'
					,@DOC_NUM = 46
...
<Data>
    <salesOrder>
        ...
        <Status>0</Status>
        <PostedBy>Alberto@DIPROCOM</PostedBy>
        ...
        <IsParent>1</IsParent>
        <ReferenceId>Alberto@DIPROCOM2017/03/16 08:43:55-2</ReferenceId>
...
*/
-- =============================================
CREATE PROCEDURE [DIPROCOM].SONDA_SP_VALIDATE_SALE_ORDER_IS_POSTED (@SALES_ORDER_ID INT
...
BEGIN
  ...
  BEGIN TRY
    ...
    FROM [DIPROCOM].[SONDA_SALES_ORDER_HEADER] [H] /*WITH(ROWLOCK,XLOCK,HOLDLOCK)*/
    ...
    SELECT TOP 1
     ...
     ,@POSTED_DATETIME = MAX([H].[POSTED_DATETIME])
     ,@DETAIL_QTY_IN_DB = COUNT(D.SALES_ORDER_ID)
     ,@CODE_CUSTOMER = MAX([H].[CLIENT_ID])
     ,@CODE_ROUTE = MAX([H].[POS_TERMINAL])
    FROM [DIPROCOM].[SONDA_SALES_ORDER_HEADER] [H] /*WITH(ROWLOCK,XLOCK,HOLDLOCK)*/
    INNER JOIN [DIPROCOM].[SONDA_SALES_ORDER_DETAIL] [D]
      ON (
    ...
    BEGIN
      PRINT 'Existia una viejo'
      --
      UPDATE [DIPROCOM].[SONDA_SALES_ORDER_HEADER]
      SET DOC_NUM = NEXT VALUE FOR [DIPROCOM].SALES_ORDER_NEGATIVE_SEQUENCE
      ...
      SELECT
       ...
       ,@INSERT = 1
      --
      INSERT INTO [DIPROCOM].[SONDA_SALES_ORDER_LOG_EXISTS] ([LOG_DATETIME]
      , [EXISTS_SALES_ORDER]
      , [DOC_SERIE]
      , [DOC_NUM]
      , [CODE_ROUTE]
    ...
    BEGIN
      IF @EXISTS = 1
        AND @DETAIL_NUM = @DETAIL_QTY_IN_DB
      BEGIN
        -- ------------------------------------------------------------------------------------
  ...
  BEGIN CATCH
    ...
    EXEC [DIPROCOM].[SONDA_SP_INSERT_SONDA_SERVER_ERROR_LOG] @CODE_ROUTE = @CODE_ROUTE
                                                         ,@LOGIN = NULL
                                                         ,@SOURCE_ERROR = 'SONDA_SP_VALIDATE_SALE_ORDER_IS_POSTED'
                                                         ,@DOC_RESOLUTION = NULL
                                                         ,@DOC_SERIE = @DOC_SERIE
                                                         ,@DOC_NUM = @DOC_NUM
