...
Ejemplo de Ejecución:
	...
	EXEC [acsa].[SWIFT_SP_ADD_CUSTOMER_IN_TRADE_AGREEMENT]
		@TRADE_AGREEMENT_ID = 21
		,@CODE_CUSTOMER = 'C002'

==================================================*/

CREATE PROCEDURE [acsa].[SWIFT_SP_ADD_CUSTOMER_IN_TRADE_AGREEMENT]
(
...
BEGIN
	BEGIN TRY
		...
		--
		SELECT @EXISTE_EN_ACUERDO = 1, @OLD_TRADE_AGREEMENT_ID = TA.TRADE_AGREEMENT_ID FROM [acsa].[SWIFT_TRADE_AGREEMENT_BY_CUSTOMER] AS TA
		...
		IF(@EXISTE_EN_ACUERDO = 1) BEGIN
			...
			--
			EXEC [acsa].[SWIFT_SP_DELETE_CUSTOMER_FROM_TRADE_AGREEMENT]
				@TRADE_AGREEMENT_ID = @OLD_TRADE_AGREEMENT_ID
				,@CODE_CUSTOMER = @CODE_CUSTOMER

			--
			INSERT INTO [acsa].[SWIFT_TRADE_AGREEMENT_BY_CUSTOMER] 
				(
					TRADE_AGREEMENT_ID
					,CODE_CUSTOMER
				) VALUES (
