...
-- Ejemplo de Ejecucion:
        ...
        GO
        --
        EXEC [PACASA].[SONDA_SP_SET_ORDER_HEADER_AMOUNT]
*/
-- =============================================
CREATE PROCEDURE [PACASA].[SONDA_SP_SET_ORDER_HEADER_AMOUNT]
AS
BEGIN
	SET NOCOUNT ON;
	--
	BEGIN TRY
		...
		FROM (SELECT d.[SALES_ORDER_ID],SUM([TOTAL_LINE]) TOTAL_LINES FROM [PACASA].[SONDA_SALES_ORDER_DETAIL] d
		INNER JOIN [PACASA].[SONDA_SALES_ORDER_HEADER] h ON [h].[SALES_ORDER_ID] = [d].[SALES_ORDER_ID]
		...
		INNER JOIN [PACASA].[SONDA_SALES_ORDER_HEADER] B ON [B].[SALES_ORDER_ID] = [A].[SALES_ORDER_ID]
