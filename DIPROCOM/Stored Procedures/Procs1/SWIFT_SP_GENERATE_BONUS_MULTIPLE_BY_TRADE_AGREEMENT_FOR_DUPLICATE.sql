...
-- Description:			SP que genera las bonificaciones por multiplo de los clientes en acuerdo comercial por cliente y canal
...
-- Ejemplo de Ejecucion:
				EXEC [DIPROCOM].[SWIFT_SP_GENERATE_BONUS_MULTIPLE_BY_TRADE_AGREEMENT_FOR_DUPLICATE]
					@CODE_ROUTE = '44'
					,@ORDER = 1
*/
-- =============================================
CREATE PROCEDURE [DIPROCOM].[SWIFT_SP_GENERATE_BONUS_MULTIPLE_BY_TRADE_AGREEMENT_FOR_DUPLICATE] (@CODE_ROUTE VARCHAR(250)
, @ORDER INT)
AS
BEGIN
  SET NOCOUNT ON;
  ...
  -- ------------------------------------------------------------------------------------                    
  -- Se comento este segmento debido que se corre primero el SP que genera las bonificaciones para escalas --[SWIFT_SP_GENERATE_BONUS_BY_TRADE_AGREEMENT_FOR_DUPLICATE] Ó [SWIFT_SP_GENERATE_BONUS_BY_CHANNEL_FOR_DUPLICATE]
  /*IF @ORDER = 1
	BEGIN
		-- ------------------------------------------------------------------------------------
  ...
  -- ------------------------------------------------------------------------------------
  INSERT INTO [DIPROCOM].[SWIFT_BONUS_LIST_BY_SKU_MULTIPLE] ([BONUS_LIST_ID]
  , [CODE_SKU]
  , [CODE_PACK_UNIT]
  , [MULTIPLE]
  , [CODE_SKU_BONUS]
  ...
  , [FREQUENCY])
    ...
    LEFT JOIN [DIPROCOM].[SWIFT_BONUS_LIST_BY_SKU_MULTIPLE] [BLBS]
      ON (
      [BLBS].[BONUS_LIST_ID] = [BL].[BONUS_LIST_ID]
      AND [BLBS].[CODE_SKU] = [TAM].[CODE_SKU]
      )
