...
-- EJEMPLO DE EJECUCION: 
		EXEC [PACASA].[SWIFT_SP_GET_TOP5_CUSTOMER_CHANGE] @OWNER='diprocom'

=========================================================*/
CREATE PROCEDURE [PACASA].[SWIFT_SP_GET_TOP5_CUSTOMER_CHANGE](
	@OWNER VARCHAR(125)
)
AS
BEGIN
	...
	--
	SELECT @FIELD_NAME_FOR_TAX_ID = [PACASA].[SWIFT_FN_GET_PARAMETER]('SCOUTING', 'FIELD_NAME_FOR_TAX_ID')
	...
	BEGIN TRY
		...
		FROM [PACASA].[SWIFT_CUSTOMER_CHANGE] AS CC
			INNER JOIN [PACASA].[SWIFT_VIEW_ALL_COSTUMER] [VAC] ON [CC].[CODE_CUSTOMER] = [VAC].[CODE_CUSTOMER]
