...
-- EJEMPLO DE EJECUCION: 
		EXEC [DIPROCOM].[SWIFT_SP_GET_TOP5_CUSTOMER_CHANGE] @OWNER='diprocom'

=========================================================*/
CREATE PROCEDURE [DIPROCOM].[SWIFT_SP_GET_TOP5_CUSTOMER_CHANGE](
	@OWNER VARCHAR(125)
)
AS
BEGIN
	...
	--
	SELECT @FIELD_NAME_FOR_TAX_ID = [DIPROCOM].[SWIFT_FN_GET_PARAMETER]('SCOUTING', 'FIELD_NAME_FOR_TAX_ID')
	...
	BEGIN TRY
		...
		FROM [DIPROCOM].[SWIFT_CUSTOMER_CHANGE] AS CC
			INNER JOIN [DIPROCOM].[SWIFT_VIEW_ALL_COSTUMER] [VAC] ON [CC].[CODE_CUSTOMER] = [VAC].[CODE_CUSTOMER]
