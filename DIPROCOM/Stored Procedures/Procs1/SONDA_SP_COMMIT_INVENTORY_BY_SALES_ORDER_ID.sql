...
-- Ejemplo de Ejecucion:
			EXEC [PACASA].[SONDA_SP_COMMIT_INVENTORY_BY_SALES_ORDER_ID] @SALE_ORDER_ID = 

  SELECT * FROM [PACASA].[SONDA_IS_COMITED_BY_WAREHOUSE] [C] WHERE C.CODE_SKU IN ('100003'
,'100007'
, '100012'
,'100018') AND C.CODE_WAREHOUSE = 'C001' 

   SELECT * FROM [PACASA].[SONDA_SALES_ORDER_HEADER] [SH]
  INNER JOIN [PACASA].[SONDA_SALES_ORDER_DETAIL] [SD]
    ON [SH].[SALES_ORDER_ID] = [SD].[SALES_ORDER_ID] 

  WHERE SH.SALES_ORDER_ID = 36954
*/
-- =============================================
CREATE PROCEDURE [PACASA].[SONDA_SP_COMMIT_INVENTORY_BY_SALES_ORDER_ID] (@SALE_ORDER_ID INT)
AS
BEGIN
    SET NOCOUNT ON;
    --
    ...
    FROM [PACASA].[SONDA_SALES_ORDER_HEADER] [SH]
        INNER JOIN [PACASA].[SONDA_SALES_ORDER_DETAIL] [SD]
            ON [SH].[SALES_ORDER_ID] = [SD].[SALES_ORDER_ID]
        INNER JOIN [PACASA].[SONDA_IS_COMITED_BY_WAREHOUSE] [C]
