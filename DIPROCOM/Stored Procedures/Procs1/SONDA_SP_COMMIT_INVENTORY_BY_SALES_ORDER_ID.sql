...
-- Ejemplo de Ejecucion:
			EXEC [DIPROCOM].[SONDA_SP_COMMIT_INVENTORY_BY_SALES_ORDER_ID] @SALE_ORDER_ID = 

  SELECT * FROM [DIPROCOM].[SONDA_IS_COMITED_BY_WAREHOUSE] [C] WHERE C.CODE_SKU IN ('100003'
,'100007'
, '100012'
,'100018') AND C.CODE_WAREHOUSE = 'C001' 

   SELECT * FROM [DIPROCOM].[SONDA_SALES_ORDER_HEADER] [SH]
  INNER JOIN [DIPROCOM].[SONDA_SALES_ORDER_DETAIL] [SD]
    ON [SH].[SALES_ORDER_ID] = [SD].[SALES_ORDER_ID] 

  WHERE SH.SALES_ORDER_ID = 36954
*/
-- =============================================
CREATE PROCEDURE [DIPROCOM].[SONDA_SP_COMMIT_INVENTORY_BY_SALES_ORDER_ID] (@SALE_ORDER_ID INT)
AS
BEGIN
    SET NOCOUNT ON;
    --
    ...
    FROM [DIPROCOM].[SONDA_SALES_ORDER_HEADER] [SH]
        INNER JOIN [DIPROCOM].[SONDA_SALES_ORDER_DETAIL] [SD]
            ON [SH].[SALES_ORDER_ID] = [SD].[SALES_ORDER_ID]
        INNER JOIN [DIPROCOM].[SONDA_IS_COMITED_BY_WAREHOUSE] [C]
