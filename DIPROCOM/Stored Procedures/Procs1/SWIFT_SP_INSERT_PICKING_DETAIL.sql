...
-- Ejemplo de Ejecucion:
        ...
        GO
        --
        EXEC [PACASA].[SWIFT_SP_INSERT_PICKING_DETAIL]
			@PICKING_HEADER = 1
			,@CODE_SKU = '100001'
			...
			,@DIFFERENCE = 2
		--
		SELECT * FROM [PACASA].[SWIFT_PICKING_DETAIL] WHERE PICKING_HEADER = 1 AND DESCRIPTION_SKU = 'DESCRIPCION'
*/
-- =============================================
CREATE PROCEDURE [PACASA].[SWIFT_SP_INSERT_PICKING_DETAIL]
		@PICKING_HEADER INT
		,@CODE_SKU VARCHAR(50)
...
BEGIN TRY
	...
	SELECT  @VALIDATION = [PACASA].[SWIFT_FN_VALIDATE_STOCK_INVENTORY_BY_WS_AND_SKU](@CODE_WAREHOUSE,@CODE_SKU,@DISPATCH)
	--
	IF @VALIDATION = 1
	BEGIN
		-- ------------------------------------------------------------------------------------
		-- Inserta el detalle
		-- ------------------------------------------------------------------------------------
		INSERT INTO [PACASA].[SWIFT_PICKING_DETAIL](
			PICKING_HEADER
			,CODE_SKU
			,DESCRIPTION_SKU
			,DISPATCH
