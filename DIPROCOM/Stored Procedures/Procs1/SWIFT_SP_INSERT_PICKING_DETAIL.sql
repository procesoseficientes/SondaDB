...
-- Ejemplo de Ejecucion:
        ...
        GO
        --
        EXEC [DIPROCOM].[SWIFT_SP_INSERT_PICKING_DETAIL]
			@PICKING_HEADER = 1
			,@CODE_SKU = '100001'
			...
			,@DIFFERENCE = 2
		--
		SELECT * FROM [DIPROCOM].[SWIFT_PICKING_DETAIL] WHERE PICKING_HEADER = 1 AND DESCRIPTION_SKU = 'DESCRIPCION'
*/
-- =============================================
CREATE PROCEDURE [DIPROCOM].[SWIFT_SP_INSERT_PICKING_DETAIL]
		@PICKING_HEADER INT
		,@CODE_SKU VARCHAR(50)
...
BEGIN TRY
	...
	SELECT  @VALIDATION = [DIPROCOM].[SWIFT_FN_VALIDATE_STOCK_INVENTORY_BY_WS_AND_SKU](@CODE_WAREHOUSE,@CODE_SKU,@DISPATCH)
	--
	IF @VALIDATION = 1
	BEGIN
		-- ------------------------------------------------------------------------------------
		-- Inserta el detalle
		-- ------------------------------------------------------------------------------------
		INSERT INTO [DIPROCOM].[SWIFT_PICKING_DETAIL](
			PICKING_HEADER
			,CODE_SKU
			,DESCRIPTION_SKU
			,DISPATCH
