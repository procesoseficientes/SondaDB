...
SET @XML = '<Data>
	<documents>
		...
		<deposit>
			...
			<amount>15</amount>
			<postedBy>NL0015@DIPROCOM</postedBy>
...
EXECUTE @RC = [DIPROCOM].SONDA_SP_ADD_DEPOSITS_BY_XML @XML
                                                ,@JSON
GO
...
*/
-- =============================================
CREATE PROCEDURE [DIPROCOM].SONDA_SP_ADD_DEPOSITS_BY_XML (@XML XML
, @JSON VARCHAR(MAX))
AS
BEGIN
  SET NOCOUNT ON;
  ...
  BEGIN TRY
    ...
    SELECT
     ...
     ,@LOGIN = [DIPROCOM].[SWIFT_FN_GET_LOGIN_BY_ROUTE]([x].[Rec].[query]('./routeid').[value]('.', 'varchar(50)'))
    ...
    BEGIN
      ...
      EXEC [DIPROCOM].[SONDA_SP_VALIDATE_IF_EXISTS_DEPOSIT] @CODE_ROUTE = @CODE_ROUTE
                                                        ,@DOC_SERIE = @DOC_SERIE
                                                        ,@DOC_NUM = @DOC_NUM
      ...
      BEGIN TRY
        ...
        INSERT INTO [DIPROCOM].SONDA_DEPOSITS (TRANS_TYPE, TRANS_DATETIME, BANK_ID, ACCOUNT_NUM, AMOUNT, POSTED_BY, POSTED_DATETIME, POS_TERMINAL, GPS_URL, TRANS_REF, IS_OFFLINE, STATUS, DOC_SERIE, DOC_NUM, LIQUIDATION_ID, IMAGE_1, IS_READY_TO_SEND)
          SELECT
            d.TRANS_TYPE
           ,d.TRANS_DATE_TIME
           ,d.BANK_ID
