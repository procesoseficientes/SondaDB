CREATE PROC [PACASA].[SWIFT_GET_HIST_SKU]
@SKU VARCHAR(MAX),
@DTBEGIN VARCHAR(50),
@DTEND VARCHAR(50)
AS
...
INTO #temp FROM [PACASA].SWIFT_VIEW_HIST_SKU SKU WHERE SKU IN 
(select Data from [PACASA].Split
(@SKU,','))
...
--SELECT INVENTORY = IDENTITY(int,1,1) , *
--INTO #temp  FROM [PACASA].SWIFT_VIEW_HIST_SKU WHERE SKU IN ('+ @SKU +') ORDER BY DESCRIPTION_SKU ASC
