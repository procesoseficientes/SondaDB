CREATE PROC [DIPROCOM].[SWIFT_GET_HIST_SKU]
@SKU VARCHAR(MAX),
@DTBEGIN VARCHAR(50),
@DTEND VARCHAR(50)
AS
...
INTO #temp FROM [DIPROCOM].SWIFT_VIEW_HIST_SKU SKU WHERE SKU IN 
(select Data from [DIPROCOM].Split
(@SKU,','))
...
--SELECT INVENTORY = IDENTITY(int,1,1) , *
--INTO #temp  FROM [DIPROCOM].SWIFT_VIEW_HIST_SKU WHERE SKU IN ('+ @SKU +') ORDER BY DESCRIPTION_SKU ASC
