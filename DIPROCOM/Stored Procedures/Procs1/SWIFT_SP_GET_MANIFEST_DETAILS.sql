...
-- Ejemplo de Ejecucion:
		--
		EXEC [PACASA].[SWIFT_SP_GET_MANIFEST_DETAILS]
			@MANIFEST_HEADER = 3071
*/
-- =============================================
CREATE PROCEDURE [PACASA].[SWIFT_SP_GET_MANIFEST_DETAILS] (
	@MANIFEST_HEADER INT
)
AS
BEGIN
	...
	INNER JOIN [PACASA].[SWIFT_PICKING_HEADER] [PH] ON (
	...
	BEGIN
		...
		SELECT @QUERY = CASE @IS_FROM_SALES_ORDER
			WHEN 1 THEN N'INSERT INTO #SALES_ORDER_DETAIL (
				...
				EXEC [PACASA].[SWIFT_SP_GET_SAP_PICKING] @pERP_DOC = ' + CAST(@ERP_DOC AS VARCHAR)
			...
			EXEC [PACASA].[SWIFT_SP_GET_SAP_PICKING_ITR] @pERP_DOC = ' + CAST(@ERP_DOC AS VARCHAR)
			END
		--
		PRINT '--> @QUERY: ' + @QUERY
		--
