...
-- Ejemplo de Ejecucion:
		--
		EXEC [acsa].[SWIFT_SP_GET_MANIFEST_DETAILS]
			@MANIFEST_HEADER = 3071
*/
-- =============================================
CREATE PROCEDURE [acsa].[SWIFT_SP_GET_MANIFEST_DETAILS] (
	@MANIFEST_HEADER INT
)
AS
BEGIN
	...
	BEGIN
		...
		SELECT @QUERY = CASE @IS_FROM_SALES_ORDER
			WHEN 1 THEN N'INSERT INTO #SALES_ORDER_DETAIL (
				...
				EXEC [acsa].[SWIFT_SP_GET_SAP_PICKING] @pERP_DOC = ' + CAST(@ERP_DOC AS VARCHAR)
			...
			EXEC [acsa].[SWIFT_SP_GET_SAP_PICKING_ITR] @pERP_DOC = ' + CAST(@ERP_DOC AS VARCHAR)
			END
		--
		PRINT '--> @QUERY: ' + @QUERY
		--
