...
*/
-- =============================================
CREATE PROCEDURE [DIPROCOM].[SWIFT_SP_GET_MANIFEST_DETAILS] (
	@MANIFEST_HEADER INT
)
AS
BEGIN
	...
	INNER JOIN [DIPROCOM].[SWIFT_PICKING_HEADER] [PH] ON (
	...
	BEGIN
		...
		SELECT @QUERY = CASE @IS_FROM_SALES_ORDER
			WHEN 1 THEN N'INSERT INTO #SALES_ORDER_DETAIL (
				...
				EXEC [DIPROCOM].[SWIFT_SP_GET_SAP_PICKING] @pERP_DOC = ' + CAST(@ERP_DOC AS VARCHAR)
			...
			EXEC [DIPROCOM].[SWIFT_SP_GET_SAP_PICKING_ITR] @pERP_DOC = ' + CAST(@ERP_DOC AS VARCHAR)
			END
		--
		PRINT '--> @QUERY: ' + @QUERY
		--
