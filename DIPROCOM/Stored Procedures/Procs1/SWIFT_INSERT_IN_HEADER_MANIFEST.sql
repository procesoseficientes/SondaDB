...
/*
	Ejemplo Ejecucion: 
    EXEC [PACASA].[SWIFT_INSERT_IN_HEADER_MANIFEST] @CODEMANIFEST = '123111'
												,@CODE_DRIVER = 'PL002'
												,@CODE_VEHICLE = '001'
 ...
 */
-- =============================================
CREATE PROCEDURE [PACASA].[SWIFT_INSERT_IN_HEADER_MANIFEST] @CODEMANIFEST VARCHAR(50),
...
BEGIN
	...
	BEGIN
		...
		FROM [PACASA].[SWIFT_VEHICLES] [sv]
		WHERE [sv].[CODE_VEHICLE] = @CODE_VEHICLE

		INSERT INTO [PACASA].SWIFT_MANIFEST_HEADER (CODE_MANIFEST_HEADER,
		CODE_DRIVER,
		CODE_VEHICLE,
		COMMENTS,
		STATUS,
