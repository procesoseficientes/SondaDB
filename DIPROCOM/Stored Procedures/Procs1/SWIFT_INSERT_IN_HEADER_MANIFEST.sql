...
/*
	Ejemplo Ejecucion: 
    EXEC [DIPROCOM].[SWIFT_INSERT_IN_HEADER_MANIFEST] @CODEMANIFEST = '123111'
												,@CODE_DRIVER = 'PL002'
												,@CODE_VEHICLE = '001'
 ...
 */
-- =============================================
CREATE PROCEDURE [DIPROCOM].[SWIFT_INSERT_IN_HEADER_MANIFEST] @CODEMANIFEST VARCHAR(50),
...
BEGIN
	...
	BEGIN
		...
		FROM [DIPROCOM].[SWIFT_DRIVERS] [sd]
		WHERE [sd].[CODE_DRIVER] = @CODE_DRIVER

		SELECT
			@NAME_ROUTE = [sr].[NAME_ROUTE]
		FROM [DIPROCOM].[SWIFT_ROUTES] [sr]
		WHERE [sr].[CODE_ROUTE] = @ROUTE

		SELECT
			@PLATE_VEHICLE = [sv].[PLATE_VEHICLE]
		FROM [DIPROCOM].[SWIFT_VEHICLES] [sv]
		WHERE [sv].[CODE_VEHICLE] = @CODE_VEHICLE

		INSERT INTO DIPROCOM.SWIFT_MANIFEST_HEADER (CODE_MANIFEST_HEADER,
		CODE_DRIVER,
		CODE_VEHICLE,
		COMMENTS,
		STATUS,
