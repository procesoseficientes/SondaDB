...
-- Ejemplo de Ejecucion:
				EXEC [PACASA].[SWIFT_SP_ADD_MANUAL_FRECUENCY] 
					@TRADE_AGREEMENT_ID = 1
*/
-- =============================================
CREATE PROCEDURE [PACASA].[SWIFT_SP_ADD_MANUAL_FRECUENCY] (@XML XML
...
BEGIN TRY
  ...
  -- ------------------------------------------------------
  MERGE [PACASA].[SWIFT_ROUTES] AS [R]
  ...
  -- ------------------------------------------------------
  MERGE [PACASA].[SWIFT_ROUTE_BY_USER] AS [RU]
  USING @TABLE_SELLER [F]
  ...
  BEGIN
    ...
    FROM [PACASA].[SWIFT_POLYGON_BY_ROUTE] AS [PBR]
    INNER JOIN [PACASA].[SWIFT_FREQUENCY] AS [F]
    ...
    -- -----------------------------------------------
    INSERT INTO [PACASA].[SWIFT_FREQUENCY] ([CODE_FREQUENCY]
    , [SUNDAY]
    , [MONDAY]
    , [TUESDAY]
    , [WEDNESDAY]
    ...
    , [LAST_WEEK_VISITED])
      SELECT
       ...
       ,[TF].[LAST_WEEK]
      FROM @TABLE_FRECUENCY [TF]
      INNER JOIN [PACASA].[SWIFT_FREQUENCY] [SF]
        ON [SF].[CODE_FREQUENCY] = [TF].[CODE_FREQUENCY];

    -- --------------------------------------------------------
    -- Insertamos las nuevas frequencias por cliente generadas
    -- --------------------------------------------------------
    INSERT INTO [PACASA].[SWIFT_FREQUENCY_X_CUSTOMER] ([ID_FREQUENCY]
    , [CODE_CUSTOMER]
    , [PRIORITY]
    , [LAST_WEEK_VISITED])
      SELECT
  ...
  BEGIN
    ...
    , [LAST_WEEK_VISITED])
      ...
      LEFT JOIN [PACASA].[SWIFT_FREQUENCY_X_CUSTOMER] AS [FC]
