...
-- Description:	11/15/2018 - SP que obtiene las respuestas de las microencuestas validas por ruta		     
...
-- Ejemplo de Ejecucion:
        EXEC [DIPROCOM].[SONDA_SP_GET_ANSWERS_BY_ROUTE]
		@CODE_ROUTE = '136'
*/
-- =============================================
CREATE PROCEDURE [DIPROCOM].[SONDA_SP_GET_ANSWERS_BY_ROUTE]
...
BEGIN
    ...
    SELECT [SA].[ANSWER_ID],
           [SA].[QUESTION_ID],
           [SA].[ANSWER]
    FROM [DIPROCOM].[SWIFT_ANSWER] [SA]
        INNER JOIN [DIPROCOM].[SWIFT_QUESTION] [SQU]
            ON ([SA].[QUESTION_ID] = [SQU].[QUESTION_ID])
