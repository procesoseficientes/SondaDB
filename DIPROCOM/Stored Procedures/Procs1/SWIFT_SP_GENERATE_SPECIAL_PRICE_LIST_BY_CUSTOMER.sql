...
-- Ejemplo de Ejecucion:
		-- 
		EXEC [DIPROCOM].[SWIFT_SP_GENERATE_SPECIAL_PRICE_LIST_BY_CUSTOMER]
		@CODE_ROUTE = '136'
*/
-- =============================================
CREATE PROCEDURE [DIPROCOM].[SWIFT_SP_GENERATE_SPECIAL_PRICE_LIST_BY_CUSTOMER]
	(
...
AS
	BEGIN
		...
		INNER JOIN [DIPROCOM].[SWIFT_SPECIAL_PRICE_LIST] [DL]
		ON	(
		...
		BEGIN
			...
			FROM
				[DIPROCOM].[SWIFT_TRADE_AGREEMENT] [TA]
			WHERE
			...
			BEGIN
				...
				SELECT TOP 1
					@IS_ALREADY = 1
				FROM
					[DIPROCOM].[SWIFT_SPECIAL_PRICE_LIST_BY_CUSTOMER] [DLC]
				INNER JOIN [DIPROCOM].[SWIFT_SPECIAL_PRICE_LIST] [DL]
    ...
    --
			...
			BEGIN
				...
				FROM
					[DIPROCOM].[SWIFT_TRADE_AGREEMENT] [T]
				WHERE
		...
		INNER JOIN [DIPROCOM].[SWIFT_PROMO_SPECIAL_PRICE_LIST_BY_SCALE] [PDS]
