...
-- Ejemplo de Ejecucion:
		-- 
		EXEC [PACASA].[SWIFT_SP_GENERATE_SPECIAL_PRICE_LIST_BY_CUSTOMER]
		@CODE_ROUTE = '136'
*/
-- =============================================
CREATE PROCEDURE [PACASA].[SWIFT_SP_GENERATE_SPECIAL_PRICE_LIST_BY_CUSTOMER]
	(
...
AS
	BEGIN
		...
		-- ------------------------------------------------------------------------------------
		INSERT	INTO [PACASA].[SWIFT_SPECIAL_PRICE_LIST_BY_CUSTOMER]
				(
		...
		INNER JOIN [PACASA].[SWIFT_SPECIAL_PRICE_LIST] [DL]
		ON	(
		...
		BEGIN
			...
			FROM
				[PACASA].[SWIFT_TRADE_AGREEMENT] [TA]
			WHERE
			...
			BEGIN
				...
				SELECT TOP 1
					@IS_ALREADY = 1
				FROM
					[PACASA].[SWIFT_SPECIAL_PRICE_LIST_BY_CUSTOMER] [DLC]
				INNER JOIN [PACASA].[SWIFT_SPECIAL_PRICE_LIST] [DL]
    ...
    --
			...
			BEGIN
				...
				FROM
					[PACASA].[SWIFT_TRADE_AGREEMENT] [T]
				WHERE
		...
		-- ------------------------------------------------------------------------------------
		INSERT INTO [PACASA].[SWIFT_SPECIAL_PRICE_LIST_BY_SCALE]
				(
					[SPECIAL_PRICE_LIST_ID]
					,[CODE_SKU]
					,[PACK_UNIT]
		...
		INNER JOIN [PACASA].[SWIFT_PROMO_SPECIAL_PRICE_LIST_BY_SCALE] [PDS]
