...
/*
  EXEC [acsa].[SWIFT_SP_INSERT_TAG_X_CUSTOMER]
  	 @TAG_COLOR ='#123'
    ,@CUSTOMER = 1
	,@DEVICE_NETWORK_TYPE = '3G'
	,@IS_POSTED_OFFLINE = 0
	
    SELECT * FROM [acsa].[SWIFT_TAG_X_CUSTOMER_NEW] WHERE TAG_COLOR = '#123' AND CUSTOMER = 1
*/

-- =============================================
CREATE PROCEDURE [acsa].[SWIFT_SP_INSERT_TAG_X_CUSTOMER]
	@TAG_COLOR VARCHAR(8)
	,@CUSTOMER VARCHAR(50)
