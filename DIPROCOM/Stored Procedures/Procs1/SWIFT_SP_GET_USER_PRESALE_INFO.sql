...
-- EJEMPLO DE EJECUCION: 
		EXEC [PACASA].[SWIFT_SP_GET_USER_PRESALE_INFO]
			@LOGIN = 'gerente@DIPROCOM'

*/		
-- =============================================
CREATE  PROCEDURE [PACASA].[SWIFT_SP_GET_USER_PRESALE_INFO]
		@LOGIN VARCHAR(50)
AS
BEGIN
	SET NOCOUNT ON;
	...
	SELECT
		...
		,CASE
			WHEN ((CONVERT(NUMERIC(18,6),[PACASA].[SWIFT_FN_GET_PARAMETER]('SALES_ORDER', 'DOC_SEQUENCE_ALERT')) * [D].[DOC_TO]) / 100) >= ([D].[DOC_TO] - [D].[CURRENT_DOC])
				THEN 1
			ELSE 0
		END [HAVE_ALERT]
	FROM [PACASA].[USERS] [U]
	LEFT JOIN [PACASA].[SWIFT_DOCUMENT_SEQUENCE] [D] ON (
