...
CREATE PROCEDURE [DIPROCOM].[SWIFT_SP_DELETE_QUIZ] (@QUIZ_ID INT)
AS
BEGIN TRY
...
-- ------------------------------------------------------------------------------------
  ...
  DELETE [DIPROCOM].SWIFT_ANSWER
  WHERE ANSWER_ID in (SELECT ANSWER_ID FROM DIPROCOM.SWIFT_QUESTION WHERE [QUIZ_ID] = @QUIZ_ID)
	
  DELETE [DIPROCOM].SWIFT_QUESTION
  WHERE [QUIZ_ID] = @QUIZ_ID

  DELETE [DIPROCOM].[SWIFT_QUIZ]
  WHERE [QUIZ_ID] = @QUIZ_ID
