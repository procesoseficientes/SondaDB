...
CREATE PROCEDURE [acsa].[SWIFT_SP_DELETE_QUIZ] (@QUIZ_ID INT)
AS
BEGIN TRY
...
-- ------------------------------------------------------------------------------------
  ...
  WHERE ANSWER_ID in (SELECT ANSWER_ID FROM [acsa].SWIFT_QUESTION WHERE [QUIZ_ID] = @QUIZ_ID)
	
  DELETE [acsa].SWIFT_QUESTION
  WHERE [QUIZ_ID] = @QUIZ_ID

  DELETE [acsa].[SWIFT_QUIZ]
  WHERE [QUIZ_ID] = @QUIZ_ID
