...
-- Ejemplo de Ejecucion:
				EXEC [PACASA].[SWIFT_SP_ADD_DISCOUNT_BY_GENERAL_AMOUNT_TO_PROMOTION_OF_DISCOUNT_BY_GENERAL_AMOUNT]
				@PROMO_ID = 8,
...
*/
-- =============================================
CREATE PROCEDURE [PACASA].[SWIFT_SP_ADD_DISCOUNT_BY_GENERAL_AMOUNT_TO_PROMOTION_OF_DISCOUNT_BY_GENERAL_AMOUNT](
	@PROMO_ID INT,
...
BEGIN
	...
	BEGIN TRY
		...
		-- ----------------------------------------------------------------------------------------------------------
		EXEC [PACASA].[SWIFT_SP_VALID_DISCOUNT_SCALE_FOR_DISCOUNT_BY_GENERAL_AMOUNT] @PROMO_ID,@LOW_AMOUNT,@HIGH_AMOUNT;
		
		-- -----------------------------------------------------------------------------------------------------------
		-- Si no hay conflicto con el rango de descuento, éste se agrega a la promoción de Descuento por Monto General
		-- -----------------------------------------------------------------------------------------------------------
		INSERT INTO [PACASA].[SWIFT_PROMO_DISCOUNT_BY_GENERAL_AMOUNT]
				(
					[PROMO_ID]
					,[LOW_AMOUNT]
					,[HIGH_AMOUNT]
