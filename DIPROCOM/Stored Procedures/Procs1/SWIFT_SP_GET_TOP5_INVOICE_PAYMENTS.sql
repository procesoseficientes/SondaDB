...
-- Ejemplo de Ejecucion:
                EXEC [DIPROCOM].SWIFT_SP_GET_TOP5_INVOICE_PAYMENTS
*/
-- =============================================
CREATE PROCEDURE [DIPROCOM].[SWIFT_SP_GET_TOP5_INVOICE_PAYMENTS]
AS
BEGIN
  SET NOCOUNT ON;
  --
  ...
  SELECT
    @SHIPPING_ATTEMPTS = [DIPROCOM].[SWIFT_FN_GET_PARAMETER]('PAYMENT',
    'SHIPPING_ATTEMPTS')
   ,@CASH_ACCOUNT_IN_ERP = [DIPROCOM].[SWIFT_FN_GET_PARAMETER]('PAYMENT',
    'CASH_ACCOUNT_IN_ERP')
   ,@CHECK_ACCOUNT_IN_ERP = [DIPROCOM].[SWIFT_FN_GET_PARAMETER]('PAYMENT',
    'CHECK_ACCOUNT_IN_ERP')
   ,@DEPOSIT_ACCOUNT_IN_ERP = [DIPROCOM].[SWIFT_FN_GET_PARAMETER]('PAYMENT',
    'DEPOSIT_ACCOUNT_IN_ERP')
   ,@GENERATE_PAYMENT = [DIPROCOM].[SWIFT_FN_GET_PARAMETER]('INVOICE',
    'GENERATE_PAYMENT');
