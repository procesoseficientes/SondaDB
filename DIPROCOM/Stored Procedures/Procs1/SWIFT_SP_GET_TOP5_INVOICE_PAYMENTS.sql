...
-- Ejemplo de Ejecucion:
                EXEC [PACASA].SWIFT_SP_GET_TOP5_INVOICE_PAYMENTS
*/
-- =============================================
CREATE PROCEDURE [PACASA].[SWIFT_SP_GET_TOP5_INVOICE_PAYMENTS]
AS
BEGIN
  SET NOCOUNT ON;
  --
  ...
  SELECT
    @SHIPPING_ATTEMPTS = [PACASA].[SWIFT_FN_GET_PARAMETER]('PAYMENT',
    'SHIPPING_ATTEMPTS')
   ,@CASH_ACCOUNT_IN_ERP = [PACASA].[SWIFT_FN_GET_PARAMETER]('PAYMENT',
    'CASH_ACCOUNT_IN_ERP')
   ,@CHECK_ACCOUNT_IN_ERP = [PACASA].[SWIFT_FN_GET_PARAMETER]('PAYMENT',
    'CHECK_ACCOUNT_IN_ERP')
   ,@DEPOSIT_ACCOUNT_IN_ERP = [PACASA].[SWIFT_FN_GET_PARAMETER]('PAYMENT',
    'DEPOSIT_ACCOUNT_IN_ERP')
   ,@GENERATE_PAYMENT = [PACASA].[SWIFT_FN_GET_PARAMETER]('INVOICE',
    'GENERATE_PAYMENT');
