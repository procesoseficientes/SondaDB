...
-- Modificacion 		7/24/2019 @ G-Force Team Sprint Groenlandia
-- Autor: 				diego.as
-- Historia/Bug:		Product Backlog Item 30512: Toma de multiples fotos en entrega de Sonda SD
-- Descripcion: 		7/24/2019 - Se agregan columnas para almacenar multiples imagenes (4 en total)

/*
-- Ejemplo de Ejecucion:
				EXEC [DIPROCOM].[SONDA_SP_ADD_DELIVERY_NOTE_BY_XML]
				@XML = '
...
<Data>
    ...
    <routeid>46</routeid>
    <loginId>Adolfo@DIPROCOM</loginId>
...
*/
-- =============================================
CREATE PROCEDURE [DIPROCOM].[SONDA_SP_ADD_DELIVERY_NOTE_BY_XML] (@XML XML)
AS
BEGIN
    BEGIN TRY
        DECLARE @ID INT,
        ...
        (
            ...
            [DELIVERY_IMAGE_2] VARCHAR(MAX) NULL,
            [DELIVERY_IMAGE_3] VARCHAR(MAX) NULL,
            [DELIVERY_IMAGE_4] VARCHAR(MAX) NULL,
            [DELIVERY_SIGNATURE] VARCHAR(MAX) NULL,
            UNIQUE
            (
                [DOC_SERIE],
                [DOC_NUM]
        ...
        -- --------------------------------------------------------------------------------------------------------
        SELECT @CODE_ROUTE = [x].[Rec].[query]('./routeid').[value]('.', 'varchar(50)'),
        ...
        SELECT [x].[Rec].[query]('./docSerie').[value]('.', 'varchar(250)'),
               ...
               CASE [x].[Rec].[query]('./devolutionId').[value]('.', 'varchar(50)')
                   ...
                   WHEN 'UNDEFINED' THEN
                       NULL
                   ELSE
                       [x].[Rec].[query]('./devolutionId').[value]('.', 'int')
               END,
               CASE [x].[Rec].[query]('./deliveryImage').[value]('.', 'varchar(MAX)')
                   WHEN 'NULL' THEN
                       NULL
                   WHEN 'UNDEFINED' THEN
                       NULL
               ...
               CASE [x].[Rec].[query]('./billedFromSonda').[value]('.', 'varchar(50)')
                   ...
                   WHEN 'UNDEFINED' THEN
                       NULL
                   ELSE
                       [x].[Rec].[query]('./billedFromSonda').[value]('.', 'int')
               END,
               CASE [x].[Rec].[query]('./isCanceled').[value]('.', 'varchar(50)')
                   ...
                   WHEN 'UNDEFINED' THEN
                       0
                   ELSE
                       [x].[Rec].[query]('./isCanceled').[value]('.', 'int')
               END,
               CASE [x].[Rec].[query]('./reasonCancel').[value]('.', 'varchar(250)')
                   ...
                   WHEN 'UNDEFINED' THEN
                       NULL
                   ELSE
                       [x].[Rec].[query]('./reasonCancel').[value]('.', 'varchar(250)')
               END,
               ...
               END,
               CASE [x].[Rec].[query]('./deliveryImage2').[value]('.', 'varchar(MAX)')
                   WHEN 'NULL' THEN
                       NULL
                   WHEN 'UNDEFINED' THEN
                       NULL
                   ELSE
                       [x].[Rec].[query]('./deliveryImage2').[value]('.', 'varchar(MAX)')
               END,
               CASE [x].[Rec].[query]('./deliveryImage3').[value]('.', 'varchar(MAX)')
                   WHEN 'NULL' THEN
                       NULL
                   WHEN 'UNDEFINED' THEN
                       NULL
                   ELSE
                       [x].[Rec].[query]('./deliveryImage3').[value]('.', 'varchar(MAX)')
               END,
               CASE [x].[Rec].[query]('./deliveryImage4').[value]('.', 'varchar(MAX)')
                   WHEN 'NULL' THEN
                       NULL
                   WHEN 'UNDEFINED' THEN
                       NULL
                   ELSE
                       [x].[Rec].[query]('./deliveryImage4').[value]('.', 'varchar(MAX)')
               END,
               CASE [x].[Rec].[query]('./deliverySignature').[value]('.', 'varchar(MAX)')
                   WHEN 'NULL' THEN
                       NULL
                   WHEN 'UNDEFINED' THEN
                       NULL
                   ELSE
                       [x].[Rec].[query]('./deliverySignature').[value]('.', 'varchar(MAX)')
               END
        ...
        SELECT [x].[Rec].[query]('./deliveryNoteId').[value]('.', 'int'),
               ...
               CASE [x].[Rec].[query]('./appliedDiscount').[value]('.', 'varchar(50)')
                   ...
                   WHEN 'UNDEFINED' THEN
                       0
                   ELSE
                       [x].[Rec].[query]('./appliedDiscount').[value]('.', 'numeric(18,6)')
               END,
               [x].[Rec].[query]('./createdDateTime').[value]('.', 'varchar(50)'),
               CASE [x].[Rec].[query]('./relatedPickingDemandHeaderId').[value]('.', 'varchar(50)')
                   ...
                   WHEN 'UNDEFINED' THEN
                       0
                   ELSE
                       [x].[Rec].[query]('./relatedPickingDemandHeaderId').[value]('.', 'int')
               END
        FROM @XML.[nodes]('Data/notasDeEntrega/detalleNotaDeEntrega') AS [x]([Rec]);

        -- ----------------------------------------------------------------------------------------------------------

        WHILE EXISTS (SELECT TOP 1 1 FROM @DELIVERY_NOTE_HEADER)
        BEGIN
            ...
            BEGIN TRY
                --
                INSERT INTO [DIPROCOM].[SONDA_DELIVERY_NOTE_HEADER]
                (
                    [DOC_SERIE],
                    [DOC_NUM],
                    [CODE_CUSTOMER],
                ...
                --
                INSERT INTO [DIPROCOM].[SONDA_DELIVERY_NOTE_DETAIL]
                (
                    [DELIVERY_NOTE_ID],
                    [CODE_SKU],
                    [QTY],
                ...
                BEGIN
                    --
                    EXEC [DIPROCOM].[SONDA_SP_ADD_DELIVERY_NOTE_BY_INVOICE] @DELIVERY_NOTE_DOC_SERIE = @DELIVERY_NOTE_DOC_SERIE,
                ...
                --
                INSERT INTO @DELIVERY_NOTE_RESULT
                (
                    [DELIVERY_NOTE_ID],
                ...
                VALUES
                (@ID, @DELIVERY_NOTE_DOC_SERIE, @DELIVERY_NOTE_DOC_NUM, 2, @DELIVERY_NOTE_BY_INVOICE_ID,
                 @DELIVERY_NOTE_BY_CONSIGNMENT_ID, @DELIVERY_NOTE_BY_DEVOLUTION_ID);
                --
            ...
            BEGIN CATCH
                ...
                VALUES
                (@ID, @DELIVERY_NOTE_DOC_SERIE, @DELIVERY_NOTE_DOC_NUM, 0, @DELIVERY_NOTE_BY_INVOICE_ID,
                 @DELIVERY_NOTE_BY_CONSIGNMENT_ID, @DELIVERY_NOTE_BY_DEVOLUTION_ID);
