...
-- Ejemplo de Ejecucion:
	EXEC [PACASA].[SONDA_SP_STATISTIC_SALES_BY_CUSTOMER_OUT_OF_ROUTE_PLAN]
	@CODE_ROUTE = '46', @CLIENT_ID = 'SO-152137'
*/
-- =============================================

CREATE PROCEDURE [PACASA].[SONDA_SP_STATISTIC_SALES_BY_CUSTOMER_OUT_OF_ROUTE_PLAN]
(
...
BEGIN
    ...
    FROM [PACASA].[SONDA_STATISTIC_SALES_BY_CUSTOMER] AS [sssbc]
        INNER JOIN [PACASA].[SONDA_ROUTE_PLAN] AS [srp]
            ON [srp].[RELATED_CLIENT_CODE] = [sssbc].[CLIENT_ID]
        INNER JOIN [PACASA].[SWIFT_VIEW_ALL_SKU] AS [svas]
