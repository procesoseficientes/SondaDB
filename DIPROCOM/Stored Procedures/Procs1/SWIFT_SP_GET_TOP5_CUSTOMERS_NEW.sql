...
EXECUTE @RC = [DIPROCOM].SWIFT_SP_GET_TOP5_CUSTOMERS_NEW @OWNER='diprocom'
EXEC [DIPROCOM].SWIFT_SP_GET_TOP5_CUSTOMERS_NEW @OWNER='diprocom'

GO
  
*/
-- =============================================
CREATE PROCEDURE [DIPROCOM].[SWIFT_SP_GET_TOP5_CUSTOMERS_NEW](
	@OWNER VARCHAR(25)
)
AS
  ...
  SELECT
    @FIELD_NAME_FOR_TAX_ID = [DIPROCOM].[SWIFT_FN_GET_PARAMETER]('SCOUTING', 'FIELD_NAME_FOR_TAX_ID')
   ,@CREATE_NEW_CUSTOMER_AS_LID = CAST([DIPROCOM].[SWIFT_FN_GET_PARAMETER]('SCOUTING', 'CREATE_NEW_CUSTOMER_AS_LID') AS INT)
   ,@DEFAULT_GROUP_CODE = CAST( [DIPROCOM].[SWIFT_FN_GET_PARAMETER]('SCOUTING', 'DEFAULT_GROUP_CODE') AS INT)
   ,@SHIPPING_ATTEMPTS = CAST( [DIPROCOM].[SWIFT_FN_GET_PARAMETER]('SCOUTING', 'SHIPPING_ATTEMPTS') AS INT)
  ...
  SELECT TOP 5
   ...
   ,@DEFAULT_GROUP_CODE AS DEFAULT_GROUP_CODE
	...
	,[scn].[IS_FROM]
  FROM [DIPROCOM].[SWIFT_VIEW_ALL_CUSTOMER_NEW] [scn]
  INNER JOIN [DIPROCOM].[SWIFT_VW_GET_NEWEST_ORDER_FOR_DUPLICATE_CUSTOMER_NEW] [OCN]
    ON (
  ...
  LEFT JOIN [DIPROCOM].[SWIFT_CUSTOMER_FREQUENCY_NEW] [scfn]
    ON (
    [scn].[CODE_CUSTOMER] = [scfn].[CODE_CUSTOMER]
    )
  INNER JOIN [DIPROCOM].[SWIFT_COMPANY] [sc]
	ON(
