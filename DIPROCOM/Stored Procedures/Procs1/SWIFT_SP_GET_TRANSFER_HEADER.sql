...
-- Ejemplo de Ejecucion:
				EXEC [PACASA].[SWIFT_SP_GET_TRANSFER_HEADER]
				@START_DATETIME = '20170101 00:00:00.000'
				,@END_DATETIME = '20170201 00:00:00.000'
*/
-- =============================================
CREATE PROCEDURE [PACASA].[SWIFT_SP_GET_TRANSFER_HEADER](
...
BEGIN
	...
	FROM [PACASA].[SWIFT_TRANSFER_HEADER] [TH]
		INNER JOIN [PACASA].[SWIFT_SELLER] [S] ON [S].[SELLER_CODE] = [TH].[SELLER_CODE]
		INNER JOIN [PACASA].[SWIFT_ROUTES] [R] ON [R].[CODE_ROUTE] = [TH].[SELLER_ROUTE]
		INNER JOIN [PACASA].[SWIFT_WAREHOUSES] [CWS] ON [CWS].[CODE_WAREHOUSE] = [TH].[CODE_WAREHOUSE_SOURCE]
		INNER JOIN [PACASA].[SWIFT_WAREHOUSES] [CWT] ON [CWT].[CODE_WAREHOUSE] = [TH].[CODE_WAREHOUSE_TARGET]
