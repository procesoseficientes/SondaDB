...
Ejemplo de Ejecucion:
            ...
            EXEC [PACASA].SONDA_SP_GENERATE_ITEM_HISTORY
			--
			SELECT * FROM [PACASA].[SONDA_ITEM_HISTORY]
*/
-- =============================================
CREATE PROCEDURE [PACASA].[SONDA_SP_GENERATE_ITEM_HISTORY]
AS
BEGIN
    SET NOCOUNT ON;
    --
    ...
    -- Limpia la tabla
    -- ------------------------------------------------------------------------------------
    TRUNCATE TABLE [PACASA].[SONDA_ITEM_HISTORY];

    -- ------------------------------------------------------------------------------------
    -- Se obtiene el tipo de orden de venta
    -- ------------------------------------------------------------------------------------
    SELECT @DOC_TYPE = [PACASA].[SWIFT_FN_GET_PARAMETER]('SALES_ORDER', 'DOC_TYPE');
    ...
    (
        ...
        LEFT JOIN [PACASA].[SONDA_ROUTE_PLAN] [P]
            ON (
    ...
    FROM [PACASA].[SONDA_SALES_ORDER_DETAIL] AS [D]
    ...
    WHERE [D].[CODE_PACK_UNIT] IS NOT NULL
    ...
    -- ------------------------------------------------------------------------------------
    INSERT INTO [PACASA].[SONDA_ITEM_HISTORY]
    (
        [CODE_ROUTE],
        [DOC_TYPE],
        [CODE_CUSTOMER],
    ...
    -- ------------------------------------------------------------------------------------
    SELECT @DOC_TYPE = [PACASA].[SWIFT_FN_GET_PARAMETER]('TAKE_INVENTORY', 'DOC_TYPE');
    ...
    FROM [PACASA].[SONDA_TAKE_INVENTORY_HEADER] [H]
        INNER JOIN [PACASA].[SONDA_ROUTE_PLAN] [P]
    ...
    FROM [#TAKE_INVENTORY_BY_CUSTOMER] [C]
        INNER JOIN [PACASA].[SONDA_TAKE_INVENTORY_HEADER] [H]
            ON ([H].[TAKE_INVENTORY_ID] = [C].[TAKE_INVENTORY_ID])
        INNER JOIN [PACASA].[SONDA_TAKE_INVENTORY_DETAIL] [D]
