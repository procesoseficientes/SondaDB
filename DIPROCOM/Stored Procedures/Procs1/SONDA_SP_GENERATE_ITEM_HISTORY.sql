...
Ejemplo de Ejecucion:
            ...
            EXEC [DIPROCOM].SONDA_SP_GENERATE_ITEM_HISTORY
			--
			SELECT * FROM [DIPROCOM].[SONDA_ITEM_HISTORY]
*/
-- =============================================
CREATE PROCEDURE [DIPROCOM].[SONDA_SP_GENERATE_ITEM_HISTORY]
AS
BEGIN
    SET NOCOUNT ON;
    --
    ...
    -- Limpia la tabla
    -- ------------------------------------------------------------------------------------
    TRUNCATE TABLE [DIPROCOM].[SONDA_ITEM_HISTORY];

    -- ------------------------------------------------------------------------------------
    -- Se obtiene el tipo de orden de venta
    -- ------------------------------------------------------------------------------------
    SELECT @DOC_TYPE = [DIPROCOM].[SWIFT_FN_GET_PARAMETER]('SALES_ORDER', 'DOC_TYPE');
    ...
    (
        ...
        LEFT JOIN [DIPROCOM].[SONDA_ROUTE_PLAN] [P]
            ON (
    ...
    -- ------------------------------------------------------------------------------------
    INSERT INTO [DIPROCOM].[SONDA_ITEM_HISTORY]
    (
        [CODE_ROUTE],
        [DOC_TYPE],
        [CODE_CUSTOMER],
    ...
    -- ------------------------------------------------------------------------------------
    SELECT @DOC_TYPE = [DIPROCOM].[SWIFT_FN_GET_PARAMETER]('TAKE_INVENTORY', 'DOC_TYPE');
    ...
    FROM [DIPROCOM].[SONDA_TAKE_INVENTORY_HEADER] [H]
        INNER JOIN [DIPROCOM].[SONDA_ROUTE_PLAN] [P]
    ...
    FROM [#TAKE_INVENTORY_BY_CUSTOMER] [C]
        INNER JOIN [DIPROCOM].[SONDA_TAKE_INVENTORY_HEADER] [H]
            ON ([H].[TAKE_INVENTORY_ID] = [C].[TAKE_INVENTORY_ID])
        INNER JOIN [DIPROCOM].[SONDA_TAKE_INVENTORY_DETAIL] [D]
