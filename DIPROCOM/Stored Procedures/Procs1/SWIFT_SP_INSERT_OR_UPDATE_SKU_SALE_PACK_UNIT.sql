...
-- Ejemplo de Ejecucion:
				...
				EXEC [DIPROCOM].SWIFT_SP_INSERT_OR_UPDATE_SKU_SALE_PACK_UNIT
					@CODE_PACK_UNIT = @CODE_PACK_UNIT
					,@CODE_SKU = @CODE_SKU
				
				--
				SELECT * FROM [DIPROCOM].SWIFT_SKU_SALE_PACK_UNIT WHERE CODE_PACK_UNIT = @CODE_PACK_UNIT AND CODE_SKU = @CODE_SKU
*/
-- =============================================
CREATE PROCEDURE [DIPROCOM].[SWIFT_SP_INSERT_OR_UPDATE_SKU_SALE_PACK_UNIT] 
...
BEGIN TRY
  ...
  -------------------------------------------------
	 IF (SELECT COUNT(ssspu.CODE_PACK_UNIT) FROM DIPROCOM.SWIFT_SKU_SALE_PACK_UNIT ssspu
          WHERE ssspu.CODE_SKU = @CODE_SKU)<1
    BEGIN  
    -------------------------------------------------
  --inserta la informacion si no tiene datos el sku
  -------------------------------------------------
   	  INSERT INTO DIPROCOM.SWIFT_SKU_SALE_PACK_UNIT(
		      CODE_PACK_UNIT
          ,CODE_SKU
	    ) VALUES (
		      @CODE_PACK_UNIT
  ...
  -------------------------------------------------
      UPDATE [DIPROCOM].SWIFT_SKU_SALE_PACK_UNIT
