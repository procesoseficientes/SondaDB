...
-- Ejemplo de Ejecucion:
				EXEC [PACASA].[SWIFT_SP_GET_TRADE_AGREEMENT_SKU_BY_BONUS_RULE]
					@TRADE_AGREEMENT_ID = 1043
					,@COMBO_ID = 1003
*/
-- =============================================
CREATE PROCEDURE [PACASA].[SWIFT_SP_GET_TRADE_AGREEMENT_SKU_BY_BONUS_RULE](
...
BEGIN
	...
	INNER JOIN [PACASA].[SWIFT_TRADE_AGREEMENT_SKU_BY_BONUS_RULE] [CRS] ON (
		[CRS].[CODE_SKU] = [S].[CODE_SKU]
	)
	INNER JOIN [PACASA].[SONDA_PACK_UNIT] [PU] ON (
		[PU].[PACK_UNIT] = [CRS].[PACK_UNIT]
	)
	INNER JOIN [PACASA].[SWIFT_TRADE_AGREEMENT_BY_COMBO_BONUS_RULE] [CR] ON(
		[CR].[TRADE_AGREEMENT_BONUS_RULE_BY_COMBO_ID] = [CRS].[TRADE_AGREEMENT_BONUS_RULE_BY_COMBO_ID]
	)
	INNER JOIN [PACASA].[SWIFT_TRADE_AGREEMENT_BY_BONUS_RULE] [TAR] ON (
