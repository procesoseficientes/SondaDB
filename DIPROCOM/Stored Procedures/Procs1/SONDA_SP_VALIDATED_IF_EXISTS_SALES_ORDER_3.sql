...
-- Modificacion 8/3/2017 @ Reborn-Team Sprint Bearbeitung
					-- diego.as
					-- Se agrega columna SALES_ORDER_POSTED_DATETIME que indica la fecha en la que se posteo el documento en el servidor

/*
-- EJEMPLO DE EJECUCION: 
		--
		EXEC [DIPROCOM].[SONDA_SP_VALIDATED_IF_EXISTS_SALES_ORDER_3]
			@DOC_SERIE = 'ESC0003@DIPROCOM'
			,@DOC_NUM = 4102
			,@CODE_ROUTE = 'ESC0003@DIPROCOM'
...
*/
-- =====================================================
CREATE PROCEDURE [DIPROCOM].SONDA_SP_VALIDATED_IF_EXISTS_SALES_ORDER_3 (@DOC_SERIE VARCHAR(100)
, @DOC_NUM INT
...
BEGIN
  ...
  BEGIN TRY
    ...
    SELECT
	 ...
	 ,@SALES_ORDER_POSTED_DATETIME AS [SALES_ORDER_POSTED_DATETIME]
  END TRY
  BEGIN CATCH
	--
    SET @SP_ERROR = ERROR_MESSAGE();
    PRINT 'SP_CATCH: ' + @SP_ERROR;
	--
    EXEC [DIPROCOM].[SONDA_SP_INSERT_SONDA_SERVER_ERROR_LOG] @CODE_ROUTE = @CODE_ROUTE
                                                         ,@LOGIN = NULL
                                                         ,@SOURCE_ERROR = 'SONDA_SP_VALIDATED_IF_EXISTS_SALES_ORDER_3'
                                                         ,@DOC_RESOLUTION = NULL
                                                         ,@DOC_SERIE = @DOC_SERIE
                                                         ,@DOC_NUM = @DOC_NUM
