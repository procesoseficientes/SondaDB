...
-- Ejemplo de Ejecucion:
				-- 
				EXEC [DIPROCOM].[SWIFT_SP_ASOSOCIATE_CUSTOMER_FREQUENCY]
				   @FREQUENCY_WEEKS = 4
				   ,@SUNDAY = 1
				   ,@MONDAY = 0
				   ,@TUESDAY = 1
				   ...
				   ,@CODE_CUSTOMER = '205'
				--
				SELECT * FROM [DIPROCOM].SWIFT_FREQUENCY
				SELECT * FROM [DIPROCOM].SWIFT_FREQUENCY_X_CUSTOMER WHERE CODE_CUSTOMER = '205'

*/
-- =============================================
CREATE PROCEDURE [DIPROCOM].[SWIFT_SP_ASOSOCIATE_CUSTOMER_FREQUENCY](
	@FREQUENCY_WEEKS INT
	,@SUNDAY INT
	,@MONDAY INT
	,@TUESDAY INT
...
BEGIN
	...
	BEGIN TRY
		...
		SELECT @LAST_DATE_VISITED = 
		CASE 
			WHEN @FREQUENCY_WEEKS = 1 THEN
		...
		-- ------------------------------------------------------------------------------------
		MERGE [DIPROCOM].[SWIFT_POLYGON_X_CUSTOMER] AS [PC]
		USING (
				SELECT
