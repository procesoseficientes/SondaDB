...
CREATE PROCEDURE [PACASA].[SWIFT_INSERT_OR_UPDATE_QUERY_LIST_BY_GRIDS_LAYOUT] (
		@QUERY_LIST_ID INT
		,@LOGIN_ID VARCHAR(50)
		,@LAYOUT_XML XML
	)
...
BEGIN
	BEGIN TRY
		...
		IF NOT EXISTS ( SELECT TOP 1
							1
						FROM
							[PACASA].[SWIFT_QUERY_LIST_BY_GRIDS_LAYOUT]
						WHERE
							[QUERY_LIST_ID] = @QUERY_LIST_ID
							AND [LOGIN_ID] = @LOGIN_ID )
		BEGIN
			INSERT	INTO [PACASA].[SWIFT_QUERY_LIST_BY_GRIDS_LAYOUT]
					(
						[QUERY_LIST_ID]
						,[LOGIN_ID]
						,[LAYOUT_XML]
