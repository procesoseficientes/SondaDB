...
-- Ejemplo de Ejecucion:
				exec [PACASA].SONDA_SP_GET_ROUTE_PLAN @CODE_ROUTE = '101109'
  select * from [PACASA].
*/
-- =============================================
CREATE PROCEDURE [PACASA].[SONDA_SP_GET_ROUTE_PLAN] @CODE_ROUTE VARCHAR(50)
AS
BEGIN
  SET NOCOUNT ON;
  --
  SELECT
   ...
   ,[RP].[EMAIL_TO_CONFIRM]
   ,upper([RP].[RELATED_CLIENT_CODE]) as [RELATED_CLIENT_CODE]
  ...
  FROM [PACASA].[SONDA_ROUTE_PLAN] [RP]
    INNER JOIN [PACASA].[SWIFT_VIEW_ALL_COSTUMER] [C] ON [C].[CODE_CUSTOMER] = [RP].[RELATED_CLIENT_CODE]
