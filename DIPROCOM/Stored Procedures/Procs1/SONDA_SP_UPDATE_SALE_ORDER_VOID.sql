...
-- Ejemplo de Ejecucion:
				DECLARE 
					...
					,@VOID INT = 1
				--
				SELECT S.DOC_SERIE,S.DOC_NUM,IS_VOID FROM [DIPROCOM].[SONDA_SALES_ORDER_HEADER] S WHERE DOC_SERIE = @SERIE AND DOC_NUM = @DOC_NUM
				--
				EXEC [DIPROCOM].[SONDA_SP_UPDATE_SALE_ORDER_VOID]
					@SERIE = @SERIE
					,@DOC_NUM = @DOC_NUM
					,@VOID = @VOID
				--
...
*/
-- =============================================
CREATE PROCEDURE [DIPROCOM].[SONDA_SP_UPDATE_SALE_ORDER_VOID]
(	
	@SERIE VARCHAR(100)
	,@DOC_NUM INT
	,@VOID INT = 1
...
BEGIN
	...
	BEGIN TRY
		...
		IF @VOID = 1
		BEGIN
			SELECT @CAN_VOID = [DIPROCOM].[SONDA_FN_CAN_VOID_SALE_ORDER](@SERIE,@DOC_NUM)
			--
			IF @CAN_VOID = 0
			BEGIN
