...
/*
  EXEC [PACASA].[SONDA_SP_INSERT_TAKE_INVENTORY_DETAIL_IF_SKU_EXIST]
...
-- =============================================
CREATE PROCEDURE [PACASA].[SONDA_SP_INSERT_TAKE_INVENTORY_DETAIL_IF_SKU_EXIST]
	-- ----------------------------------------------------------------------------------
...
BEGIN
	...
	DECLARE @RESULT INT=0
	  ...
	  -- ----------------------------------------------------------------------------------
			...
			SELECT @RESULT=1 
			FROM [PACASA].SWIFT_VIEW_ALL_SKU VAS 
			INNER JOIN [PACASA].SONDA_PACK_CONVERSION SPC ON (VAS.CODE_SKU = SPC.CODE_SKU)
	  ...
	  -- ----------------------------------------------------------------------------------
		...
		IF @RESULT = 1
	BEGIN 
		INSERT INTO [PACASA].[SONDA_TAKE_INVENTORY_DETAIL](
             [TAKE_INVENTORY_ID]
    	    ,[CODE_SKU]
        	,[QTY]
        	,[POSTED_DATETIME]
