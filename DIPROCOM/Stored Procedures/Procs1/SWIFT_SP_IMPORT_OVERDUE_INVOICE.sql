...
-- Ejemplo de Ejecucion:
      		EXEC [PACASA].[SWIFT_SP_IMPORT_OVERDUE_INVOICE]
*/
-- =============================================
CREATE PROCEDURE [PACASA].[SWIFT_SP_IMPORT_OVERDUE_INVOICE]
AS
BEGIN
    --
    SET NOCOUNT ON;
    ...
    BEGIN TRY
        ...
        -- Limpiamos la tabla
        -- -------------------------------------------------------------
        TRUNCATE TABLE [PACASA].[SWIFT_OVERDUE_INVOICE_BY_CUSTOMER];
            ...
            = '
				INSERT INTO [#OVERDUE_INVOICE_BY_CUSTOMER]
					...
					FROM OPENQUERY([DIPROCOM_SERVER], ''
						SELECT
							INV.DocNum INVOICE_ID
							,INV.DocEntry DOC_ENTRY
							,INV.CardCode CODE_CUSTOMER
							,INV.DocDate CREATED_DATE
							,INV.DocDueDate DUE_DATE
							,INV.DocTotal TOTAL_AMOUNT
							,(INV.DocTotal - INV.PaidToDate) as PENDING_TO_PAID
						FROM [ME_LLEGA_DB].DBO.OINV AS INV
						where GETDATE() > INV.DocDueDate
						AND (INV.DocTotal - INV.PaidToDate) > 0
				'') AS IV
        ...
        -- -------------------------------------------------------------------
        INSERT INTO [PACASA].[SWIFT_OVERDUE_INVOICE_BY_CUSTOMER]
        (
            [INVOICE_ID],
            [DOC_ENTRY],
            [CODE_CUSTOMER],
    ...
    BEGIN CATCH
        ...
        --
        EXEC [PACASA].[SONDA_SP_INSERT_SONDA_SERVER_ERROR_LOG] @CODE_ROUTE = '',                                  -- varchar(50)
                                                              @LOGIN = '',                                       -- varchar(50)
