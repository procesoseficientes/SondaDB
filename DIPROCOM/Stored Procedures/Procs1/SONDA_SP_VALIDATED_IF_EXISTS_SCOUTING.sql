...
*/			
-- =============================================
CREATE PROCEDURE [PACASA].[SONDA_SP_VALIDATED_IF_EXISTS_SCOUTING](
...
BEGIN
	...
	SELECT TOP 1
		@EXISTS = 1
		,@ID = [CN].[CUSTOMER_ID]
	FROM [PACASA].[SONDA_CUSTOMER_NEW] [CN] WITH(ROWLOCK,XLOCK,HOLDLOCK)
	...
	BEGIN
		...
		FROM [PACASA].[SONDA_CUSTOMER_NEW] [CN] WITH(ROWLOCK,XLOCK,HOLDLOCK)
		INNER JOIN [PACASA].[SONDA_TAG_X_CUSTOMER_NEW] [T] ON ([T].[CUSTOMER_ID] = [CN].[CUSTOMER_ID])
		WHERE [CN].[CODE_CUSTOMER] = @ID_FROM_XML
		GROUP BY [CN].[CUSTOMER_ID];
	END
