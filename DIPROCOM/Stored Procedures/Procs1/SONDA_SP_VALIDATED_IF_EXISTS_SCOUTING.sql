...
-- Ejemplo de Ejecucion:
				EXEC [DIPROCOM].[SONDA_SP_VALIDATED_IF_EXISTS_SCOUTING]
					@CODE_ROUTE = '46', -- varchar(50)
					@CODE_CUSTOMER = 'SO-1322', -- varchar(50)
					@DOC_SERIE = 'ADOLFO@DIPROCOM', -- varchar(50)
					@DOC_NUM = 3, -- int
					@SYNC_ID = '46|Adolfo@DIPROCOM|2017/06/16 10:24:19|-4', -- varchar(250)
					...
					@XML = 
						N'<Data>
						  <Scouting>
							...
							<DocSerie>ADOLFO@DIPROCOM</DocSerie>
							<DocNum>3</DocNum>
							<SyncId>46|Adolfo@DIPROCOM|2017/06/16 10:24:19|-4</SyncId>
...
*/			
-- =============================================
CREATE PROCEDURE [DIPROCOM].[SONDA_SP_VALIDATED_IF_EXISTS_SCOUTING](
...
BEGIN
	...
	SELECT TOP 1
		@EXISTS = 1
		,@ID = [CN].[CUSTOMER_ID]
	FROM [DIPROCOM].[SONDA_CUSTOMER_NEW] [CN] WITH(ROWLOCK,XLOCK,HOLDLOCK)
	...
	BEGIN
		...
		FROM [DIPROCOM].[SONDA_CUSTOMER_NEW] [CN] WITH(ROWLOCK,XLOCK,HOLDLOCK)
		INNER JOIN [DIPROCOM].[SONDA_TAG_X_CUSTOMER_NEW] [T] ON ([T].[CUSTOMER_ID] = [CN].[CUSTOMER_ID])
		WHERE [CN].[CODE_CUSTOMER] = @ID_FROM_XML
		GROUP BY [CN].[CUSTOMER_ID];
	END
	...
	-- ------------------------------------------------------------------------------------
	UPDATE [DIPROCOM].[SONDA_CUSTOMER_NEW]
	...
	-- Inserta el log
	-- ------------------------------------------------------------------------------------
	EXEC [DIPROCOM].[SONDA_SP_CUSTOMER_NEW_INSERT_LOG] 
