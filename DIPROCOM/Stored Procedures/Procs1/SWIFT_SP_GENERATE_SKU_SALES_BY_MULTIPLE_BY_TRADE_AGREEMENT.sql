...
*/
-- =============================================
CREATE PROCEDURE [acsa].[SWIFT_SP_GENERATE_SKU_SALES_BY_MULTIPLE_BY_TRADE_AGREEMENT] (@CODE_ROUTE VARCHAR(50)) WITH RECOMPILE
AS
BEGIN
  SET NOCOUNT ON;
  ...
  , [NAME_SALES_BY_MULTIPLE_LIST])
    ...
    LEFT JOIN [acsa].[SWIFT_TRADE_AGREEMENT] [TA]
      ON (
      [TAC].[TRADE_AGREEMENT_ID] = [TA].[TRADE_AGREEMENT_ID]
      )
  ...
  , [CODE_CUSTOMER])
    ...
    INNER JOIN [acsa].[SWIFT_SKU_SALES_BY_MULTIPLE_LIST] [S]
      ON (
  ...
  BEGIN
    ...
    FROM [acsa].[SWIFT_TRADE_AGREEMENT] [TA]
    ...
    BEGIN
      SELECT TOP 1
        @IS_ALREADY = 1
      FROM [acsa].[SWIFT_SKU_SALES_BY_MULTIPLE_LIST_BY_CUSTOMER] [SLC]
      INNER JOIN [acsa].[SWIFT_SKU_SALES_BY_MULTIPLE_LIST] [SL]
        ON (
