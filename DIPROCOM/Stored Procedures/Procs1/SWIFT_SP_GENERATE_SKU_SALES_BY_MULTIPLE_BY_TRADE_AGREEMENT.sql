...
*/
-- =============================================
CREATE PROCEDURE [PACASA].[SWIFT_SP_GENERATE_SKU_SALES_BY_MULTIPLE_BY_TRADE_AGREEMENT] (@CODE_ROUTE VARCHAR(50)) WITH RECOMPILE
AS
BEGIN
  SET NOCOUNT ON;
  ...
  , [NAME_SALES_BY_MULTIPLE_LIST])
    ...
    LEFT JOIN [PACASA].[SWIFT_TRADE_AGREEMENT] [TA]
      ON (
      [TAC].[TRADE_AGREEMENT_ID] = [TA].[TRADE_AGREEMENT_ID]
      )
  ...
  -- ------------------------------------------------------------------------------------
  INSERT INTO [PACASA].[SWIFT_SKU_SALES_BY_MULTIPLE_LIST_BY_CUSTOMER] ([SALES_BY_MULTIPLE_LIST_ID]
  , [CODE_CUSTOMER])
    SELECT DISTINCT
      [S].[SALES_BY_MULTIPLE_LIST_ID]
     ,[TA].[CODE_CUSTOMER]
    FROM @TRADE_AGREEMENT_BY_CUSTOMER [TA]
    INNER JOIN [PACASA].[SWIFT_SKU_SALES_BY_MULTIPLE_LIST] [S]
      ON (
  ...
  -- ------------------------------------------------------------------------------------
  IF (SELECT
        [PACASA].[SWIFT_FN_VALIDATE_TRADE_AGREEMENT_BY_ROUTE](@CODE_ROUTE))
    = 1
  BEGIN
    ...
    --
    SELECT
      @TRADE_AGREEMENT_ID = [PACASA].[SWIFT_FN_GET_TRADE_AGREEMENT_BY_ROUTE](@CODE_ROUTE)
    --
    SELECT
      @CODE_TRADE_AGREEMENT = [TA].[CODE_TRADE_AGREEMENT]
    FROM [PACASA].[SWIFT_TRADE_AGREEMENT] [TA]
    ...
    BEGIN
      SELECT TOP 1
        @IS_ALREADY = 1
      FROM [PACASA].[SWIFT_SKU_SALES_BY_MULTIPLE_LIST_BY_CUSTOMER] [SLC]
      INNER JOIN [PACASA].[SWIFT_SKU_SALES_BY_MULTIPLE_LIST] [SL]
        ON (
    ...
    BEGIN
      ...
      , [NAME_SALES_BY_MULTIPLE_LIST])
        ...
        FROM [PACASA].[SWIFT_TRADE_AGREEMENT] [T]
  ...
  -- ------------------------------------------------------------------------------------
  INSERT INTO [PACASA].[SWIFT_SKU_SALES_BY_MULTIPLE_LIST_BY_SKU] ([SALES_BY_MULTIPLE_LIST_ID]
  , [CODE_SKU]
  , [CODE_PACK_UNIT]
  , [MULTIPLE]
  , [PROMO_ID]
  ...
  , [FREQUENCY])
    ...
    INNER JOIN [PACASA].[SWIFT_PROMO_SKU_SALES_BY_MULTIPLE] [TAS]
      ON (
      [TAS].[PROMO_ID] = [P].[PROMO_ID]
      )
    INNER JOIN [PACASA].[SWIFT_SKU_SALES_BY_MULTIPLE_LIST] [SM]
      ON (
