...
-- Ejemplo de Ejecucion:
				-- 
				EXEC [DIPROCOM].[SWIFT_SP_GENERATE_DISCOUNT_LIST_BY_ROUTE_FOR_REPEATED_CUSTOMER]
					@CODE_ROUTE = 'RUDI@DIPROCOM'
*/
-- =============================================
CREATE PROCEDURE [DIPROCOM].[SWIFT_SP_GENERATE_DISCOUNT_LIST_BY_ROUTE_FOR_REPEATED_CUSTOMER] (@CODE_ROUTE VARCHAR(250))
AS
BEGIN
  SET NOCOUNT ON;
  ...
  SELECT TOP 1
    @LINKED_TO = [LINKED_TO]
  FROM [DIPROCOM].[SWIFT_DISCOUNT_PRIORITY]
  WHERE [ORDER] > 0
  ...
  BEGIN
    ...
    -- ------------------------------------------------------------------------------------
    INSERT INTO [DIPROCOM].[SWIFT_DISCOUNT_LIST] ([NAME_DISCOUNT_LIST]
    , [CODE_ROUTE])
    ...
    BEGIN
      ...
      , [FREQUENCY])
        SELECT
         ...
         ,[P].[PROMO_TYPE]
         ,[TAP].[FREQUENCY]
        FROM [DIPROCOM].[SWIFT_PROMO_DISCOUNT_BY_SCALE] [TAD]
      ...
      , [FREQUENCY])
        ...
        LEFT JOIN [DIPROCOM].[SWIFT_DISCOUNT_LIST_BY_SKU] [DLS]
          ON (
