...
-- Ejemplo de Ejecucion:
		--
		EXEC [acsa].SWIFT_SP_GET_MANIFEST_HEADER_FROM_PICKING
			@MANIFEST_HEADER = 3071 
					EXEC [acsa].SWIFT_SP_GET_MANIFEST_HEADER
			@MANIFEST_HEADER = 3071 
*/
-- =============================================
CREATE PROCEDURE [acsa].[SWIFT_SP_GET_MANIFEST_HEADER_FROM_PICKING] (@MANIFEST_HEADER INT)
AS
BEGIN
  SET NOCOUNT ON;
  --
  ...
  INNER JOIN [acsa].[SWIFT_MANIFEST_DETAIL] [MD]
    ON (
  ...
  INNER JOIN [acsa].[SWIFT_PICKING_HEADER] [PH]
    ON (
    [MD].[CODE_PICKING] = [PH].[PICKING_HEADER]
    )
  INNER JOIN [acsa].[SWIFT_PICKING_DETAIL] [PD]
    ON (
