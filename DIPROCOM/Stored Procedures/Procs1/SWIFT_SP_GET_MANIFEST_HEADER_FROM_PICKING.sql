...
-- Ejemplo de Ejecucion:
		--
		EXEC [DIPROCOM].SWIFT_SP_GET_MANIFEST_HEADER_FROM_PICKING
			@MANIFEST_HEADER = 3071 
					EXEC [DIPROCOM].SWIFT_SP_GET_MANIFEST_HEADER
			@MANIFEST_HEADER = 3071 
*/
-- =============================================
CREATE PROCEDURE [DIPROCOM].[SWIFT_SP_GET_MANIFEST_HEADER_FROM_PICKING] (@MANIFEST_HEADER INT)
AS
BEGIN
  SET NOCOUNT ON;
  --
  SELECT
   ...
   ,SUM([PD].[SCANNED]) QTY
   ,0 [TOTAL_AMOUNT]
  FROM [DIPROCOM].[SWIFT_MANIFEST_HEADER] [MH]
  INNER JOIN [DIPROCOM].[SWIFT_MANIFEST_DETAIL] [MD]
    ON (
  ...
  INNER JOIN [DIPROCOM].[SWIFT_PICKING_HEADER] [PH]
    ON (
    [MD].[CODE_PICKING] = [PH].[PICKING_HEADER]
    )
  INNER JOIN [DIPROCOM].[SWIFT_PICKING_DETAIL] [PD]
    ON (
