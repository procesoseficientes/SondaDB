...
-- Ejemplo de Ejecucion:
				EXEC [acsa].[SWIFT_SP_GET_SKU_WITH_INVETORY_BY_DISTRIBUTION_CENTER]
					@LOGIN = 'GERENTE@DIPROCOM'
*/
-- =============================================
CREATE PROCEDURE [acsa].[SWIFT_SP_GET_SKU_WITH_INVETORY_BY_DISTRIBUTION_CENTER](
	@LOGIN VARCHAR(50)
)
AS
BEGIN
	...
	INNER JOIN [acsa].[SWIFT_WAREHOUSE_X_DISTRIBUTION_CENTER] [WDC] ON ([I].[WAREHOUSE] = [WDC].[CODE_WAREHOUSE])
	...
	INNER JOIN [SWIFT_INTERFACES].[acsa].[ERP_VIEW_SKU] [EVS] ON [EVS].[CODE_SKU] = [S].[CODE_SKU]
	LEFT JOIN [acsa].[SWIFT_PROVIDERS] [PR] ON [S].[CODE_PROVIDER] = [PR].[CODE_PROVIDER] 
