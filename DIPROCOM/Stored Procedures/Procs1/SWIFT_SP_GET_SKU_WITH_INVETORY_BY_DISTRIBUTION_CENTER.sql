...
-- Ejemplo de Ejecucion:
				EXEC [PACASA].[SWIFT_SP_GET_SKU_WITH_INVETORY_BY_DISTRIBUTION_CENTER]
					@LOGIN = 'GERENTE@DIPROCOM'
*/
-- =============================================
CREATE PROCEDURE [PACASA].[SWIFT_SP_GET_SKU_WITH_INVETORY_BY_DISTRIBUTION_CENTER](
	@LOGIN VARCHAR(50)
)
AS
BEGIN
	...
	INNER JOIN [PACASA].[SWIFT_WAREHOUSE_X_DISTRIBUTION_CENTER] [WDC] ON ([I].[WAREHOUSE] = [WDC].[CODE_WAREHOUSE])
	...
	SELECT 
		...
		,[EVS].[LONG_SKU]
		,[EVS].[WIDTH_SKU]
	FROM [PACASA].[SWIFT_VIEW_ALL_SKU] [S]
	INNER JOIN [SWIFT_INTERFACES].[PACASA].[ERP_VIEW_SKU] [EVS] ON [EVS].[CODE_SKU] = [S].[CODE_SKU]
	LEFT JOIN [PACASA].[SWIFT_PROVIDERS] [PR] ON [S].[CODE_PROVIDER] = [PR].[CODE_PROVIDER] 
