...
-- Ejemplo de Ejecucion:
				EXEC [DIPROCOM].[SWIFT_SP_GET_SKU_WITH_INVETORY_BY_DISTRIBUTION_CENTER]
					@LOGIN = 'GERENTE@DIPROCOM'
*/
-- =============================================
CREATE PROCEDURE [DIPROCOM].[SWIFT_SP_GET_SKU_WITH_INVETORY_BY_DISTRIBUTION_CENTER](
	@LOGIN VARCHAR(50)
)
AS
BEGIN
	...
	--
	SELECT @DISTRIBUTION_CENTER_ID = [DIPROCOM].[SWIFT_FN_GET_DISTRIBUTION_CENTER_BY_LOGIN](@LOGIN)
	...
	INNER JOIN [DIPROCOM].[SWIFT_WAREHOUSE_X_DISTRIBUTION_CENTER] [WDC] ON ([I].[WAREHOUSE] = [WDC].[CODE_WAREHOUSE])
	...
	INNER JOIN [SWIFT_INTERFACES].[DIPROCOM].[ERP_VIEW_SKU] [EVS] ON [EVS].[CODE_SKU] = [S].[CODE_SKU]
	LEFT JOIN [DIPROCOM].[SWIFT_PROVIDERS] [PR] ON [S].[CODE_PROVIDER] = [PR].[CODE_PROVIDER] 
