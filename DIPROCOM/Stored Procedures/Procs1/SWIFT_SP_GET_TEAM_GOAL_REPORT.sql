...
-- Ejemplo de Ejecucion:
				EXEC [DIPROCOM].[SWIFT_SP_GET_TEAM_GOAL_REPORT]
					@TEAM_ID = 2
*/
-- =============================================
CREATE PROCEDURE [DIPROCOM].[SWIFT_SP_GET_TEAM_GOAL_REPORT] (@TEAM_ID INT)
AS
BEGIN
    SET NOCOUNT ON;
	--
    ...
    SELECT
       ...
       ,'PRE'
    FROM
        [DIPROCOM].[SONDA_SALES_ORDER_HEADER] [SOH]
    INNER JOIN [DIPROCOM].[SWIFT_TASKS] [TS] ON [TS].[TASK_ID] = [SOH].[TASK_ID]
    INNER JOIN [DIPROCOM].[USERS] [U] ON [SOH].[POSTED_BY] = [U].[LOGIN]
    INNER JOIN [DIPROCOM].[SWIFT_USER_BY_TEAM] [UT] ON [UT].[USER_ID] = [U].[CORRELATIVE]
    INNER JOIN [DIPROCOM].[SWIFT_TEAM] [T] ON [T].[TEAM_ID] = [UT].[TEAM_ID]
    INNER JOIN [DIPROCOM].[USERS] [S] ON [S].[CORRELATIVE] = [T].[SUPERVISOR]
    WHERE
        [UT].[TEAM_ID] = @TEAM_ID
    ...
    SELECT
       ...
       ,[DIPROCOM].[SWIFT_FN_GET_GOAL_WORK_DAYS](GETDATE(), [GH].[GOAL_DATE_TO],
                                              [GH].[INCLUDE_SATURDAY]) [DAYS_LEFT]
       ,[DIPROCOM].[SWIFT_FN_GET_GOAL_WORK_DAYS]([GH].[GOAL_DATE_FROM], GETDATE(),
       ...
       ,@CURRENCY [CURRENCY]
    FROM
        [DIPROCOM].[SWIFT_GOAL_HEADER] [GH]
    INNER JOIN [DIPROCOM].[SWIFT_GOAL_DETAIL] [GD] ON [GD].[GOAL_HEADER_ID] = [GH].[GOAL_HEADER_ID]
    INNER JOIN [DIPROCOM].[USERS] [U] ON [U].[CORRELATIVE] = [GD].[SELLER_ID]
    LEFT JOIN [DIPROCOM].[SWIFT_DAILY_GOAL_BY_SELLER] [DG] ON [DG].[TEAM_ID] = [GH].[TEAM_ID]
