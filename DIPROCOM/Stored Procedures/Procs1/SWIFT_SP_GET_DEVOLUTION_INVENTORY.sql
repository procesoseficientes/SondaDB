...
-- Ejemplo de Ejecucion:
				EXEC [PACASA].[SWIFT_SP_GET_DEVOLUTION_INVENTORY]
					@START_DATETIME = '20160101 00:00:00.000'
					,@END_DATETIME = '20170101 00:00:00.000'
					,@LOGIN = 'GERENTE@PACASA'
				--
				SELECT * FROM [PACASA].[SONDA_DEVOLUTION_INVENTORY_DETAIL] WHERE DEVOLUTION_ID = 1005
				SELECT * FROM [PACASA].[SONDA_DEVOLUTION_INVENTORY_HEADER] WHERE DEVOLUTION_ID = 1005
				--
				SELECT * 
				FROM [PACASA].[SONDA_HISTORICAL_TRACEABILITY_CONSIGNMENT] 
				WHERE [DOC_SERIE_TARGET] = 'Serie Recoger Inventario'
					AND [DOC_NUM_TARGET] = 19

*/
-- =============================================
CREATE PROCEDURE [PACASA].[SWIFT_SP_GET_DEVOLUTION_INVENTORY](
...
BEGIN
	...
	SET @QUERY = N'SELECT
		...
		,CONVERT(DECIMAL(18,' + CAST(@DEFAULT_DISPLAY_DECIMALS AS VARCHAR) + '),[PACASA].SWIFT_FN_GET_DISPLAY_NUMBER(MAX([PLS].[COST]))) [PRICE]
		...
		,CONVERT(DECIMAL(18,' + CAST(@DEFAULT_DISPLAY_DECIMALS AS VARCHAR) + '),[PACASA].SWIFT_FN_GET_DISPLAY_NUMBER(SUM(CASE CAST([DID].[IS_GOOD_STATE] AS NUMERIC(18,2))
		...
		,CONVERT(DECIMAL(18,' + CAST(@DEFAULT_DISPLAY_DECIMALS AS VARCHAR) + '),[PACASA].SWIFT_FN_GET_DISPLAY_NUMBER(SUM([DID].[TOTAL_AMOUNT]))) [TOTAL_AMOUNT]
	...
	FROM [PACASA].[SONDA_DEVOLUTION_INVENTORY_DETAIL] [DID]
	INNER JOIN [PACASA].[SONDA_DEVOLUTION_INVENTORY_HEADER] [DIH] ON (
	...
	INNER JOIN [PACASA].[SWIFT_PRICE_LIST_BY_CUSTOMER] [PLC] ON (
		[PLC].[CODE_CUSTOMER] = [DIH].[CODE_CUSTOMER]
	)
	INNER JOIN [PACASA].[SWIFT_PRICE_LIST_BY_SKU] [PLS] ON (
