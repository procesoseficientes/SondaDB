...
-- Ejemplo de Ejecucion:
		EXEC [acsa].[SONDA_SP_GET_PRICE_LIST_OUTSIDE_ROUTE]
		@TRANSFER_ID = 177,
		@ROUTE_ID = '46',
		@XML = '<skus>
					<skus>100018</skus>
...
CREATE PROCEDURE [acsa].[SONDA_SP_GET_PRICE_LIST_OUTSIDE_ROUTE]
	(
		@TRANSFER_ID INT
		,@ROUTE_ID VARCHAR(25)
		,@XML XML
...
AS
	BEGIN
		...
		BEGIN TRY 
			...
			FROM
				[acsa].[SWIFT_INVENTORY] [SI]
			INNER JOIN [acsa].[SWIFT_VIEW_ALL_SKU] [s]
			ON	([s].[CODE_SKU] = [SI].[SKU])
			INNER JOIN [acsa].[SWIFT_TRANSFER_DETAIL] AS [TD]
			ON	(
					[TD].[SKU_CODE] = [SI].[SKU]
			...
			LEFT JOIN [acsa].[SONDA_POS_SKUS] AS [PS]
			ON	(
					[PS].[SKU] = [TS].[SKU]
			...
			SELECT
				...
				,[PLPS].[PRICE]
			FROM
				[acsa].[SWIFT_PRICE_LIST_BY_SKU_PACK_SCALE] [PLPS]
			WHERE
				[PLPS].[CODE_PRICE_LIST] IN (
				SELECT DISTINCT
					[PL].[CODE_PRICE_LIST]
				FROM
					[acsa].[SWIFT_PRICE_LIST_BY_CUSTOMER_FOR_ROUTE] AS [PL]
				INNER JOIN [acsa].[SONDA_ROUTE_PLAN] AS [RP]
			...
			SELECT
				...
				,[PC].[ORDER]
			FROM
				[acsa].[SONDA_PACK_CONVERSION] AS [PC]
			INNER JOIN @TEMP_SKU AS [TS]
