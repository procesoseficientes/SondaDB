...
-- Ejemplo de Ejecucion:
		EXEC [PACASA].[SONDA_SP_GET_PRICE_LIST_OUTSIDE_ROUTE]
		@TRANSFER_ID = 177,
		@ROUTE_ID = '46',
		@XML = '<skus>
					<skus>100018</skus>
...
CREATE PROCEDURE [PACASA].[SONDA_SP_GET_PRICE_LIST_OUTSIDE_ROUTE]
	(
		@TRANSFER_ID INT
		,@ROUTE_ID VARCHAR(25)
		,@XML XML
...
AS
	BEGIN
		...
		BEGIN TRY 
			...
			-- ------------------------------------------------------
			INSERT	INTO [PACASA].[SONDA_POS_SKUS]
					(
						[SKU]
						,[SKU_NAME]
						,[SKU_PRICE]
			...
			SELECT
				...
				,'ST' [CODE_PACK_UNIT_STOCK]
			FROM
				[PACASA].[SWIFT_INVENTORY] [SI]
			INNER JOIN [PACASA].[SWIFT_VIEW_ALL_SKU] [s]
			ON	([s].[CODE_SKU] = [SI].[SKU])
			INNER JOIN [PACASA].[SWIFT_TRANSFER_DETAIL] AS [TD]
			ON	(
					[TD].[SKU_CODE] = [SI].[SKU]
			...
			LEFT JOIN [PACASA].[SONDA_POS_SKUS] AS [PS]
			ON	(
					[PS].[SKU] = [TS].[SKU]
			...
			SELECT
				...
				,[PLPS].[PRICE]
			FROM
				[PACASA].[SWIFT_PRICE_LIST_BY_SKU_PACK_SCALE] [PLPS]
			WHERE
				[PLPS].[CODE_PRICE_LIST] IN (
				SELECT DISTINCT
					[PL].[CODE_PRICE_LIST]
				FROM
					[PACASA].[SWIFT_PRICE_LIST_BY_CUSTOMER_FOR_ROUTE] AS [PL]
				INNER JOIN [PACASA].[SONDA_ROUTE_PLAN] AS [RP]
			...
			SELECT
				...
				,[PC].[ORDER]
			FROM
				[PACASA].[SONDA_PACK_CONVERSION] AS [PC]
			INNER JOIN @TEMP_SKU AS [TS]
