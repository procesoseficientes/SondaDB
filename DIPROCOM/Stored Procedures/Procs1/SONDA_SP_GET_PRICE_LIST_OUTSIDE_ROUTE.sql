...
-- Ejemplo de Ejecucion:
		EXEC [DIPROCOM].[SONDA_SP_GET_PRICE_LIST_OUTSIDE_ROUTE]
		@TRANSFER_ID = 177,
		@ROUTE_ID = '46',
		@XML = '<skus>
					<skus>100018</skus>
...
CREATE PROCEDURE [DIPROCOM].[SONDA_SP_GET_PRICE_LIST_OUTSIDE_ROUTE]
	(
		@TRANSFER_ID INT
		,@ROUTE_ID VARCHAR(25)
		,@XML XML
...
AS
	BEGIN
		...
		BEGIN TRY 
			...
			FROM
				[DIPROCOM].[SWIFT_INVENTORY] [SI]
			INNER JOIN [DIPROCOM].[SWIFT_VIEW_ALL_SKU] [s]
			ON	([s].[CODE_SKU] = [SI].[SKU])
			INNER JOIN [DIPROCOM].[SWIFT_TRANSFER_DETAIL] AS [TD]
			ON	(
					[TD].[SKU_CODE] = [SI].[SKU]
			...
			LEFT JOIN [DIPROCOM].[SONDA_POS_SKUS] AS [PS]
			ON	(
					[PS].[SKU] = [TS].[SKU]
			...
			SELECT
				...
				,[PLPS].[PRICE]
			FROM
				[DIPROCOM].[SWIFT_PRICE_LIST_BY_SKU_PACK_SCALE] [PLPS]
			WHERE
				[PLPS].[CODE_PRICE_LIST] IN (
				SELECT DISTINCT
					[PL].[CODE_PRICE_LIST]
				FROM
					[DIPROCOM].[SWIFT_PRICE_LIST_BY_CUSTOMER_FOR_ROUTE] AS [PL]
				INNER JOIN [DIPROCOM].[SONDA_ROUTE_PLAN] AS [RP]
			...
			SELECT
				...
				,[PC].[ORDER]
			FROM
				[DIPROCOM].[SONDA_PACK_CONVERSION] AS [PC]
			INNER JOIN @TEMP_SKU AS [TS]
