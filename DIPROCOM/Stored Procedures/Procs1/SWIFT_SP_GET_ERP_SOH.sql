...
-- Ejemplo de Ejecucion:
      ...
      SET @PICKING_HEADER = '1' 
      
      EXECUTE @RC = DIPROCOM.SWIFT_SP_GET_ERP_SOH @PICKING_HEADER
      GO
*/
CREATE PROC [DIPROCOM].[SWIFT_SP_GET_ERP_SOH] 
@PICKING_HEADER VARCHAR(50)
AS 
DECLARE @SCHEMA VARCHAR(20)='DIPROCOM';
DECLARE @DOC_SAP_RECEPTION VARCHAR(MAX)='-1';
DECLARE @SQL VARCHAR(MAX);
SET NOCOUNT ON

DECLARE @CLASSIFICATION_PICKING VARCHAR(50);

SELECT @DOC_SAP_RECEPTION= ISNULL(DOC_SAP_RECEPTION,'-1'), @CLASSIFICATION_PICKING=sph.CLASSIFICATION_PICKING   FROM DIPROCOM.SWIFT_PICKING_HEADER sph
  WHERE sph.PICKING_HEADER= @PICKING_HEADER;



IF (@CLASSIFICATION_PICKING='3')  
  BEGIN  
  	EXEC DIPROCOM.SWIFT_SP_GET_ERP_ITWH @PICKING_HEADER;
    RETURN;
  END
...
FROM         ['+@SCHEMA+'].[SWIFT_PICKING_HEADER] AS t INNER JOIN
                          (SELECT     so.DocEntry, so.DocNum, so.DocType, so.CANCELED, so.Handwrtten, so.Printed, so.DocStatus, so.InvntSttus, so.Transfered, so.ObjType, so.DocDate, so.DocDueDate, so.CardCode, 
                                                   ...
                                                   so.PaidDpmF, so.PaidDpmS, so.U_Guatex UGuatex, NULL UKm, so.Ship_To_Address_Type AS ShipToAddressType, so.Ship_To_Street AS ShipToStreet, so.Ship_To_State AS ShipToState, 
  ...
  so.DocEntry, so.DocNum, so.DocType, so.CANCELED, so.Handwrtten, so.Printed, so.DocStatus, so.InvntSttus, so.Transfered, so.ObjType, so.DocDate, so.DocDueDate, so.CardCode, 
                                                   ...
                                                   so.EDocCancel, so.EDocTest, so.EDocPrefix, so.CUP, so.CIG, so.DpmAsDscnt, so.Attachment, so.AtcEntry, so.SupplCode, so.GTSRlvnt, so.BaseDisc, so.BaseDiscSc, so.BaseDiscFc, 
                                                   so.BaseDiscPr, so.CreateTS, so.UpdateTS, so.SrvTaxRule, so.AnnInvDecR, so.Supplier, so.Releaser, so.Receiver, so.AgrNo, so.IsAlt, so.AltBaseTyp, so.AltBaseEnt, so.PaidDpm, 
                                                   so.PaidDpmF, so.PaidDpmS, NULL AS U_Guatex, NULL AS U_KM, ae.AddrTypeS AS Ship_To_Address_Type, ae.StreetS AS Ship_To_Street, ae.StateS AS Ship_To_State, 
                                                   ae.CountryS AS Ship_To_Country
FROM  [BD_Prueba_Viscosa].dbo.ORDR AS so  LEFT OUTER JOIN
                                                   [BD_Prueba_Viscosa].dbo.RDR12 AS ae ON ae.DocEntry = so.DocEntry                                                   
...
and t.picking_header ='+ @PICKING_HEADER+'
  AND t.CLASSIFICATION_PICKING= 4
';

PRINT ('====================EJECUCION DEL SCRIPT=============================');
EXEC (@SQL);
PRINT ('==================FIN EJECUCION DEL SCRIPT===========================');
