...
-- Ejemplo de Ejecucion:
				EXEC [DIPROCOM].[SWIFT_SP_GENERATE_PRICE_LIST]
					@CODE_ROUTE = 'ES000007'
*/
-- =============================================
CREATE PROCEDURE [DIPROCOM].[SWIFT_SP_GENERATE_PRICE_LIST](
...
BEGIN
	...
	INNER JOIN [DIPROCOM].[SWIFT_VIEW_ALL_COSTUMER] [VC] ON ([VC].[CODE_CUSTOMER] = [RP].[RELATED_CLIENT_CODE])
	...
	SELECT
		...
		,[PLC].[CODE_PRICE_LIST]
		,[C].[OWNER]
	FROM [DIPROCOM].[SWIFT_PRICE_LIST_BY_CUSTOMER] [PLC]
	...
	BEGIN
		...
		SELECT DISTINCT
			...
			,[PLS].[HIGH_LIMIT]
			,[PLS].[PRICE]
		FROM [DIPROCOM].[SWIFT_PRICE_LIST_BY_SKU_PACK_SCALE] [PLS]
	...
	BEGIN
		...
		BEGIN
			...
			SELECT
				...
				,[SPS].[HIGH_LIMIT]
				,[SPS].[PRICE]
			FROM [DIPROCOM].[SWIFT_PRICE_LIST_BY_SKU_PACK_SCALE] [SPS]
	...
	-- ------------------------------------------------------------------------------------
	INSERT INTO [DIPROCOM].[SWIFT_PRICE_LIST_BY_CUSTOMER_FOR_ROUTE]
	(
		[CODE_ROUTE]
		,[CODE_PRICE_LIST]
		,[CODE_CUSTOMER]
	...
	--
	INSERT INTO [DIPROCOM].[SWIFT_PRICE_LIST_BY_SKU_PACK_SCALE_FOR_ROUTE]
			(
				[CODE_ROUTE]
				,[CODE_PRICE_LIST]
				,[CODE_SKU]
