...
-- Ejemplo de Ejecucion:
				EXEC [DIPROCOM].[SWIFT_SP_GET_TOP_SALES_TO_POLYGON_REPORT]
...
*/
-- =============================================
CREATE PROCEDURE [DIPROCOM].SWIFT_SP_GET_TOP_SALES_TO_POLYGON_REPORT (@START_DATETIME DATETIME
...
BEGIN
  ...
  BEGIN TRY
    ...
    FROM [DIPROCOM].[SWIFT_POLYGON] [P]
		INNER JOIN [DIPROCOM].[SWIFT_POLYGON] [P2] ON [P].[POLYGON_ID_PARENT] = [P2].[POLYGON_ID]
    ...
    SET @QUERY = N'SELECT 
			...
			,[A].[CUSTOMERS_COUNT]
			,CONVERT(DECIMAL(18,' + CAST(@DEFAULT_DISPLAY_DECIMALS AS VARCHAR) + '),[DIPROCOM].SWIFT_FN_GET_DISPLAY_NUMBER([PA].[TOTAL_AMOUNT])) [TOTAL_SALES]
			,[R].[CODE_ROUTE]
			,[R].[NAME_ROUTE]
      ,[PA].[SELLER_CODE]
      ,[PA].[SELLER_NAME]
		...
		INNER JOIN [DIPROCOM].[SWIFT_POLYGON] [PS] ON (
			[P].[POLYGON_ID_PARENT] = [PS].[POLYGON_ID]
		)
		INNER JOIN [DIPROCOM].[SWIFT_POLYGON] [PR] ON (
			[PS].[POLYGON_ID_PARENT] = [PR].[POLYGON_ID]
		)
		LEFT JOIN [DIPROCOM].[SWIFT_POLYGON_BY_ROUTE] [PBR] ON(
