...
-- Ejemplo de Ejecucion:
			EXEC  [acsa].[SWIFT_SP_GET_BONUS_BY_COMBO_OF_PROMO] @PROMO_ID = 6
*/
-- =============================================

CREATE PROCEDURE [acsa].[SWIFT_SP_GET_BONUS_BY_COMBO_OF_PROMO] (@PROMO_ID INT)
AS
BEGIN
  SET NOCOUNT ON;
  --
  ...
  INNER JOIN [acsa].[SWIFT_SKU_BY_COMBO] [SBC]
    ON [C].[COMBO_ID] = [SBC].[COMBO_ID]
  INNER JOIN [acsa].[SWIFT_PROMO_BY_COMBO_PROMO_RULE] [PBC]
    ON [C].[COMBO_ID] = [PBC].[COMBO_ID]
  INNER JOIN [acsa].[SWIFT_PROMO_BY_BONUS_RULE] [PBBR]
