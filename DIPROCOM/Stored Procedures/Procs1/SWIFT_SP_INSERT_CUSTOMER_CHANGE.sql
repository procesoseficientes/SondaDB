...
/*
  EXEC [acsa].[SWIFT_SP_INSERT_CUSTOMER_CHANGE]
  	 @CODE_CUSTOMER = 'RD001'
    ...
    SELECT * FROM [acsa].SWIFT_CUSTOMER_CHANGE WHERE CODE_CUSTOMER = 'RD001'
*/
...
BEGIN TRY
	...
	DELETE
		[T]
	FROM
		[acsa].[SWIFT_TAG_X_CUSTOMER_CHANGE] [T]
	INNER JOIN [acsa].[SWIFT_CUSTOMER_CHANGE] [CC]
	...
	--
	DELETE FROM
		[acsa].[SWIFT_CUSTOMER_CHANGE]
	WHERE
		[CODE_CUSTOMER] = @CODE_CUSTOMER
		AND [POSTED_DATETIME] = @POSTED_DATETIME;
	--
	SELECT @OWNER = [OWNER] , @OWNER_ID = [OWNER_ID] 
	FROM [acsa].[SWIFT_VIEW_ALL_COSTUMER] WHERE [CODE_CUSTOMER] = @CODE_CUSTOMER
	--
	INSERT	INTO [acsa].[SWIFT_CUSTOMER_CHANGE]
			(
				[CODE_CUSTOMER]
				,[PHONE_CUSTOMER]
				,[ADRESS_CUSTOMER]
