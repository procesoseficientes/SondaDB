...
/*
  EXEC [PACASA].[SWIFT_SP_INSERT_CUSTOMER_CHANGE]
  	 @CODE_CUSTOMER = 'RD001'
    ...
    SELECT * FROM [PACASA].SWIFT_CUSTOMER_CHANGE WHERE CODE_CUSTOMER = 'RD001'
*/


-- =============================================
CREATE PROCEDURE [PACASA].[SWIFT_SP_INSERT_CUSTOMER_CHANGE] 
...
BEGIN TRY
	...
	DELETE
		[T]
	FROM
		[PACASA].[SWIFT_TAG_X_CUSTOMER_CHANGE] [T]
	INNER JOIN [PACASA].[SWIFT_CUSTOMER_CHANGE] [CC]
	...
	--
	DELETE FROM
		[PACASA].[SWIFT_CUSTOMER_CHANGE]
	WHERE
		[CODE_CUSTOMER] = @CODE_CUSTOMER
		AND [POSTED_DATETIME] = @POSTED_DATETIME;
	--
	SELECT @OWNER = [OWNER] , @OWNER_ID = [OWNER_ID] 
	FROM [PACASA].[SWIFT_VIEW_ALL_COSTUMER] WHERE [CODE_CUSTOMER] = @CODE_CUSTOMER
	--
	INSERT	INTO [PACASA].[SWIFT_CUSTOMER_CHANGE]
			(
				[CODE_CUSTOMER]
				,[PHONE_CUSTOMER]
				,[ADRESS_CUSTOMER]
