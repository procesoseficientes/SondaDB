...
-- Fecha Modificación:	26/11/2019
-- Descripción:			Se agregaron campos necesarios para proceso FEL
-- =============================================

CREATE PROCEDURE [PACASA].[SONDA_SP_GET_TASKS_FOR_SALE_ROUTE_PLAN] @CODE_ROUTE VARCHAR(50)
AS
BEGIN
    SET NOCOUNT ON;
  ...
  FROM [PACASA].[SONDA_ROUTE_PLAN] [RP]
        ...
        LEFT JOIN [PACASA].[SWIFT_PRICE_LIST_BY_CUSTOMER_FOR_ROUTE] [LCR]
            ON [LCR].[CODE_CUSTOMER] = [C].[CODE_CUSTOMER]
        LEFT JOIN SWIFT_INTERFACES.[PACASA].[SWIFT_ERP_CUSTOMERS] [EC]
            ON [RP].[RELATED_CLIENT_CODE] = [EC].[CODE_CUSTOMER] COLLATE DATABASE_DEFAULT
