...
create PROCEDURE diprocom.SONDA_SP_GET_TASKS_FOR_SALE_ROUTE_PLAN
...
AS
    BEGIN
        ...
        FROM    [diprocom].[SONDA_ROUTE_PLAN] [RP]
                INNER JOIN [diprocom].[SWIFT_VIEW_ALL_COSTUMER] [C] ON [C].[CODE_CUSTOMER] = [RP].[RELATED_CLIENT_CODE]
                LEFT JOIN [diprocom].SWIFT_PRICE_LIST_BY_CUSTOMER_FOR_ROUTE [LCR] ON [LCR].[CODE_CUSTOMER] = [C].[CODE_CUSTOMER]
