...
-- Ejemplo de Ejecucion:
		--
        EXEC [DIPROCOM].SONDA_SP_GET_CURRENT_ACCOUNT_BY_CUSTOMER @CODE_COSTUMER = '1', @CURRENT_AMOUT_PAYMENT = 1500, @CODE_ROUTE = 'rudi@DIPROCOM'
*/

-- =============================================
CREATE PROCEDURE [DIPROCOM].[SONDA_SP_GET_CURRENT_ACCOUNT_BY_CUSTOMER]	
...
BEGIN
	...
	-- ------------------------------------------------------------------------------------
	IF ([DIPROCOM].[SWIFT_FN_VALIDATE_EVENT_FOR_ROUTE]('NoValidarAntiguedadDeSaldos',@CODE_ROUTE) = 1)
	BEGIN
		GOTO ENDSP
	END
	...
	ELSE
	BEGIN
		IF (@SALES_ORDER_TYPE = (SELECT [DIPROCOM].[SWIFT_FN_GET_PARAMETER]('SALES_ORDER_TYPE','CASH' )))
		BEGIN
			SET @IS_CASH = 1
		END
	END
	...
	INSERT INTO #INVOICE
	EXEC [DIPROCOM].SONDA_SP_GET_INVOICE_ACTIVE_BY_COSTUMER @CODE_COSTUMER
