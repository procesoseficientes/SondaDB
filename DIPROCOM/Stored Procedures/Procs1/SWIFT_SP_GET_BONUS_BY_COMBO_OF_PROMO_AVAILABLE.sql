...
-- Ejemplo de Ejecucion:
			EXEC  [DIPROCOM].[SWIFT_SP_GET_BONUS_BY_COMBO_OF_PROMO_AVAILABLE] @PROMO_ID = 66
*/
-- =============================================
CREATE PROCEDURE [DIPROCOM].[SWIFT_SP_GET_BONUS_BY_COMBO_OF_PROMO_AVAILABLE] (@PROMO_ID INT = NULL)
AS
BEGIN
  SET NOCOUNT ON;
  --
  ...
  INNER JOIN [DIPROCOM].[SWIFT_SKU_BY_COMBO] [SC]
    ON ([C].[COMBO_ID] = [SC].[COMBO_ID])
  LEFT JOIN [DIPROCOM].[SWIFT_VIEW_COMBO_BY_PROMO] [VCBP]
