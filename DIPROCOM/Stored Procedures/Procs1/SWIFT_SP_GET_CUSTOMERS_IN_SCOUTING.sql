...
*/
-- =============================================
CREATE PROCEDURE [PACASA].[SWIFT_SP_GET_CUSTOMERS_IN_SCOUTING]
...
BEGIN
	...
	INTO #TAG_TIENDA
	FROM [PACASA].SWIFT_TAG_X_CUSTOMER_NEW T
	INNER JOIN [PACASA].SWIFT_TAGS TA ON (T.TAG_COLOR = TA.TAG_COLOR)
	GROUP BY T.CUSTOMER
	...
	INNER JOIN [PACASA].SWIFT_TAG_X_CUSTOMER_NEW T ON (C.CODE_CUSTOMER = T.CUSTOMER)
	INNER JOIN [PACASA].SWIFT_TAGS TA ON (T.TAG_COLOR = TA.TAG_COLOR)
	...
	INTO #TAG_TIENDA_2
	FROM #TAG_TIENDA TT
	INNER JOIN [PACASA].SWIFT_TAG_X_CUSTOMER_NEW T ON (TT.CUSTOMER = T.CUSTOMER)
	INNER JOIN [PACASA].SWIFT_TAGS TA ON (T.TAG_COLOR = TA.TAG_COLOR)
	...
	SELECT
		...
		,ISNULL(TA.TAG_VALUE_TEXT,'Sin Etiqueta') Etiqueta
	FROM [PACASA].SWIFT_CUSTOMERS_NEW C
	LEFT JOIN [PACASA].SWIFT_TAG_X_CUSTOMER_NEW T ON (C.CODE_CUSTOMER = T.CUSTOMER)
	LEFT JOIN [PACASA].SWIFT_TAGS TA ON (T.TAG_COLOR = TA.TAG_COLOR)
	---WHERE C.LAST_UPDATE < '20160127 00:00:00.000'
