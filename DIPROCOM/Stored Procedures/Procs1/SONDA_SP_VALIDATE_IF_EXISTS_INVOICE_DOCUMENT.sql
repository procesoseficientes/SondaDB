...
*/
-- =============================================
CREATE PROCEDURE [PACASA].[SONDA_SP_VALIDATE_IF_EXISTS_INVOICE_DOCUMENT](
...
BEGIN
	...
	SELECT TOP 1
		@EXISTS = 1
		,@ID = H.[ID]
	FROM [PACASA].[SONDA_POS_INVOICE_HEADER] [H] WITH(ROWLOCK,XLOCK,HOLDLOCK)
	...
	BEGIN
		...
		FROM [PACASA].[SONDA_POS_INVOICE_HEADER] [H] WITH(ROWLOCK,XLOCK,HOLDLOCK)
		INNER JOIN [PACASA].[SONDA_POS_INVOICE_DETAIL] [D] ON ([H].[ID] = [D].[ID])
		WHERE [H].[ID] = @ID_BO
		GROUP BY [H].[ID];
	END
	-- ------------------------------------------------------------------------------------
	...
	BEGIN
		BEGIN TRY
			BEGIN TRAN
			--
			EXEC [PACASA].[SONDA_SP_DECREASE_INVENTORY_BY_INVOICE] @ID = @ID
			--
...
--	-- Inserta el log
--	-- ------------------------------------------------------------------------------------
--	EXEC [PACASA].[SONDA_SP_INSERT_INVOICE_LOG_EXISTS]		
