CREATE PROCEDURE [PACASA].[SWIFT_SP_INSERT_CUSTOMER_FREQUENCY_TASK]
AS
BEGIN
...
FROM  
	[PACASA].SWIFT_VIEW_ALL_COSTUMER  A,
	[PACASA].SWIFT_CUSTOMER_FREQUENCY B
WHERE 
	...
	AND '1' = CASE WHEN @columName = 'MONDAY' THEN B.MONDAY 
			...
			END
	AND  A.CODE_CUSTOMER NOT IN (SELECT COSTUMER_CODE FROM [PACASA].SWIFT_TASKS WHERE SCHEDULE_FOR = CONVERT(DATE,GETDATE()) AND (TASK_TYPE = 'PRESALE' OR TASK_TYPE = 'SALE') AND TASK_STATUS = 'ASSIGNED')
	AND  CURRENT_TIMESTAMP >= DATEADD(WEEK,CONVERT(INT,B.FREQUENCY_WEEKS), B.LAST_DATE_VISITED)

-------------------------------------------------------------------------------------------------------------------------------------------------------
SET @USER_ROLE = ISNULL((SELECT [USER_TYPE] FROM [PACASA].USERS WHERE RELATED_SELLER = (SELECT SELLER_CODE FROM [PACASA].SWIFT_VIEW_ALL_SELLERS WHERE SELLER_NAME = (SELECT DISTINCT SELLER_DEFAULT_CODE FROM [PACASA].SWIFT_VIEW_ALL_COSTUMER WHERE CODE_CUSTOMER = @CLIENT_CODE))),'NULL')
PRINT @USER_ROLE
PRINT @columName
PRINT @CLIENT_CODE
PRINT @NAME_CLIENT
...
VALUES(
	...
	GETDATE(),
	--MODIFICAR
	ISNULL((SELECT TOP 1 BRANCH_ADDRESS FROM [PACASA].SWIFT_BRANCHES WHERE CUSTOMER_CODE = @CLIENT_CODE AND IS_DEFAULT = 1 ),'N/A'),
	(SELECT [LOGIN] FROM [PACASA].USERS WHERE RELATED_SELLER = (SELECT SELLER_CODE FROM SWIFT_VIEW_ALL_SELLERS WHERE SELLER_NAME = (SELECT DISTINCT SELLER_DEFAULT_CODE FROM [PACASA].SWIFT_VIEW_ALL_COSTUMER WHERE CODE_CUSTOMER = @CLIENT_CODE))),
	'ASSIGNED',
	1,
	'0,0',
	ISNULL((SELECT TOP 1 RECOLLECT_EMAIL FROM [PACASA].SWIFT_BRANCHES WHERE CUSTOMER_CODE = @CLIENT_CODE AND IS_DEFAULT = 1 ),'N/A'),
	...
	UPDATE [PACASA].[SWIFT_CUSTOMER_FREQUENCY]
		SET [LAST_DATE_VISITED] = CURRENT_TIMESTAMP
	WHERE CODE_CUSTOMER = @CLIENT_CODE

	PRINT 'POR EL EXECUTE'
	EXECUTE [PACASA].SWIFT_SP_INSERT_CUSTOMER_FREQUENCY_TASK
END
