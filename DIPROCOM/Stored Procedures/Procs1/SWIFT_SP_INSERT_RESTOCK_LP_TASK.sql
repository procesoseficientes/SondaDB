...
/*
-- DROP PROCEDURE [DIPROCOM].[SWIFT_SP_INSERT_RESTOCK_LP_TASK]
-- Ejemplo de Ejecucion:
        EXEC  [DIPROCOM].[SWIFT_SP_INSERT_RESTOCK_LP_TASK] @MATERIAL_ID ='100003',
        @MATERIAL_DESCRIPTION = 'DU CB AL AA 1SB X 120CS 12SW HLLY',
        @LOCATION_SPOT = 'R1C02N1P02',
        @RESTOCK_USER = 'RUDI@DIPROCOM',
        @LOGIN = 'MBL',
        @QTY = '5000000',
        @LP_TASK_CODE = '123123',
        @SOURCE_WAREHOUSE = 'BODEGA_CENTRAL'
  
          SELECT  TOP(10)  *  FROM [DIPROCOM].[SWIFT_PICKING_DETAIL] order by PICKING_HEADER DESC
          SELECT  TOP(10)  *  FROM [DIPROCOM].[SWIFT_PICKING_HEADER] order by PICKING_HEADER DESC
          SELECT  TOP(10)  *  FROM [DIPROCOM].[SWIFT_TASKS] order by PICKING_NUMBER DESC
 -- WITH  q AS (SELECT TOP(1)  * FROM [DIPROCOM].[SWIFT_TASKS] order by PICKING_NUMBER DESC)DELETE FROM q
 -- WITH  q AS (SELECT TOP(1)  * FROM [DIPROCOM].[SWIFT_PICKING_DETAIL] order by PICKING_HEADER DESC)DELETE FROM q
 -- WITH  q AS (SELECT TOP(1)  * FROM [DIPROCOM].[SWIFT_PICKING_HEADER] order by PICKING_HEADER DESC)DELETE FROM q
        
*/
-- =============================================
CREATE PROCEDURE [DIPROCOM].[SWIFT_SP_INSERT_RESTOCK_LP_TASK] (@MATERIAL_ID VARCHAR(25),
...
BEGIN
	  ...
	  EXEC [DIPROCOM].SWIFT_SP_INSERT_PICKING_HEADER @CLASSIFICATION_PICKING = '3076'
	  ...
	  INSERT INTO #RESULT_HEADER
	  EXEC [DIPROCOM].SWIFT_SP_INSERT_PICKING_DETAIL @PICKING_HEADER = @I_PICKING_HEADER
	  ...
	  END
	  INSERT INTO #RESULT_HEADER
	  EXEC [DIPROCOM].SWIFT_SP_INSERT_TASK_FROM_PICKING @PICKING_HEADER = @I_PICKING_HEADER
