...
/*
-- DROP PROCEDURE [acsa].[SWIFT_SP_INSERT_RESTOCK_LP_TASK]
-- Ejemplo de Ejecucion:
        EXEC  [acsa].[SWIFT_SP_INSERT_RESTOCK_LP_TASK] @MATERIAL_ID ='100003',
        ...
        @SOURCE_WAREHOUSE = 'BODEGA_CENTRAL'
          ...
          SELECT  TOP(10)  *  FROM [acsa].[SWIFT_PICKING_DETAIL] order by PICKING_HEADER DESC
          SELECT  TOP(10)  *  FROM [acsa].[SWIFT_PICKING_HEADER] order by PICKING_HEADER DESC
          SELECT  TOP(10)  *  FROM [acsa].[SWIFT_TASKS] order by PICKING_NUMBER DESC
 -- WITH  q AS (SELECT TOP(1)  * FROM [acsa].[SWIFT_TASKS] order by PICKING_NUMBER DESC)DELETE FROM q
 -- WITH  q AS (SELECT TOP(1)  * FROM [acsa].[SWIFT_PICKING_DETAIL] order by PICKING_HEADER DESC)DELETE FROM q
 -- WITH  q AS (SELECT TOP(1)  * FROM [acsa].[SWIFT_PICKING_HEADER] order by PICKING_HEADER DESC)DELETE FROM q
        
*/
-- =============================================
CREATE PROCEDURE [acsa].[SWIFT_SP_INSERT_RESTOCK_LP_TASK] (@MATERIAL_ID VARCHAR(25),
...
BEGIN
	  ...
	  EXEC [acsa].SWIFT_SP_INSERT_PICKING_HEADER @CLASSIFICATION_PICKING = '3076'
	  ...
	  INSERT INTO #RESULT_HEADER
	  EXEC [acsa].SWIFT_SP_INSERT_PICKING_DETAIL @PICKING_HEADER = @I_PICKING_HEADER
	  ...
	  END
	  INSERT INTO #RESULT_HEADER
	  EXEC [acsa].SWIFT_SP_INSERT_TASK_FROM_PICKING @PICKING_HEADER = @I_PICKING_HEADER
