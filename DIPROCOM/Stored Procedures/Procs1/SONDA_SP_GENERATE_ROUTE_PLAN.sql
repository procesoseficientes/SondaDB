...
-- Ejemplo de Ejecucion:
				...
				--
				exec SWIFT_EXPRESS.[acsa].SONDA_SP_GENERATE_ROUTE_PLAN @CODE_FREQUENCY_OLD = '' ,@CODE_FREQUENCY_NEW = ''
				--
				exec [acsa].SONDA_SP_GENERATE_ROUTE_PLAN @CODE_FREQUENCY_NEW = 'SALE240010101124'
*/
-- =============================================
CREATE PROCEDURE [acsa].[SONDA_SP_GENERATE_ROUTE_PLAN]
...
BEGIN
  ...
  --SELECT
  --  @GENERATE_ROUTE_PLAN_FROM_POLYGON = [acsa].[SWIFT_FN_GET_PARAMETER]('IMPLEMENTATION',
  ...
  BEGIN
    TRUNCATE TABLE  SWIFT_EXPRESS.[acsa].[SONDA_ROUTE_PLAN];
    --
    DELETE FROM  SWIFT_EXPRESS.[acsa].[SWIFT_TASKS]
  ...
  BEGIN
    ...
    WHERE [ASSIGNED_BY] = 'Proceso diario'
      ...
      AND [TASK_ID] IN (SELECT
          [TASK_ID]
        FROM  SWIFT_EXPRESS.[acsa].[SONDA_ROUTE_PLAN]
  ...
  EXEC  SWIFT_EXPRESS.[acsa].[SWIFT_SP_GET_FREQUENCY_X_TASK] @DATE = @DATE
                                              ,@CODE_FREQUENCY = NULL;
  --
  SELECT DISTINCT
    [F].*
  ...
  INNER JOIN [acsa].[SWIFT_FREQUENCY_X_CUSTOMER] [X]
  ...
  -- -----------------------------------------------------------------
  INSERT INTO SWIFT_EXPRESS.[acsa].[SWIFT_TASKS] ([TASK_TYPE]
  , [TASK_DATE]
  , [SCHEDULE_FOR]
  , [CREATED_STAMP]
  , [ASSIGEND_TO]
  ...
  -- -----------------------------------------------------------------
  INSERT INTO  SWIFT_EXPRESS.[acsa].[SONDA_ROUTE_PLAN] ([TASK_ID]
  , [CODE_FREQUENCY]
  , [SCHEDULE_FOR]
  , [ASSIGNED_BY]
  , [DOC_PARENT]
  ...
  , [CREATE_BY])
    SELECT
     ...
     ,'BY_CALENDAR'
    FROM [#frecuencia] [F]
    INNER JOIN  SWIFT_EXPRESS.[acsa].[SWIFT_TASKS] [T]
      ON (
