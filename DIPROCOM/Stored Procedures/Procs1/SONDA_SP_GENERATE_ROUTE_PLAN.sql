...
-- Ejemplo de Ejecucion:
				exec [PACASA].SONDA_SP_GENERATE_ROUTE_PLAN
				--
				exec SWIFT_EXPRESS.[PACASA].SONDA_SP_GENERATE_ROUTE_PLAN @CODE_FREQUENCY_OLD = '' ,@CODE_FREQUENCY_NEW = ''
				--
				exec [PACASA].SONDA_SP_GENERATE_ROUTE_PLAN @CODE_FREQUENCY_NEW = 'SALE240010101124'
*/
-- =============================================
CREATE PROCEDURE [PACASA].[SONDA_SP_GENERATE_ROUTE_PLAN]
...
BEGIN
  ...
  --SELECT
  --  @GENERATE_ROUTE_PLAN_FROM_POLYGON = [PACASA].[SWIFT_FN_GET_PARAMETER]('IMPLEMENTATION',
  ...
  BEGIN
    TRUNCATE TABLE  SWIFT_EXPRESS.[PACASA].[SONDA_ROUTE_PLAN];
    --
    DELETE FROM  SWIFT_EXPRESS.[PACASA].[SWIFT_TASKS]
  ...
  BEGIN
    ...
    WHERE [ASSIGNED_BY] = 'Proceso diario'
      ...
      AND [TASK_ID] IN (SELECT
          [TASK_ID]
        FROM  SWIFT_EXPRESS.[PACASA].[SONDA_ROUTE_PLAN]
        WHERE [CODE_FREQUENCY] = @CODE_FREQUENCY_OLD);
    --
    DELETE [PACASA].[SONDA_ROUTE_PLAN]
    WHERE [CODE_FREQUENCY] = @CODE_FREQUENCY_OLD;
  END;
  ...
  EXEC  SWIFT_EXPRESS.[PACASA].[SWIFT_SP_GET_FREQUENCY_X_TASK] @DATE = @DATE
                                              ,@CODE_FREQUENCY = NULL;
  --
  SELECT DISTINCT
    [F].*
  ...
  INNER JOIN [PACASA].[SWIFT_FREQUENCY_X_CUSTOMER] [X]
  ...
  -- -----------------------------------------------------------------
  INSERT INTO SWIFT_EXPRESS.[PACASA].[SWIFT_TASKS] ([TASK_TYPE]
  , [TASK_DATE]
  , [SCHEDULE_FOR]
  , [CREATED_STAMP]
  , [ASSIGEND_TO]
  ...
  -- -----------------------------------------------------------------
  INSERT INTO  SWIFT_EXPRESS.[PACASA].[SONDA_ROUTE_PLAN] ([TASK_ID]
  , [CODE_FREQUENCY]
  , [SCHEDULE_FOR]
  , [ASSIGNED_BY]
  , [DOC_PARENT]
  ...
  , [CREATE_BY])
    SELECT
     ...
     ,'BY_CALENDAR'
    FROM [#frecuencia] [F]
    INNER JOIN  SWIFT_EXPRESS.[PACASA].[SWIFT_TASKS] [T]
      ON (
