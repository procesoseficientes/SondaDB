...
-- Ejemplo de Ejecucion:
				...
				--
				exec SWIFT_EXPRESS.[SONDA].SONDA_SP_GENERATE_ROUTE_PLAN @CODE_FREQUENCY_OLD = '' ,@CODE_FREQUENCY_NEW = ''
				--
				exec [SONDA].SONDA_SP_GENERATE_ROUTE_PLAN @CODE_FREQUENCY_NEW = 'SALE240010101124'
*/
-- =============================================
...
BEGIN
  ...
  BEGIN
    TRUNCATE TABLE  SWIFT_EXPRESS.[SONDA].[SONDA_ROUTE_PLAN];
    --
    DELETE FROM  SWIFT_EXPRESS.[SONDA].[SWIFT_TASKS]
  ...
  BEGIN
    ...
    WHERE [ASSIGNED_BY] = 'Proceso diario'
      ...
      AND [TASK_ID] IN (SELECT
          [TASK_ID]
        FROM  SWIFT_EXPRESS.[SONDA].[SONDA_ROUTE_PLAN]
  ...
  EXEC  SWIFT_EXPRESS.[SONDA].[SWIFT_SP_GET_FREQUENCY_X_TASK] @DATE = @DATE
                                              ,@CODE_FREQUENCY = NULL;
  --
  SELECT DISTINCT
    [F].*
  ...
  -- -----------------------------------------------------------------
  INSERT INTO SWIFT_EXPRESS.[SONDA].[SWIFT_TASKS] ([TASK_TYPE]
  , [TASK_DATE]
  , [SCHEDULE_FOR]
  , [CREATED_STAMP]
  , [ASSIGEND_TO]
  ...
  -- -----------------------------------------------------------------
  INSERT INTO  SWIFT_EXPRESS.[SONDA].[SONDA_ROUTE_PLAN] ([TASK_ID]
  , [CODE_FREQUENCY]
  , [SCHEDULE_FOR]
  , [ASSIGNED_BY]
  , [DOC_PARENT]
  ...
  , [CREATE_BY])
    SELECT
     ...
     ,'BY_CALENDAR'
    FROM [#frecuencia] [F]
    INNER JOIN  SWIFT_EXPRESS.[SONDA].[SWIFT_TASKS] [T]
      ON (
