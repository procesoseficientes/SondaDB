...
-- Ejemplo de Ejecucion:
        exec [PACASA].[SWIFT_SP_GET_CUSTUMER_FOR_SCOUTING] @CODE_ROUTE = 'rp-01'
*/
-- =============================================
CREATE PROCEDURE [PACASA].[SWIFT_SP_GET_CUSTUMER_FOR_SCOUTING]
...
BEGIN
  ...
  WHERE [SELLER_ROUTE] = @CODE_ROUTE*/
    ...
    FROM [PACASA].[SWIFT_DISCOUNT_LIST] [DL]
        INNER JOIN [PACASA].[SWIFT_DISCOUNT_LIST_BY_CUSTOMER] [DLBC]
            ON (
    ...
    FROM [PACASA].[SWIFT_BONUS_LIST] [BL]
        INNER JOIN [PACASA].[SWIFT_BONUS_LIST_BY_CUSTOMER] [BLBC]
    ...
    FROM [PACASA].[SWIFT_SKU_SALES_BY_MULTIPLE_LIST] [SM]
        INNER JOIN [PACASA].[SWIFT_SKU_SALES_BY_MULTIPLE_LIST_BY_CUSTOMER] [SLBC]
    ...
    SELECT [SPL].[SPECIAL_PRICE_LIST_ID],
           [SPL].[CODE_ROUTE],
           [SPLBC].[CODE_CUSTOMER]
    FROM [PACASA].[SWIFT_SPECIAL_PRICE_LIST] AS [SPL]
        INNER JOIN [PACASA].[SWIFT_SPECIAL_PRICE_LIST_BY_CUSTOMER] AS [SPLBC]
            ON (
   ...
   FROM [PACASA].[SWIFT_CHANNEL] [SC]
        INNER JOIN [PACASA].[SWIFT_CHANNEL_X_CUSTOMER] [SCXC]
    ...
    SELECT LTRIM(RTRIM([C].[CODE_CUSTOMER])),
           ...
           (
               ...
               FROM [PACASA].[SONDA_ITEM_HISTORY] AS [IH]
    ...
    FROM [PACASA].[SWIFT_VIEW_ALL_COSTUMER] [C]
        ...
        LEFT JOIN [PACASA].[SWIFT_PRICE_LIST_BY_CUSTOMER_FOR_ROUTE] [PLC]
            ON (
    ...
    SELECT [VAC].[CODE_CUSTOMER],
        ...
        FROM [SWIFT_EXPRESS].[PACASA].[SWIFT_VIEW_ALL_COSTUMER] [VAC]
        INNER JOIN [PACASA].[SONDA_ROUTE_PLAN] [RP]
