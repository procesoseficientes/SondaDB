...
-- Ejemplo de Ejecucion:
				EXEC [DIPROCOM].[SWIFT_SET_INVENTORY_BY_DISTRIBUTION_CENTER]
...
*/
-- =============================================
CREATE PROCEDURE [DIPROCOM].[SWIFT_SET_INVENTORY_BY_DISTRIBUTION_CENTER](
	@LOGIN VARCHAR(50)
	,@SKU XML
	,@ACTIVATING INT
)
...
BEGIN
	...
	BEGIN TRY
		...
		FROM [DIPROCOM].[SWIFT_WAREHOUSE_X_DISTRIBUTION_CENTER] [WDC]
		...
		IF @ACTIVATING = 1
		BEGIN
			SELECT @ON_HAND = CAST([DIPROCOM].[SWIFT_FN_GET_PARAMETER]('INVENTORY','SET_INVENTORY') AS INT)
			--
			SELECT DISTINCT
			...
			-- ------------------------------------------------------------------------------------
			MERGE [DIPROCOM].[SWIFT_INVENTORY] SIV
			...
			INSERT INTO [DIPROCOM].[SWIFT_INVENTORY]
					(
						[SERIAL_NUMBER]
						,[WAREHOUSE]
						,[LOCATION]
