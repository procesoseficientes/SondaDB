...
-- Ejemplo de Ejecucion:
				--
				EXEC [DIPROCOM].[SWIFT_SP_GET_COMPLIANCE_ROUTE]
					@TASK_DATE = '20160623'
...
*/
-- =============================================
CREATE PROCEDURE [DIPROCOM].SWIFT_SP_GET_COMPLIANCE_ROUTE
	@TASK_DATE DATE
...
BEGIN
	...
	INTO #ROUTE
	FROM [DIPROCOM].[Split](@CODE_ROUTE,@DELIMITER)
	--FROM [DIPROCOM].[SWIFT_FN_SPLIT](@CODE_ROUTE,@DELIMITER)
	...
	SELECT
		[T].[TASK_ID]
		,SUM([H].[TOTAL_AMOUNT]) [TOTAL_AMOUNT]
	FROM [DIPROCOM].[SONDA_POS_INVOICE_HEADER] [H]
	INNER JOIN [#TASK] [T] 
	...
	SELECT
		...
		,[T].[COMPLETED_STAMP]
		,[T].[LAPSE_TIME]
		,CONVERT(DECIMAL(18,' + CAST(@DEFAULT_DISPLAY_DECIMALS AS VARCHAR) + '),[DIPROCOM].[SWIFT_FN_GET_DISPLAY_NUMBER](ISNULL([S].[TOTAL_AMOUNT],0.00))) [TOTAL_AMOUNT]
	FROM [#TASK] [T]
