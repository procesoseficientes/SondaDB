...
-- Ejemplo de Ejecucion:
				--
				EXEC [PACASA].[SWIFT_SP_GET_COMPLIANCE_ROUTE]
					@TASK_DATE = '20160623'
					,@CODE_ROUTE = 'rudi@DIPROCOM'
				--
				EXEC [PACASA].[SWIFT_SP_GET_COMPLIANCE_ROUTE]
					@TASK_DATE = '20160614'
					,@CODE_ROUTE = 'rudi@DIPROCOM|oper1@DIPROCOM'
*/
-- =============================================
CREATE PROCEDURE [PACASA].SWIFT_SP_GET_COMPLIANCE_ROUTE
	@TASK_DATE DATE
...
BEGIN
	...
	INTO #ROUTE
	FROM [PACASA].[Split](@CODE_ROUTE,@DELIMITER)
	--FROM [PACASA].[SWIFT_FN_SPLIT](@CODE_ROUTE,@DELIMITER)
	...
	SELECT
		...
		,[T].[COMPLETED_STAMP]
		,[T].[LAPSE_TIME]
		,CONVERT(DECIMAL(18,' + CAST(@DEFAULT_DISPLAY_DECIMALS AS VARCHAR) + '),[PACASA].[SWIFT_FN_GET_DISPLAY_NUMBER](ISNULL([S].[TOTAL_AMOUNT],0.00))) [TOTAL_AMOUNT]
	FROM [#TASK] [T]
