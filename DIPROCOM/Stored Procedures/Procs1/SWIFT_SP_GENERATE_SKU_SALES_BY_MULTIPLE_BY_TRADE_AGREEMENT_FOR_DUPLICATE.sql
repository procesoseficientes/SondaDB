...
-- Description:			SP que genera las venta por multiplo de los clientes en acuerdo comercial por cliente y canal
...
-- Ejemplo de Ejecucion:
				EXEC [DIPROCOM].[SWIFT_SP_GENERATE_SKU_SALES_BY_MULTIPLE_BY_TRADE_AGREEMENT_FOR_DUPLICATE]
					@CODE_ROUTE = '44'
					,@ORDER = 1
*/
-- =============================================
CREATE PROCEDURE [DIPROCOM].[SWIFT_SP_GENERATE_SKU_SALES_BY_MULTIPLE_BY_TRADE_AGREEMENT_FOR_DUPLICATE] (@CODE_ROUTE VARCHAR(50)
, @ORDER INT)
AS
BEGIN
  SET NOCOUNT ON;
  ...
  BEGIN
    ...
    -- ------------------------------------------------------------------------------------
    INSERT INTO [DIPROCOM].[SWIFT_SKU_SALES_BY_MULTIPLE_LIST] ([NAME_SALES_BY_MULTIPLE_LIST]
    , [CODE_ROUTE])
      SELECT DISTINCT
    ...
    -- ------------------------------------------------------------------------------------
    INSERT INTO [DIPROCOM].[SWIFT_SKU_SALES_BY_MULTIPLE_LIST_BY_CUSTOMER] ([SALES_BY_MULTIPLE_LIST_ID]
    , [CODE_CUSTOMER])
      SELECT DISTINCT
        [SM].[SALES_BY_MULTIPLE_LIST_ID]
       ,[TA].[CODE_CUSTOMER]
      FROM @TRADE_AGREEMENT [TA]
      INNER JOIN [DIPROCOM].[SWIFT_SKU_SALES_BY_MULTIPLE_LIST] [SM]
        ON (
  ...
  -- ------------------------------------------------------------------------------------
  INSERT INTO [DIPROCOM].[SWIFT_SKU_SALES_BY_MULTIPLE_LIST_BY_SKU] ([SALES_BY_MULTIPLE_LIST_ID]
  , [CODE_SKU]
  , [CODE_PACK_UNIT]
  , [MULTIPLE]
  , [PROMO_ID]
  ...
  , [FREQUENCY])
    ...
    INNER JOIN [DIPROCOM].[SWIFT_PROMO_SKU_SALES_BY_MULTIPLE] [TAS]
      ON (
    ...
    LEFT JOIN [DIPROCOM].[SWIFT_SKU_SALES_BY_MULTIPLE_LIST_BY_SKU] [SMLS]
      ON (
