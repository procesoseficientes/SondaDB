...
-- Ejemplo de Ejecucion:
        ...
        GO
        --
        EXEC [DIPROCOM].SWIFT_SP_GET_DETAIL_SALES_ORDER_FOR_PICKING			    
			    @SALES_ORDERS_IDS = '33698|33699'
			    
*/
-- =============================================
CREATE PROCEDURE [DIPROCOM].SWIFT_SP_GET_DETAIL_SALES_ORDER_FOR_PICKING(
...
BEGIN
  ...
  INTO #SALES_ORDER
  FROM [DIPROCOM].[SWIFT_FN_SPLIT](@SALES_ORDERS_IDS, @DELIMITER) S
  ...
  INNER JOIN [DIPROCOM].SONDA_SALES_ORDER_HEADER SO ON(
    SO.SALES_ORDER_ID = SOT.SALES_ORDER_ID
  )
  INNER JOIN [DIPROCOM].SWIFT_VIEW_ALL_SKU VS ON (
    VS.CODE_SKU = SOD.SKU
  )
  INNER JOIN [DIPROCOM].SWIFT_VIEW_ALL_COSTUMER VC ON(
