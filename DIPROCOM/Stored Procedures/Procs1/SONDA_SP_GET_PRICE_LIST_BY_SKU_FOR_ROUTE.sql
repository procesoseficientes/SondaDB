...
-- EJEMPLO DE EJECUCION: 
		...
		EXEC [DIPROCOM].[SONDA_SP_GET_PRICE_LIST_BY_SKU_FOR_ROUTE]
			@CODE_ROUTE = '011202'

*/
-- =====================================================
CREATE PROCEDURE [DIPROCOM].[SONDA_SP_GET_PRICE_LIST_BY_SKU_FOR_ROUTE] (@CODE_ROUTE VARCHAR(50))
AS
BEGIN
	SET NOCOUNT ON;
	--
	...
	--
	SELECT 
		@DEFAULT_CODE_PRICE_LIST = [DIPROCOM].SWIFT_FN_GET_PARAMETER('ERP_HARDCODE_VALUES','PRICE_LIST')
		,@GENERATE_DEFAULT_PRICE_LIST = [DIPROCOM].SWIFT_FN_GET_PARAMETER('ERP_HARDCODE_VALUES','GENERATE_DEFAULT_PRICE_LIST')
	--
	SELECT DISTINCT
		splbc.CODE_PRICE_LIST
	INTO #PRICE_LIST
	FROM [DIPROCOM].[SONDA_ROUTE_PLAN] srp
	INNER JOIN [DIPROCOM].[SWIFT_PRICE_LIST_BY_CUSTOMER] splbc 
	ON (srp.[RELATED_CLIENT_CODE] = [splbc].[CODE_CUSTOMER])
	...
	SELECT DISTINCT
		...
		,[PL].[CODE_PACK_UNIT]
		,[PL].[UM_ENTRY]
	FROM [DIPROCOM].[SWIFT_PRICE_LIST_BY_SKU] AS PL
