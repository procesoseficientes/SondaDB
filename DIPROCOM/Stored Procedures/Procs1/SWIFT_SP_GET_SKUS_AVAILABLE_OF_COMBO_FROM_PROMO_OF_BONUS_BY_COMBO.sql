...
-- Ejemplo de Ejecucion:
				EXEC [PACASA].[SWIFT_SP_GET_SKUS_AVAILABLE_OF_COMBO_FROM_PROMO_OF_BONUS_BY_COMBO]
					@PROMO_RULE_BY_COMBO_ID = 8
*/
-- =============================================
CREATE PROCEDURE [PACASA].[SWIFT_SP_GET_SKUS_AVAILABLE_OF_COMBO_FROM_PROMO_OF_BONUS_BY_COMBO] (@PROMO_RULE_BY_COMBO_ID INT)
AS
BEGIN
  SET NOCOUNT ON;
  --
  ...
  LEFT JOIN [PACASA].[SWIFT_FAMILY_SKU] [FS]
    ON (
    [FS].[CODE_FAMILY_SKU] = [VAS].[CODE_FAMILY_SKU]
    )
  INNER JOIN [PACASA].[SONDA_PACK_CONVERSION] [PC]
    ON (
  ...
  LEFT JOIN [PACASA].[SWIFT_PROMO_SKU_BY_PROMO_RULE] [PSR]
