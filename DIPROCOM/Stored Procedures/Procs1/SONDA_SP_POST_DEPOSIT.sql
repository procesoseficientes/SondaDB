...
-- Ejemplo de Ejecucion:
		...
		--
		EXEC [PACASA].[SONDA_SP_POST_DEPOSIT]
		...
		SELECT @pResult
		--
        SELECT * FROM [PACASA].[SONDA_DEPOSITS] ORDER BY 1 DESC
*/
-- =============================================
CREATE PROCEDURE [PACASA].[SONDA_SP_POST_DEPOSIT] (
...
BEGIN
    ...
    BEGIN TRY
		...
		-- -----------------------------------------------------------------
	   	EXEC [PACASA].[SONDA_SP_VALIDATE_DEVICE_ID_OF_USER_FOR_TRANSACTION] @CODE_ROUTE = @pPOS_ID , -- varchar(50)
	   		@DEVICE_ID = @DEVICE_ID -- varchar(50)

		--
    	INSERT INTO [PACASA].[SONDA_DEPOSITS]
    			(
    				[TRANS_TYPE]
    				,[TRANS_DATETIME]
    				,[BANK_ID]
