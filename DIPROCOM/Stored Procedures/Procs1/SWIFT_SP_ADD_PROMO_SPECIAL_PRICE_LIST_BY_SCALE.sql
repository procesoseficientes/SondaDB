...
-- Ejemplo de Ejecucion:
		EXEC [DIPROCOM].[SWIFT_SP_ADD_PROMO_SPECIAL_PRICE_LIST_BY_SCALE]
		@PROMO_ID = 3512
		,@CODE_SKU = '100001'
		,@PACK_UNIT = 1
		,@LOW_LIMIT = 1
...
*/
-- =============================================
CREATE PROCEDURE [DIPROCOM].[SWIFT_SP_ADD_PROMO_SPECIAL_PRICE_LIST_BY_SCALE](
@PROMO_ID INT
	, @CODE_SKU VARCHAR(50)
	, @PACK_UNIT INT
	, @LOW_LIMIT INT
...
BEGIN
	BEGIN TRY
		...
		-- --------------------------------------------------------------------------
		EXEC [DIPROCOM].[SWIFT_SP_VALIDATE_DISCOUNT_SCALE_FOR_PROMO]
		@PROMO_ID = @PROMO_ID
		, @CODE_SKU = @CODE_SKU
		, @PACK_UNIT = @PACK_UNIT
		, @LOW_LIMIT = @LOW_LIMIT
		...
		IF (
				SELECT COUNT(D.CODE_SKU) FROM 
					(SELECT
						*
					FROM
						[DIPROCOM].[SWIFT_PROMO_SPECIAL_PRICE_LIST_BY_SCALE]
					WHERE
...
ELSE 
	BEGIN 
		INSERT INTO DIPROCOM.[SWIFT_PROMO_SPECIAL_PRICE_LIST_BY_SCALE]
			(
				[PROMO_ID]
				,[CODE_SKU]
				,[PACK_UNIT]
