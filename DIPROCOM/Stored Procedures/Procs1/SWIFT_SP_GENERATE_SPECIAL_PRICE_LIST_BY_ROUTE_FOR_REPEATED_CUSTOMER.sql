...
-- Ejemplo de Ejecucion:
        EXEC [PACASA].[SWIFT_SP_GENERATE_SPECIAL_PRICE_LIST_BY_ROUTE_FOR_REPEATED_CUSTOMER]
		@CODE_ROUTE = '136'
*/
-- =============================================
CREATE PROCEDURE [PACASA].[SWIFT_SP_GENERATE_SPECIAL_PRICE_LIST_BY_ROUTE_FOR_REPEATED_CUSTOMER]
	(
...
AS
	BEGIN
		...
		SELECT TOP 1
			@LINKED_TO = [LINKED_TO]
		FROM
			[PACASA].[SWIFT_SPECIAL_PRICE_PRIORITY]
		WHERE
			[ORDER] > 0
		...
		BEGIN
			...
			-- ------------------------------------------------------------------------------------
			INSERT	INTO [PACASA].[SWIFT_SPECIAL_PRICE_LIST]
					(
			...
			BEGIN
				...
				FROM
					[PACASA].[SWIFT_PROMO_SPECIAL_PRICE_LIST_BY_SCALE] [TAD]
				...
				INNER JOIN [PACASA].[SWIFT_TRADE_AGREEMENT_BY_CUSTOMER] [TAC]
				ON	([TAC].[TRADE_AGREEMENT_ID] = [TAP].[TRADE_AGREEMENT_ID])
				LEFT JOIN [PACASA].[SWIFT_SPECIAL_PRICE_LIST_BY_SCALE] [DLS]
				ON	(
