...
-- Ejemplo de Ejecucion:
        EXEC [DIPROCOM].[SWIFT_SP_GENERATE_SPECIAL_PRICE_LIST_BY_ROUTE_FOR_REPEATED_CUSTOMER]
		@CODE_ROUTE = '136'
*/
-- =============================================
CREATE PROCEDURE [DIPROCOM].[SWIFT_SP_GENERATE_SPECIAL_PRICE_LIST_BY_ROUTE_FOR_REPEATED_CUSTOMER]
	(
...
AS
	BEGIN
		...
		INNER JOIN [DIPROCOM].[SWIFT_TRADE_AGREEMENT_BY_CHANNEL] [TA]
		...
		SELECT TOP 1
			@LINKED_TO = [LINKED_TO]
		FROM
			[DIPROCOM].[SWIFT_SPECIAL_PRICE_PRIORITY]
		WHERE
			[ORDER] > 0
		...
		BEGIN
			...
			-- ------------------------------------------------------------------------------------
			INSERT	INTO [DIPROCOM].[SWIFT_SPECIAL_PRICE_LIST]
					(
			...
			-- ------------------------------------------------------------------------------------
			INSERT	INTO [DIPROCOM].[SWIFT_SPECIAL_PRICE_LIST_BY_CUSTOMER]
					(
			...
			BEGIN
				INSERT	INTO [DIPROCOM].[SWIFT_SPECIAL_PRICE_LIST_BY_SCALE]
						(
							[SPECIAL_PRICE_LIST_ID]
							,[CODE_SKU]
							,[PACK_UNIT]
				...
				FROM
					[DIPROCOM].[SWIFT_PROMO_SPECIAL_PRICE_LIST_BY_SCALE] [TAD]
				...
				INNER JOIN [DIPROCOM].[SWIFT_TRADE_AGREEMENT_BY_CUSTOMER] [TAC]
				ON	([TAC].[TRADE_AGREEMENT_ID] = [TAP].[TRADE_AGREEMENT_ID])
				LEFT JOIN [DIPROCOM].[SWIFT_SPECIAL_PRICE_LIST_BY_SCALE] [DLS]
				ON	(
