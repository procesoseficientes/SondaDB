...
-- Ejemplo de Ejecucion:
				EXEC [acsa].[SWIFT_SP_GET_COUNT_CUSTOMER_IN_POLYGON_FOR_DAY_BY_SELLER]
...
*/
-- =============================================
CREATE PROCEDURE [acsa].[SWIFT_SP_GET_COUNT_CUSTOMER_IN_POLYGON_FOR_DAY_BY_SELLER](
	@LOGIN VARCHAR(50)
...
BEGIN
	...
	INNER JOIN [acsa].[SWIFT_SELLER] [SS] ON (
	...
	FROM [acsa].[SWIFT_POLYGON_BY_ROUTE] [PBR]
	INNER JOIN [acsa].[SWIFT_FREQUENCY] [SF] ON (
	...
	INNER JOIN [acsa].[SWIFT_FREQUENCY_X_CUSTOMER] [FC] ON (
	...
	INNER JOIN [acsa].[SWIFT_POLYGON] [P2] ON (
		[P2].[POLYGON_ID] = [P].[POLYGON_ID_PARENT]
	)
	INNER JOIN [acsa].[SWIFT_POLYGON] [P3] ON (
