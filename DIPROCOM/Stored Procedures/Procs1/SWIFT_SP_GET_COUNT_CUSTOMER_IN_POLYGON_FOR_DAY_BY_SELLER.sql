...
-- Ejemplo de Ejecucion:
				EXEC [PACASA].[SWIFT_SP_GET_COUNT_CUSTOMER_IN_POLYGON_FOR_DAY_BY_SELLER]
...
*/
-- =============================================
CREATE PROCEDURE [PACASA].[SWIFT_SP_GET_COUNT_CUSTOMER_IN_POLYGON_FOR_DAY_BY_SELLER](
	@LOGIN VARCHAR(50)
...
BEGIN
	...
	INNER JOIN [PACASA].[SWIFT_SELLER] [SS] ON (
	...
	-- ------------------------------------------------------------------------------------
	-- Obtiene las frecuencias de Rutas Frecuencia Unica
	-- ------------------------------------------------------------------------------------
	...
	-- ------------------------------------------------------------------------------------
	-- Obtiene las frecuencias de Rutas Frecuencia Multiple
	-- ------------------------------------------------------------------------------------
	INSERT INTO [#FREQUENCY]
	        ( [ID_FREQUENCY] )
	SELECT DISTINCT [SF].[ID_FREQUENCY]
	FROM [PACASA].[SWIFT_POLYGON_BY_ROUTE] [PBR]
	INNER JOIN [PACASA].[SWIFT_FREQUENCY] [SF] ON (
		[SF].[POLYGON_ID] = [PBR].[POLYGON_ID]
	)
	INNER JOIN [PACASA].[SWIFT_ROUTES] [R] ON (
		[R].[ROUTE] = [PBR].[ROUTE]
	)
	...
	INNER JOIN [PACASA].[SWIFT_FREQUENCY_X_CUSTOMER] [FC] ON (
	...
	INNER JOIN [PACASA].[SWIFT_POLYGON] [P2] ON (
		[P2].[POLYGON_ID] = [P].[POLYGON_ID_PARENT]
	)
	INNER JOIN [PACASA].[SWIFT_POLYGON] [P3] ON (
