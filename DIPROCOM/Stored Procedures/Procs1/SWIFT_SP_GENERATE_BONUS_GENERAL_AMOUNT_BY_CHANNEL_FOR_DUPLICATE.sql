...
-- Description:			SP que genera las bonificaciones desde acuerdo comercial de canal para los clientes que tambien esten en un acuerdo comercial de cliente

/*
-- Ejemplo de Ejecucion:
				EXEC [DIPROCOM].[SWIFT_SP_GENERATE_BONUS_GENERAL_AMOUNT_BY_CHANNEL_FOR_DUPLICATE]
					@CODE_ROUTE = '44'
					,@ORDER = 2
*/
-- =============================================
CREATE PROCEDURE [DIPROCOM].[SWIFT_SP_GENERATE_BONUS_GENERAL_AMOUNT_BY_CHANNEL_FOR_DUPLICATE] (@CODE_ROUTE VARCHAR(250)
, @ORDER INT)
AS
BEGIN
  SET NOCOUNT ON;
  ...
  -- ------------------------------------------------------------------------------------
  --Se comena por que ya se hace en el sp de escalas [SWIFT_SP_GENERATE_BONUS_BY_TRADE_AGREEMENT_FOR_DUPLICATE] ó [SWIFT_SP_GENERATE_BONUS_BY_CHANNEL_FOR_DUPLICATE]
  --  IF @ORDER = 1
  --  BEGIN
  --    -- ------------------------------------------------------------------------------------
  --    -- Genera las listas de bonificaciones
  --    -- ------------------------------------------------------------------------------------
  --    INSERT INTO [DIPROCOM].[SWIFT_BONUS_LIST] ([NAME_BONUS_LIST]
  --    , [CODE_ROUTE])
  --      SELECT DISTINCT
  ...
  , [FREQUENCY])
    ...
    INNER JOIN [DIPROCOM].[SWIFT_PROMO_BONUS_BY_GENERAL_AMOUNT] [TAB]
      ON (
    ...
    LEFT JOIN [DIPROCOM].[SWIFT_BONUS_LIST_BY_GENERAL_AMOUNT] [BLBS]
      ON (
