...
-- Ejemplo de Ejecucion:
				EXEC [PACASA].[SWIFT_SP_GENERATE_BONUS_GENERAL_AMOUNT_BY_CHANNEL_FOR_DUPLICATE]
					@CODE_ROUTE = '44'
					,@ORDER = 2
*/
-- =============================================
CREATE PROCEDURE [PACASA].[SWIFT_SP_GENERATE_BONUS_GENERAL_AMOUNT_BY_CHANNEL_FOR_DUPLICATE] (@CODE_ROUTE VARCHAR(250)
, @ORDER INT)
AS
BEGIN
  SET NOCOUNT ON;
  ...
  --    -- ------------------------------------------------------------------------------------
  --    INSERT INTO [PACASA].[SWIFT_BONUS_LIST] ([NAME_BONUS_LIST]
  --    , [CODE_ROUTE])
  --      SELECT DISTINCT
  ...
  , [FREQUENCY])
    ...
    INNER JOIN [PACASA].[SWIFT_PROMO_BONUS_BY_GENERAL_AMOUNT] [TAB]
      ON (
    ...
    LEFT JOIN [PACASA].[SWIFT_BONUS_LIST_BY_GENERAL_AMOUNT] [BLBS]
      ON (
