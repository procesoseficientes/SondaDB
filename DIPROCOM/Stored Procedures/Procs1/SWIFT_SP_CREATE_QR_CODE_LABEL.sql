...
Ejemplo de Ejecucion:
	...
	EXEC [DIPROCOM].[SWIFT_SP_CREATE_QR_CODE_LABEL] 
		@pTASK_ID = 16574
...
CREATE PROC [DIPROCOM].[SWIFT_SP_CREATE_QR_CODE_LABEL] 
	@pTASK_ID int,
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET @SQL = 'select SHIP_TO_ADDRESSES from openquery (ERPSERVER,''SELECT PO.Address2 AS SHIP_TO_ADDRESSES
				FROM    [BD_Prueba_Viscosa].dbo.ORDR AS PO 
    ...
    EXEC @return_value = [DIPROCOM].[SWIFT_SP_GET_NEXT_SEQUENCE] 
		@SEQUENCE_NAME = 'QR_CODE_LABEL'
        ,@pRESULT = @pID OUTPUT

    INSERT INTO DIPROCOM.SWIFT_QR_CODE_LABEL (
		ID
		, TASK_ID
		, CUSTOMER_NAME
		, CODE_SKU
