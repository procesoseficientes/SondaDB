...
-- Ejemplo de Ejecucion:
		EXEC [acsa].[SWIFT_SP_GET_SKU_OF_SALE_BY_MULTIPLE_ASSOCIATED_TO_PROMOTION_OF_SALE_BY_MULTIPLE]
		@PROMO_ID = 9
*/
-- =============================================
CREATE PROCEDURE [acsa].[SWIFT_SP_GET_SKU_OF_SALE_BY_MULTIPLE_ASSOCIATED_TO_PROMOTION_OF_SALE_BY_MULTIPLE](
	@PROMO_ID INT
)
AS
BEGIN
	...
	SELECT SBM.[SWIFT_PROMO_SKU_SALES_BY_MULTIPLE_ID]
			...
			,SBM.[MULTIPLE]
	FROM [acsa].[SWIFT_PROMO_SKU_SALES_BY_MULTIPLE] SBM
	INNER JOIN [acsa].[SONDA_PACK_UNIT] AS PU ON(
		SBM.[PACK_UNIT] = PU.[PACK_UNIT]
	)
	INNER JOIN [acsa].[SWIFT_VIEW_ALL_SKU] VSKU ON(
		SBM.[CODE_SKU] = [VSKU].[CODE_SKU]
	)
	LEFT JOIN [acsa].[SWIFT_FAMILY_SKU] AS FSKU ON(
