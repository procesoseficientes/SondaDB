...
-- Ejemplo de Ejecucion:
    ...
    EXECUTE @RC = [DIPROCOM].SWIFT_SP_GET_INVOICE_HEADER @idFactura
    GO
*/
CREATE PROCEDURE [DIPROCOM].[SWIFT_SP_GET_INVOICE_HEADER]
	(
		@INVOICE_ID INT
...
AS
  ...
  -- ------------------------------------------------------------------------------------
		SELECT
			@INTERFACE_PAYMENT_TYPE = [DIPROCOM].[SWIFT_FN_GET_PARAMETER]('INVOICE' ,
		...
		INNER JOIN [DIPROCOM].[SWIFT_WAREHOUSES] [sw]
		ON	([u].[DEFAULT_WAREHOUSE] = [sw].[CODE_WAREHOUSE])
		INNER JOIN [DIPROCOM].[SWIFT_VIEW_COST_CENTER_BY_WAREHOUSE] [CCW]
