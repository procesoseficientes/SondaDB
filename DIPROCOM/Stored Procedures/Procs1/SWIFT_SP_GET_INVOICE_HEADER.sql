...
-- Ejemplo de Ejecucion:
    ...
    EXECUTE @RC = [acsa].SWIFT_SP_GET_INVOICE_HEADER @idFactura
    GO
*/
CREATE PROCEDURE [acsa].[SWIFT_SP_GET_INVOICE_HEADER]
	(
		@INVOICE_ID INT
...
AS
  ...
  -- ------------------------------------------------------------------------------------
		SELECT
			@INTERFACE_PAYMENT_TYPE = [acsa].[SWIFT_FN_GET_PARAMETER]('INVOICE' ,
		...
		INNER JOIN [acsa].[SWIFT_WAREHOUSES] [sw]
		ON	([u].[DEFAULT_WAREHOUSE] = [sw].[CODE_WAREHOUSE])
		INNER JOIN [acsa].[SWIFT_VIEW_COST_CENTER_BY_WAREHOUSE] [CCW]
