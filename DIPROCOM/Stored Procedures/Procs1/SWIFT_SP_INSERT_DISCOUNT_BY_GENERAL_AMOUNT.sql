...
-- Ejemplo de Ejecucion:
				EXEC [PACASA].[SWIFT_SP_INSERT_DISCOUNT_BY_GENERAL_AMOUNT]
				@TRADE_AGREEMENT_ID = 21
				,@LOW_AMOUNT = '100'
				,@HIGH_AMOUNT = '500'
				,@DISCOUNT = 25
				-- 
				SELECT * FROM [PACASA].[SWIFT_TRADE_AGREEMENT_DISCOUNT_BY_GENERAL_AMOUNT] 
				WHERE TRADE_AGREEMENT_ID = 21
*/
-- =============================================
CREATE PROCEDURE [PACASA].[SWIFT_SP_INSERT_DISCOUNT_BY_GENERAL_AMOUNT](
...
BEGIN
	BEGIN TRY
		DECLARE @ID INT
		--
		EXEC [PACASA].[SWIFT_SP_VALIDATED_DISCOUNT_SCALE_FOR_GENERAL_AMOUNT] @TRADE_AGREEMENT_ID, @LOW_AMOUNT,@HIGH_AMOUNT
		--
		INSERT INTO [PACASA].[SWIFT_TRADE_AGREEMENT_DISCOUNT_BY_GENERAL_AMOUNT] (
			TRADE_AGREEMENT_ID
			,LOW_AMOUNT
			,HIGH_AMOUNT
			,DISCOUNT
