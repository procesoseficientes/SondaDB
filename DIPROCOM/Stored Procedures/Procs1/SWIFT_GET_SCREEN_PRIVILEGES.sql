...
-- Ejemplo de Ejecucion:
				EXEC [DIPROCOM].[SWIFT_GET_SCREEN_PRIVILEGES]
					@USER_LOGGED = 'gerente@DIPROCOM'
          ,@PARENT_PRIVILEGE_ID = 'btnBonusForPromo'
*/
-- =============================================

CREATE PROCEDURE [DIPROCOM].[SWIFT_GET_SCREEN_PRIVILEGES] (@USER_LOGGED VARCHAR(50)
...
AS
	...
	FROM
		[DIPROCOM].[USERS] [U]
	INNER JOIN [DIPROCOM].[SWIFT_ROLE] [R] ON ([R].[ROLE_ID] = [U].[USER_ROLE])
	INNER JOIN [DIPROCOM].[SWIFT_PRIVILEGES_X_ROLE] [PR] ON ([R].[ROLE_ID] = [PR].[ROLE_ID])
	INNER JOIN [DIPROCOM].[SWIFT_PRIVILEGES] [P] ON (
