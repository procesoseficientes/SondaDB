...
--Ejemplo de Ejecucion:
						exec [acsa].[SWIFT_SP_GET_SCOUNTING] 
...
-- =============================================
CREATE PROCEDURE [acsa].[SWIFT_SP_GET_SCOUNTING]
	@TAGS VARCHAR(MAX)
	,@Type AS VARCHAR(20)
	,@NOT_TAGS AS INT
...
BEGIN
	...
	LEFT JOIN [acsa].[SWIFT_VIEW_ALL_TAG_X_CUSTOMER_NEW] [TCN] ON ([CN].[CODE_CUSTOMER] = [TCN].[CUSTOMER])
	INNER JOIN [acsa].[SWIFT_COMPANY] [C] ON ([C].[COMPANY_ID] = [CN].[OWNER_ID])
	WHERE [RU].[LOGIN] = @LOGIN
		...
		AND (@NOT_TAGS = 0 OR 0 < (SELECT COUNT(*) FROM [acsa].[SWIFT_VIEW_ALL_TAG_X_CUSTOMER_NEW] [TC] WHERE [CN].[CODE_CUSTOMER] = [TC].[CUSTOMER]))
