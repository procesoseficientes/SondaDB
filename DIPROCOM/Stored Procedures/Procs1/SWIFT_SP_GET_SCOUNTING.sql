...
--Ejemplo de Ejecucion:
						exec [DIPROCOM].[SWIFT_SP_GET_SCOUNTING] 
...
-- =============================================
CREATE PROCEDURE [DIPROCOM].[SWIFT_SP_GET_SCOUNTING]
	@TAGS VARCHAR(MAX)
	,@Type AS VARCHAR(20)
	,@NOT_TAGS AS INT
...
BEGIN
	...
	INTO [#FREQUENCY]
	FROM [DIPROCOM].[SWIFT_CUSTOMER_FREQUENCY_NEW] [CFN]
	--
	SELECT DISTINCT
		[CN].[CUSTOMER]
		,[CN].[CODE_CUSTOMER]
		...
		,[CN].[DOC_SERIE]
		,[CN].[DOC_NUM]
	FROM [DIPROCOM].[SWIFT_VIEW_ALL_CUSTOMER_NEW] [CN]
	INNER JOIN [DIPROCOM].[SWIFT_VIEW_ALL_ROUTE] [VR] ON ([CN].[CODE_ROUTE] = [VR].[CODE_ROUTE])
	LEFT JOIN [#FREQUENCY] [F] ON ([CN].[CODE_CUSTOMER] = [F].[CODE_CUSTOMER])
	INNER JOIN [DIPROCOM].[SWIFT_ROUTE_BY_USER] [RU] ON ([RU].[CODE_ROUTE] = [CN].[CODE_ROUTE])
	LEFT JOIN [DIPROCOM].[SWIFT_VIEW_ALL_TAG_X_CUSTOMER_NEW] [TCN] ON ([CN].[CODE_CUSTOMER] = [TCN].[CUSTOMER])
	INNER JOIN [DIPROCOM].[SWIFT_COMPANY] [C] ON ([C].[COMPANY_ID] = [CN].[OWNER_ID])
	WHERE [RU].[LOGIN] = @LOGIN
		AND [CN].[POST_DATETIME] BETWEEN @FechaIncio AND @FechaFinal
		AND (@TAGS = '' OR [TCN].[TAG_COLOR] IN (SELECT [Splitcolumn] FROM [DIPROCOM].[SWIFT_FUNC_GET_TABLE_SPLIT](@TAGS,',')))
		AND (@Type = '' OR [CN].[STATUS] = @Type)
		AND (@NOT_TAGS = 0 OR 0 < (SELECT COUNT(*) FROM [DIPROCOM].[SWIFT_VIEW_ALL_TAG_X_CUSTOMER_NEW] [TC] WHERE [CN].[CODE_CUSTOMER] = [TC].[CUSTOMER]))
