...
-- Ejemplo de Ejecucion:
        exec [PACASA].[SWIFT_SP_GET_CUSTUMER_FOR_SCOUTING_TEST] @CODE_ROUTE = 'es000128'
*/
-- =============================================
CREATE PROCEDURE [PACASA].[SWIFT_SP_GET_CUSTUMER_FOR_SCOUTING_TEST] (
...
--
	...
	-- ------------------------------------------------------------------------------------
	/*SELECT @DEFAULT_PRICE_LIST = ISNULL([CODE_PRICE_LIST], [PACASA].[SWIFT_FN_GET_PARAMETER] ('ERP_HARDCODE_VALUES','PRICE_LIST'))
	FROM [PACASA].[USERS]
	...
	FROM [PACASA].[SWIFT_DISCOUNT_LIST] [DL]
	...
	FROM [PACASA].[SWIFT_BONUS_LIST] [BL]
	...
	FROM [PACASA].[SWIFT_SKU_SALES_BY_MULTIPLE_LIST] [SM]
	...
	SELECT DISTINCT
		...
		,[TAX_ID_NUMBER]
		,REPLACE([dbo].[FUNC_REMOVE_SPECIAL_CHARS](COALESCE([ADRESS_CUSTOMER] ,'')) ,'"' ,'') AS [ADRESS_CUSTOMER]
		...
		,[PACASA].[SONDA_FN_GET_LAST_PURCHASE_FOR_CUSTOMER] ([C].[CODE_CUSTOMER]) AS LAST_PURCHASE
		,[C].[INVOICE_NAME]
	FROM [PACASA].[SWIFT_VIEW_ALL_COSTUMER] [C]
	INNER JOIN [PACASA].[USERS] [U] ON (
	      [C].[SELLER_DEFAULT_CODE] = [U].[RELATED_SELLER]
	)
	LEFT JOIN [PACASA].[SWIFT_DISCOUNT_LIST_BY_CUSTOMER] [DLC] ON (
		[DLC].[CODE_CUSTOMER] = [C].[CODE_CUSTOMER]
	)
	LEFT JOIN [PACASA].[SWIFT_BONUS_LIST_BY_CUSTOMER] [BLC] ON (
		[BLC].[CODE_CUSTOMER] = [C].[CODE_CUSTOMER]
	)
	LEFT JOIN [PACASA].[SWIFT_PRICE_LIST_BY_CUSTOMER_FOR_ROUTE] [PLC] ON (
		[PLC].[CODE_CUSTOMER] = [C].[CODE_CUSTOMER]
		AND [PLC].[CODE_ROUTE] = @CODE_ROUTE
	)
	LEFT JOIN [PACASA].[SWIFT_SKU_SALES_BY_MULTIPLE_LIST_BY_CUSTOMER] [SMC] ON (
	...
	WHERE [U].[SELLER_ROUTE] = @CODE_ROUTE
		AND (
			...
			OR
			DLC.DISCOUNT_LIST_ID IN (SELECT DISCOUNT_LIST_ID FROM @DISCOUNT_LIST D WHERE D.DISCOUNT_LIST_ID = DLC.DISCOUNT_LIST_ID)
		)
		AND (
			[BLC].BONUS_LIST_ID IS NULL
			OR
			[BLC].BONUS_LIST_ID IN (SELECT BONUS_LIST_ID FROM @BONUS_LIST B WHERE B.BONUS_LIST_ID = BLC.BONUS_LIST_ID)
		)
		AND (
			[SMC].SALES_BY_MULTIPLE_LIST_ID IS NULL
			OR
			[SMC].SALES_BY_MULTIPLE_LIST_ID IN (SELECT SALES_BY_MULTIPLE_LIST_ID FROM @SKU_SALES_BY_MULTIPLE_LIST S WHERE S.SALES_BY_MULTIPLE_LIST_ID = SMC.SALES_BY_MULTIPLE_LIST_ID)
		)
	-- ------------------------------------------------------------------------------------
	...
	SELECT DISTINCT
		...
		,[PACASA].[SONDA_FN_GET_LAST_PURCHASE_FOR_CUSTOMER] ([VAC].[CODE_CUSTOMER]) AS LAST_PURCHASE
		,[VAC].[INVOICE_NAME]
	FROM [PACASA].[SWIFT_VIEW_ALL_COSTUMER] [VAC]
	INNER JOIN [PACASA].[SONDA_ROUTE_PLAN] [RP] ON (
