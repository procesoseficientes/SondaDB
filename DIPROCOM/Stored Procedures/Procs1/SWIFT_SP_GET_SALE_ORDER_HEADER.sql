...
-- Ejemplo de Ejecucion:
        ...
        GO
        --
        EXEC [DIPROCOM].[SWIFT_SP_GET_SALE_ORDER_HEADER]
			@idOrdenDeVenta = '5160'
*/
-- =============================================
CREATE PROCEDURE [DIPROCOM].[SWIFT_SP_GET_SALE_ORDER_HEADER] (
@idOrdenDeVenta INT
) AS
BEGIN
	SET NOCOUNT ON;
	...
	INNER JOIN [DIPROCOM].[SWIFT_PARAMETER] [sp] ON (
	...
	--
	SELECT
		@SALES_ORDER_TYPE = [DIPROCOM].[SWIFT_FN_GET_PARAMETER]('SALES_ORDER_TYPE', 'CASH')
		,@SEND_SALES_ORDER_TO_DELIVERY_DATE = [DIPROCOM].[SWIFT_FN_GET_PARAMETER]('SALES_ORDER', 'SEND_SALES_ORDER_TO_DELIVERY_DATE')
	...
	SELECT
		...
		,'S' AS [SONDA]
		,[SSOH].[COMMENT]
		,[DIPROCOM].[SWIFT_FN_VALIDATE_IS_SCOUTING]([SSOH].[CLIENT_ID]) AS [IS_SCOUTING]
		,[SVAC].[EXTRA_DAYS]
		,[SVAC].[PAYMENT_CONDITIONS]
	FROM [DIPROCOM].[SONDA_SALES_ORDER_HEADER] [SSOH]
	INNER JOIN [DIPROCOM].[SWIFT_WAREHOUSES] [SW] ON ([SSOH].[WAREHOUSE] = [SW].[CODE_WAREHOUSE])
	LEFT JOIN [DIPROCOM].[SWIFT_VIEW_ALL_COSTUMER] [SVAC] ON ([SSOH].[CLIENT_ID] = [SVAC].[CODE_CUSTOMER])
	INNER JOIN [DIPROCOM].[USERS] [U] ON ([SSOH].[POSTED_BY] = [U].[LOGIN])
	LEFT JOIN [DIPROCOM].[SWIFT_SELLER] [S] ON ([U].[RELATED_SELLER] = [S].[SELLER_CODE])
	LEFT JOIN [DIPROCOM].[SWIFT_SALES_OFFICE] [SO] ON ([SO].[SALES_OFFICE_ID] = [S].[SALES_OFFICE_ID])
	LEFT JOIN [DIPROCOM].[SWIFT_SALES_ORGANIZATION] [ORG] ON ([ORG].[SALES_ORGANIZATION_ID] = [SO].[SALES_ORGANIZATION_ID])
	WHERE
