...
-- Ejemplo de Ejecucion:
        ...
        GO
        --
        EXEC [PACASA].[SWIFT_SP_GET_SALE_ORDER_HEADER]
			@idOrdenDeVenta = '5160'
*/
-- =============================================
CREATE PROCEDURE [PACASA].[SWIFT_SP_GET_SALE_ORDER_HEADER] (
@idOrdenDeVenta INT
) AS
BEGIN
	SET NOCOUNT ON;
	...
	INNER JOIN [PACASA].[SWIFT_PARAMETER] [sp] ON (
	...
	--
	SELECT
		@SALES_ORDER_TYPE = [PACASA].[SWIFT_FN_GET_PARAMETER]('SALES_ORDER_TYPE', 'CASH')
		,@SEND_SALES_ORDER_TO_DELIVERY_DATE = [PACASA].[SWIFT_FN_GET_PARAMETER]('SALES_ORDER', 'SEND_SALES_ORDER_TO_DELIVERY_DATE')
	...
	SELECT
		...
		,'S' AS [SONDA]
		,[SSOH].[COMMENT]
		,[PACASA].[SWIFT_FN_VALIDATE_IS_SCOUTING]([SSOH].[CLIENT_ID]) AS [IS_SCOUTING]
		,[SVAC].[EXTRA_DAYS]
		,[SVAC].[PAYMENT_CONDITIONS]
	FROM [PACASA].[SONDA_SALES_ORDER_HEADER] [SSOH]
	INNER JOIN [PACASA].[SWIFT_WAREHOUSES] [SW] ON ([SSOH].[WAREHOUSE] = [SW].[CODE_WAREHOUSE])
	LEFT JOIN [PACASA].[SWIFT_VIEW_ALL_COSTUMER] [SVAC] ON ([SSOH].[CLIENT_ID] = [SVAC].[CODE_CUSTOMER])
	INNER JOIN [PACASA].[USERS] [U] ON ([SSOH].[POSTED_BY] = [U].[LOGIN])
	LEFT JOIN [PACASA].[SWIFT_SELLER] [S] ON ([U].[RELATED_SELLER] = [S].[SELLER_CODE])
	LEFT JOIN [PACASA].[SWIFT_SALES_OFFICE] [SO] ON ([SO].[SALES_OFFICE_ID] = [S].[SALES_OFFICE_ID])
	LEFT JOIN [PACASA].[SWIFT_SALES_ORGANIZATION] [ORG] ON ([ORG].[SALES_ORGANIZATION_ID] = [SO].[SALES_ORGANIZATION_ID])
	WHERE
