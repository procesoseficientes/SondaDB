...
-- Ejemplo de Ejecucion:
				EXEC [PACASA].[SWIFT_SP_GET_PACK_UNIT_TO_POLYGON_REPORT]
				    	@POLYGON_TYPE = 'REGION'
...
*/
-- =============================================
CREATE PROCEDURE [PACASA].SWIFT_SP_GET_PACK_UNIT_TO_POLYGON_REPORT (@POLYGON_TYPE VARCHAR(250)
  ...
  BEGIN TRY
    ...
    BEGIN
      ...
      INSERT INTO @TOTAL_QTY_BY_SKU_AND_PACK_UNIT
        SELECT
         ...
         ,SUM([SD].[QTY])
         ,[PBR].[POLYGON_ID]
        FROM [PACASA].[SONDA_SALES_ORDER_DETAIL] [SD]
        INNER JOIN [PACASA].[SONDA_SALES_ORDER_HEADER] [SO]
          ON (
        ...
        INNER JOIN [PACASA].[SONDA_PACK_UNIT] [PU]
          ON (
        ...
        INNER JOIN [PACASA].[SWIFT_POLYGON_BY_ROUTE] [PBR]
          ON (
          [PBR].[ROUTE] = [R].[ROUTE]
          )
    ...
    BEGIN
      ...
      INSERT INTO @TOTAL_QTY_BY_SKU_AND_PACK_UNIT
        ...
        INNER JOIN [PACASA].[SONDA_POS_INVOICE_HEADER] [I]
          ON (
    ...
    SET @QUERY = N'SELECT
			...
			,[P].[POLYGON_NAME]
			,[PPU].[PACK_UNIT]
			,CONVERT(DECIMAL(18,' + CAST(@DEFAULT_DISPLAY_DECIMALS AS VARCHAR) + '),[PACASA].SWIFT_FN_GET_DISPLAY_NUMBER([PPU].[QTY])) [QTY]
