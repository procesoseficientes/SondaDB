...
-- EJEMPLO DE EJECUCION: 
		EXEC [PACASA].[SWIFT_SP_GET_SALES_DETAIL]

=========================================================*/
CREATE PROCEDURE [PACASA].SWIFT_SP_GET_SALES_DETAIL
AS
BEGIN
	
	DROP INDEX IDX_SALES_ROUTE ON [PACASA].SWIFT_SALES_INDICATOR
	DROP INDEX IDX_SALES_DATE ON [PACASA].SWIFT_SALES_INDICATOR

	DELETE FROM [PACASA].SWIFT_SALES_INDICATOR 
	...
	LEFT JOIN [PACASA].USERS us ON st.ASSIGEND_TO = us.login
	WHERE SCHEDULE_FOR = CAST(GETDATE() AS DATE)
	...
	LEFT JOIN [PACASA].SONDA_SALES_ORDER_DETAIL ssod ON ssoh.SALES_ORDER_ID = ssod.SALES_ORDER_ID
	LEFT JOIN [PACASA].SWIFT_VIEW_ALL_SKU svas ON svas.CODE_SKU = ssod.SKU
	LEFT JOIN [PACASA].[SWIFT_SKU_FAMILY] ssf ON ssf.CODE_FAMILY_SKU = svas.CODE_FAMILY_SKU
	WHERE ssoh.IS_DRAFT=0 AND CAST(ssoh.POSTED_DATETIME AS DATE) = CAST(GETDATE() AS DATE)
	AND ssoh.IS_READY_TO_SEND=1

	INSERT INTO [PACASA].SWIFT_SALES_INDICATOR ([TASK_ID]
      ,[COSTUMER_CODE]
      ,[COSTUMER_NAME]
      ,[SCHEDULE_FOR]
      ,[EXPECTED_GPS]
	...
	select
		...
		,[PACASA].[SWIFT_CALCULATE_DISTANCE] (st.EXPECTED_GPS,ISNULL(ssoh.POSTED_GPS,st.POSTED_GPS),'M') as DISTANCE
		,ISNULL([PACASA].[SWIFT_FN_GET_KPI] 
			([PACASA].[SWIFT_CALCULATE_DISTANCE] (st.EXPECTED_GPS,ISNULL(ssoh.POSTED_GPS,st.POSTED_GPS),'M'),
	...
	CREATE INDEX IDX_SALES_ROUTE ON [PACASA].SWIFT_SALES_INDICATOR ([SELLER_ROUTE])
	CREATE INDEX IDX_SALES_DATE ON [PACASA].SWIFT_SALES_INDICATOR (SCHEDULE_FOR)
