...
-- Ejemplo de Ejecucion:
      ...
      EXEC	@return_value = [DIPROCOM].[SWIFT_SP_GET_PRIVILEGES]
...
*/
-- =============================================
CREATE PROCEDURE [DIPROCOM].SWIFT_SP_GET_PRIVILEGES @USER VARCHAR(50)  WITH RECOMPILE
AS
  ...
  INNER JOIN [DIPROCOM].[SWIFT_ROLE] R
    ON (
    R.ROLE_ID = U.USER_ROLE
    )
  INNER JOIN [DIPROCOM].[SWIFT_PRIVILEGES_X_ROLE] PR
    ON (
    R.ROLE_ID = PR.ROLE_ID
    )
  INNER JOIN [DIPROCOM].[SWIFT_PRIVILEGES] P
    ON (
