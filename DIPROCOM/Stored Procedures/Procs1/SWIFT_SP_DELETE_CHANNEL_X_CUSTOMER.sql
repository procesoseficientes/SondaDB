...
-- Ejemplo de Ejecucion:
				EXEC [acsa].SWIFT_SP_DELETE_CHANNEL_X_CUSTOMER
					@CHANNEL_ID = 5
					,@CODE_CUSTOMER = 'SO-005'
				-- 
				SELECT * FROM [acsa].SWIFT_CHANNEL_X_CUSTOMER
*/
-- =============================================
CREATE PROCEDURE [acsa].SWIFT_SP_DELETE_CHANNEL_X_CUSTOMER (@CHANNEL_ID INT
, @CODE_CUSTOMER VARCHAR(50))
AS
BEGIN
  BEGIN TRY
    DELETE FROM [acsa].SWIFT_CHANNEL_X_CUSTOMER
    WHERE CHANNEL_ID = @CHANNEL_ID
      AND CODE_CUSTOMER = @CODE_CUSTOMER
    --

    UPDATE [acsa].[SWIFT_TRADE_AGREEMENT_BY_CHANNEL]
