...
-- Ejemplo de Ejecucion:
				DECLARE 
					...
					,@DOC_NUM INT = 2
				--
				SELECT D.DOC_TYPE,D.SERIE,D.CURRENT_DOC FROM [PACASA].[SWIFT_DOCUMENT_SEQUENCE] D WHERE DOC_TYPE = @DOC_TYPE AND SERIE = @SERIE
				--
				EXEC [PACASA].[SONDA_SP_UPDATE_DOCUMENT_SEQUENCE_BO]
					@DOC_TYPE = @DOC_TYPE
					,@SERIE = @SERIE
					,@DOC_NUM = @DOC_NUM
				--
...
*/
-- =============================================
CREATE PROCEDURE [PACASA].[SONDA_SP_UPDATE_DOCUMENT_SEQUENCE_BO]
(	
	@DOC_TYPE VARCHAR(50)
...
BEGIN
	...
	BEGIN TRY
		UPDATE [PACASA].[SWIFT_DOCUMENT_SEQUENCE]
