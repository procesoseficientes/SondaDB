...
-- Ejemplo de Ejecucion:
        ...
        EXECUTE @RC = DIPROCOM.SWIFT_SP_GET_SAP_PICKING_ITR @pERP_DOC
        GO
*/
-- =============================================
CREATE PROCEDURE [DIPROCOM].[SWIFT_SP_GET_SAP_PICKING_ITR] 
	@pERP_DOC VARCHAR(50)
AS
BEGIN
  ...
  SELECT
	...
	SELECT
	 ...
	 FROM openquery (ERPSERVER,''
		...
		FROM [BD_Prueba_Viscosa].dbo.WTQ1 AS POD
		INNER JOIN [BD_Prueba_Viscosa].dbo.OWTQ AS PO
		  ON PO.DocEntry = POD.DocEntry
		INNER JOIN [BD_Prueba_Viscosa].dbo.OSLP AS SE
		  ON SE.SlpCode = PO.SlpCode
		INNER JOIN [BD_Prueba_Viscosa].dbo.OWHS o1
