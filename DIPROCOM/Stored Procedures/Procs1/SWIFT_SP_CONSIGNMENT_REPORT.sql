...
Ejemplo de Ejecucion:
	EXEC [DIPROCOM].[SWIFT_SP_CONSIGNMENT_REPORT]
		@QTY = -1
		,@INIT_DATE = '7/28/2015 2:10:03 PM' 
		,@END_DATE = '10/31/2016 2:13:43 PM'
======================================================*/
CREATE PROCEDURE [DIPROCOM].[SWIFT_SP_CONSIGNMENT_REPORT]
	@QTY		INT = -1
	,@INIT_DATE	DATETIME 
	,@END_DATE		DATETIME 
AS
...
BEGIN
  ...
  set @query += ' , CASE WHEN ISNULL(d.SERIAL_NUMBER,''0'') = ''0'' THEN ''N/A'' WHEN d.SERIAL_NUMBER = ''NULL'' THEN ''N/A'' ELSE d.SERIAL_NUMBER END AS [SERIAL_NUMBER]'
	set @query += ' FROM [DIPROCOM].[SWIFT_CONSIGNMENT_HEADER] h'
	set @query += ' INNER JOIN [DIPROCOM].[SWIFT_CONSIGNMENT_DETAIL] d ON (h.CONSIGNMENT_ID = d.CONSIGNMENT_ID)'
	set @query += ' INNER JOIN [DIPROCOM].[SWIFT_VIEW_ALL_SKU] AS VS ON(d.SKU = VS.CODE_SKU)'
	set @query += ' INNER JOIN [DIPROCOM].[SWIFT_VIEW_ALL_COSTUMER] c ON (h.CUSTOMER_ID = c.CODE_CUSTOMER)'
	set @query += ' INNER JOIN [DIPROCOM].[SWIFT_VIEW_ROUTES] R ON (R.[CODE_ROUTE] = H.POS_TERMINAL)'
