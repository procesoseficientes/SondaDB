...
-- Description:			Obtiene las promos que no estan asociadas a un acuerdo comercial
...
-- Ejemplo de Ejecucion:
				EXEC [DIPROCOM].[SWIFT_SP_GET_PROMO_NO_ASSOCIATED_TO_TRADE_AGREEMENT] @TRADE_AGREEMENT_ID = 1029
*/
-- =============================================
CREATE PROCEDURE [DIPROCOM].[SWIFT_SP_GET_PROMO_NON_ASSOCIATED_FOR_TRADE_AGREEMENT] (@TRADE_AGREEMENT_ID INT)
AS
BEGIN
  SET NOCOUNT ON;
  --
  SELECT
   ...
   ,CASE [SP].[PROMO_TYPE]
    ...
    END [PROMO_TYPE]
  FROM [DIPROCOM].[SWIFT_PROMO] [SP]
  LEFT JOIN [DIPROCOM].[SWIFT_TRADE_AGREEMENT_BY_PROMO] [STAP]
    ON ([STAP].[PROMO_ID] = [SP].[PROMO_ID]
    AND [STAP].[TRADE_AGREEMENT_ID] = @TRADE_AGREEMENT_ID)
  WHERE [STAP].[TRADE_AGREEMENT_ID] IS NULL
  AND [SP].[PROMO_ID] > 0
