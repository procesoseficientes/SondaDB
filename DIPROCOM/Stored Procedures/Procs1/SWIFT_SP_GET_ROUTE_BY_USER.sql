...
/*
	Ejemplo Ejecucion: 
    EXEC [DIPROCOM].SWIFT_SP_GET_ROUTE_BY_USER 
		@LOGIN = 'gerente@DIPROCOM'
 */
-- =============================================
CREATE PROCEDURE [DIPROCOM].[SWIFT_SP_GET_ROUTE_BY_USER] (
	@LOGIN VARCHAR(50)
)AS
BEGIN
	SELECT DISTINCT
    ...
    ,[S].[SELLER_CODE]
    ,[S].[SELLER_NAME]
	FROM [DIPROCOM].[SWIFT_VIEW_ALL_ROUTE] [SR]
	INNER JOIN [DIPROCOM].[SWIFT_ROUTE_BY_USER] [srbu] ON (
		[SR].[CODE_ROUTE] = [srbu].[CODE_ROUTE]
	)
	LEFT JOIN [DIPROCOM].[USERS] [U] ON (
		[U].[SELLER_ROUTE] = [SR].[CODE_ROUTE]
	)
	LEFT JOIN [DIPROCOM].[SWIFT_SELLER] [S] ON (
