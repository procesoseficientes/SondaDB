...
/*
	Ejemplo Ejecucion: 
    EXEC [PACASA].SWIFT_SP_GET_ROUTE_BY_USER 
		@LOGIN = 'gerente@DIPROCOM'
 */
-- =============================================
CREATE PROCEDURE [PACASA].[SWIFT_SP_GET_ROUTE_BY_USER] (
	@LOGIN VARCHAR(50)
)AS
BEGIN
	SELECT DISTINCT
    ...
    ,[S].[SELLER_CODE]
    ,[S].[SELLER_NAME]
	FROM [PACASA].[SWIFT_VIEW_ALL_ROUTE] [SR]
	INNER JOIN [PACASA].[SWIFT_ROUTE_BY_USER] [srbu] ON (
		[SR].[CODE_ROUTE] = [srbu].[CODE_ROUTE]
	)
	LEFT JOIN [PACASA].[USERS] [U] ON (
		[U].[SELLER_ROUTE] = [SR].[CODE_ROUTE]
	)
	LEFT JOIN [PACASA].[SWIFT_SELLER] [S] ON (
		[S].[SELLER_CODE] = [U].[RELATED_SELLER]
	)
	LEFT JOIN [PACASA].[SWIFT_CLASSIFICATION] [C] ON (
