...
-- Ejemplo de Ejecucion:
				EXEC [PACASA].[SWIFT_SP_ASSOCIATE_CUSTOMER_TO_POLYGON]
*/
-- =============================================
CREATE PROCEDURE [PACASA].[SWIFT_SP_ASSOCIATE_CUSTOMER_TO_POLYGON]
AS
	BEGIN
		SET NOCOUNT ON;
		-- ------------------------------------------------------------------------------------
		...
		DECLARE	@CUSTOMERS_FILTERED AS TABLE
			...
			truncate table  [PACASA].[SWIFT_CUSTOMER_ASSOCIATE_TO_POLYGON]
			truncate table   [PACASA].[SWIFT_CUSTOMER_GPS_ASSOCIATE_TO_POLYGON]
		...
		LEFT JOIN [PACASA].[SWIFT_CUSTOMER_GPS_ASSOCIATE_TO_POLYGON] AS [CAP]
		...
		DELETE
			[CAP]
		FROM
			[PACASA].[SWIFT_CUSTOMER_ASSOCIATE_TO_POLYGON] AS [CAP]
		...
		DELETE
			[CAP]
		FROM
			[PACASA].[SWIFT_CUSTOMER_GPS_ASSOCIATE_TO_POLYGON] AS [CAP]
		INNER JOIN @CUSTOMER AS [C]
		ON	([C].[CODE_CUSTOMER] = [CAP].[CODE_CUSTOMER]);
		...
		BEGIN
			...
			-- ------------------------------------------------------------------------------------
			SET @GEOMETRY_POLYGON = [PACASA].[SWIFT_GET_GEOMETRY_POLYGON_BY_POLIGON_ID](@POLYGON_ID);	  
	  
			-- ------------------------------------------------------------------------------------
			-- INSERTA EL CLIENTE
			-- ------------------------------------------------------------------------------------
			INSERT	INTO [PACASA].[SWIFT_CUSTOMER_ASSOCIATE_TO_POLYGON]
					(
						[POLYGON_ID]
						,[POLYGON_NAME]
						,[POLYGON_TYPE]
		...
		SELECT DISTINCT
			...
			,[VC].[GPS_CUSTOMER]
		FROM
			[PACASA].[SWIFT_CUSTOMER_ASSOCIATE_TO_POLYGON] AS [VC];

		-- ------------------------------------------------------------------------------------
		-- Guarda el historico de gps de cliente asociado a poligono
		-- ------------------------------------------------------------------------------------
		INSERT	INTO [PACASA].[SWIFT_CUSTOMER_GPS_ASSOCIATE_TO_POLYGON]
				(
					[CODE_CUSTOMER]
					,[GPS]
				)
