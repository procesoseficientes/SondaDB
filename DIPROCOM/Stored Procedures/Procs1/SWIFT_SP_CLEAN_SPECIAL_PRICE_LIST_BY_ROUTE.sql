...
-- Ejemplo de Ejecucion:
        EXEC [DIPROCOM].[SWIFT_SP_CLEAN_SPECIAL_PRICE_LIST_BY_ROUTE]
		@CODE_ROUTE = '136'
*/
-- =============================================
CREATE PROCEDURE [DIPROCOM].[SWIFT_SP_CLEAN_SPECIAL_PRICE_LIST_BY_ROUTE]
	(
...
AS
  ...
  --
		...
		FROM
			[DIPROCOM].[SWIFT_SPECIAL_PRICE_LIST] AS [SP]
		WHERE
		...
		DELETE
			[SPLBC]
		FROM
			[DIPROCOM].[SWIFT_SPECIAL_PRICE_LIST_BY_CUSTOMER] AS [SPLBC]
  ...
  --
		DELETE
			[SPBL]
		FROM
			[DIPROCOM].[SWIFT_SPECIAL_PRICE_LIST_BY_SCALE] AS [SPBL]
		INNER JOIN @SPECIAL_PRICE_LIST AS [SPL]
  ...
  --
		DELETE
			[SPL]
		FROM
			[DIPROCOM].[SWIFT_SPECIAL_PRICE_LIST] AS [SPL]
