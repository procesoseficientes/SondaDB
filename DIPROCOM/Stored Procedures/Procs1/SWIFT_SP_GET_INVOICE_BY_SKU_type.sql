...
-- Ejemplo de Ejecucion:
				EXEC [SONDA].[SWIFT_SP_GET_INVOICE_BY_SKU_TYPE]
					@START_DATETIME = '20160101 00:00:00.000'
					,@END_DATETIME = '20170101 00:00:00.000'
					,@LOGIN = 'GERENTE@DIPROCOM'
*/
-- =============================================
CREATE PROCEDURE [SONDA].[SWIFT_SP_GET_INVOICE_BY_SKU_type](
...
BEGIN
    ...
    ,SUBSTRING([IH].[GPS_URL], CHARINDEX('','', [IH].[GPS_URL]) + 1, LEN([IH].[GPS_URL])) AS LONGITUDE
		...
		,[S].[DESCRIPTION_SKU]
		,[ID].[QTY]
		,CONVERT(DECIMAL(18,' + CAST(@DEFAULT_DISPLAY_DECIMALS AS VARCHAR) + '),[SONDA].SWIFT_FN_GET_DISPLAY_NUMBER([ID].[PRICE])) [PRICE]
		,CONVERT(DECIMAL(18,' + CAST(@DEFAULT_DISPLAY_DECIMALS AS VARCHAR) + '),[SONDA].SWIFT_FN_GET_DISPLAY_NUMBER([ID].[TOTAL_LINE])) [TOTAL_LINE]
		,CONVERT(DECIMAL(18,' + CAST(@DEFAULT_DISPLAY_DECIMALS AS VARCHAR) + '),[SONDA].SWIFT_FN_GET_DISPLAY_NUMBER([IH].[TOTAL_AMOUNT])) [TOTAL_AMOUNT]
    ,[IH].[IS_CREDIT_NOTE]
