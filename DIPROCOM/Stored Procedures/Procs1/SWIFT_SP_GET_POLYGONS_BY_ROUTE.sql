...
/*
	Ejemplo Ejecucion: 
    EXEC [acsa].[SWIFT_SP_GET_POLYGONS_BY_ROUTE] @CODE_ROUTE = N'3111'
 */
-- =============================================

CREATE PROCEDURE [acsa].[SWIFT_SP_GET_POLYGONS_BY_ROUTE]
...
BEGIN
	...
	FROM [SWIFT_EXPRESS].[acsa].SWIFT_VIEW_ALL_ROUTE Routes
	INNER JOIN [SWIFT_EXPRESS].[acsa].[SWIFT_POLYGON_BY_ROUTE] PbR
		on Routes.ROUTE = PbR.ROUTE
	INNER JOIN [SWIFT_EXPRESS].[acsa].[SWIFT_POLYGON] Poly
		on Poly.POLYGON_ID = PbR.POLYGON_ID
  LEFT JOIN [SWIFT_EXPRESS].[acsa].[SWIFT_POLYGON] Parent 
