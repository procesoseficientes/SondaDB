...
/*
	Ejemplo Ejecucion: 
    EXEC [DIPROCOM].[SWIFT_SP_GET_POLYGONS_BY_ROUTE] @CODE_ROUTE = N'3111'
 */
-- =============================================

CREATE PROCEDURE [DIPROCOM].[SWIFT_SP_GET_POLYGONS_BY_ROUTE]
...
BEGIN
	...
	FROM [SWIFT_EXPRESS].[DIPROCOM].SWIFT_VIEW_ALL_ROUTE Routes
	INNER JOIN [SWIFT_EXPRESS].[DIPROCOM].[SWIFT_POLYGON_BY_ROUTE] PbR
		on Routes.ROUTE = PbR.ROUTE
	INNER JOIN [SWIFT_EXPRESS].[DIPROCOM].[SWIFT_POLYGON] Poly
		on Poly.POLYGON_ID = PbR.POLYGON_ID
  LEFT JOIN [SWIFT_EXPRESS].[DIPROCOM].[SWIFT_POLYGON] Parent 
