...
-- Ejemplo de Ejecucion:
		--
		EXEC [PACASA].[SONDA_SP_INSERT_TRACEABILITY_CONSIGMENT]
			@CONSIGNMENT_ID = 26
			...
			--
			SELECT * FROM [PACASA].[SONDA_HISTORICAL_TRACEABILITY_CONSIGNMENT]
*/
-- =============================================
CREATE PROCEDURE [PACASA].[SONDA_SP_INSERT_TRACEABILITY_CONSIGMENT]
(
	@CONSIGNMENT_ID INT
...
BEGIN
  ...
  SELECT @HANDLE_SERIAL = CASE WHEN @HANDLE_SERIAL IS NULL THEN 0 ELSE @HANDLE_SERIAL END
		...
		SELECT @EXIST = 1 FROM [PACASA].[SONDA_HISTORICAL_TRACEABILITY_CONSIGNMENT] AS H
		...
		--
		IF(@EXIST = 0) BEGIN
			INSERT INTO [PACASA].[SONDA_HISTORICAL_TRACEABILITY_CONSIGNMENT](
			[CONSIGNMENT_ID] 
			,[DOC_SERIE]
			,[DOC_NUM]
			,[CODE_SKU] 
