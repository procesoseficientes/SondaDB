CREATE PROCEDURE [PACASA].[SWIFT_SP_CANCEL_SCAN]
@pTXN_ID	int
,@pSKU		varchar(75)
...
AS
	...
	BEGIN TRY
		...
		delete from [PACASA].[SWIFT_INVENTORY] where TXN_ID = @pTXN_ID
		--
		if (@cantTxns - @cantInventory) > 1
		BEGIN
			update [PACASA].[SWIFT_TXNS] 
			set TXN_QTY = (@cantTxns - @cantInventory)
			where TXN_ID = @pTXN_ID
		END
		else
		BEGIN
			delete from [PACASA].[SWIFT_TXNS] where TXN_ID = @pTXN_ID
		END
		--
		update [PACASA].[SWIFT_RECEPTION_DETAIL] 
