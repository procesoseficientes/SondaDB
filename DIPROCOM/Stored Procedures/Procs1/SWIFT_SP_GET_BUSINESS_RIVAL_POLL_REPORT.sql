...
-- Ejemplo de Ejecucion:
				EXEC [DIPROCOM].SWIFT_SP_GET_BUSINESS_RIVAL_POLL_REPORT
					@START_DATETIME = '20170101 00:00:00.000'
					,@END_DATETIME = '20170606 00:00:00.000'
					,@CODE_ROUTE = '42|43'
*/
-- =============================================
CREATE PROCEDURE [DIPROCOM].SWIFT_SP_GET_BUSINESS_RIVAL_POLL_REPORT (@START_DATETIME DATETIME
...
BEGIN
  ...
  , CONVERT(DECIMAL(18,' + CAST(@DEFAULT_DISPLAY_DECIMALS AS VARCHAR) + '),[DIPROCOM].SWIFT_FN_GET_DISPLAY_NUMBER(sbrp.BUSSINESS_RIVAL_TOTAL_AMOUNT)) [BUSSINESS_RIVAL_TOTAL_AMOUNT]
  , CONVERT(DECIMAL(18,' + CAST(@DEFAULT_DISPLAY_DECIMALS AS VARCHAR) + '),[DIPROCOM].SWIFT_FN_GET_DISPLAY_NUMBER(sbrp.CUSTOMER_TOTAL_AMOUNT)) [CUSTOMER_TOTAL_AMOUNT]
   ,sbrp.COMMENT
   ,sbrp.ROUTE
   ,svar.CODE_ROUTE
   ...
   ,CASE
      ...
      WHEN (sbrp.CUSTOMER_TOTAL_AMOUNT - sbrp.BUSSINESS_RIVAL_TOTAL_AMOUNT) > 0 THEN ''+''
    END AS COLOR
  FROM [DIPROCOM].SONDA_BUSINESS_RIVAL_POLL sbrp
      INNER JOIN [DIPROCOM].SWIFT_FN_SPLIT_2( ''' + @CODE_ROUTE + ''' , ''' + @DELIMITER + ''') [CR] 
    ON ([CR].VALUE = CAST(sbrp.ROUTE as VARCHAR))
      LEFT JOIN [DIPROCOM].SWIFT_VIEW_ALL_COSTUMER svac 
    ON sbrp.CODE_CUSTOMER = svac.CODE_CUSTOMER
      LEFT JOIN [DIPROCOM].SWIFT_VIEW_ALL_ROUTE svar
