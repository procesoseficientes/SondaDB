...
EXECUTE @RC = [DIPROCOM].SWIFT_SP_GET_SALE_ORDER_DETAIL @SALES_ORDER_ID
GO

*/
-- =============================================  
CREATE PROCEDURE [DIPROCOM].SWIFT_SP_GET_SALE_ORDER_DETAIL (
	@SALES_ORDER_ID INT
...
BEGIN
	...
	--
	SELECT @PARAMETER_OWNER = [DIPROCOM].[SWIFT_FN_GET_PARAMETER]('SALES_ORDER_INTERCOMPANY','SEND_ALL_DETAIL')
			,@SHIPPING_ATTEMPTS = [DIPROCOM].[SWIFT_FN_GET_PARAMETER]('SALES_ORDER', 'SHIPPING_ATTEMPTS')
	...
	BEGIN	
		...
		,ISNULL((SELECT
			spu.UM_ENTRY
			FROM [DIPROCOM].SONDA_PACK_UNIT spu
		...
		END) AS [LONG]
		,[svas].[OWNER]
		FROM [DIPROCOM].[SONDA_SALES_ORDER_DETAIL] AS D
			INNER JOIN [DIPROCOM].SONDA_SALES_ORDER_HEADER ssoh ON (D.SALES_ORDER_ID = ssoh.SALES_ORDER_ID)
			INNER JOIN [DIPROCOM].SWIFT_VIEW_ALL_SKU svas ON(svas.CODE_SKU = D.SKU) 
			INNER JOIN [DIPROCOM].[SWIFT_SKU_INTERCOMPANY] [SKUI] ON [svas].[CODE_SKU] = [SKUI].[MASTER_ID] AND SKUI.[SOURCE]=@INTERFACE_OWNER
	...
	-- ------------------------------------------------------------------------------------
		...
		FROM [DIPROCOM].[SONDA_SALES_ORDER_DETAIL] AS D
			INNER JOIN [DIPROCOM].SONDA_SALES_ORDER_HEADER ssoh ON (D.SALES_ORDER_ID = ssoh.SALES_ORDER_ID)
			INNER JOIN [DIPROCOM].SWIFT_VIEW_ALL_SKU svas ON(svas.CODE_SKU = D.SKU) 
			INNER JOIN [DIPROCOM].[SWIFT_SKU_INTERCOMPANY] [SKUI] ON [svas].[CODE_SKU] = [SKUI].[MASTER_ID]
		WHERE D.SALES_ORDER_ID = @SALES_ORDER_ID
			AND SSOH.IS_READY_TO_SEND=1
			AND ISNULL([D].[IS_POSTED_ERP], 0) = 0
			AND (ISNULL([D].ATTEMPTED_WITH_ERROR,0) < CAST(@SHIPPING_ATTEMPTS AS INT))
