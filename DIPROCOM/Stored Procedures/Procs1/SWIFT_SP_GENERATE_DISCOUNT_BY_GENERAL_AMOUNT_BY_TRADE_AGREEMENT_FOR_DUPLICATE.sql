...
-- Ejemplo de Ejecucion:
				EXEC [DIPROCOM].[SWIFT_SP_GENERATE_DISCOUNT_BY_GENERAL_AMOUNT_BY_TRADE_AGREEMENT_FOR_DUPLICATE]
					@CODE_ROUTE = '44'
					,@ORDER = 1
*/
-- =============================================
CREATE PROCEDURE [DIPROCOM].[SWIFT_SP_GENERATE_DISCOUNT_BY_GENERAL_AMOUNT_BY_TRADE_AGREEMENT_FOR_DUPLICATE] (@CODE_ROUTE VARCHAR(250)
, @ORDER INT) 
AS
BEGIN
  SET NOCOUNT ON;
  ...
  /*IF @ORDER = 1
	BEGIN
		...
		-- ------------------------------------------------------------------------------------
		INSERT INTO [DIPROCOM].[SWIFT_DISCOUNT_LIST]
				([NAME_DISCOUNT_LIST])
		...
		-- ------------------------------------------------------------------------------------
		INSERT INTO [DIPROCOM].[SWIFT_DISCOUNT_LIST_BY_CUSTOMER]
				(
					[DISCOUNT_LIST_ID]
					,[CODE_CUSTOMER]
				)
  ...
  , [FREQUENCY]  
    ...
    INNER JOIN [DIPROCOM].[SWIFT_PROMO_DISCOUNT_BY_GENERAL_AMOUNT] [TAG]
      ON (
    ...
    LEFT JOIN [DIPROCOM].[SWIFT_DISCOUNT_LIST_BY_GENERAL_AMOUNT] [DLG]
      ON (
