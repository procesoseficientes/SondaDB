...
-- Description:			    SP que genera los descuentos generales por acuerdo comercial de los clientes en acuerdo comercial por cliente y canal
...
-- Ejemplo de Ejecucion:
				EXEC [PACASA].[SWIFT_SP_GENERATE_DISCOUNT_BY_GENERAL_AMOUNT_BY_TRADE_AGREEMENT_FOR_DUPLICATE]
					@CODE_ROUTE = '44'
					,@ORDER = 1
*/
-- =============================================
CREATE PROCEDURE [PACASA].[SWIFT_SP_GENERATE_DISCOUNT_BY_GENERAL_AMOUNT_BY_TRADE_AGREEMENT_FOR_DUPLICATE] (@CODE_ROUTE VARCHAR(250)
, @ORDER INT) 
AS
BEGIN
  SET NOCOUNT ON;
  ...
  /*IF @ORDER = 1
	BEGIN
		...
		-- ------------------------------------------------------------------------------------
		INSERT INTO [PACASA].[SWIFT_DISCOUNT_LIST]
				([NAME_DISCOUNT_LIST])
		...
		SELECT DISTINCT 
			...
			,[TA].[CODE_CUSTOMER]
		FROM @TRADE_AGREEMENT [TA]
		INNER JOIN [PACASA].[SWIFT_DISCOUNT_LIST] [DL] ON (
  ...
  , [FREQUENCY]  
    ...
    INNER JOIN [PACASA].[SWIFT_PROMO_DISCOUNT_BY_GENERAL_AMOUNT] [TAG]
      ON (
    ...
    LEFT JOIN [PACASA].[SWIFT_DISCOUNT_LIST_BY_GENERAL_AMOUNT] [DLG]
      ON (
