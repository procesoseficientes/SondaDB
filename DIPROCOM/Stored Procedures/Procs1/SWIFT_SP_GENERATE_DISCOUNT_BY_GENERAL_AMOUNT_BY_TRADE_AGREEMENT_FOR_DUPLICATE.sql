...
-- Ejemplo de Ejecucion:
				EXEC [acsa].[SWIFT_SP_GENERATE_DISCOUNT_BY_GENERAL_AMOUNT_BY_TRADE_AGREEMENT_FOR_DUPLICATE]
					@CODE_ROUTE = '44'
					,@ORDER = 1
*/
-- =============================================
CREATE PROCEDURE [acsa].[SWIFT_SP_GENERATE_DISCOUNT_BY_GENERAL_AMOUNT_BY_TRADE_AGREEMENT_FOR_DUPLICATE] (@CODE_ROUTE VARCHAR(250)
, @ORDER INT) 
AS
BEGIN
  SET NOCOUNT ON;
  ...
  /*IF @ORDER = 1
	BEGIN
		...
		-- ------------------------------------------------------------------------------------
		INSERT INTO [acsa].[SWIFT_DISCOUNT_LIST]
				([NAME_DISCOUNT_LIST])
  ...
  , [FREQUENCY]  
    ...
    INNER JOIN [acsa].[SWIFT_PROMO_DISCOUNT_BY_GENERAL_AMOUNT] [TAG]
      ON (
    ...
    LEFT JOIN [acsa].[SWIFT_DISCOUNT_LIST_BY_GENERAL_AMOUNT] [DLG]
      ON (
