...
-- Descripcion: 		7/8/2019 - Se cambia tipo de datos de las columnas INVOICE_ID, DOC_ENTRY a varchar(250)

/*
-- Ejemplo de Ejecucion:
				EXEC [DIPROCOM].[SONDA_SP_ADD_OVERDUE_INVOICE_PAYMENT_BY_XML]
				@XML = '
					
				'
*/
-- =============================================
CREATE PROCEDURE [DIPROCOM].[SONDA_SP_ADD_OVERDUE_INVOICE_PAYMENT_BY_XML]
(@XML XML)
AS
BEGIN
    SET NOCOUNT ON;
    ...
    (
		...
		AMOUNT_TO_DATE NUMERIC(18,6) NULL DEFAULT(0),
		PENDING_AMOUNT NUMERIC(18,6) NULL DEFAULT(0),
        PRIMARY KEY (
                        [INVOICE_ID],
                        [DOC_ENTRY],
                        [DOC_SERIE],
    ...
    BEGIN TRY
        ...
        SELECT [x].[Rec].[query]('./codeCustomer').[value]('.', 'varchar(250)'),
               ...
               [x].[Rec].[query]('./paymentAmount').[value]('.', 'numeric(18,6)'),
               [x].[Rec].[query]('./paidComment').[value]('.', 'varchar(250)')
        FROM @XML.[nodes]('/Data/overdueInvoicePayment') AS [x]([Rec]);
        ...
        SELECT [x].[Rec].[query]('./invoiceId').[value]('.', 'VARCHAR(250)'),
               [x].[Rec].[query]('./docEntry').[value]('.', 'VARCHAR(250)'),
               [x].[Rec].[query]('./docSerie').[value]('.', 'varchar(250)'),
               [x].[Rec].[query]('./docNum').[value]('.', 'int'),
               [x].[Rec].[query]('./payedAmount').[value]('.', 'numeric(18,6)'),
			   [x].[Rec].[query]('./amountToDate').[value]('.', 'numeric(18,6)'),
			   [x].[Rec].[query]('./pendingAmount').[value]('.', 'numeric(18,6)')
        FROM @XML.[nodes]('/Data/overdueInvoicePayment/overdueInvoicePaymentDetail') AS [x]([Rec]);
        ...
        BEGIN
            ...
            BEGIN TRY
                ...
                FROM [DIPROCOM].[SONDA_OVERDUE_INVOICE_PAYMENT_HEADER]
                ...
                BEGIN
                    ...
                    -- ----------------------------------------------------------------------------
                    INSERT INTO [DIPROCOM].[SONDA_OVERDUE_INVOICE_PAYMENT_HEADER]
                    (
                        [CODE_CUSTOMER],
                        [DOC_SERIE],
                        [DOC_NUM],
                    ...
                    SELECT [H].[CODE_CUSTOMER],
                           ...
                           [H].[COMMENT]
                    FROM @HEADER AS [H]
                    WHERE [H].[DOC_SERIE] = @CURRENT_DOC_SERIE
                    ...
                    -- --------------------------------------------------------------------------------------
                    INSERT INTO [DIPROCOM].[SONDA_OVERDUE_INVOICE_PAYMENT_DETAIL]
                    (
                        [PAYMENT_HEADER_ID],
                        [INVOICE_ID],
                        [DOC_ENTRY],
                    ...
                    SELECT @PAYMENT_ID_BO,
						   ...
						   [D].[PENDING_AMOUNT]
                    FROM @DETAIL AS [D]
                    WHERE [D].[DOC_SERIE] = @CURRENT_DOC_SERIE
                          AND [D].[DOC_NUM] = @CURRENT_DOC_NUM;

                    -- --------------------------------------------------------------------------------------
                    -- Agregamos el detalle de los tipos de pago realizados en el documento
                    -- --------------------------------------------------------------------------------------
                    INSERT INTO [DIPROCOM].[SONDA_PAYMENT_TYPE_DETAIL_FOR_OVERDUE_INVOICE_PAYMENT]
                    (
                        [PAYMENT_HEADER_ID],
                        [PAYMENT_TYPE],
                        [FRONT_IMAGE],
                    ...
                    SELECT @PAYMENT_ID_BO,
                           ...
                           [BANK_NAME],
                           [AMOUNT]
                    FROM @PAYMENT_TYPE_DETAIL AS [PTD]
                    WHERE [PTD].[DOC_SERIE] = @CURRENT_DOC_SERIE
