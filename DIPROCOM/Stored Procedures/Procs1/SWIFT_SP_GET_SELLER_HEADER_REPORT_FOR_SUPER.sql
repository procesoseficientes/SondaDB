...
-- Ejemplo de Ejecucion:
				EXEC [DIPROCOM].[SWIFT_SP_GET_SELLER_HEADER_REPORT_FOR_SUPER]
					@TEAM_ID = 2
*/
-- =============================================
CREATE PROCEDURE [DIPROCOM].[SWIFT_SP_GET_SELLER_HEADER_REPORT_FOR_SUPER] (@TEAM_ID INT)
AS
BEGIN
    SET NOCOUNT ON;
	--
    ...
    SELECT TOP 1
       ...
       ,@DISPLAY_DECIMALS = [DIPROCOM].[SWIFT_FN_GET_PARAMETER]('CALCULATION_RULES',
    ...
    SELECT
       ...
       ,0 [DOCUMENT_TOTAL]
    FROM
        [DIPROCOM].[SWIFT_TEAM] [T]
    INNER JOIN [DIPROCOM].[USERS] [U] ON [U].[CORRELATIVE] = [T].[SUPERVISOR]
    WHERE
        [T].[TEAM_ID] = @TEAM_ID;
	-- ------------------------------------------------------------------------------------
	-- Resultado final
