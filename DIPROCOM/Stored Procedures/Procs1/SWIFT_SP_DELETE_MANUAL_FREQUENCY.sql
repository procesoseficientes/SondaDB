CREATE PROCEDURE [DIPROCOM].[SWIFT_SP_DELETE_MANUAL_FREQUENCY]	
    @XML AS XML
...
------------------------------------------------------------
	...
	SELECT ID_FREQUENCY FROM DIPROCOM.SWIFT_FREQUENCY_X_CUSTOMER 
...
------------------------------------------------------------
	...
	IF NOT EXISTS(SELECT * FROM DIPROCOM.SWIFT_FREQUENCY_X_CUSTOMER
			  WHERE ID_FREQUENCY IN (SELECT * FROM @TEMP_TABLE_IDS))
		BEGIN
			DELETE FROM DIPROCOM.SWIFT_FREQUENCY WHERE ID_FREQUENCY IN (SELECT ID_FREQUENCY FROM @TEMP_TABLE_IDS);
		END
