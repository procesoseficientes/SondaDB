...
-- Ejemplo de Ejecucion:
				EXEC [PACASA].[SONDA_SP_GET_READY_TO_START_ROUTE]
					@CODE_ROUTE = '1'
*/
-- =============================================
CREATE PROCEDURE [PACASA].[SONDA_SP_GET_READY_TO_START_ROUTE]
	(
...
AS
	BEGIN
		...
		EXEC [PACASA].[SWIFT_SP_VALIDATE_IF_ROUTE_GENERATE_TRADE_AGREEMENT] @CODE_ROUTE = @CODE_ROUTE ,
			@GENERATE_TRADE_AGREEMENT = @GENERATE_TRADE_AGREEMENT_OUTPUT OUTPUT;

		EXEC [PACASA].[SWIFT_SP_VALIDATE_IF_ROUTE_GENERATE_BONUS] @CODE_ROUTE = @CODE_ROUTE ,
			@GENERATE_BONUS = @GENERATE_BONUS OUTPUT;

		EXEC [PACASA].[SWIFT_SP_VALIDATE_IF_ROUTE_GENERATE_DISCOUNT] @CODE_ROUTE = @CODE_ROUTE ,
			@GENERATE_DISCOUNT = @GENERATE_DISCOUNT OUTPUT;

		EXEC [PACASA].[SWIFT_SP_VALIDATE_IF_ROUTE_GENERATE_SKU_SALES_BY_MULTIPLE] @CODE_ROUTE = @CODE_ROUTE ,
			@GENERATE_SKU_SALES_BY_MULTIPLE = @GENERATE_SKU_SALES_BY_MULTIPLE OUTPUT;

		EXEC [PACASA].[SWIFT_SP_VALIDATE_IF_ROUTE_GENERATE_SPECIAL_PRICES_BY_SCALE] @CODE_ROUTE = @CODE_ROUTE , -- varchar(50)
		...
		BEGIN
			EXEC [PACASA].[SWIFT_SP_GENERATE_DISCOUNT_FROM_TRADE_AGREEMENT] @CODE_ROUTE = @CODE_ROUTE;
		END;
		-- ------------------------------------------------------------------------------------
		...
		IF @GENERATE_TRADE_AGREEMENT_OUTPUT = 1
			OR @GENERATE_BONUS = 1
		BEGIN
			EXEC [PACASA].[SWIFT_SP_GENERATE_BONUS_FROM_TRADE_AGREEMENT] @CODE_ROUTE = @CODE_ROUTE;
		END;
		-- ------------------------------------------------------------------------------------
		-- Genera las bonificaciones para la ruta
		-- ------------------------------------------------------------------------------------
		...
		BEGIN
			EXEC [PACASA].[SWIFT_SP_GENERATE_SKU_SALES_BY_MULTIPLE_FROM_TRADE_AGREEMENT] @CODE_ROUTE = @CODE_ROUTE;
		END;
		...
		BEGIN
			EXEC [PACASA].[SWIFT_SP_GENERATE_SPECIAL_PRICE_FROM_TRADE_AGREEMENT] @CODE_ROUTE = @CODE_ROUTE;
		END;
		...
		BEGIN
			...
			-- ------------------------------------------------------------------------------------
			EXEC [PACASA].[SWIFT_SP_GENERATE_PRICE_LIST_FOR_ROUTE] @CODE_ROUTE = @CODE_ROUTE;
