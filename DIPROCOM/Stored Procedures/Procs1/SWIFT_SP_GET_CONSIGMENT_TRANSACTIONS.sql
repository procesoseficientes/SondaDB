...
-- Ejemplo de Ejecucion:
				EXEC [PACASA].[SWIFT_SP_GET_CONSIGMENT_TRANSACTIONS]  @DOC_SERIE = 'Serie Re-Consignación',@DOC_NUM = 22 
 */
-- =============================================
CREATE PROCEDURE [PACASA].[SWIFT_SP_GET_CONSIGMENT_TRANSACTIONS] (  
...
DECLARE @CONSIGNMENT_BO_NUM INT
  --
  SELECT @CONSIGNMENT_BO_NUM = CONSIGNMENT_ID FROM [PACASA].SWIFT_CONSIGNMENT_HEADER sch WHERE sch.DOC_SERIE =@DOC_SERIE AND sch.DOC_NUM = @DOC_NUM
  --
  SELECT
		[scd].[DOC_SERIE]		
		,[scd].[DOC_NUM]
	...
	FROM [PACASA].[SONDA_HISTORICAL_TRACEABILITY_CONSIGNMENT] [scd]
		INNER JOIN [PACASA].[SWIFT_CONSIGNMENT_HEADER] [sch] ON 
		...
		INNER JOIN [PACASA].[SWIFT_VIEW_ALL_SKU] AS VS ON VS.CODE_SKU = scd.CODE_SKU 
