...
-- Ejemplo de Ejecucion:
				...
				--
				EXEC [acsa].SWIFT_SP_DELETE_PROMO_OF_BONUS_BY_SCALE
					@PROMO_BONUS_BY_SCALE_ID = 1
				-- 
				SELECT * FROM [acsa].SWIFT_PROMO_BONUS_BY_SCALE
*/
-- =============================================
CREATE PROCEDURE [acsa].[SWIFT_SP_DELETE_PROMO_OF_BONUS_BY_SCALE] (@PROMO_BONUS_BY_SCALE_ID INT)
AS
BEGIN
  SET NOCOUNT ON;
  --
  ...
  SET @PROMO_ID = (SELECT TOP 1
      [PROMO_ID]
    FROM [acsa].[SWIFT_PROMO_BONUS_BY_SCALE]
