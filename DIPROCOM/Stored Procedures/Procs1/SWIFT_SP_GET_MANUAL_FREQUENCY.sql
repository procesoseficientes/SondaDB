...
-- Ejemplo de Ejecucion:				
				--exec [DIPROCOM].[SWIFT_SP_GET_MANUAL_FRECUENCY]  
*/
-- =============================================
CREATE PROCEDURE [DIPROCOM].[SWIFT_SP_GET_MANUAL_FREQUENCY]
AS
	BEGIN
		SELECT
			[VAC].[CODE_CUSTOMER]
		...
		INTO
			[#CUSTOMERS]
		FROM
			[DIPROCOM].[ERP_VIEW_COSTUMER] AS [VAC];
		...
		FROM
			[DIPROCOM].[SWIFT_FREQUENCY] [SF]
		INNER JOIN [DIPROCOM].[SWIFT_FREQUENCY_X_CUSTOMER] [FXC]
		ON	[SF].[ID_FREQUENCY] = [FXC].[ID_FREQUENCY]
		INNER JOIN [#CUSTOMERS] [VAC]
		ON	[VAC].[CODE_CUSTOMER] = [FXC].[CODE_CUSTOMER]
		INNER JOIN [DIPROCOM].[SWIFT_ROUTES] [US]
