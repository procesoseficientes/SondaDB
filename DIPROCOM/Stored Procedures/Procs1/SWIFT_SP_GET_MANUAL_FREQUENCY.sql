...
-- Ejemplo de Ejecucion:				
				--exec [acsa].[SWIFT_SP_GET_MANUAL_FREQUENCY] 
*/
-- =============================================
CREATE PROCEDURE [acsa].[SWIFT_SP_GET_MANUAL_FREQUENCY]
AS
	BEGIN
		SELECT
			[VAC].[CODE_CUSTOMER]
		...
		INTO
			[#CUSTOMERS]
		FROM
			[SWIFT_INTERFACES].[acsa].[ERP_VIEW_COSTUMER] AS [VAC];
		...
		FROM
			[acsa].[SWIFT_FREQUENCY] [SF]
		INNER JOIN [acsa].[SWIFT_FREQUENCY_X_CUSTOMER] [FXC]
		...
		INNER JOIN [acsa].[SWIFT_ROUTES] [US]
