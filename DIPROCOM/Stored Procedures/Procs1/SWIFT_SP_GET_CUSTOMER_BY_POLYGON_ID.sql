...
-- Ejemplo de Ejecucion:
        --
        EXEC [PACASA].[SWIFT_SP_GET_CUSTOMER_BY_POLYGON_ID]
        @POLYGON_ID = 9304
*/
-- =============================================
CREATE PROCEDURE [PACASA].[SWIFT_SP_GET_CUSTOMER_BY_POLYGON_ID] (@POLYGON_ID INT,
  ...
  BEGIN TRY
    ...
    BEGIN
      INSERT INTO @TAG ([TAG_COLOR])
        SELECT
          [VALUE]
        FROM [PACASA].[SWIFT_FN_SPLIT_2](@TAG_COLOR, '|')
    ...
    BEGIN
      INSERT INTO @CHANNEL ([CHANNEL_ID])
        SELECT
          [VALUE]
        FROM [PACASA].[SWIFT_FN_SPLIT_2](@CHANNEL_ID, '|')
    ...
    BEGIN
      INSERT INTO @CUSTOMER
        SELECT DISTINCT
         ...
         ,C.LONGITUDE
         ,1 IS_IN
        FROM [PACASA].[SWIFT_CUSTOMER_ASSOCIATE_TO_POLYGON] [CAP]
    ...
    BEGIN
      ...
      SELECT
        @GEOMETRY_POLYGON = [PACASA].SWIFT_GET_GEOMETRY_POLYGON_BY_POLIGON_ID(@POLYGON_ID)
      ...
      INSERT INTO @CUSTOMER
        ...
        LEFT JOIN [PACASA].[SWIFT_TAG_X_CUSTOMER] [TXC]
          ON [TXC].[CUSTOMER] = [C].[CODE_CUSTOMER]
        LEFT JOIN [PACASA].[SWIFT_CHANNEL_X_CUSTOMER] [CHXC]
