...
-- Ejemplo de Ejecucion:
        --
        EXEC [DIPROCOM].[SWIFT_SP_GET_CUSTOMER_BY_POLYGON_ID]
        @POLYGON_ID = 9304
*/
-- =============================================
CREATE PROCEDURE [DIPROCOM].[SWIFT_SP_GET_CUSTOMER_BY_POLYGON_ID] (@POLYGON_ID INT,
  ...
  BEGIN TRY
    ...
    BEGIN
      INSERT INTO @TAG ([TAG_COLOR])
        SELECT
          [VALUE]
        FROM [DIPROCOM].[SWIFT_FN_SPLIT_2](@TAG_COLOR, '|')
    ...
    BEGIN
      INSERT INTO @CHANNEL ([CHANNEL_ID])
        SELECT
          [VALUE]
        FROM [DIPROCOM].[SWIFT_FN_SPLIT_2](@CHANNEL_ID, '|')
    ...
    BEGIN
      ...
      SELECT
        @GEOMETRY_POLYGON = DIPROCOM.SWIFT_GET_GEOMETRY_POLYGON_BY_POLIGON_ID(@POLYGON_ID)
      ...
      INSERT INTO @CUSTOMER
        ...
        LEFT JOIN [DIPROCOM].[SWIFT_TAG_X_CUSTOMER] [TXC]
          ON [TXC].[CUSTOMER] = [C].[CODE_CUSTOMER]
        LEFT JOIN [DIPROCOM].[SWIFT_CHANNEL_X_CUSTOMER] [CHXC]
    ...
    SELECT
     ...
     ,ISNULL(CONVERT(INT, ISNULL(F.SUNDAY,CF.SUNDAY)), 0) AS SUNDAY
     ,ISNULL(CONVERT(INT, ISNULL(F.MONDAY,CF.MONDAY)), 0) AS MONDAY
     ,ISNULL(CONVERT(INT, ISNULL(F.TUESDAY,CF.TUESDAY)), 0) AS TUESDAY
     ,ISNULL(CONVERT(INT, ISNULL(F.WEDNESDAY,CF.WEDNESDAY)), 0) AS WEDNESDAY
     ,ISNULL(CONVERT(INT, ISNULL(F.THURSDAY,CF.THURSDAY)), 0) AS THURSDAY
     ,ISNULL(CONVERT(INT, ISNULL(F.FRIDAY,CF.FRIDAY)), 0) AS FRIDAY
     ,ISNULL(CONVERT(INT, ISNULL(F.SATURDAY,CF.SATURDAY)), 0) AS SATURDAY
     ,ISNULL(CONVERT(INT, ISNULL(F.FREQUENCY_WEEKS,CF.FREQUENCY_WEEKS)), 0) AS FREQUENCY_WEEKS
    ...
    FROM @CUSTOMER C
    LEFT JOIN [DIPROCOM].[SWIFT_POLYGON_X_CUSTOMER] PC
      ON (C.CODE_CUSTOMER = PC.CODE_CUSTOMER
      AND PC.POLYGON_ID = @POLYGON_ID
      )
    LEFT JOIN [DIPROCOM].SWIFT_CUSTOMER_FREQUENCY CF
      ON (CF.CODE_CUSTOMER = C.CODE_CUSTOMER)
    LEFT JOIN [DIPROCOM].[SWIFT_FREQUENCY_X_CUSTOMER] FC
      ON (C.CODE_CUSTOMER = FC.CODE_CUSTOMER)
	LEFT JOIN [DIPROCOM].[SWIFT_FREQUENCY] F
