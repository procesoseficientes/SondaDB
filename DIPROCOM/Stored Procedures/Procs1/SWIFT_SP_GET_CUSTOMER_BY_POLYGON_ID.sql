...
*/
-- =============================================
CREATE PROCEDURE [acsa].[SWIFT_SP_GET_CUSTOMER_BY_POLYGON_ID] (@POLYGON_ID INT,
  ...
  BEGIN TRY
    ...
    BEGIN
      INSERT INTO @TAG ([TAG_COLOR])
        SELECT
          [VALUE]
        FROM [acsa].[SWIFT_FN_SPLIT_2](@TAG_COLOR, '|')
    ...
    BEGIN
      INSERT INTO @CHANNEL ([CHANNEL_ID])
        SELECT
          [VALUE]
        FROM [acsa].[SWIFT_FN_SPLIT_2](@CHANNEL_ID, '|')
    ...
    BEGIN
      ...
      SELECT
        @GEOMETRY_POLYGON = [acsa].SWIFT_GET_GEOMETRY_POLYGON_BY_POLIGON_ID(@POLYGON_ID)
      ...
      INSERT INTO @CUSTOMER
        ...
        LEFT JOIN [acsa].[SWIFT_TAG_X_CUSTOMER] [TXC]
          ON [TXC].[CUSTOMER] = [C].[CODE_CUSTOMER]
        LEFT JOIN [acsa].[SWIFT_CHANNEL_X_CUSTOMER] [CHXC]
