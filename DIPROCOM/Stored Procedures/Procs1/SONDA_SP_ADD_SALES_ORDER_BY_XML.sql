...
-- Descripcion: 		7/12/2019 - Se agrega columna [TOTAL_AMOUNT_DISPLAY] al encabezado de la orden de venta para almacenar el monto que incluye todo el proceso de descuentos (realizados en el movil)

-- Modificacion 		9/12/2019 @ G-Force Team Sprint Jakarta
-- Autor: 				diego.as
-- Historia/Bug:		
-- Descripcion: 		9/12/2019 - Se corrige insercion del campo "ComboReference" en el detalle del documento de venta

-- Modificacion 		11/21/2019 @ G-Force Team Sprint Oslo
-- Autor: 				diego.as
-- Historia/Bug:		IMPLEMENTACION CENDALZA
-- Descripcion: 		11/21/2019 - Se agrega columna PURCHASE_ORDER_NUMBER en el encabezado del documento de venta

/*			
-- Ejemplo de Ejecucion:
				EXEC [DIPROCOM].[SONDA_SP_ADD_SALES_ORDER_BY_XML]
					@XML = '<?xml version="1.0"?>
<Data>
  <salesOrder>
    ...
    <PosTerminal>SOL0011@DIPROCOM</PosTerminal>
    <GpsUrl>14.99522969,-90.80447165</GpsUrl>
    <TotalAmount>132.75</TotalAmount>
    <Status>0</Status>
    <PostedBy>sol0011@DIPROCOM</PostedBy>
    <Image1 />
    <Image2 />
    <Image3>null</Image3>
    ...
    <IsParent>1</IsParent>
    <ReferenceId>sol0011@DIPROCOM2017/07/10 10:44:56-3</ReferenceId>
    <TimesPrinted>0</TimesPrinted>
    <DocSerie>SOL0011@DIPROCOM</DocSerie>
    <DocNum>6034</DocNum>
    <IsVoid>0</IsVoid>
...
BEGIN
    ...
    BEGIN TRY
	  ...
	  -- ------------------------------------------------------------------------------------
			-- INSERTAR EL XML EN LA TABLA DE LOG
    -- ------------------------------------------------------------------------------------
	  ...
	  SELECT
		...
		INSERT INTO diprocom.SONDA_SALES_ORDER_LOG_EXISTS
				( LOG_DATETIME ,
				  EXISTS_SALES_ORDER ,
				  DOC_SERIE ,
				  DOC_NUM ,
        ...
        SELECT @HEADER_POSTEDDATIME = [x].[Rec].[query]('./PostedDatetime').[value]('.', 'datetime'),
               ...
               @POSTED_BY = [x].[Rec].[query]('./PostedBy').[value]('.', 'varchar(50)')
        FROM @XML.[nodes]('/Data/salesOrder') AS [x]([Rec]);
        ...
        SELECT [x].[Rec].[query]('./Sku').[value]('.', 'varchar(50)'),
               ...
               CASE [x].[Rec].[query]('./Long').[value]('.', 'varchar(50)')
                   WHEN 'null' THEN
                       NULL
                   ELSE
                       [x].[Rec].[query]('./Long').[value]('.', 'varchar(50)')
               END,
               CASE [x].[Rec].[query]('./DiscountType').[value]('.', 'varchar(50)')
                   WHEN 'null' THEN
                       NULL
                   ELSE
                       [x].[Rec].[query]('./DiscountType').[value]('.', 'varchar(50)')
               END,
               CASE [x].[Rec].[query]('./DiscountByFamily').[value]('.', 'varchar(50)')
                   WHEN 'null' THEN
                       0
                   ELSE
                       [x].[Rec].[query]('./DiscountByFamily').[value]('.', 'numeric(18,6)')
               END,
               CASE [x].[Rec].[query]('./DiscountByGeneralAmount').[value]('.', 'varchar(50)')
                   WHEN 'null' THEN
                       0
                   ELSE
                       [x].[Rec].[query]('./DiscountByGeneralAmount').[value]('.', 'numeric(18,6)')
               END,
               CASE [x].[Rec].[query]('./DiscountByFamilyAndPaymentType').[value]('.', 'varchar(50)')
                   WHEN 'null' THEN
                       0
                   ELSE
                       [x].[Rec].[query]('./DiscountByFamilyAndPaymentType').[value]('.', 'numeric(18,6)')
               END,
               CASE [x].[Rec].[query]('./TypeOfDiscountByFamily').[value]('.', 'varchar(50)')
                   WHEN 'null' THEN
                       NULL
                   ELSE
                       [x].[Rec].[query]('./TypeOfDiscountByFamily').[value]('.', 'varchar(50)')
               END,
               CASE [x].[Rec].[query]('./TypeOfDiscountByGeneralAmount').[value]('.', 'varchar(50)')
                   WHEN 'null' THEN
                       NULL
                   ELSE
                       [x].[Rec].[query]('./TypeOfDiscountByGeneralAmount').[value]('.', 'varchar(50)')
               END,
               CASE [x].[Rec].[query]('./TypeOfDiscountByFamilyAndPaymentType').[value]('.', 'varchar(50)')
                   WHEN 'null' THEN
                       NULL
                   ELSE
                       [x].[Rec].[query]('./TypeOfDiscountByFamilyAndPaymentType').[value]('.', 'varchar(50)')
               END,
               [x].[Rec].[query]('./BasePrice').[value]('.', 'numeric(18,6)'),
               [x].[Rec].[query]('./CodeFamily').[value]('.', 'varchar(50)'),
               [x].[Rec].[query]('./UniqueDiscountByScaleApplied').[value]('.', 'int'),
               [x].[Rec].[query]('./TotalAmountDisplay').[value]('.', 'numeric(18,6)')
        ...
        BEGIN
            ...
            EXEC [DIPROCOM].[SONDA_SP_VALIDATED_IF_EXISTS_SALES_ORDER_3] @DOC_SERIE = @DOC_SERIE,
                                                                      @DOC_NUM = @DOC_NUM,                     -- int
            ...
            BEGIN
                ...
                BEGIN TRY
                    ...
                    (
                        SELECT 1
                        FROM [DIPROCOM].[SONDA_SALES_ORDER_HEADER] [SH]
                        WHERE [SH].[IS_READY_TO_SEND] = 1
                    ...
                    BEGIN
                        ...
                        --
                        SET @LOG_MESSAGE
                            = 'Listo para envio SAP Serie: ' + @DOC_SERIE + ' DocNum: ' + CAST(@DOC_NUM AS VARCHAR);
                        EXEC [DIPROCOM].[SONDA_SP_INSERT_SONDA_SERVER_ERROR_LOG] @CODE_ROUTE = @CODE_ROUTE,
                                                                              @LOGIN = NULL,
                    ...
                    --
                    DELETE [SOD]
                    FROM [DIPROCOM].[SONDA_SALES_ORDER_DETAIL] [SOD]
                        INNER JOIN [DIPROCOM].[SONDA_SALES_ORDER_HEADER] [SOH]
                            ON [SOD].[SALES_ORDER_ID] = [SOH].[SALES_ORDER_ID]
                    WHERE [SOH].[DOC_SERIE] = @DOC_SERIE
                          AND [SOH].[DOC_NUM] = @DOC_NUM
                          AND [SOH].[IS_READY_TO_SEND] = 0;
                    --
                    DELETE [DIPROCOM].[SONDA_SALES_ORDER_HEADER]
            ...
            BEGIN
                BEGIN TRY
                    ...
                    SELECT NULL,
                           ...
                           CASE [X].[Rec].[query]('./VoidDatetime').[value]('.', 'varchar(50)')
                               WHEN 'null' THEN
                                   NULL
                               ELSE
                                   [X].[Rec].[query]('./VoidDatetime').[value]('.', 'DATETIME')
                           END,
                           CASE [X].[Rec].[query]('./VoidReason').[value]('.', 'varchar(50)')
                               WHEN 'null' THEN
                                   NULL
                               ELSE
                                   [X].[Rec].[query]('./VoidReason').[value]('.', 'VARCHAR(25)')
                           END,
                           ...
                           CASE [X].[Rec].[query]('./ClosedRouteDatetime').[value]('.', 'varchar(50)')
                               WHEN 'null' THEN
                                   NULL
                               ELSE
                                   [X].[Rec].[query]('./ClosedRouteDatetime').[value]('.', 'DATETIME')
                           END,
                           [X].[Rec].[query]('./IsActiveRoute').[value]('.', 'int'),
                           [X].[Rec].[query]('./GpsUrl').[value]('.', 'varchar(50)'),
                           ...
                           1,
                           @SERVER_POSTED_DATETIME,
                           [X].[Rec].[query]('./DeviceNetworkType').[value]('.', 'varchar(15)'),
                           CASE [X].[Rec].[query]('./IsPostedOffline').[value]('.', 'varchar(50)')
                               WHEN 'null' THEN
                                   NULL
                               ELSE
                                   [X].[Rec].[query]('./IsPostedOffline').[value]('.', 'int')
                           END,
                           CASE [X].[Rec].[query]('./GoalHeaderId').[value]('.', 'varchar(50)')
                               WHEN 'null' THEN
                                   NULL
                               ELSE
                           ...
                           END,
                           [X].[Rec].[query]('./TotalAmountDisplay').[value]('.', 'numeric(18,6)'),
                           [X].[Rec].[query]('./PurchaseOrderNumber').[value]('.', 'varchar(100)')
                    FROM @XML.[nodes]('/Data/salesOrder') AS [X]([Rec]);
                    --
                    ...
                    -- inserta el detalle
                    -- ------------------------------------------------------------------------------------
                    INSERT INTO [DIPROCOM].[SONDA_SALES_ORDER_DETAIL]
                    (
                        [SALES_ORDER_ID],
                        [SKU],
                        [LINE_SEQ],
