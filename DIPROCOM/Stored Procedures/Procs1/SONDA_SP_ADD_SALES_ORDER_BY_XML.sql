...
-- Ejemplo de Ejecucion:
				EXEC [acsa].[SONDA_SP_ADD_SALES_ORDER_BY_XML]
					@XML = '<?xml version="1.0"?>
<Data>
  <salesOrder>
  ...
  <uuid>a6941e7844e1768e</uuid>
				...
				--
				SELECT TOP 10 * FROM [acsa].[SONDA_SALES_ORDER_HEADER] ORDER BY 1 DESC
*/
-- =============================================
CREATE PROCEDURE [acsa].[SONDA_SP_ADD_SALES_ORDER_BY_XML]
(
    @XML XML,
    @JSON VARCHAR(MAX)
)
...
BEGIN
    ...
    -- ------------------------------------------------------------------------------------
	  ...
	  SELECT
		...
		INSERT INTO [acsa].SONDA_SALES_ORDER_LOG_EXISTS
				( LOG_DATETIME ,
				  EXISTS_SALES_ORDER ,
				  DOC_SERIE ,
				  DOC_NUM ,
        ...
        BEGIN
            ...
            EXEC [acsa].[SONDA_SP_VALIDATED_IF_EXISTS_SALES_ORDER_3] @DOC_SERIE = @DOC_SERIE,
                                                                      @DOC_NUM = @DOC_NUM,                     -- int
            ...
            BEGIN
                ...
                BEGIN TRY
                    ...
                    --
                    DELETE [SOD]
                    FROM [acsa].[SONDA_SALES_ORDER_DETAIL] [SOD]
                        INNER JOIN [acsa].[SONDA_SALES_ORDER_HEADER] [SOH]
                            ON [SOD].[SALES_ORDER_ID] = [SOH].[SALES_ORDER_ID]
                    WHERE [SOH].[DOC_SERIE] = @DOC_SERIE
                          AND [SOH].[DOC_NUM] = @DOC_NUM
                          AND [SOH].[IS_READY_TO_SEND] = 0;
                    --
                    DELETE [acsa].[SONDA_SALES_ORDER_HEADER]
