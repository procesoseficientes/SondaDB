...
-- Ejemplo de Ejecucion:
				EXEC [DIPROCOM].[SONDA_SP_VALIDATE_IF_EXISTS_DEPOSIT]
*/
-- =============================================
CREATE PROCEDURE [DIPROCOM].[SONDA_SP_VALIDATE_IF_EXISTS_DEPOSIT] (@CODE_ROUTE VARCHAR(50)
...
BEGIN
  ...
  SELECT TOP 1
    @EXISTS = 1
   ,@ID = sd.TRANS_ID
  FROM [DIPROCOM].SONDA_DEPOSITS sd WITH (ROWLOCK, XLOCK, HOLDLOCK)
  ...
  -- Inserta el log
  -- ------------------------------------------------------------------------------------
  EXEC [DIPROCOM].[SONDA_SP_INSERT_DEPOSIT_LOG_EXISTS] @EXISTS_DEPOSIT = @EXISTS
                                                   , -- int                                                   
                                                    @DOC_SERIE = @DOC_SERIE
                                                   , -- varchar(100)
                                                    @DOC_NUM = @DOC_NUM
