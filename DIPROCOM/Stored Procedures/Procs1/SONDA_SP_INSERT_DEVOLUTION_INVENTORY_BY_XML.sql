...
-- Ejemplo de Ejecucion:
				EXEC [DIPROCOM].[SONDA_SP_INSERT_DEVOLUTION_INVENTORY_BY_XML]
				@XML = '
					<Data>
					   <documentoDeDevolucion>
						  ...
						  <LAST_UPDATE_BY>Nl0015@DIPROCOM</LAST_UPDATE_BY>
...
*/
-- =============================================
CREATE PROCEDURE [DIPROCOM].[SONDA_SP_INSERT_DEVOLUTION_INVENTORY_BY_XML](
	@XML XML
)
AS
BEGIN
		...
		-- -------------------------------------------------------------------------------
		 SELECT
		  ...
		  FROM @XML.nodes('Data/documentoDeDevolucion') AS x (Rec);
			...
			-- --------------------------------------------------------------------------
			EXEC [DIPROCOM].[SONDA_SP_VALIDATE_DEVOLUTION_INVENTORY] 
				@DOC_SERIE = @DOC_SERIE ,
				@DOC_NUM = @DOC_NUM,
			...
			BEGIN TRANSACTION INSERT_DEVOLUTION_TRANS;
				...
				-- --------------------------------------------------------------------------
				INSERT INTO [DIPROCOM].[SONDA_DEVOLUTION_INVENTORY_HEADER](
					[CODE_CUSTOMER]
					,[DOC_SERIE]
					,[DOC_NUM]
					,[CODE_ROUTE]
				...
				-- --------------------------------------------------------------------------
				INSERT INTO [DIPROCOM].[SONDA_DEVOLUTION_INVENTORY_DETAIL]
						(
							[DEVOLUTION_ID]
							,[CODE_SKU]
							,[QTY_SKU]
					...
					IF (@IS_GOOD_STATE = 1) BEGIN
						...
						-- -------------------------------------------------------------------------------------
						  IF (@HANDLE_SERIAL = 0) BEGIN
							...
							UPDATE [DIPROCOM].[SONDA_POS_SKUS]
