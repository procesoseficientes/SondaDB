...
-- Ejemplo de Ejecucion:
				exec [DIPROCOM].SWIFT_SP_GET_FREQUENCY_X_TASK @DATE = '20151111'
				--
				exec [DIPROCOM].SWIFT_SP_GET_FREQUENCY_X_TASK @DATE = '20151111', @CODE_FREQUENCY = '00111101SALE001'
*/
-- =============================================
CREATE PROCEDURE [DIPROCOM].[SWIFT_SP_GET_FREQUENCY_X_TASK]
	@DATE DATETIME
...
BEGIN
	...
	--
	DECLARE 
		@query nvarchar(1000) = N'SELECT F.ID_FREQUENCY FROM [DIPROCOM].SWIFT_FREQUENCY F'
		,@dia int = (select datepart(dw,@DATE))
	--
	SELECT @query = @query + CASE 
								WHEN @CODE_FREQUENCY IS NOT NULL THEN ' INNER JOIN [DIPROCOM].[SWIFT_FREQUENCY_X_CUSTOMER] X ON (F.ID_FREQUENCY = X.ID_FREQUENCY)' 
								ELSE ''
							END
	--
	SELECT @query = @query 
