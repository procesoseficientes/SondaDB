...
-- Ejemplo de Ejecucion:
				EXEC [PACASA].SWIFT_SP_GET_DISCOUNT_FROM_PROMO_OF_DISCOUNT_BY_FAMILY
				@PROMO_ID = 3299
*/
-- =============================================
CREATE PROCEDURE [PACASA].[SWIFT_SP_GET_DISCOUNT_FROM_PROMO_OF_DISCOUNT_BY_FAMILY](
	@PROMO_ID INT = NULL
)
AS
BEGIN
	...
	SELECT [DF].[PROMO_DISCOUNT_ID] AS [PROMO_IDENTITY]
			...
			END [DISCOUNT_TYPE]	
	FROM [PACASA].[SWIFT_PROMO_DISCOUNT_BY_FAMILY] [DF]
		INNER JOIN [PACASA].[SWIFT_FAMILY_SKU] [FS] ON ([FS].[CODE_FAMILY_SKU] = [DF].[CODE_FAMILY_SKU])		
