...
-- Ejemplo de Ejecucion:
				EXEC [DIPROCOM].[SWIFT_SALES_ORDER_LOG]
*/
-- =============================================
CREATE PROCEDURE [DIPROCOM].[SWIFT_SALES_ORDER_LOG] 
AS 
BEGIN
	SET NOCOUNT ON
	--
	...
	INTO #T
	FROM DIPROCOM.SONDA_SALES_ORDER_HEADER SSOH WITH (NOLOCK)
		INNER JOIN DIPROCOM.SONDA_SALES_ORDER_DETAIL SSOD WITH (NOLOCK)ON (SSOH.SALES_ORDER_ID=SSOD.SALES_ORDER_ID)
	...
	BEGIN
		...
		-- Inserta el log
		-- ---------------------------------------------------------------
		INSERT INTO DIPROCOM.SWIFT_SALES_ORDER
		SELECT	
			*
		FROM #RESUTL AS T
			WHERE T.ENCABEZADO>T.DETALLE
		...
		SET @BODY += '<p>En DIPROCOM en el servidor ' + @@SERVERNAME + ' con la IP ' + CAST(CONNECTIONPROPERTY('local_net_address') AS VARCHAR) + ',' + CAST(CONNECTIONPROPERTY('local_tcp_port') AS VARCHAR) + ' se han encontrado las siguientes ordenes de venta incompletas:</p>'
		SET @BODY += '<table>'
		SET @BODY += '<tr>'
