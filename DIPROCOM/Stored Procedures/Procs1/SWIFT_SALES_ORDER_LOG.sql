...
-- Ejemplo de Ejecucion:
				EXEC [SONDA].[SWIFT_SALES_ORDER_LOG]
*/
-- =============================================
CREATE PROCEDURE [SONDA].[SWIFT_SALES_ORDER_LOG] 
AS 
BEGIN
	SET NOCOUNT ON
	--
	...
	INTO #T
	FROM [SONDA].SONDA_SALES_ORDER_HEADER SSOH WITH (NOLOCK)
		INNER JOIN [SONDA].SONDA_SALES_ORDER_DETAIL SSOD WITH (NOLOCK)ON (SSOH.SALES_ORDER_ID=SSOD.SALES_ORDER_ID)
	...
	BEGIN
		...
		-- Inserta el log
		-- ---------------------------------------------------------------
		INSERT INTO [SONDA].SWIFT_SALES_ORDER
		SELECT	
			*
		FROM #RESUTL AS T
			WHERE T.ENCABEZADO>T.DETALLE
