...
-- Ejemplo de Ejecucion:
        ...
        GO
        --
        EXEC [PACASA].[SWIFT_SP_GET_TOP5_SALE_ORDER_HEADER]
*/
-- =============================================
CREATE PROCEDURE [PACASA].[SWIFT_SP_GET_TOP5_SALE_ORDER_HEADER](
	@OWNER VARCHAR(125)
)
AS
BEGIN
	...
	SELECT
		...
		,@PARAMETER_OWNER = [PACASA].[SWIFT_FN_GET_PARAMETER]('SALES_ORDER_INTERCOMPANY','SEND_ALL_DETAIL')
	...
	BEGIN
		...
		INNER JOIN [PACASA].[SWIFT_VIEW_ALL_COSTUMER] [SVAC] ON ([SSOH].[CLIENT_ID] = [SVAC].[CODE_CUSTOMER])
		INNER JOIN [PACASA].[SWIFT_WAREHOUSES_PRESALE_FOR_3PL] SWPF ON (SSOH.WAREHOUSE=SWPF.CODE_WAREHOUSE)
		INNER JOIN [PACASA].[USERS] [U] ON [SSOH].[POSTED_BY] = [U].[LOGIN]
		INNER JOIN [PACASA].[SWIFT_SELLER] [SELL] ON [U].[RELATED_SELLER] = [SELL].[SELLER_CODE]
		INNER JOIN [PACASA].[SONDA_SALES_ORDER_DETAIL] [SSOD] ON [SSOD].[SALES_ORDER_ID] = [SSOH].[SALES_ORDER_ID] 
		INNER JOIN [PACASA].[SWIFT_VIEW_ALL_SKU] [SVAS] ON [SVAS].[CODE_SKU] = [SSOD].[SKU]
		INNER JOIN [PACASA].[SWIFT_CUSTOMER_INTERCOMPAY] [CC] ON [CC].[MASTER_ID] = [SSOH].[CLIENT_ID]
		INNER JOIN [PACASA].[SWIFT_SELLER_INTERCOMPAY] [SI] ON [SELL].[SELLER_CODE] = [SI].[MASTER_ID]
		WHERE SWPF.[IS_WAREHOUSE_3PL] = 0
			...
			AND SSOH.TOTAL_AMOUNT= (SELECT SUM([OD].[TOTAL_LINE]) FROM [PACASA].[SONDA_SALES_ORDER_DETAIL] [OD] WHERE [OD].[SALES_ORDER_ID]=[SSOH].[SALES_ORDER_ID])
	...
	FROM [PACASA].[SONDA_SALES_ORDER_HEADER] [SOH]
	...
	INNER JOIN [PACASA].[USERS] [U] ON ([SSOH].[POSTED_BY] = [U].[LOGIN])
	LEFT JOIN [PACASA].[SWIFT_SALES_OFFICE] [SO] ON ([SO].[SALES_OFFICE_ID] = [SW].[SALES_OFFICE_ID])
	LEFT JOIN [PACASA].[SWIFT_SALES_ORGANIZATION] [ORG] ON ([ORG].[SALES_ORGANIZATION_ID] = [SVAC].[ORGANIZACION_VENTAS])
	INNER JOIN [PACASA].[SWIFT_CUSTOMER_INTERCOMPAY] [CC] ON [CC].[MASTER_ID] = [SSOH].[CLIENT_ID]
	INNER JOIN [PACASA].[SWIFT_SELLER_INTERCOMPAY] [SI] ON [U].[RELATED_SELLER] = [SI].[MASTER_ID]
