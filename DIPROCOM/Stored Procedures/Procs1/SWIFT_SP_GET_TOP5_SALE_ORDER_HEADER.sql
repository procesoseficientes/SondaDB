...
-- Ejemplo de Ejecucion:
        ...
        GO
        --
        EXEC [DIPROCOM].[SWIFT_SP_GET_TOP5_SALE_ORDER_HEADER]
*/
-- =============================================
CREATE PROCEDURE [DIPROCOM].SWIFT_SP_GET_TOP5_SALE_ORDER_HEADER(
	@OWNER VARCHAR(125)
)
AS
BEGIN
	...
	SELECT
		...
		,@PARAMETER_OWNER = [DIPROCOM].[SWIFT_FN_GET_PARAMETER]('SALES_ORDER_INTERCOMPANY','SEND_ALL_DETAIL')
	...
	BEGIN
		...
		INNER JOIN [DIPROCOM].[SWIFT_VIEW_ALL_COSTUMER] [SVAC] ON ([SSOH].[CLIENT_ID] = [SVAC].[CODE_CUSTOMER])
		INNER JOIN [DIPROCOM].[SWIFT_WAREHOUSES_PRESALE_FOR_3PL] SWPF ON (SSOH.WAREHOUSE=SWPF.CODE_WAREHOUSE)
		INNER JOIN [DIPROCOM].[USERS] [U] ON [SSOH].[POSTED_BY] = [U].[LOGIN]
		INNER JOIN [DIPROCOM].[SWIFT_SELLER] [SELL] ON [U].[RELATED_SELLER] = [SELL].[SELLER_CODE]
		INNER JOIN [DIPROCOM].[SONDA_SALES_ORDER_DETAIL] [SSOD] ON [SSOD].[SALES_ORDER_ID] = [SSOH].[SALES_ORDER_ID] 
		INNER JOIN [DIPROCOM].[SWIFT_VIEW_ALL_SKU] [SVAS] ON [SVAS].[CODE_SKU] = [SSOD].[SKU]
		INNER JOIN [DIPROCOM].[SWIFT_CUSTOMER_INTERCOMPAY] [CC] ON [CC].[MASTER_ID] = [SSOH].[CLIENT_ID]
		INNER JOIN [DIPROCOM].[SWIFT_SELLER_INTERCOMPAY] [SI] ON [SELL].[SELLER_CODE] = [SI].[MASTER_ID]
		WHERE SWPF.[IS_WAREHOUSE_3PL] = 0
			...
			AND SSOH.TOTAL_AMOUNT= (SELECT SUM([OD].[TOTAL_LINE]) FROM [DIPROCOM].[SONDA_SALES_ORDER_DETAIL] [OD] WHERE [OD].[SALES_ORDER_ID]=[SSOH].[SALES_ORDER_ID])
	...
	FROM [DIPROCOM].[SONDA_SALES_ORDER_HEADER] [SOH]
	...
	INNER JOIN [DIPROCOM].[USERS] [U] ON ([SSOH].[POSTED_BY] = [U].[LOGIN])
	LEFT JOIN [DIPROCOM].[SWIFT_SALES_OFFICE] [SO] ON ([SO].[SALES_OFFICE_ID] = [SW].[SALES_OFFICE_ID])
	LEFT JOIN [DIPROCOM].[SWIFT_SALES_ORGANIZATION] [ORG] ON ([ORG].[SALES_ORGANIZATION_ID] = [SVAC].[ORGANIZACION_VENTAS])
	INNER JOIN [DIPROCOM].[SWIFT_CUSTOMER_INTERCOMPAY] [CC] ON [CC].[MASTER_ID] = [SSOH].[CLIENT_ID]
	INNER JOIN [DIPROCOM].[SWIFT_SELLER_INTERCOMPAY] [SI] ON [U].[RELATED_SELLER] = [SI].[MASTER_ID]
