CREATE PROC [PACASA].[SWIFT_GET_DELIVERY_COMPLIANCE]
@DTBEGIN DATE,
@DTEND DATE
AS
SELECT A.MANIFEST_HEADER, (SELECT COUNT(J.CODE_PICKING) FROM [PACASA].SWIFT_MANIFEST_DETAIL J WHERE J.CODE_MANIFEST_HEADER = A.MANIFEST_HEADER 
) AS 'CANTIDAD_PEDIDOS', A.CREATED_DATE, A.ACCEPTED_STAMP,A.LAST_UPDATE,A.COMPLETED_STAMP, B.NAME_DRIVER, c.NAME_ROUTE
 FROM [PACASA].SWIFT_MANIFEST_HEADER A, [PACASA].SWIFT_DRIVERS B, [PACASA].SWIFT_ROUTES C
