...
*/
-- =============================================
CREATE PROCEDURE [PACASA].[SWIFT_SP_GET_MANIFEST_HEADER] (
	@MANIFEST_HEADER INT
)
AS
BEGIN
	...
	INNER JOIN [PACASA].[SWIFT_MANIFEST_DETAIL] [MD] ON (
	...
	INNER JOIN [PACASA].[SWIFT_PICKING_DETAIL] [PD] ON (
		[PH].[PICKING_HEADER] = [PD].[PICKING_HEADER]
	)
	INNER JOIN [PACASA].[USERS] [U] ON (
		[U].[RELATED_SELLER] = [PH].[CODE_SELLER]
	)
	INNER JOIN [SWIFT_INTERFACES].[PACASA].[ERP_VIEW_DOC_FOR_PICKING] [EDP] ON (
