...
Ejemplo de Ejecucion:
    exec [PACASA].SWIFT_SP_INSERT_RESOLUTION
         @AUTH_ID = 'Prueba'
          , @SERIE = 'Prueba'
          , @BRANCH_NAME = '...'
          , @BRANCH_ADDRES = '...'
...
CREATE PROC [PACASA].[SWIFT_SP_INSERT_RESOLUTION]
    @AUTH_ID VARCHAR(50),
    @SERIE VARCHAR(100),
...
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM .[SONDA_POS_RES_SAT] WHERE [AUTH_ID] = @AUTH_ID and [AUTH_SERIE] = @SERIE )
    BEGIN
        SELECT @pResult = 'El numero de resolucion ya fue ingresado.';
        RETURN -1;
    END;
    --
    ...
    SELECT @DATE_I = DATEADD(DAY, +1, GETDATE());
    SELECT @DATE_F = DATEADD(DAY, -9, @DATE_I);

    IF @DATE_I < GETDATE()
       OR @DATE_F > GETDATE()
    BEGIN
        SELECT @pResult
            = 'La fecha no puede ser mayor a ' + CONVERT(VARCHAR(10), @DATE_I) + ' y menor a '
              + CONVERT(VARCHAR(10), @DATE_F);
        RETURN -1;
    END;
    ...
    --    SELECT @ASSIGNED_DATETIME = GETDATE();

    --    IF @ASSIGNED_TO = ''
    --    BEGIN
    --        SELECT @ASSIGNED_DATETIME = NULL;
    --    END;
    --    --			
    --    SELECT @LIMIT_DATETIME = DATEADD(YEAR, 2, @POST_DATETIME);
    --    --VAlida si es la primera resolu8cion que se ingresa
    --    IF
    --    (
    --        SELECT COUNT(*) FROM [SONDA_POS_RES_SAT]
    --    ) = 0
    --    BEGIN
    --        SELECT @LIMIT_DATETIME = DATEADD(YEAR, 1, @POST_DATETIME);
    --    END;

        INSERT INTO [SONDA_POS_RES_SAT]
        (
            [AUTH_ID],
            ...
            [BRANCH_ADDRESS3],
            [BRANCH_ADDRESS4],
            [FEL_DOCUMENT_TYPE_CLASSIFICATION_ID],
            [FEL_DOCUMENT_TYPE],
            [FEL_STABLISHMENT_CODE_CLASSIFICATION_ID],
            [FEL_STABLISHMENT_CODE]
        )
        VALUES
        (   @AUTH_ID, GETDATE(), GETDATE(), @ASSIGNED_BY, @DOC_FROM, @DOC_TO, @SERIE, 'FACTURA',
            @ASSIGNED_TO, @DOC_FROM, @POST_DATETIME, 1, @BRANCH_NAME, @BRANCH_ADDRES, @BRANCH_ADDRESS2,
            @BRANCH_ADDRESS3, @BRANCH_ADDRESS4, @FEL_DOCUMENT_TYPE,
            -1
            , @FEL_STABLISHMENT_CODE,
            -1);
    END;
