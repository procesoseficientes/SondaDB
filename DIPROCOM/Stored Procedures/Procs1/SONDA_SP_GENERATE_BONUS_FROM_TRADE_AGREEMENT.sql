...
-- Ejemplo de Ejecucion:
				-- 
				EXEC [DIPROCOM].[SONDA_SP_GENERATE_BONUS_FROM_TRADE_AGREEMENT]
					@CODE_ROUTE = 'RUDI@DIPROCOM'
*/
-- =============================================
CREATE PROCEDURE [DIPROCOM].[SONDA_SP_GENERATE_BONUS_FROM_TRADE_AGREEMENT]
(
...
BEGIN
	...
	--
	SELECT @SELLER_CODE = DIPROCOM.SWIFT_FN_GET_SELLER_BY_ROUTE(@CODE_ROUTE)
	...
	-- ------------------------------------------------------------------------------------	
	DELETE BL
	FROM DIPROCOM.SWIFT_BONUS_LIST_BY_CUSTOMER BL
	INNER JOIN #BONUS B ON (
		BL.BONUS_LIST_ID = B.BONUS_LIST_ID
	)
	--
	DELETE BL 
	FROM DIPROCOM.SWIFT_BONUS_LIST_BY_SKU BL
	INNER JOIN #BONUS B ON (
		BL.BONUS_LIST_ID = B.BONUS_LIST_ID
	)
	--
	...
	INNER JOIN DIPROCOM.SWIFT_TRADE_AGREEMENT_BY_CUSTOMER TAC ON(
		C.CODE_CUSTOMER = TAC.CODE_CUSTOMER
	)
	INNER JOIN DIPROCOM.SWIFT_TRADE_AGREEMENT TA ON (
	...
	SELECT
		@CODE_ROUTE + '|' + TA.CODE_TRADE_AGREEMENT AS NAME_BONUS_LIST
	FROM DIPROCOM.SWIFT_VIEW_ALL_COSTUMER C
	INNER JOIN DIPROCOM.SWIFT_CHANNEL_X_CUSTOMER CC ON(
		C.CODE_CUSTOMER = CC.CODE_CUSTOMER
	)
	INNER JOIN DIPROCOM.SWIFT_TRADE_AGREEMENT_BY_CHANNEL TAC ON (
	...
	INNER JOIN DIPROCOM.SWIFT_BONUS_LIST DL ON (
	...
	-- ------------------------------------------------------------------------------------
	INSERT INTO DIPROCOM.SWIFT_BONUS_LIST_BY_SKU
	SELECT DISTINCT
		BL.BONUS_LIST_ID
		,TAB.CODE_SKU
		,TAB.PACK_UNIT
		...
		,TAB.PACK_UNIT_BONUS
	FROM #TRADE_AGREEMENT_BY_CHANNEL TA
	INNER JOIN DIPROCOM.SWIFT_TRADE_AGREEMENT_BONUS TAB ON (
