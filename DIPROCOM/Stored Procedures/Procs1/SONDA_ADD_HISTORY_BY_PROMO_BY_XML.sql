...
-- Ejemplo de Ejecucion:
				EXEC [PACASA].[SONDA_ADD_HISTORY_BY_PROMO_BY_XML]
@XML = '<?xml version="1.0"?>
<Data>
...
*/
-- =============================================
CREATE PROCEDURE [PACASA].[SONDA_ADD_HISTORY_BY_PROMO_BY_XML] (@XML XML)
AS
BEGIN
  SET NOCOUNT ON;
  --
  ...
  BEGIN
	...
	BEGIN TRY
   BEGIN TRAN
    UPDATE [PACASA].[SWIFT_HISTORY_BY_PROMO]
    SET [LAST_APPLIED] = 0
    WHERE [CODE_CUSTOMER] = @CODE_CUSTOMER
    AND [PROMO_ID] = @PROMO_ID;


		INSERT	INTO [PACASA].[SWIFT_HISTORY_BY_PROMO]
				(
					[DOC_SERIE]
					,[DOC_NUM]
					,[CODE_ROUTE]
		...
		IF(@@ERROR = 2627) BEGIN
			...
			VALUES
					(
						...
						,1  -- RESULT - int
						,NULL  -- ERROR_MESSAGE - varchar(250)
						,(SELECT [SERVER_POSTED_DATETIME] FROM [PACASA].[SWIFT_HISTORY_BY_PROMO] WHERE [DOC_SERIE] = @DOC_SERIE AND [DOC_NUM] = @DOC_NUM)
					);
		END
		ELSE BEGIN
			INSERT INTO @RESULT
