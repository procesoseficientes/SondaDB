...
-- Ejemplo de Ejecucion:
				EXEC [PACASA].[SWIFT_SP_GET_LIQUIDATION_DOCUMENT_BY_SKU]
					@LIQUIDATION_ID = 6
					,@CODE_SKU = '100002'
*/
-- =============================================
CREATE PROCEDURE [PACASA].[SWIFT_SP_GET_LIQUIDATION_DOCUMENT_BY_SKU](
...
BEGIN
	...
	--
	DECLARE	@RESULT [PACASA].LIQUIDATION_DOCUMENT_BY_SKU
	...
	FROM [PACASA].[SONDA_LIQUIDATION] [L]
	INNER JOIN [PACASA].[SONDA_POS_SKU_HISTORICAL] [PSH] ON (
	...
	SELECT
		...
		,[ID].[PRICE]
		,[ID].[TOTAL_LINE]
	FROM [PACASA].[SONDA_POS_INVOICE_HEADER] [IH]
	INNER JOIN [PACASA].[SONDA_POS_INVOICE_DETAIL] [ID] ON (
	...
	INNER JOIN [PACASA].[SWIFT_CONSIGNMENT_DETAIL] [CD] ON (
	...
	SELECT DISTINCT
		...
		,[CD].[PRICE]
		,[CD].[TOTAL_LINE]
	FROM [PACASA].[SONDA_DEVOLUTION_INVENTORY_HEADER] [DH]
	INNER JOIN [PACASA].[SONDA_DEVOLUTION_INVENTORY_DETAIL] [DD] ON (
		[DD].[DEVOLUTION_ID] = [DH].[DEVOLUTION_ID]
	)
	INNER JOIN [PACASA].[SONDA_HISTORICAL_TRACEABILITY_CONSIGNMENT] [HTC] ON (
		[HTC].[DOC_SERIE_TARGET] = [DH].[DOC_SERIE]
		AND [HTC].[DOC_NUM_TARGET] = [DH].[DOC_NUM]
		AND [HTC].[CODE_SKU] = [DD].[CODE_SKU]
	)
	INNER JOIN [PACASA].[SWIFT_CONSIGNMENT_HEADER] [CH] ON (
	...
	SET @QUERY = N'SELECT
		...
		,[R].[SKU_DESCRIPTION]
		,[R].[QTY]
		,CONVERT(DECIMAL(18,' + CAST(@DEFAULT_DISPLAY_DECIMALS AS VARCHAR) + '),[PACASA].SWIFT_FN_GET_DISPLAY_NUMBER([R].[PRICE])) AS [PRICE]
		,CONVERT(DECIMAL(18,' + CAST(@DEFAULT_DISPLAY_DECIMALS AS VARCHAR) + '),[PACASA].SWIFT_FN_GET_DISPLAY_NUMBER([R].[TOTAL_LINE])) AS [TOTAL_LINE]
	FROM @RESULT [R]'
	--
	PRINT '----> @QUERY: ' + @QUERY
	--
