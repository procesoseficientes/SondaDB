...
-- Ejemplo de Ejecucion:
				--
				EXEC [DIPROCOM].[SWIFT_SP_GET_WEEKLY_EFFICIENT_ROUTE]
					@STAR_DATE = '20160612'
					,@END_DATE = '20160618'
...
*/
-- =============================================
CREATE PROCEDURE [DIPROCOM].SWIFT_SP_GET_WEEKLY_EFFICIENT_ROUTE (
	@STAR_DATE DATETIME
	,@END_DATE DATETIME
...
BEGIN
	...
	SELECT 
		...
		,[R].[NAME_ROUTE]
	INTO #PRE_ROUTE
	FROM [DIPROCOM].[Split](@CODE_ROUTE,@DELIMITER) [S]
	INNER JOIN [DIPROCOM].[SWIFT_ROUTES] [R] ON (
		[S].[Data] = [R].[CODE_ROUTE]
	)
	/*FROM [DIPROCOM].[SWIFT_FN_SPLIT](@CODE_ROUTE,@DELIMITER) [S]
	INNER JOIN [DIPROCOM].[SWIFT_ROUTES] [R] ON (
	...
	SELECT
		...
		END [TOTAL_AMOUNT]
	INTO #SALES_ORDER
	FROM [DIPROCOM].[SONDA_SALES_ORDER_HEADER] [S]
	...
	SELECT
		...
		--END 
		,[S].[TOTAL_AMOUNT]
	FROM [DIPROCOM].[SONDA_POS_INVOICE_HEADER] [S]
	INNER JOIN [#ROUTE] [R] ON (
		[S].[POS_TERMINAL] = [R].[CODE_ROUTE]
		AND CAST([S].[POSTED_DATETIME] AS DATE) = [R].[DATE]
	)
	...
	SELECT 
		...
		,[R].[CODE_ROUTE]
		,[R].[NAME_ROUTE]
		,CONVERT(DECIMAL(18,' + CAST(@DEFAULT_DISPLAY_DECIMALS AS VARCHAR) + '),[DIPROCOM].[SWIFT_FN_GET_DISPLAY_NUMBER]([R].[SUNDAY])) [SUNDAY]
		,CONVERT(DECIMAL(18,' + CAST(@DEFAULT_DISPLAY_DECIMALS AS VARCHAR) + '),[DIPROCOM].[SWIFT_FN_GET_DISPLAY_NUMBER]([R].[MONDAY])) [MONDAY]
		,CONVERT(DECIMAL(18,' + CAST(@DEFAULT_DISPLAY_DECIMALS AS VARCHAR) + '),[DIPROCOM].[SWIFT_FN_GET_DISPLAY_NUMBER]([R].[TUESDAY])) [TUESDAY]
		,CONVERT(DECIMAL(18,' + CAST(@DEFAULT_DISPLAY_DECIMALS AS VARCHAR) + '),[DIPROCOM].[SWIFT_FN_GET_DISPLAY_NUMBER]([R].[WEDNESDAY])) [WEDNESDAY]
		,CONVERT(DECIMAL(18,' + CAST(@DEFAULT_DISPLAY_DECIMALS AS VARCHAR) + '),[DIPROCOM].[SWIFT_FN_GET_DISPLAY_NUMBER]([R].[THURSDAY])) [THURSDAY]
		,CONVERT(DECIMAL(18,' + CAST(@DEFAULT_DISPLAY_DECIMALS AS VARCHAR) + '),[DIPROCOM].[SWIFT_FN_GET_DISPLAY_NUMBER]([R].[FRIDAY])) [FRIDAY]
		,CONVERT(DECIMAL(18,' + CAST(@DEFAULT_DISPLAY_DECIMALS AS VARCHAR) + '),[DIPROCOM].[SWIFT_FN_GET_DISPLAY_NUMBER]([R].[SATURDAY])) [SATURDAY]
		,CONVERT(DECIMAL(18,' + CAST(@DEFAULT_DISPLAY_DECIMALS AS VARCHAR) + '),[DIPROCOM].[SWIFT_FN_GET_DISPLAY_NUMBER](([R].[SUNDAY] + [R].[FRIDAY] + [R].[MONDAY] + [R].[SATURDAY] + [R].[THURSDAY] + [R].[TUESDAY] + [R].[WEDNESDAY]))) [TOTAL_AMOUNT] --,[DIPROCOM].[SWIFT_FN_GET_DISPLAY_NUMBER](([R].[SUNDAY] + [R].[FRIDAY] + [R].[MONDAY] + [R].[SATURDAY] + [R].[THURSDAY] + [R].[TUESDAY] + [R].[WEDNESDAY])) [TOTAL_AMOUNT]
	FROM [#RESULT] [R]
	INNER JOIN [DIPROCOM].[USERS] [U] ON (
