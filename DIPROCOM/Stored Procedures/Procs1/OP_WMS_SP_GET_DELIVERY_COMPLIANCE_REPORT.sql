...
-- Ejemplo de Ejecucion:
			EXEC [PACASA].OP_WMS_SP_GET_DELIVERY_COMPLIANCE_REPORT @VEHICLE_CODES = '17|19|21|25' ,@PILOT_CODES = '20', @START_DATE = '2017-10-19 6:50:05 PM' , @END_DATE = '2017-11-10 11:09:53 AM'
*/
-- =============================================
CREATE PROCEDURE [PACASA].[OP_WMS_SP_GET_DELIVERY_COMPLIANCE_REPORT] (@VEHICLE_CODES VARCHAR(MAX) = NULL,
...
BEGIN
  ...
  INSERT INTO #VEHICLES
    SELECT DISTINCT
      [VS].[VALUE]
     ,[V].[PLATE_NUMBER]
    FROM [PACASA].[OP_WMS_FUNC_SPLIT_3](@VEHICLE_CODES, '|') [VS]
    INNER JOIN [PACASA].[OP_WMS_VEHICLE] [V]
  ...
  INSERT INTO #PILOTS
    SELECT DISTINCT
     ...
     ,[P].[NAME]
     ,[P].[LAST_NAME]
    FROM [PACASA].[OP_WMS_FUNC_SPLIT_3](@PILOT_CODES, '|') [PS]
    INNER JOIN [PACASA].[OP_WMS_PILOT] [P]
  ...
  EXEC [PACASA].OP_WMS_SP_GET_MANIFEST_PERCENTAGE_AND_STATUS_BY_DEMAND_PICKING @VEHICLE_CODES = @VEHICLE_CODES
  ...
  INSERT INTO @EXTERNAL_SOURCE
    ...
    FROM [PACASA].[OP_SETUP_EXTERNAL_SOURCE] [ES]
...
INSERT INTO #DELIVERY_COMPLIANCE_REPORT
    SELECT
     ...
     ,[T].[EXPECTED_GPS]
     ,[T].[POSTED_GPS]
     ,[PACASA].[OP_WMS_FN_CALCULATE_DISTANCE]([T].[EXPECTED_GPS], T.[POSTED_GPS]) AS [DISTANCE]
     ,[T].[ACCEPTED_STAMP]
     ,[T].[COMPLETED_STAMP]
    ...
    FROM [PACASA].[OP_WMS_MANIFEST_HEADER] [MH]
    INNER JOIN [PACASA].[OP_WMS_MANIFEST_DETAIL] [MD]
    ...
    INNER JOIN [PACASA].[OP_WMS_NEXT_PICKING_DEMAND_HEADER] [PDH]
      ON [PDH].[PICKING_DEMAND_HEADER_ID] = [PBT].[PICKING_DEMAND_HEADER_ID]      
    INNER JOIN [PACASA].[OP_WMS_NEXT_PICKING_DEMAND_DETAIL] [PDD]
      ON ([PDH].[PICKING_DEMAND_HEADER_ID] = [PDD].[PICKING_DEMAND_HEADER_ID])
    INNER JOIN [PACASA].[OP_WMS_MATERIALS] [M]
