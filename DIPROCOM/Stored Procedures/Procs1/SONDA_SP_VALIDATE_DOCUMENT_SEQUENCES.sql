...
EXEC [DIPROCOM].[SONDA_SP_VALIDATE_DOCUMENT_SEQUENCES] 
	@XML = 
	'
<Data>
	<docSquences>
...
*/
-- =============================================
-- DROP PROCEDURE [DIPROCOM].[SONDA_SP_VALIDATE_DOCUMENT_SEQUENCES]
CREATE PROCEDURE [DIPROCOM].[SONDA_SP_VALIDATE_DOCUMENT_SEQUENCES] (@XML XML)
AS
	BEGIN
		SET NOCOUNT ON;
		DECLARE
		...
		BEGIN
			...
			BEGIN TRY
				...
				SELECT TOP 1
					@RESULT = 1
				FROM
					[DIPROCOM].[SWIFT_DOCUMENT_SEQUENCE]
				WHERE
					[CURRENT_DOC] = @CURRENT_DOC
				...
				INSERT	INTO @RESULTS
				VALUES
						(@DOC_TYPE ,@SERIE ,@CURRENT_DOC ,@RESULT);
