...
-- Description:			SP que genera las bonificaciones por combo desde acuerdo comercial de canal para los clientes asociados a un canla que tambien esten en un acuerdo comercial de cliente
...
-- Ejemplo de Ejecucion:
				EXEC [DIPROCOM].[SWIFT_SP_GENERATE_BONUS_COMBO_BY_CHANNEL_FOR_DUPLICATE]
					@CODE_ROUTE = '44'
					,@ORDER = 2
*/
-- =============================================
CREATE PROCEDURE [DIPROCOM].[SWIFT_SP_GENERATE_BONUS_COMBO_BY_CHANNEL_FOR_DUPLICATE] (@CODE_ROUTE VARCHAR(250)
, @ORDER INT)
AS
BEGIN
  SET NOCOUNT ON;
  ...
  , [NAME_BONUS_LIST])
    ...
    AND @ACTIVE_STATUS BETWEEN [TA].[VALID_START_DATETIME] AND [TA].[VALID_END_DATETIME]
  ...
  /*IF @ORDER = 1
	BEGIN
		...
		-- ------------------------------------------------------------------------------------
		INSERT INTO [DIPROCOM].[SWIFT_BONUS_LIST]
				([NAME_BONUS_LIST])
		...
		-- ------------------------------------------------------------------------------------
		INSERT INTO [DIPROCOM].[SWIFT_BONUS_LIST_BY_CUSTOMER]
				(
					[BONUS_LIST_ID]
					,[CODE_CUSTOMER]
				)
		SELECT DISTINCT 
			...
			,[TA].[CODE_CUSTOMER]
		FROM @TRADE_AGREEMENT [TA]
		INNER JOIN [DIPROCOM].[SWIFT_BONUS_LIST] [BL] ON (
  ...
  , [FREQUENCY]  
    ...
    LEFT JOIN [DIPROCOM].[SWIFT_BONUS_LIST_BY_COMBO] [BLC]
      ON (
  ...
  , [IS_MULTIPLE])
    ...
    LEFT JOIN [DIPROCOM].[SWIFT_BONUS_LIST_BY_COMBO_SKU] [BLC]
      ON (
      [BLC].[BONUS_LIST_ID] = [BL].[BONUS_LIST_ID]
      AND [BLC].[COMBO_ID] = [TAR].[COMBO_ID]
