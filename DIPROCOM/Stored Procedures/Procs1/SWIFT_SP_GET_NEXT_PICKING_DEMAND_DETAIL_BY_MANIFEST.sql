...
-- Modificacion 11/13/2018 @ G-Force Team Sprint 
-- Autor: diego.as
-- Historia/Bug: Product Backlog Item 25566: Entregas con unidad de medida base Sonda SD
-- Descripcion: 11/13/2018 - Se agrega envio de unidades de medida para utilizacion en Sonda SD

/*
-- Ejemplo de Ejecucion:
				EXEC [DIPROCOM].[SWIFT_SP_GET_NEXT_PICKING_DEMAND_DETAIL_BY_MANIFEST] @MANIFEST_HEADER_ID = 2153
*/
-- =============================================
CREATE PROCEDURE [DIPROCOM].[SWIFT_SP_GET_NEXT_PICKING_DEMAND_DETAIL_BY_MANIFEST] (@MANIFEST_HEADER_ID INT)
AS
BEGIN
  --
  SET NOCOUNT ON;
  ...
  FROM [DIPROCOM].[SWIFT_SETUP_EXTERNAL_SOURCE] AS [S]
  ...
  SET
    @QUERY = '	SELECT * FROM OPENQUERY( [MOBILITYSERVER],''  
        SELECT
		 ...
		 ,[PDD].[IS_BONUS]
		 ,[PDD].[DISCOUNT]
		 ,[M].[BASE_MEASUREMENT_UNIT] AS [CODE_PACK_UNIT_STOCK]
		 ,[M].[BASE_MEASUREMENT_UNIT] AS [SALES_PACK_UNIT]
