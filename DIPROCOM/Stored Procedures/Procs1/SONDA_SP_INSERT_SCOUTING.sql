...
/*
  EXEC [PACASA].SONDA_SP_INSERT_SCOUTING
		  ...
		  ,@IS_POSTED_OFFLINE = 0
					...
					SELECT * 
          FROM [PACASA].SWIFT_CUSTOMERS_NEW
          WHERE CODE_CUSTOMER_BO = 'SO-374'

          SELECT *
          FROM [PACASA].SWIFT_CUSTOMER_FREQUENCY_NEW 
          --WHERE CODE_CUSTOMER = 'SO-125'
          ORDER BY CODE_FREQUENCY DESC
*/

-- =============================================
CREATE PROCEDURE [PACASA].[SONDA_SP_INSERT_SCOUTING] (
-- ----------------------------------------------------------------------------------
...
BEGIN
  ...
  BEGIN TRY
	...
	BEGIN 
		...
		FROM [PACASA].SWIFT_CUSTOMERS_NEW SCN
    ...
    FROM [PACASA].[SWIFT_VIEW_ALL_COSTUMER] [C]
		INNER JOIN [PACASA].[SWIFT_COMPANY] [SC] ON [C].[OWNER] = [SC].[COMPANY_NAME]
    ...
    -- ----------------------------------------------------------------------------------
	INSERT	INTO [PACASA].[SWIFT_CUSTOMERS_NEW]
			(
				[CODE_CUSTOMER]
				,[NAME_CUSTOMER]
    ...
    -- ----------------------------------------------------------------------------------
    INSERT [PACASA].SWIFT_CUSTOMER_FREQUENCY_NEW (CODE_CUSTOMER
    , SUNDAY
    , MONDAY
    , TUESDAY
    , WEDNESDAY
