...
-- Ejemplo de Ejecucion:
				EXEC [DIPROCOM].SWIFT_SP_GET_CHANNEL_NOT_IN_TRADE_AGREEMENT 
						@TRADE_AGREEMENT_ID = 1
*/
-- =============================================
CREATE PROCEDURE [DIPROCOM].SWIFT_SP_GET_CHANNEL_NOT_IN_TRADE_AGREEMENT(
...
BEGIN
	...
	--
	SET @TYPE_CHANNEL = [DIPROCOM].SWIFT_FN_GET_PARAMETER('TRADE_AGREEMENT','CHANNEL_FOR_TRADE_AGREEMENT')
	--
	SELECT
		C.CHANNEL_ID
		,C.CODE_CHANNEL
	...
	FROM [DIPROCOM].SWIFT_CHANNEL C
	LEFT JOIN [DIPROCOM].SWIFT_TRADE_AGREEMENT_BY_CHANNEL TAC ON (
