...
*/
-- =============================================
CREATE PROCEDURE [SONDA].[SWIFT_SP_GET_TRANSFER_DETAIL_BY_TRANSFER_ID](
	@TRANSFER_ID INT
)
AS
BEGIN
	...
	SELECT
		...
		,[S].[VAT_CODE]
    FROM [SONDA].[SWIFT_TRANSFER_DETAIL] [TD]
    INNER JOIN [SONDA].[SWIFT_VIEW_ALL_SKU] [S] ON([S].[CODE_SKU] = [TD].[SKU_CODE])
	INNER JOIN [SONDA].[SWIFT_TRANSFER_HEADER] [TH] ON (TH.TRANSFER_ID = TD.TRANSFER_ID)
