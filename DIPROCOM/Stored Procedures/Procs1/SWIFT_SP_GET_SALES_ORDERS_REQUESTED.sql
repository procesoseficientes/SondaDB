...
-- Ejemplo de Ejecucion:
				EXEC [DIPROCOM].[SWIFT_SP_GET_SALES_ORDERS_REQUESTED]
					@POS_TERMINAL = '137'

					SELECT * FROM [DIPROCOM].[SONDA_SALES_ORDER_DETAIL_REQUESTED]

*/
-- =============================================
CREATE PROCEDURE [DIPROCOM].[SWIFT_SP_GET_SALES_ORDERS_REQUESTED] (@POS_TERMINAL VARCHAR(25))
AS
BEGIN
  SET NOCOUNT ON;
  --
  ...
  FROM [DIPROCOM].[SONDA_SALES_ORDER_HEADER_REQUESTED] AS HR
  INNER JOIN [DIPROCOM].[SONDA_SALES_ORDER_DETAIL_REQUESTED] AS DR
    ON ([DR].[ORDER_ID] = [HR].[ORDER_ID])
  LEFT JOIN [DIPROCOM].[SONDA_SALES_ORDER_HEADER] AS OH
    ON ([OH].[SALES_ORDER_ID] = [HR].[SALES_ORDER_ID])
  INNER JOIN [DIPROCOM].[SWIFT_VIEW_ALL_SKU] AS VS
