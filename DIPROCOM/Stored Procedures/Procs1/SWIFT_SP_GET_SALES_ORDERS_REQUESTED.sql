...
-- Ejemplo de Ejecucion:
				EXEC [PACASA].[SWIFT_SP_GET_SALES_ORDERS_REQUESTED]
					@POS_TERMINAL = '137'

					SELECT * FROM [PACASA].[SONDA_SALES_ORDER_DETAIL_REQUESTED]

*/
-- =============================================
CREATE PROCEDURE [PACASA].[SWIFT_SP_GET_SALES_ORDERS_REQUESTED] (@POS_TERMINAL VARCHAR(25))
AS
BEGIN
  SET NOCOUNT ON;
  --
  ...
  FROM [PACASA].[SONDA_SALES_ORDER_HEADER_REQUESTED] AS HR
  INNER JOIN [PACASA].[SONDA_SALES_ORDER_DETAIL_REQUESTED] AS DR
    ON ([DR].[ORDER_ID] = [HR].[ORDER_ID])
  LEFT JOIN [PACASA].[SONDA_SALES_ORDER_HEADER] AS OH
    ON ([OH].[SALES_ORDER_ID] = [HR].[SALES_ORDER_ID])
  INNER JOIN [PACASA].[SWIFT_VIEW_ALL_SKU] AS VS
