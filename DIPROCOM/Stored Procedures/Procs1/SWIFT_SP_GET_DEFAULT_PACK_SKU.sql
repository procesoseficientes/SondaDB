...
-- EJEMPLO DE EJECUCION: 
		...
		EXEC [acsa].[SWIFT_SP_GET_DEFAULT_PACK_SKU]
		@CODE_ROUTE = 'RUDI@DIPROCOM'
			

*/
-- =====================================================
CREATE PROCEDURE [acsa].[SWIFT_SP_GET_DEFAULT_PACK_SKU] (@CODE_ROUTE VARCHAR(50))
AS
BEGIN
  ...
  --
  INSERT INTO #SKUS
  EXEC [acsa].[SONDA_SP_GET_SKU_PRESALE] @WAREHOUSES = @PRESALE_WAREHOUSE
  ...
  SELECT DISTINCT
   ...
   ,PU.CODE_SKU
  FROM #SKUS AS PS
  INNER JOIN [acsa].[SWIFT_SKU_SALE_PACK_UNIT] AS PU
    ON (
    PU.CODE_SKU = PS.SKU
    )
