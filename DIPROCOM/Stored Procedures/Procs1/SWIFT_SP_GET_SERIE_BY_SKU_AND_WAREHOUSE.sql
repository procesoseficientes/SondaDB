...
-- Ejemplo de Ejecucion:
				EXEC [PACASA].[SWIFT_SP_GET_SERIE_BY_SKU_AND_WAREHOUSE]
					@CODE_WAREHOUSE = 'C001'
					,@CODE_SKU = '100003'
*/
-- =============================================
CREATE PROCEDURE [PACASA].[SWIFT_SP_GET_SERIE_BY_SKU_AND_WAREHOUSE](
...
BEGIN
	...
	LEFT JOIN [PACASA].[SWIFT_TRANSFER_DETAIL] [TD] ON (
		[TD].[SKU_CODE] = [I].[SKU]
		AND ISNULL([I].[SERIAL_NUMBER],'NA') = ISNULL([TD].[SERIE],'NA')
		--AND [TD].[STATUS] = 'CANCELADO'
	)
	LEFT JOIN [PACASA].[SWIFT_TRANSFER_HEADER] [TH] ON (
