...
*/
-- =============================================
CREATE PROCEDURE [DIPROCOM].[SWIFT_SP_GET_CUSTOMER_BY_POLYGON_ID_TUNNING] ( @POLYGON_ID INT )
AS
    BEGIN
        BEGIN TRY

            DECLARE @GEOMETRY_POLYGON GEOMETRY;            
            SELECT  @GEOMETRY_POLYGON = DIPROCOM.SWIFT_GET_GEOMETRY_POLYGON_BY_POLIGON_ID(@POLYGON_ID);
    ...
    -- ------------------------------------------------------------------------------------
            ...
            INTO    #RESULT
            FROM    #CUSTOMER C
                    LEFT JOIN [DIPROCOM].[SWIFT_POLYGON_X_CUSTOMER] PC ON ( C.CODE_CUSTOMER = PC.CODE_CUSTOMER
                                                              AND PC.POLYGON_ID = @POLYGON_ID
                                                              )
                    LEFT JOIN DIPROCOM.SWIFT_CUSTOMER_FREQUENCY CF ON ( CF.CODE_CUSTOMER = C.CODE_CUSTOMER )
                    LEFT JOIN [DIPROCOM].[SWIFT_FREQUENCY_X_CUSTOMER] FC ON ( C.CODE_CUSTOMER = FC.CODE_CUSTOMER );
