...
-- Ejemplo de Ejecucion:
				EXEC [PACASA].[SWIFT_SP_GET_INVOICE_BY_SKU]
...
CREATE PROCEDURE [PACASA].[SWIFT_SP_GET_INVOICE_BY_SKU](
...
BEGIN
	...
	SELECT @SERIAL_NR = ISNULL([PACASA].[SWIFT_FN_GET_PARAMETER]('ERP_HARDCODE_VALUES','SERIAL_NR'),'')
			,@PAY_DEAL = ISNULL([PACASA].[SWIFT_FN_GET_PARAMETER]('ERP_HARDCODE_VALUES','PAY_DEAL'),'');
	-- ------------------------------------------------------------------------------------
    ...
    ,SUBSTRING([IH].[GPS_URL], CHARINDEX('','', [IH].[GPS_URL]) + 1, LEN([IH].[GPS_URL])) AS LONGITUDE
		...
		,CASE ISNULL([IH].[VOIDED_INVOICE],0)
				WHEN 0 THEN CONVERT(DECIMAL(18,' + CAST(@DEFAULT_DISPLAY_DECIMALS AS VARCHAR) + '),[PACASA].SWIFT_FN_GET_DISPLAY_NUMBER([ID].[PRICE]))
				ELSE 0
				END AS [PRICE]
		,CASE ISNULL([IH].[VOIDED_INVOICE],0)
				WHEN 0 THEN CONVERT(DECIMAL(18,' + CAST(@DEFAULT_DISPLAY_DECIMALS AS VARCHAR) + '),[PACASA].SWIFT_FN_GET_DISPLAY_NUMBER([ID].[TOTAL_LINE]))
				ELSE 0
				END AS [TOTAL_LINE]
		,CASE ISNULL([IH].[VOIDED_INVOICE],0)
				WHEN 0 THEN CONVERT(DECIMAL(18,' + CAST(@DEFAULT_DISPLAY_DECIMALS AS VARCHAR) + '),[PACASA].SWIFT_FN_GET_DISPLAY_NUMBER([IH].[CREDIT_AMOUNT]))
				ELSE 0
				END AS [CREDIT_AMOUNT]
		,CASE ISNULL([IH].[VOIDED_INVOICE],0)
				WHEN 0 THEN CONVERT(DECIMAL(18,' + CAST(@DEFAULT_DISPLAY_DECIMALS AS VARCHAR) + '),[PACASA].SWIFT_FN_GET_DISPLAY_NUMBER([IH].[CASH_AMOUNT]))
				ELSE 0
				END AS [CASH_AMOUNT]
		,CASE ISNULL([IH].[VOIDED_INVOICE],0)
				WHEN 0 THEN CONVERT(DECIMAL(18,' + CAST(@DEFAULT_DISPLAY_DECIMALS AS VARCHAR) + '),[PACASA].SWIFT_FN_GET_DISPLAY_NUMBER([IH].[TOTAL_AMOUNT]))
				ELSE 0
				END AS [TOTAL_AMOUNT]
  ...
  ,CASE [IH].[IS_EXPORTED_TO_XML]
	...
	,CASE 
     ...
     ELSE [IH].[COMMENT]
    END AS [COMMENT]
	FROM [PACASA].[SONDA_POS_INVOICE_DETAIL] [ID]
	INNER JOIN [PACASA].[SONDA_POS_INVOICE_HEADER] [IH] ON (
		IH.ID = ID.ID
	)
	INNER JOIN [PACASA].[SWIFT_VIEW_ALL_COSTUMER] AS VAC ON(
		VAC.[CODE_CUSTOMER] = IH.[CLIENT_ID]
	)
	INNER JOIN [PACASA].[SWIFT_ROUTE_BY_USER] [RBU] ON (
