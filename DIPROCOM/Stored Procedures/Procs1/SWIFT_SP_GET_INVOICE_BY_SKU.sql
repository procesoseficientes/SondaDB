...
-- Ejemplo de Ejecucion:
				EXEC [DIPROCOM].[SWIFT_SP_GET_INVOICE_BY_SKU]
...
CREATE PROCEDURE [DIPROCOM].[SWIFT_SP_GET_INVOICE_BY_SKU](
...
BEGIN
	...
	SELECT @SERIAL_NR = ISNULL([DIPROCOM].[SWIFT_FN_GET_PARAMETER]('ERP_HARDCODE_VALUES','SERIAL_NR'),'')
			,@PAY_DEAL = ISNULL([DIPROCOM].[SWIFT_FN_GET_PARAMETER]('ERP_HARDCODE_VALUES','PAY_DEAL'),'');
	-- ------------------------------------------------------------------------------------
    ...
    ,SUBSTRING([IH].[GPS_URL], CHARINDEX('','', [IH].[GPS_URL]) + 1, LEN([IH].[GPS_URL])) AS LONGITUDE
		...
		,CASE ISNULL([IH].[VOIDED_INVOICE],0)
				WHEN 0 THEN CONVERT(DECIMAL(18,' + CAST(@DEFAULT_DISPLAY_DECIMALS AS VARCHAR) + '),[DIPROCOM].SWIFT_FN_GET_DISPLAY_NUMBER([ID].[PRICE]))
				ELSE 0
				END AS [PRICE]
		,CASE ISNULL([IH].[VOIDED_INVOICE],0)
				WHEN 0 THEN CONVERT(DECIMAL(18,' + CAST(@DEFAULT_DISPLAY_DECIMALS AS VARCHAR) + '),[DIPROCOM].SWIFT_FN_GET_DISPLAY_NUMBER([ID].[TOTAL_LINE]))
				ELSE 0
				END AS [TOTAL_LINE]
		,CASE ISNULL([IH].[VOIDED_INVOICE],0)
				WHEN 0 THEN CONVERT(DECIMAL(18,' + CAST(@DEFAULT_DISPLAY_DECIMALS AS VARCHAR) + '),[DIPROCOM].SWIFT_FN_GET_DISPLAY_NUMBER([IH].[CREDIT_AMOUNT]))
				ELSE 0
				END AS [CREDIT_AMOUNT]
		,CASE ISNULL([IH].[VOIDED_INVOICE],0)
				WHEN 0 THEN CONVERT(DECIMAL(18,' + CAST(@DEFAULT_DISPLAY_DECIMALS AS VARCHAR) + '),[DIPROCOM].SWIFT_FN_GET_DISPLAY_NUMBER([IH].[CASH_AMOUNT]))
				ELSE 0
				END AS [CASH_AMOUNT]
		,CASE ISNULL([IH].[VOIDED_INVOICE],0)
				WHEN 0 THEN CONVERT(DECIMAL(18,' + CAST(@DEFAULT_DISPLAY_DECIMALS AS VARCHAR) + '),[DIPROCOM].SWIFT_FN_GET_DISPLAY_NUMBER([IH].[TOTAL_AMOUNT]))
				ELSE 0
				END AS [TOTAL_AMOUNT]
  ...
  ,CASE [IH].[IS_EXPORTED_TO_XML]
	...
	,CASE 
     ...
     ELSE [IH].[COMMENT]
    END AS [COMMENT]
	FROM [DIPROCOM].[SONDA_POS_INVOICE_DETAIL] [ID]
	INNER JOIN [DIPROCOM].[SONDA_POS_INVOICE_HEADER] [IH] ON (
		IH.ID = ID.ID
	)
	INNER JOIN [DIPROCOM].[SWIFT_VIEW_ALL_COSTUMER] AS VAC ON(
		VAC.[CODE_CUSTOMER] = IH.[CLIENT_ID]
	)
	INNER JOIN [DIPROCOM].[SWIFT_ROUTE_BY_USER] [RBU] ON (
		[RBU].[CODE_ROUTE] = [IH].[POS_TERMINAL]
	)
	INNER JOIN [DIPROCOM].[SWIFT_VIEW_ALL_SKU] [S] ON (
		[S].[CODE_SKU] = [ID].[SKU]
	)
	...
	WHERE IH.IS_READY_TO_SEND = 1 
		...
		AND [RBU].[LOGIN] = ''' + @LOGIN + '''
		AND [IH].[POSTED_DATETIME] BETWEEN ''' + CONVERT(VARCHAR,@START_DATETIME,121) + ''' AND ''' + CONVERT(VARCHAR,DATEADD(DAY,1,@END_DATETIME),121) + '''
