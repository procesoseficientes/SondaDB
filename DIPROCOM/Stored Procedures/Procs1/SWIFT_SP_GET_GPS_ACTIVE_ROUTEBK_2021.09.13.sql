...
*/
-- =============================================
CREATE PROCEDURE [DIPROCOM].[SWIFT_SP_GET_GPS_ACTIVE_ROUTEBK_2021.09.13] (@ASSIGNED_TO VARCHAR(50)
, @START_DATE DATE
, @END_DATE DATE)
AS
BEGIN
  ...
  INSERT INTO @TASK_TEMP ([TASK_ID], [TASK_TYPE], [TASK_DATE], [SCHEDULE_FOR], [CREATED_STAMP], [ASSIGEND_TO], [ASSIGNED_BY], [ASSIGNED_STAMP], [CANCELED_STAMP], [CANCELED_BY], [ACCEPTED_STAMP], [COMPLETED_STAMP], [RELATED_PROVIDER_CODE], [RELATED_PROVIDER_NAME], [EXPECTED_GPS], [POSTED_GPS], [TASK_STATUS], [TASK_COMMENTS], [TASK_SEQ], [REFERENCE], [SAP_REFERENCE], [COSTUMER_CODE], [COSTUMER_NAME], [RECEPTION_NUMBER], [PICKING_NUMBER], [COUNT_ID], [ACTION], [SCANNING_STATUS], [ALLOW_STORAGE_ON_DIFF], [CUSTOMER_PHONE], [TASK_ADDRESS], [VISIT_HOUR], [ROUTE_IS_COMPLETED], [EMAIL_TO_CONFIRM], [DISTANCE_IN_KMS], [DOC_RESOLUTION], [DOC_SERIE], [DOC_NUM], [COMPLETED_SUCCESSFULLY], [REASON], [TASK_ID_HH], [IN_PLAN_ROUTE], [CREATE_BY], [DEVICE_NETWORK_TYPE], [IS_POSTED_OFFLINE])
    SELECT DISTINCT 
     ...
     ,[T].IS_POSTED_OFFLINE
    FROM [SWIFT_EXPRESS].[DIPROCOM].SWIFT_TASKS [T]
  ...
  INSERT INTO @SALES_ORDER_HEADER_TOTALS ([SALES_ORDER_ID], [TOTAL_AMOUNT], [QTY])
    SELECT
     ...
     ,CASE
      ...
      END AS [TOTAL_AMOUNT]
     ,SUM([SOD].[QTY]) AS [QTY]
    FROM [SWIFT_EXPRESS].[DIPROCOM].[SONDA_SALES_ORDER_HEADER] [SOH] WITH (NOLOCK)
    INNER JOIN [SWIFT_EXPRESS].[DIPROCOM].[SONDA_SALES_ORDER_DETAIL] [SOD] WITH (NOLOCK)
  ...
  INSERT INTO @TEMP_DOCUMENT_HEADER ([ID], [DOC_SERIAL], [ROUTE], [CLIENT], [CLIENT_ID], [ASSIGNED_TO], [TOTAL_AMOUNT], [TOTAL_AMOUNT_WHIT_DISCOUNT], [UNIDADES_VENDIDAS], [SIGNATURE], [IMAGE], [EXPECTED_GPS], [Latitude], [Longitude], [CREATED_DATESTAMP], [CDF_SERIE], [CDF_RESOLUCION], [STATUS], [CLOSED_ROUTE_DATETIME], [IMAGE_1], [IMAGE_2], [DOC_ID], [DOC_TYPE], [DOC_TYPE_DESCRIPTION], [SALES_ORDER_TYPE], [TASK_ID], [IS_POSTED_ERP], [ATTEMPTED_WITH_ERROR], [POSTED_RESPONSE], [POSTED_ERP], [POSTED_DATETIME], [LAST_UPDATE_IS_SENDING])
    ...
    INNER JOIN [SWIFT_EXPRESS].[DIPROCOM].[SONDA_SALES_ORDER_HEADER] [SOH] WITH (NOLOCK)
  ...
  INSERT INTO @TEMP_DOCUMENT_HEADER ([ID], [DOC_SERIAL], [ROUTE], [CLIENT], [CLIENT_ID], [ASSIGNED_TO], [TOTAL_AMOUNT], [TOTAL_AMOUNT_WHIT_DISCOUNT], [UNIDADES_VENDIDAS], [SIGNATURE], [IMAGE], [EXPECTED_GPS], [Latitude], [Longitude], [CREATED_DATESTAMP], [CDF_SERIE], [CDF_RESOLUCION], [STATUS], [CLOSED_ROUTE_DATETIME], [IMAGE_1], [IMAGE_2], [DOC_ID], [DOC_TYPE], [DOC_TYPE_DESCRIPTION], [SALES_ORDER_TYPE], [TASK_ID], [IS_POSTED_ERP], [ATTEMPTED_WITH_ERROR], [POSTED_RESPONSE], [POSTED_ERP], [POSTED_DATETIME], [LAST_UPDATE_IS_SENDING], [TYPE_ACTION], [GPS_URL])
    SELECT
      [IH].[ID] AS [ID]
     ...
     ,(SELECT
          SUM([QTY])
        FROM [SWIFT_EXPRESS].[DIPROCOM].[SONDA_POS_INVOICE_DETAIL] [D] WITH (NOLOCK)
        WHERE [D].[ID] = [IH].[ID])
     ...
     ,'SALE' AS [TYPE_ACTION]
	 ,[IH].GPS_URL
    FROM [SWIFT_EXPRESS].[DIPROCOM].[SONDA_POS_INVOICE_HEADER] [IH] WITH (NOLOCK)
  ...
  SELECT DISTINCT
   ...
   ,ISNULL([DH].TASK_ID, [TT].TASK_ID ) AS TASK_ID
   ,ISNULL([DH].[POSTED_DATETIME], [TT].[ACCEPTED_STAMP]) [ACCEPTED_STAMP]
   ,CASE [TT].[POSTED_GPS]
      WHEN '0,0' THEN  SUBSTRING(ISNULL([DH].EXPECTED_GPS, TT.EXPECTED_GPS), 0, CHARINDEX(',', ISNULL([DH].EXPECTED_GPS, TT.EXPECTED_GPS))) 
      ELSE ISNULL([DH].[Latitude], SUBSTRING(ISNULL([DH].EXPECTED_GPS, TT.EXPECTED_GPS), 0, CHARINDEX(',', ISNULL([DH].EXPECTED_GPS, TT.EXPECTED_GPS)))) 
    END AS [LATITUDE]
   ...
   --,ISNULL([DH].[Longitude], SUBSTRING([TT].EXPECTED_GPS, CHARINDEX(',', [TT].EXPECTED_GPS) + 1, LEN([TT].EXPECTED_GPS))) [LONGITUDE]
    ,CASE [TT].[POSTED_GPS]
      WHEN '0,0' THEN  SUBSTRING(ISNULL([DH].EXPECTED_GPS, TT.EXPECTED_GPS), CHARINDEX(',', ISNULL([DH].EXPECTED_GPS, TT.EXPECTED_GPS)) + 1, LEN(ISNULL([DH].EXPECTED_GPS, TT.EXPECTED_GPS)))
      ELSE ISNULL([DH].[Longitude], SUBSTRING(ISNULL([DH].EXPECTED_GPS, TT.EXPECTED_GPS), CHARINDEX(',', ISNULL([DH].EXPECTED_GPS, TT.EXPECTED_GPS)) + 1, LEN(ISNULL([DH].EXPECTED_GPS, TT.EXPECTED_GPS))))
    END AS [LONGITUDE]
   ,ISNULL([DH].[ASSIGNED_TO], [TT].[ASSIGEND_TO]) AS ASSIGEND_TO
   ,[DH].[ASSIGNED_TO]
   ,ISNULL([TT].[TASK_COMMENTS], 'Sin Comentario') [TASK_COMMENTS]
   ,ISNULL([DH].[CLIENT_ID], [TT].COSTUMER_CODE) [CODE_CUSTOMER]
   ,[C].[NAME_CUSTOMER] [CUSTOMER_NAME]
   ,ISNULL([C].[ADRESS_CUSTOMER], '') [TASK_ADDRESS]
   ,ISNULL([DH].[POSTED_DATETIME], [TT].[COMPLETED_STAMP]) [COMPLETED_STAMP]
   ,[DH].[TOTAL_AMOUNT]
   ,ISNULL([DH].TOTAL_AMOUNT_WHIT_DISCOUNT, DH.TOTAL_AMOUNT) AS TOTAL_AMOUNT_WHIT_DISCOUNT
   ,[TT].COMPLETED_STAMP
   ,[TT].ACCEPTED_STAMP
   ,ISNULL([TT].REASON, 'ASIGNADO') AS REASON
   ,ISNULL([DH].EXPECTED_GPS, TT.EXPECTED_GPS) EXPECTED_GPS
   ,[DH].GPS_URL
   ,[TT].TASK_ID
   ,(SELECT TOP 1
        SYMBOL_CURRENCY
      FROM [SWIFT_EXPRESS].[DIPROCOM].SWIFT_CURRENCY
      WHERE IS_DEFAULT = 1)
    AS SYMBOL_CURRENCY
   ,CASE
      WHEN [TT].REASON IS NULL THEN 'NARANJA'
      ELSE (SELECT
            DIPROCOM.[SWIFT_FN_GET_KPI](DIPROCOM.SWIFT_CALCULATE_DISTANCE([TT].EXPECTED_GPS, [TT].POSTED_GPS, 'M'), @ASSIGNED_TO, 'SALES_DISTANCE', CASE
              WHEN [DH].[TOTAL_AMOUNT] IS NULL THEN 'WITHOUT_SALE'
              ELSE 'SALE'
            END))
    END AS KPI_COLOR
   ,(SELECT
        [SWIFT_EXPRESS].[DIPROCOM].[SWIFT_FN_GET_KPI]([SWIFT_EXPRESS].[DIPROCOM].SWIFT_CALCULATE_DISTANCE([TT].EXPECTED_GPS, [TT].POSTED_GPS, 'M'), @ASSIGNED_TO, 'SALES_DISTANCE', CASE
          WHEN [DH].[TOTAL_AMOUNT] <= 0 THEN 'WITHOUT_SALE'
          ELSE 'SALE'
        END))
    AS COLOR
   ,CASE
      WHEN [SWIFT_EXPRESS].[DIPROCOM].SWIFT_CALCULATE_DISTANCE([TT].EXPECTED_GPS, [TT].POSTED_GPS, 'K') <= 0 THEN CONVERT(VARCHAR(50), 0) + 'km.'
      ELSE CASE
          WHEN [SWIFT_EXPRESS].[DIPROCOM].SWIFT_CALCULATE_DISTANCE([TT].EXPECTED_GPS, [TT].POSTED_GPS, 'K') < 1 THEN CONVERT(VARCHAR(50), [SWIFT_EXPRESS].[DIPROCOM].SWIFT_CALCULATE_DISTANCE([TT].EXPECTED_GPS, [TT].POSTED_GPS, 'M')) + 'm.'
          ELSE CONVERT(VARCHAR(50), [SWIFT_EXPRESS].[DIPROCOM].SWIFT_CALCULATE_DISTANCE([TT].EXPECTED_GPS, [TT].POSTED_GPS, 'k')) + 'km.'
        END
    END AS DIFERENCE
   ...
   ,[C].[SPECIAL_MARKER]
  FROM @TEMP_DOCUMENT_HEADER [DH]
  RIGHT OUTER JOIN @TASK_TEMP [TT]
    ON ([TT].TASK_ID = DH.TASK_ID)
  INNER JOIN [SWIFT_EXPRESS].[DIPROCOM].[SWIFT_VIEW_ALL_COSTUMER] [C]
    ON (ISNULL(DH.CLIENT_ID, TT.COSTUMER_CODE) = [C].[CODE_CUSTOMER])
   LEFT JOIN [SWIFT_EXPRESS].[DIPROCOM].[SWIFT_FREQUENCY_X_CUSTOMER] [FC]
   ON  (ISNULL(DH.CLIENT_ID, TT.COSTUMER_CODE) = [FC].[CODE_CUSTOMER])
