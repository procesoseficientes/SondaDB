...
-- Ejemplo de Ejecucion:
				    EXEC [DIPROCOM].[SWIFT_SP_GET_POLYGON_STADISTICS]
				    	@POLYGON_TYPE = 'REGION'
...
*/
-- =============================================
CREATE PROCEDURE [DIPROCOM].[SWIFT_SP_GET_POLYGON_STADISTICS]
	(
...
AS
  ...
  --
    ...
    -- ------------------------------------------------------------------------------------
			SELECT
				@DELIMITER = [DIPROCOM].[SWIFT_FN_GET_PARAMETER]('DELIMITER' ,
																'DEFAULT_DELIMITER')
				,@DEFAULT_DISPLAY_DECIMALS = [DIPROCOM].[SWIFT_FN_GET_PARAMETER]('CALCULATION_RULES' ,
    ...
    -- ------------------------------------------------------------------------------------
			...
			BEGIN
				...
				SELECT
					[T].[VALUE]
				FROM
					[DIPROCOM].[SWIFT_FN_SPLIT_2](@TAG_COLORS ,@DELIMITER) [T];
			END;
    ...
    -- ------------------------------------------------------------------------------------
			...
			BEGIN
				...
				SELECT
					[C].[VALUE]
				FROM
					[DIPROCOM].[SWIFT_FN_SPLIT_2](@CHANNELS ,@DELIMITER) [C];
			END;
    ...
    -- ------------------------------------------------------------------------------------
      ...
      -- ------------------------------------------------------------------------------------			
				...
				SELECT
					...
					,[PBR].[POLYGON_ID]
					,[U].[RELATED_SELLER]
				FROM [DIPROCOM].[SONDA_SALES_ORDER_HEADER] [SO]
					INNER JOIN [#CUSTOMER] [C] ON [SO].[CLIENT_ID] = [C].[CODE_CUSTOMER]
					INNER JOIN [DIPROCOM].[USERS] [U] ON [SO].[POS_TERMINAL] = [U].[SELLER_ROUTE]
					INNER JOIN [DIPROCOM].[SWIFT_ROUTES] [R] ON [R].[SELLER_CODE] = [U].[RELATED_SELLER]
					INNER JOIN [DIPROCOM].[SWIFT_POLYGON_BY_ROUTE] [PBR] ON [PBR].[ROUTE] = [R].[ROUTE]
      ...
      -- ------------------------------------------------------------------------------------
				...
				SELECT
					...
					,[PBR].[POLYGON_ID]
					,[U].[RELATED_SELLER]
				FROM [DIPROCOM].[SONDA_POS_INVOICE_HEADER] [I]
					INNER JOIN [#CUSTOMER] [C] ON [C].[CODE_CUSTOMER] = [I].[CLIENT_ID]
					INNER JOIN [DIPROCOM].[USERS] [U] ON [I].[POS_TERMINAL] = [U].[SELLER_ROUTE]
					INNER JOIN [DIPROCOM].[SWIFT_ROUTES] [R] ON [R].[SELLER_CODE] = [U].[RELATED_SELLER]
					INNER JOIN [DIPROCOM].[SWIFT_POLYGON_BY_ROUTE] [PBR] ON [PBR].[ROUTE] = [R].[ROUTE]
    ...
    -- ------------------------------------------------------------------------------------
			...
			,CONVERT(DECIMAL(18,' + CAST(@DEFAULT_DISPLAY_DECIMALS AS VARCHAR)
				+ '),[DIPROCOM].SWIFT_FN_GET_DISPLAY_NUMBER([A].[CUSTOMERS_PERCENT])) [CUSTOMERS_PERCENT]
			,CONVERT(DECIMAL(18,' + CAST(@DEFAULT_DISPLAY_DECIMALS AS VARCHAR)
				+ '),[DIPROCOM].SWIFT_FN_GET_DISPLAY_NUMBER([PA].[TOTAL_AMOUNT])) [TOTAL_SALES]
			,CONVERT(DECIMAL(18,' + CAST(@DEFAULT_DISPLAY_DECIMALS AS VARCHAR)
				+ '),[DIPROCOM].SWIFT_FN_GET_DISPLAY_NUMBER([PA].[TOTAL_AMOUNT_PERCENT])) [TOTAL_SALES_PERCENT]
			...
			FROM [#POLYGON_AVG] [A]
				...
				LEFT JOIN [DIPROCOM].[SWIFT_POLYGON_BY_ROUTE] [PR] ON(
				...
				LEFT JOIN [DIPROCOM].[SWIFT_SELLER] [SS] ON (
