...
-- Ejemplo de Ejecucion:
				    EXEC [acsa].[SWIFT_SP_GET_POLYGON_STADISTICS]
				    	@POLYGON_TYPE = 'REGION'
...
*/
-- =============================================
CREATE PROCEDURE [acsa].[SWIFT_SP_GET_POLYGON_STADISTICS]
	(
...
AS
  ...
  --
    ...
    -- ------------------------------------------------------------------------------------
			SELECT
				@DELIMITER = [acsa].[SWIFT_FN_GET_PARAMETER]('DELIMITER' ,
																'DEFAULT_DELIMITER')
				,@DEFAULT_DISPLAY_DECIMALS = [acsa].[SWIFT_FN_GET_PARAMETER]('CALCULATION_RULES' ,
    ...
    -- ------------------------------------------------------------------------------------
			...
			BEGIN
				...
				SELECT
					[T].[VALUE]
				FROM
					[acsa].[SWIFT_FN_SPLIT_2](@TAG_COLORS ,@DELIMITER) [T];
			END;
    ...
    -- ------------------------------------------------------------------------------------
			...
			BEGIN
				...
				SELECT
					[C].[VALUE]
				FROM
					[acsa].[SWIFT_FN_SPLIT_2](@CHANNELS ,@DELIMITER) [C];
			END;
    ...
    -- ------------------------------------------------------------------------------------
			...
			INTO
				[#CUSTOMER]
			FROM
				[acsa].[SWIFT_VIEW_ALL_COSTUMER] [C]
			WHERE
				[GPS] <> '0,0'
				AND (
    ...
    --
			...
			BEGIN
				SELECT
					@GEOMETRY_POLYGON = [acsa].[SWIFT_GET_GEOMETRY_POLYGON_BY_POLIGON_ID](@POLYGON_ID_PARENT);
      --
				SELECT
					@TOTAL_CUSTOMERS = [acsa].[SWIFT_GET_CUSTOMERS_COUNT_BY_GEOMETRY_POLYGON](@GEOMETRY_POLYGON);
			END;
    ...
    -- ------------------------------------------------------------------------------------
			...
			,CONVERT(DECIMAL(18,' + CAST(@DEFAULT_DISPLAY_DECIMALS AS VARCHAR)
				+ '),[acsa].SWIFT_FN_GET_DISPLAY_NUMBER([A].[CUSTOMERS_PERCENT])) [CUSTOMERS_PERCENT]
			,CONVERT(DECIMAL(18,' + CAST(@DEFAULT_DISPLAY_DECIMALS AS VARCHAR)
				+ '),[acsa].SWIFT_FN_GET_DISPLAY_NUMBER([PA].[TOTAL_AMOUNT])) [TOTAL_SALES]
			,CONVERT(DECIMAL(18,' + CAST(@DEFAULT_DISPLAY_DECIMALS AS VARCHAR)
				+ '),[acsa].SWIFT_FN_GET_DISPLAY_NUMBER([PA].[TOTAL_AMOUNT_PERCENT])) [TOTAL_SALES_PERCENT]
			...
			FROM [#POLYGON_AVG] [A]
				...
				LEFT JOIN [acsa].[SWIFT_POLYGON_BY_ROUTE] [PR] ON(
				...
				LEFT JOIN [acsa].[SWIFT_SELLER] [SS] ON (
