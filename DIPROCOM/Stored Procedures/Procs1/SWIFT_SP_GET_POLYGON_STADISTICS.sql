...
-- Ejemplo de Ejecucion:
				    EXEC [PACASA].[SWIFT_SP_GET_POLYGON_STADISTICS]
				    	@POLYGON_TYPE = 'REGION'
...
*/
-- =============================================
CREATE PROCEDURE [PACASA].[SWIFT_SP_GET_POLYGON_STADISTICS]
	(
...
AS
  ...
  --
    ...
    -- ------------------------------------------------------------------------------------
			SELECT
				@DELIMITER = [PACASA].[SWIFT_FN_GET_PARAMETER]('DELIMITER' ,
																'DEFAULT_DELIMITER')
				,@DEFAULT_DISPLAY_DECIMALS = [PACASA].[SWIFT_FN_GET_PARAMETER]('CALCULATION_RULES' ,
    ...
    -- ------------------------------------------------------------------------------------
			...
			BEGIN
				...
				SELECT
					[T].[VALUE]
				FROM
					[PACASA].[SWIFT_FN_SPLIT_2](@TAG_COLORS ,@DELIMITER) [T];
			END;
    ...
    -- ------------------------------------------------------------------------------------
			...
			BEGIN
				...
				SELECT
					[C].[VALUE]
				FROM
					[PACASA].[SWIFT_FN_SPLIT_2](@CHANNELS ,@DELIMITER) [C];
			END;
    ...
    -- ------------------------------------------------------------------------------------
			...
			INTO
				[#CUSTOMER]
			FROM
				[PACASA].[SWIFT_VIEW_ALL_COSTUMER] [C]
			WHERE
				[GPS] <> '0,0'
				AND (
    ...
    -- ------------------------------------------------------------------------------------
			...
			BEGIN
				...
				FROM
					[#CUSTOMER] [C]
				LEFT JOIN [PACASA].[SWIFT_TAG_X_CUSTOMER] [TC]
    ...
    -- ------------------------------------------------------------------------------------
			...
			BEGIN
				...
				FROM
					[#CUSTOMER] [C]
				LEFT JOIN [PACASA].[SWIFT_CHANNEL_X_CUSTOMER] [CC]
    ...
    --
			...
			BEGIN
				SELECT
					@GEOMETRY_POLYGON = [PACASA].[SWIFT_GET_GEOMETRY_POLYGON_BY_POLIGON_ID](@POLYGON_ID_PARENT);
      --
				SELECT
					@TOTAL_CUSTOMERS = [PACASA].[SWIFT_GET_CUSTOMERS_COUNT_BY_GEOMETRY_POLYGON](@GEOMETRY_POLYGON);
			END;
    ...
    -- ------------------------------------------------------------------------------------
      ...
      -- ------------------------------------------------------------------------------------			
				...
				SELECT
					...
					,[PBR].[POLYGON_ID]
					,[U].[RELATED_SELLER]
				FROM [PACASA].[SONDA_SALES_ORDER_HEADER] [SO]
					INNER JOIN [#CUSTOMER] [C] ON [SO].[CLIENT_ID] = [C].[CODE_CUSTOMER]
					INNER JOIN [PACASA].[USERS] [U] ON [SO].[POS_TERMINAL] = [U].[SELLER_ROUTE]
					INNER JOIN [PACASA].[SWIFT_ROUTES] [R] ON [R].[SELLER_CODE] = [U].[RELATED_SELLER]
					INNER JOIN [PACASA].[SWIFT_POLYGON_BY_ROUTE] [PBR] ON [PBR].[ROUTE] = [R].[ROUTE]
      ...
      -- ------------------------------------------------------------------------------------
				...
				SELECT
					...
					,[PBR].[POLYGON_ID]
					,[U].[RELATED_SELLER]
				FROM [PACASA].[SONDA_POS_INVOICE_HEADER] [I]
					INNER JOIN [#CUSTOMER] [C] ON [C].[CODE_CUSTOMER] = [I].[CLIENT_ID]
					INNER JOIN [PACASA].[USERS] [U] ON [I].[POS_TERMINAL] = [U].[SELLER_ROUTE]
					INNER JOIN [PACASA].[SWIFT_ROUTES] [R] ON [R].[SELLER_CODE] = [U].[RELATED_SELLER]
					INNER JOIN [PACASA].[SWIFT_POLYGON_BY_ROUTE] [PBR] ON [PBR].[ROUTE] = [R].[ROUTE]
    ...
    -- ------------------------------------------------------------------------------------
			...
			,CONVERT(DECIMAL(18,' + CAST(@DEFAULT_DISPLAY_DECIMALS AS VARCHAR)
				+ '),[PACASA].SWIFT_FN_GET_DISPLAY_NUMBER([A].[CUSTOMERS_PERCENT])) [CUSTOMERS_PERCENT]
			,CONVERT(DECIMAL(18,' + CAST(@DEFAULT_DISPLAY_DECIMALS AS VARCHAR)
				+ '),[PACASA].SWIFT_FN_GET_DISPLAY_NUMBER([PA].[TOTAL_AMOUNT])) [TOTAL_SALES]
			,CONVERT(DECIMAL(18,' + CAST(@DEFAULT_DISPLAY_DECIMALS AS VARCHAR)
				+ '),[PACASA].SWIFT_FN_GET_DISPLAY_NUMBER([PA].[TOTAL_AMOUNT_PERCENT])) [TOTAL_SALES_PERCENT]
			...
			FROM [#POLYGON_AVG] [A]
				...
				INNER JOIN [PACASA].[SWIFT_POLYGON] [P] ON (
					[A].[POLYGON_ID] = [P].[POLYGON_ID]
				)
				LEFT JOIN [PACASA].[SWIFT_POLYGON_BY_ROUTE] [PR] ON(
					[PR].[POLYGON_ID] = [P].[POLYGON_ID]
				)
				LEFT JOIN [PACASA].[SWIFT_ROUTES] [R] ON	(
					[R].[ROUTE] = [PR].[ROUTE]
				)
				LEFT JOIN [PACASA].[USERS] [U] ON (
					[R].[SELLER_CODE] = [U].[RELATED_SELLER]
				)
				LEFT JOIN [PACASA].[SWIFT_SELLER] [SS] ON (
