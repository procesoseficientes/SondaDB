...
--				   [DIPROCOM].[SWIFT_PALLET] 
--				   tomando como parametro el campo @CODE_LOCATION
--					al mismo tiempo hace un INNER JOIN con la tabla
--					[DIPROCOM].[SWIFT_BATCH] mediante el campo BATCH_ID
/*
Ejemplo de Ejecucion:

	EXEC [DIPROCOM].[SWIFT_SP_GET_PALLETS_BY_LOCATION] 
	@CODE_LOCATION = 'A3'
	 				
*/
-- =============================================

CREATE PROCEDURE [DIPROCOM].[SWIFT_SP_GET_PALLETS_BY_LOCATION]
(
...
BEGIN
	...
	FROM [DIPROCOM].[SWIFT_PALLET] AS SP  
	INNER JOIN [DIPROCOM].[SWIFT_BATCH] AS SB ON (
		SB.BATCH_ID = SP.BATCH_ID
	)
	INNER JOIN [DIPROCOM].[SWIFT_VIEW_SKU] AS SVS ON (
		SVS.CODE_SKU = SB.SKU
	)
