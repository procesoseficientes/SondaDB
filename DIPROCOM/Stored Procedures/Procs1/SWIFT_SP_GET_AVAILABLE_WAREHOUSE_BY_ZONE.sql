...
-- Ejemplo de Ejecucion:
				EXEC  [acsa].[SWIFT_SP_GET_AVAILABLE_WAREHOUSE_BY_ZONE] @ID_ZONE = 2
SELECT * FROM [acsa].[SWIFT_WAREHOUSE_X_ZONE] 

*/
-- =============================================
CREATE PROCEDURE [acsa].[SWIFT_SP_GET_AVAILABLE_WAREHOUSE_BY_ZONE] (@ID_ZONE INT )
AS
BEGIN
  SET NOCOUNT ON;
  --
 ...
 SELECT 
  ...
  FROM [acsa].[SWIFT_WAREHOUSES] [W]
    LEFT JOIN [acsa].[SWIFT_WAREHOUSE_X_ZONE] [SWXZ] ON [W].[CODE_WAREHOUSE] = [SWXZ].[CODE_WAREHOUSE] AND @ID_ZONE = [SWXZ].[ID_ZONE]
