...
-- Ejemplo de Ejecucion:
				EXEC  [PACASA].[SWIFT_SP_GET_AVAILABLE_WAREHOUSE_BY_ZONE] @ID_ZONE = 2
SELECT * FROM [PACASA].[SWIFT_WAREHOUSE_X_ZONE] 

*/
-- =============================================
CREATE PROCEDURE [PACASA].[SWIFT_SP_GET_AVAILABLE_WAREHOUSE_BY_ZONE] (@ID_ZONE INT )
AS
BEGIN
  SET NOCOUNT ON;
  --
 ...
 SELECT 
  ...
  FROM [PACASA].[SWIFT_WAREHOUSES] [W]
    LEFT JOIN [PACASA].[SWIFT_WAREHOUSE_X_ZONE] [SWXZ] ON [W].[CODE_WAREHOUSE] = [SWXZ].[CODE_WAREHOUSE] AND @ID_ZONE = [SWXZ].[ID_ZONE]
