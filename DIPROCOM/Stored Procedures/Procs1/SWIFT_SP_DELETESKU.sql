...
CREATE PROCEDURE [DIPROCOM].SWIFT_SP_DELETESKU
@SKU INT,
...
IF ((SELECT COUNT(A.CODE_SKU) FROM SWIFT_PICKING_DETAIL AS A WHERE A.CODE_SKU = (SELECT CODE_SKU FROM SWIFT_SKU WHERE SKU = @SKU ) )
	...
	--+ (SELECT COUNT(D.SKU) FROM [DIPROCOM].SWIFT_HIST_INVENTORY AS D WHERE D.SKU = @SKU)
...
-----------------------------------------------------------
  SELECT @CODE_PACK_UNIT= ssspu.CODE_PACK_UNIT 
    FROM [DIPROCOM].SWIFT_SKU_SALE_PACK_UNIT ssspu
    INNER JOIN [DIPROCOM].SWIFT_SKU ss ON ssspu.CODE_SKU = ss.CODE_SKU
    WHERE ss.SKU= @SKU

	EXEC	[DIPROCOM].SWIFT_SP_DELETE_SKU_SALE_PACK_UNIT 
