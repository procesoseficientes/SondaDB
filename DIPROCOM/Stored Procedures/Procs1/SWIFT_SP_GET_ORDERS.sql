...
-- Ejemplo de Ejecucion:
		...
		EXEC	@return_value = [DIPROCOM].[SWIFT_SP_GET_ORDERS]
		@DTBEGIN = '20171201',
		@DTEND = '20180925'
...
*/
-- =============================================
CREATE PROCEDURE [DIPROCOM].[SWIFT_SP_GET_ORDERS]
    (
      @DTBEGIN DATE ,
      @DTEND DATE
    )
AS
    BEGIN
        ...
        SELECT  @SHIPPING_ATTEMPTS_SALES_ORDER = [DIPROCOM].[SWIFT_FN_GET_PARAMETER]('SALES_ORDER',
                                                              'SHIPPING_ATTEMPTS') ,
                @SHIPPING_ATTEMPTS_INVOICE = [DIPROCOM].[SWIFT_FN_GET_PARAMETER]('INVOICE',
                                                              'SHIPPING_ATTEMPTS');
        ...
        INSERT  INTO [#DOCUMENTS]
                ...
                FROM    [DIPROCOM].[SONDA_SALES_ORDER_HEADER] [S] WITH ( NOLOCK )
        ...
        INSERT  INTO [#DOCUMENTS]
                ...
                FROM    [DIPROCOM].[SONDA_POS_INVOICE_HEADER] [H] WITH ( NOLOCK )
                        INNER JOIN [DIPROCOM].[USERS] AS [US] ON ( [US].[SELLER_ROUTE] = [H].[POS_TERMINAL] )
                WHERE   [H].[ID] > 0
        ...
        SELECT  [H].[ID] ,
                ...
                CASE [H].[SALES_ORDER_TYPE]
                  WHEN 'CASH'
                  THEN [DIPROCOM].[SWIFT_FN_GET_DISPLAY_NUMBER]([H].[TOTAL_AMOUNT])
                  ELSE 0
                END AS 'CASH_AMOUNT' ,
                CASE [H].[SALES_ORDER_TYPE]
                  WHEN 'CREDIT'
