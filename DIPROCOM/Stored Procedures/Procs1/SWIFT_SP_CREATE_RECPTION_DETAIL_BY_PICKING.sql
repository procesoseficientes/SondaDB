...
-- Ejemplo de Ejecucion:
		EXEC [PACASA].[SWIFT_SP_CREATE_RECPTION_DETAIL_BY_PICKING]
			@PICKING_HEADER = 1020
		...
		---------------------------------------------------------
		SELECT *
		FROM [PACASA].[SWIFT_RECEPTION_DETAIL]
		WHERE RECEPTION_DETAIL = 3031
		---------------------------------------------------------
		SELECT *
		FROM [PACASA].[SWIFT_PICKING_DETAIL]
		WHERE [PICKING_HEADER] = 1020 AND [CODE_SKU] = '100004'

*/
-- =============================================
CREATE PROCEDURE [PACASA].[SWIFT_SP_CREATE_RECPTION_DETAIL_BY_PICKING]
(
	@PICKING_HEADER INT
...
DECLARE @ID_PICKING_DETAIL INT
	...
	FROM [PACASA].[SWIFT_PICKING_DETAIL] AS PD
	...
	IF (@ID_PICKING_DETAIL IS NOT NULL) BEGIN
			...
			-- ------------------------------------------------------------------------------------
			BEGIN TRY
			INSERT INTO [PACASA].[SWIFT_RECEPTION_DETAIL] (
				RECEPTION_HEADER
				,CODE_SKU
				,DESCRIPTION_SKU
				,EXPECTED
		...
		IF (@DELETE_ROW = 1) BEGIN
			...
			-- ------------------------------------------------------------------------------------
			BEGIN TRY
			DELETE FROM [PACASA].[SWIFT_PICKING_DETAIL] 
