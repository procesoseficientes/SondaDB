...
-- Ejemplo de Ejecucion:
		EXEC [DIPROCOM].[SWIFT_SP_CREATE_RECPTION_DETAIL_BY_PICKING]
			@PICKING_HEADER = 1020
		...
		---------------------------------------------------------
		SELECT *
		FROM [DIPROCOM].[SWIFT_RECEPTION_DETAIL]
		WHERE RECEPTION_DETAIL = 3031
		---------------------------------------------------------
		SELECT *
		FROM [DIPROCOM].[SWIFT_PICKING_DETAIL]
		WHERE [PICKING_HEADER] = 1020 AND [CODE_SKU] = '100004'

*/
-- =============================================
CREATE PROCEDURE [DIPROCOM].[SWIFT_SP_CREATE_RECPTION_DETAIL_BY_PICKING]
(
	@PICKING_HEADER INT
...
DECLARE @ID_PICKING_DETAIL INT
	...
	IF (@ID_PICKING_DETAIL IS NOT NULL) BEGIN
			...
			-- ------------------------------------------------------------------------------------
			BEGIN TRY
			INSERT INTO [DIPROCOM].[SWIFT_RECEPTION_DETAIL] (
				RECEPTION_HEADER
				,CODE_SKU
				,DESCRIPTION_SKU
				,EXPECTED
		...
		IF (@DELETE_ROW = 1) BEGIN
			...
			-- ------------------------------------------------------------------------------------
			BEGIN TRY
			DELETE FROM [DIPROCOM].[SWIFT_PICKING_DETAIL] 
