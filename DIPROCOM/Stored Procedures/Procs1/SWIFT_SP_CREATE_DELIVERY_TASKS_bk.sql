...
-- EJEMPLO DE EJECUCION: 
		...
		--
		EXEC [DIPROCOM].[SWIFT_SP_CREATE_DELIVERY_TASKS]
			@MANIFEST_ID = 69,
...
create PROC [DIPROCOM].[SWIFT_SP_CREATE_DELIVERY_TASKS_bk]
...
AS
	...
	BEGIN TRY
		...
		SELECT @LOGIN_DEFAULT_WAREHOUSE = 	ISNULL((SELECT DEFAULT_WAREHOUSE FROM [DIPROCOM].[USERS]
		...
		SELECT @STARTING_POINT = ISNULL((SELECT GPS_WAREHOUSE FROM DIPROCOM.SWIFT_WAREHOUSES
		...
		DECLARE ORDERS_CURSOR CURSOR LOCAL FORWARD_ONLY 
			FOR 
				SELECT 
					...
					A.COSTUMER_CODE, 
					B.NAME_CUSTOMER, 
					ISNULL(B.PHONE_CUSTOMER,'0') AS PHONE_CUSTOMER, 
					ISNULL(B.GPS, '0,0') AS GPS, 
					ISNULL(B.ADRESS_CUSTOMER,'SIN DIRECCION') AS ADRESS_CUSTOMER
					
					FROM 
						DIPROCOM.SWIFT_TASKS A INNER JOIN 
						DIPROCOM.SWIFT_CUSTOMERS B ON
							B.[CODE_CUSTOMER] = A.COSTUMER_CODE INNER JOIN 
								[DIPROCOM].[SWIFT_MANIFEST_DETAIL] C ON
									C.[CODE_PICKING] = A.PICKING_NUMBER
									WHERE C.CODE_MANIFEST_HEADER = @MANIFEST_ID
			...
			SELECT @KMS_DISTANCE = dbo.[SONDA_FN_CALCULATE_DISTANCE](@STARTING_POINT,@CUSTOMER_GPS)
			 ...
			 VALUES
				  ...
				  UPDATE DIPROCOM.SWIFT_TASKS 
				   ...
				   UPDATE 
					DIPROCOM.SWIFT_MANIFEST_DETAIL SET DELIVERY_TASK = @LAST_TASK_ID
		...
		DECLARE TASKS_CURSOR CURSOR LOCAL FORWARD_ONLY 
			FOR 
				SELECT TASK_ID FROM DIPROCOM.SWIFT_TASKS 
		...
		WHILE @@FETCH_STATUS = 0
			BEGIN
				...
				UPDATE DIPROCOM.SWIFT_TASKS SET TASK_SEQ = @CURSOR_SEQ
		...
		UPDATE [DIPROCOM].[SWIFT_MANIFEST_HEADER] SET STATUS = 'ACCEPTED', 
