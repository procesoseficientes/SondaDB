...
-- EJEMPLO DE EJECUCION: 
		...
		--
		EXEC [acsa].[SWIFT_SP_CREATE_DELIVERY_TASKS]
			@MANIFEST_ID = 69,
...
create PROC [acsa].[SWIFT_SP_CREATE_DELIVERY_TASKS_bk]
...
AS
	...
	BEGIN TRY
		...
		SELECT @LOGIN_DEFAULT_WAREHOUSE = 	ISNULL((SELECT DEFAULT_WAREHOUSE FROM [acsa].[USERS]
		...
		SELECT @STARTING_POINT = ISNULL((SELECT GPS_WAREHOUSE FROM [acsa].SWIFT_WAREHOUSES
		...
		DECLARE ORDERS_CURSOR CURSOR LOCAL FORWARD_ONLY 
			FOR 
				SELECT 
					...
					FROM 
						[acsa].SWIFT_TASKS A INNER JOIN 
						[acsa].SWIFT_CUSTOMERS B ON
							B.[CODE_CUSTOMER] = A.COSTUMER_CODE INNER JOIN 
								[acsa].[SWIFT_MANIFEST_DETAIL] C ON
			...
			SELECT @KMS_DISTANCE = dbo.[SONDA_FN_CALCULATE_DISTANCE](@STARTING_POINT,@CUSTOMER_GPS)
			 ...
			 VALUES
				  ...
				  UPDATE [acsa].SWIFT_TASKS 
				   ...
				   UPDATE 
					[acsa].SWIFT_MANIFEST_DETAIL SET DELIVERY_TASK = @LAST_TASK_ID
		...
		DECLARE TASKS_CURSOR CURSOR LOCAL FORWARD_ONLY 
			FOR 
				SELECT TASK_ID FROM [acsa].SWIFT_TASKS 
		...
		WHILE @@FETCH_STATUS = 0
			BEGIN
				...
				UPDATE [acsa].SWIFT_TASKS SET TASK_SEQ = @CURSOR_SEQ
		...
		UPDATE [acsa].[SWIFT_MANIFEST_HEADER] SET STATUS = 'ACCEPTED', 
