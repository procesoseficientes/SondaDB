...
EXECUTE @RC = [DIPROCOM].SONDA_SP_VALIDATE_DEPOSITS_WERE_POSTED @XML
                                                          ,@JSON
GO
 
*/
-- =============================================
CREATE PROCEDURE [DIPROCOM].[SONDA_SP_VALIDATE_DEPOSIT_IS_POSTED] (@DEPOSIT_ID INT
, @DEPOSIT_ID_HH INT
...
BEGIN
  ...
  FROM [DIPROCOM].SONDA_DEPOSITS [H] /*WITH(ROWLOCK,XLOCK,HOLDLOCK)*/
  ...
  SELECT
    @MARK_AS_SEND = 1
   ,@EXISTS = 1
  FROM [DIPROCOM].SONDA_DEPOSITS sd
  ...
  EXISTE:
    ...
    PRINT 'Existe'
    --
    INSERT INTO [DIPROCOM].[SONDA_DEPOSIT_LOG_EXISTS] ([LOG_DATETIME]
    , EXISTS_DEPOSIT
    , [DOC_SERIE]
    , [DOC_NUM]
    , [CODE_ROUTE]
