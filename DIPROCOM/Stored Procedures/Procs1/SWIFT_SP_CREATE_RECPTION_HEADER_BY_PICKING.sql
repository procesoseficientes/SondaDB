...
-- Ejemplo de Ejecucion:
		EXEC [DIPROCOM].[SWIFT_SP_CREATE_RECPTION_HEADER_BY_PICKING]
			@PICKING_HEADER = 1020
		---------------------------------------------------------
		SELECT TYPE_RECEPTION
			...
			,SOURCE_DOC
			,TARGET_DOC
		FROM [DIPROCOM].[SWIFT_RECEPTION_HEADER]
		WHERE RECEPTION_HEADER = 2024
*/
-- =============================================

CREATE PROCEDURE [DIPROCOM].[SWIFT_SP_CREATE_RECPTION_HEADER_BY_PICKING]
(
	@PICKING_HEADER INT
)
AS
BEGIN
	...
	-- ------------------------------------------------------------------------------------
	BEGIN TRY
	INSERT INTO [DIPROCOM].[SWIFT_RECEPTION_HEADER] (
		TYPE_RECEPTION
		,CODE_PROVIDER
		,CODE_USER
		,DOC_SAP_RECEPTION
	...
	SELECT 'PICKING' AS TYPE_RECEPTION
			...
			,PH.PICKING_HEADER
		FROM [DIPROCOM].[SWIFT_PICKING_HEADER] AS PH
		INNER JOIN [DIPROCOM].[SWIFT_CLASSIFICATION] AS SC ON(
