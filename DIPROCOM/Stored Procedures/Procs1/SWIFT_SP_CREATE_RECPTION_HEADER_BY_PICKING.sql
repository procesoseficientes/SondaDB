...
-- Ejemplo de Ejecucion:
		EXEC [PACASA].[SWIFT_SP_CREATE_RECPTION_HEADER_BY_PICKING]
			@PICKING_HEADER = 1020
		---------------------------------------------------------
		SELECT TYPE_RECEPTION
			...
			,SOURCE_DOC
			,TARGET_DOC
		FROM [PACASA].[SWIFT_RECEPTION_HEADER]
		WHERE RECEPTION_HEADER = 2024
*/
-- =============================================

CREATE PROCEDURE [PACASA].[SWIFT_SP_CREATE_RECPTION_HEADER_BY_PICKING]
(
	@PICKING_HEADER INT
)
AS
BEGIN
	...
	-- ------------------------------------------------------------------------------------
	BEGIN TRY
	INSERT INTO [PACASA].[SWIFT_RECEPTION_HEADER] (
		TYPE_RECEPTION
		,CODE_PROVIDER
		,CODE_USER
		,DOC_SAP_RECEPTION
	...
	SELECT 'PICKING' AS TYPE_RECEPTION
			...
			,PH.PICKING_HEADER
		FROM [PACASA].[SWIFT_PICKING_HEADER] AS PH
		INNER JOIN [PACASA].[SWIFT_CLASSIFICATION] AS SC ON(
