...
-- Ejemplo de Ejecucion:				
	exec [DIPROCOM].SWIFT_SP_GET_INVOICE_OVERDUE_BY_RECEIPTS
	@PAYMENT_HEADER_ID = 111
*/
-- =============================================
CREATE PROCEDURE [DIPROCOM].[SWIFT_SP_GET_INVOICE_OVERDUE_BY_RECEIPTS]
...
BEGIN
    ...
    FROM [DIPROCOM].[SONDA_OVERDUE_INVOICE_PAYMENT_HEADER] [SPH]
        INNER JOIN [DIPROCOM].[SONDA_OVERDUE_INVOICE_PAYMENT_DETAIL] [SPD]
            ON [SPH].[ID] = [SPD].[PAYMENT_HEADER_ID]
        INNER JOIN [DIPROCOM].[SWIFT_OVERDUE_INVOICE_BY_CUSTOMER] [OIC]
