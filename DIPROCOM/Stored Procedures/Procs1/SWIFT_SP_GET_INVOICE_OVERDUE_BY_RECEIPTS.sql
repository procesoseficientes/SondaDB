...
-- Ejemplo de Ejecucion:				
	exec [PACASA].SWIFT_SP_GET_INVOICE_OVERDUE_BY_RECEIPTS
	@PAYMENT_HEADER_ID = 111
*/
-- =============================================
CREATE PROCEDURE [PACASA].[SWIFT_SP_GET_INVOICE_OVERDUE_BY_RECEIPTS]
...
BEGIN
    ...
    FROM [PACASA].[SONDA_OVERDUE_INVOICE_PAYMENT_HEADER] [SPH]
        INNER JOIN [PACASA].[SONDA_OVERDUE_INVOICE_PAYMENT_DETAIL] [SPD]
            ON [SPH].[ID] = [SPD].[PAYMENT_HEADER_ID]
        INNER JOIN [PACASA].[SWIFT_OVERDUE_INVOICE_BY_CUSTOMER] [OIC]
