...
CREATE PROCEDURE [diprocom].[SONDA_SP_GET_PACK_CONVERSION_FOR_ROUTE]
    @CODE_ROUTE VARCHAR(50)
AS
    BEGIN
        SET NOCOUNT ON;

---Se crea una tabla temporal desde [diprocom].[SONDA_SP_GET_PRICE_LIST_BY_SKU_PACK_SCALE_FOR_ROUTE]  para traer la lista de precio por defecto y la que tiene asignada para su ruta 
        ...
        --        EXEC [diprocom].[SONDA_SP_GET_PRICE_LIST_BY_SKU_PACK_SCALE_FOR_ROUTE] @CODE_ROUTE = '24';  
...
---Se hace un distinct para no mutiplexsar todos los campos del PACK_CONVERSION filtrado desde la tabla anterior	
        SELECT  DISTINCT
                ...
                [PC].[LAST_UPDATE_BY] ,
                [PC].[ORDER]
        FROM    [diprocom].[SONDA_PACK_CONVERSION] AS [PC]
