...
CREATE PROC [DIPROCOM].[SWIFT_SP_GET_SBO_POD]
...
(
	select  
		...
		t.TXN_IS_POSTED_ERP,
		t.TXN_TYPE 
	from DIPROCOM.SWIFT_RECEPTION_HEADER h
		inner join DIPROCOM.SWIFT_RECEPTION_DETAIL d
		on  h.RECEPTION_HEADER = d.RECEPTION_HEADER 
		inner join (
			select top 1  t.TASK_SOURCE_id TXN_ID, t.TXN_TYPE, t.TXN_IS_POSTED_ERP, tsk.RECEPTION_NUMBER  from DIPROCOM.SWIFT_TXNS t 
				inner join DIPROCOM.SWIFT_TASKS  tsk 
