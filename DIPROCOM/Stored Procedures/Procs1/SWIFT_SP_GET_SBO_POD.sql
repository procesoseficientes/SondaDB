...
(
	select  
		...
		t.TXN_IS_POSTED_ERP,
		t.TXN_TYPE 
	from [PACASA].SWIFT_RECEPTION_HEADER h
		inner join [PACASA].SWIFT_RECEPTION_DETAIL d
		on  h.RECEPTION_HEADER = d.RECEPTION_HEADER 
		inner join (
			select top 1  t.TASK_SOURCE_id TXN_ID, t.TXN_TYPE, t.TXN_IS_POSTED_ERP, tsk.RECEPTION_NUMBER  from [PACASA].SWIFT_TXNS t 
				inner join [PACASA].SWIFT_TASKS  tsk 
   ...
   (select * from openquery (ERPSERVER,'' SELECT  po.ItemCode, po.DocEntry, po.ObjType,  po.LineNum  LineNum
											...
											FROM  [BD_Prueba_Viscosa].dbo.POR1 AS po  
												INNER JOIN [BD_Prueba_Viscosa].dbo.opor op ON po.DocEntry = op.DocEntry
