...
CREATE PROCEDURE [DIPROCOM].[SWIFT_GET_REPORT_MANIFEST]
@MANIFEST_HEADER INT
AS
SELECT DISTINCT
	A.MANIFEST_HEADER
	...
	,(
		SELECT COUNT(E.CODE_PICKING) FROM DIPROCOM.SWIFT_MANIFEST_DETAIL E WHERE E.CODE_MANIFEST_HEADER = A.MANIFEST_HEADER 
	) AS 'CANTIDAD_PEDIDOS'
...
LEFT OUTER JOIN
	DIPROCOM.SWIFT_DRIVERS AS B
ON A.CODE_DRIVER = B.CODE_DRIVER
LEFT OUTER JOIN
	DIPROCOM.SWIFT_VEHICLES AS C
ON A.CODE_VEHICLE = C.CODE_VEHICLE
LEFT OUTER JOIN
	DIPROCOM.SWIFT_ROUTES AS D
...
WHERE  (PO.DocType = ''I'') and po.docstatus =''C'' ') WHERE ERP_DOC = B.DOC_SAP_PICKING),'N/A')
			...
			THEN 'VENDEDOR POR FACTURA'
				--THEN (SELECT SE.SlpName FROM OPENQUERY (ERPSERVER,'SELECT SlpName, SlpCode
				--	 FROM [BD_Prueba_Viscosa].dbo.OSLP ') AS SE
