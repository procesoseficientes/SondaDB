...
CREATE PROCEDURE [PACASA].[SWIFT_GET_REPORT_MANIFEST]
@MANIFEST_HEADER INT
AS
SELECT DISTINCT
	A.MANIFEST_HEADER
	...
	,(
		SELECT COUNT(E.CODE_PICKING) FROM [PACASA].SWIFT_MANIFEST_DETAIL E WHERE E.CODE_MANIFEST_HEADER = A.MANIFEST_HEADER 
	) AS 'CANTIDAD_PEDIDOS'
	...
	,B.LICENSE_DRIVER
FROM
	[PACASA].SWIFT_MANIFEST_HEADER AS A
LEFT OUTER JOIN
	[PACASA].SWIFT_DRIVERS AS B
ON A.CODE_DRIVER = B.CODE_DRIVER
LEFT OUTER JOIN
	[PACASA].SWIFT_VEHICLES AS C
ON A.CODE_VEHICLE = C.CODE_VEHICLE
LEFT OUTER JOIN
	[PACASA].SWIFT_ROUTES AS D
...
FROM
	[PACASA].SWIFT_MANIFEST_HEADER AS A
LEFT OUTER JOIN
	[PACASA].SWIFT_MANIFEST_DETAIL AS B
ON A.MANIFEST_HEADER = B.CODE_MANIFEST_HEADER
LEFT OUTER JOIN
	[PACASA].SWIFT_VIEW_ALL_COSTUMER AS C
ON B.CODE_CUSTOMER = C.CODE_CUSTOMER
LEFT OUTER JOIN
	[PACASA].SWIFT_PICKING_HEADER AS D
ON B.CODE_PICKING = D.PICKING_HEADER
LEFT OUTER JOIN
	[PACASA].SWIFT_PICKING_DETAIL AS E
ON B.CODE_PICKING = E.PICKING_HEADER
LEFT OUTER JOIN
	[PACASA].SWIFT_TXNS AS F
ON E.CODE_SKU = F.TXN_CODE_SKU AND B.CODE_PICKING = F.HEADER_REFERENCE
--LEFT OUTER JOIN
--	SWIFT_INTERFACES.[PACASA].ERP_VIEW_INVOICE_HEADER AS G
--ON G.DocEntry = B.CODE_PICKING
--LEFT OUTER JOIN
--	SWIFT_INTERFACES.[PACASA].ERP_VIEW_INVOICE_DETAIL AS H
