...
-- Ejemplo de Ejecucion:
				--
				EXEC [PACASA].[SWIFT_SP_INSERT_POLYGON]
					...
					,@CODE_WAREHOUSE = NULL
				--
				SELECT * FROM [PACASA].[SWIFT_POLYGON] WHERE POLYGON_NAME = 'RESERVACABAaaaa'
				--
				SELECT * FROM [PACASA].SWIFT_ROUTES WHERE NAME_ROUTE = 'RESERVACABAaaaa'
			
*/
-- =============================================
CREATE PROCEDURE [PACASA].[SWIFT_SP_INSERT_POLYGON] (
...
BEGIN
	...
	BEGIN TRY
		...
		DECLARE	@ID INT;
		--
		INSERT	INTO [PACASA].[SWIFT_POLYGON]
				(
					[POLYGON_NAME]
					,[POLYGON_DESCRIPTION]
					,[COMMENT]
