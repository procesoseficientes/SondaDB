...
-- Ejemplo de Ejecucion:
				EXEC [DIPROCOM].[SWIFT_SP_ADD_PROMO_OF_BONUS_BY_SCALE]
					@PROMO_ID = 82,
					...
					@BONUS_QTY = 10 -- int
				-- 
				SELECT * FROM [DIPROCOM].[SWIFT_PROMO]
				SELECT * FROM [DIPROCOM].[SWIFT_PROMO_BONUS_BY_SCALE]
*/
-- =============================================
CREATE PROCEDURE [DIPROCOM].[SWIFT_SP_ADD_PROMO_OF_BONUS_BY_SCALE] (@PROMO_ID INT
, @CODE_SKU VARCHAR(50)
, @PACK_UNIT INT
, @LOW_LIMIT INT
, @HIGH_LIMIT INT
...
BEGIN
  ...
  BEGIN TRY
    ...
    EXEC [DIPROCOM].[SWIFT_SP_VALIDATED_BONUS_SCALE_OF_PROMO] @PROMO_ID = @PROMO_ID
                                                        ,@CODE_SKU = @CODE_SKU
                                                        ,@PACK_UNIT = @PACK_UNIT
                                                        ,@LOW_LIMIT = @LOW_LIMIT
    ...
    DECLARE @ID INT
    --
    INSERT INTO [DIPROCOM].[SWIFT_PROMO_BONUS_BY_SCALE] ([PROMO_ID]
    , [CODE_SKU]
    , [PACK_UNIT]
    , [LOW_LIMIT]
    , [HIGH_LIMIT]
