...
-- Ejemplo de Ejecucion:
				EXEC [PACASA].[SWIFT_SP_GET_HISTORICAL_CONSIGMENT]
					@START_DATETIME = '20140101 00:00:00.000'
					,@END_DATETIME = '20170201 23:59:59:997'
*/
-- =============================================
CREATE PROCEDURE [PACASA].[SWIFT_SP_GET_HISTORICAL_CONSIGMENT](
...
BEGIN
	...
	SELECT @QUERY = N'SELECT 
		...
		,[S].[DESCRIPTION_SKU]
		,[CD].[QTY]
		,CONVERT(DECIMAL(18,' + CAST(@DEFAULT_DISPLAY_DECIMALS AS VARCHAR) + '),[PACASA].[SWIFT_FN_GET_DISPLAY_NUMBER]([CD].[PRICE])) [PRICE]
		,CONVERT(DECIMAL(18,' + CAST(@DEFAULT_DISPLAY_DECIMALS AS VARCHAR) + '),[PACASA].[SWIFT_FN_GET_DISPLAY_NUMBER]([CD].[TOTAL_LINE])) [TOTAL_LINE]
	...
	LEFT JOIN [PACASA].[SONDA_HISTORICAL_TRACEABILITY_CONSIGNMENT] [HC] ON (
