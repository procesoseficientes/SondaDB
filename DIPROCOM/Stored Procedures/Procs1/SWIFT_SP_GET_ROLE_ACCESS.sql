...
*/
-- =============================================
CREATE PROCEDURE [PACASA].[SWIFT_SP_GET_ROLE_ACCESS]
(@LOGIN_ID VARCHAR(50),@PRIVILEGE_ID VARCHAR(50))
AS
BEGIN

	SELECT COUNT(1) TIENE_ACCESO FROM [PACASA].SWIFT_PRIVILEGES A
	INNER JOIN [PACASA].SWIFT_PRIVILEGES_X_ROLE B
	ON A.ID=B.PRIVILEGE_ID
	INNER JOIN [PACASA].USERS C
