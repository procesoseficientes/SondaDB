CREATE PROCEDURE [acsa].SWIFT_GET_PICKING
...
AS
  ...
  LEFT JOIN [acsa].SWIFT_VIEW_ALL_COSTUMER B ON (P.CODE_CLIENT = B.CODE_CUSTOMER)
  --LEFT JOIN [acsa].SWIFT_FREQUENCY_X_CUSTOMER FC ON (P.CODE_CLIENT = FC.CODE_CUSTOMER)
  LEFT JOIN [acsa].SWIFT_VIEW_ALL_SELLERS VS ON (P.CODE_SELLER = VS.SELLER_CODE)
  LEFT JOIN [acsa].SWIFT_VIEW_ALL_ROUTE VR ON (VR.CODE_ROUTE = P.CODE_ROUTE)
  LEFT JOIN [acsa].SWIFT_MANIFEST_DETAIL MD ON (P.PICKING_HEADER = MD.CODE_PICKING)
  WHERE
...
--	,NULL AS FF_STATUS
--	,'Factura' AS [TYPE]
--FROM [SWIFT_INTERFACES].[acsa].ERP_VIEW_INVOICE_HEADER A
--WHERE 
--A.DocStatus = 'O'
--	PICKING_HEADER NOT IN(SELECT CODE_PICKING FROM [acsa].SWIFT_MANIFEST_DETAIL)
--	AND A.STATUS = 'CLOSED'  
