...
-- Ejemplo de Ejecucion:
		EXEC [DIPROCOM].[SWIFT_SP_INSERT_TASK_BY_RECEPTION]
			@RECEPTION_HEADER = 1020
		---------------------------------------------------------
		SELECT * FROM [DIPROCOM].[SWIFT_TASKS] order by TASK_DATE desc WHERE TASK_ID = 16507 <---COLOCAR ACA EL ID QUE RETORNA LA EJECUCION DEL SP
																		PARA PODER VER LOS DETALLES DE LA TAREA
*/
-- =============================================
CREATE PROCEDURE [DIPROCOM].[SWIFT_SP_INSERT_TASK_BY_RECEPTION]
(
...
BEGIN
	...
	BEGIN TRY
		-- ------------------------------------------------------------------------------------
		-- INSERTA LA TAREA TOMANDO INFORMACION DE LA TABLA  [DIPROCOM].[SWIFT_RECEPTION_HEADER]
		-- ------------------------------------------------------------------------------------
		...
		FROM [DIPROCOM].[SWIFT_RECEPTION_HEADER] AS SRH
		INNER JOIN [DIPROCOM].[SWIFT_VIEW_CUSTOMERS] AS SVC ON (
