...
-- Ejemplo de Ejecucion:
		EXEC [PACASA].[SWIFT_SP_INSERT_TASK_BY_RECEPTION]
			@RECEPTION_HEADER = 1020
		---------------------------------------------------------
		SELECT * FROM [PACASA].[SWIFT_TASKS] order by TASK_DATE desc WHERE TASK_ID = 16507 <---COLOCAR ACA EL ID QUE RETORNA LA EJECUCION DEL SP
																		PARA PODER VER LOS DETALLES DE LA TAREA
*/
-- =============================================
CREATE PROCEDURE [PACASA].[SWIFT_SP_INSERT_TASK_BY_RECEPTION]
(
...
BEGIN
	...
	BEGIN TRY
		-- ------------------------------------------------------------------------------------
		-- INSERTA LA TAREA TOMANDO INFORMACION DE LA TABLA  [PACASA].[SWIFT_RECEPTION_HEADER]
		-- ------------------------------------------------------------------------------------
		...
		FROM [PACASA].[SWIFT_RECEPTION_HEADER] AS SRH
		INNER JOIN [PACASA].[SWIFT_VIEW_CUSTOMERS] AS SVC ON (
