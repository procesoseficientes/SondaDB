...
-- Ejemplo de Ejecucion:
          ...
          SET @PICKING_HEADER = '19928' 
          
          EXECUTE @RC = DIPROCOM.SWIFT_SP_GET_ERP_SOD @PICKING_HEADER
          GO
*/

CREATE  PROC [DIPROCOM].[SWIFT_SP_GET_ERP_SOD_DELETEME]
...
FROM  DIPROCOM.SWIFT_PICKING_DETAIL spd
...
SELECT
  @OQUERY = 'INSERT INTO #TMP_ERP_SOD  SELECT r.DocEntry DocEntryErp  ,'+ @PICKING_HEADER+ '  DocEntry, r.LineNum, r.ItemCode,100 OpenQty, r.ObjType,0 Dispatch FROM (SELECT * FROM OPENQUERY(ERP_SERVER,''
SELECT   r.LineNum , r.ObjType,r.ItemCode,r.DocEntry, r.OpenQty FROM Me_Llega_DB.DBO.RDR1 r  INNER JOIN Me_Llega_DB.dbo.ORDR o
