...
        --
        EXEC [PACASA].[SWIFT_SP_GET_CUSTOMER_PERCENT_BY_POLYGON_TYPE]
			@POLYGON_TYPE = 'REGION'
		--
		EXEC [PACASA].[SWIFT_SP_GET_CUSTOMER_PERCENT_BY_POLYGON_TYPE]
			@POLYGON_TYPE = 'SECTOR'
...
*/
-- =============================================
CREATE PROCEDURE [PACASA].SWIFT_SP_GET_CUSTOMER_PERCENT_BY_POLYGON_TYPE (@POLYGON_TYPE VARCHAR(250)
...
BEGIN
  BEGIN TRY
    ...
    INSERT INTO @POLYGON
      ...
      LEFT JOIN [PACASA].[SWIFT_POLYGON_BY_ROUTE] [PR]
        ON (
    ...
    SELECT
     ...
     ,[P].[COMMENT]
     ,[PACASA].[SWIFT_FN_VALIDATE_POLYGON_HAS_CHILD]([A].[POLYGON_ID]) [HAS_CHILD]
     ,CASE CAST([PACASA].[SWIFT_FN_VALIDATE_POLYGON_HAS_CHILD]([A].[POLYGON_ID]) AS VARCHAR)
        WHEN '1' THEN 'No'
        ELSE 'SI'
      END [HAS_CHILD_MESSAGE]
     ,[PACASA].[SWIFT_FN_GET_TASK_TYPE_BY_POLYGON]([A].[POLYGON_ID]) [TYPE_TASK]
     ,[PACASA].[SWIFT_FUNC_GET_TASK_TYPE_DESCRIPTION_BY_POLYGON]([A].[POLYGON_ID]) [TYPE_TASK_DESCRIPTION]
     ,[W].[CODE_WAREHOUSE]
    ...
    FROM @POLYGON_AVG [A]
    INNER JOIN [PACASA].[SWIFT_POLYGON] [P]
      ON (
      [A].[POLYGON_ID] = [P].[POLYGON_ID]
      )
    LEFT JOIN [PACASA].[SWIFT_POLYGON] [PP]
      ON (
      [P].[POLYGON_ID_PARENT] = [PP].[POLYGON_ID]
      )
    LEFT JOIN [PACASA].[SWIFT_WAREHOUSES] [W]
      ON (
    ...
    LEFT JOIN [PACASA].[SWIFT_VIEW_ALL_SELLERS] [VS]
      ON (
