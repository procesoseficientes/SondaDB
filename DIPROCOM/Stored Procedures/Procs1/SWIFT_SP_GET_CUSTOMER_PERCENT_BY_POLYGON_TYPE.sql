...
        --
        EXEC [DIPROCOM].[SWIFT_SP_GET_CUSTOMER_PERCENT_BY_POLYGON_TYPE]
			@POLYGON_TYPE = 'REGION'
		--
		EXEC [DIPROCOM].[SWIFT_SP_GET_CUSTOMER_PERCENT_BY_POLYGON_TYPE]
			@POLYGON_TYPE = 'SECTOR'
...
*/
-- =============================================
CREATE PROCEDURE [DIPROCOM].SWIFT_SP_GET_CUSTOMER_PERCENT_BY_POLYGON_TYPE (@POLYGON_TYPE VARCHAR(250)
...
BEGIN
  BEGIN TRY
    ...
    INSERT INTO @POLYGON
      ...
      LEFT JOIN [DIPROCOM].[SWIFT_POLYGON_BY_ROUTE] [PR]
        ON (
    ...
    SELECT
     ...
     ,[P].[COMMENT]
     ,[DIPROCOM].[SWIFT_FN_VALIDATE_POLYGON_HAS_CHILD]([A].[POLYGON_ID]) [HAS_CHILD]
     ,CASE CAST([DIPROCOM].[SWIFT_FN_VALIDATE_POLYGON_HAS_CHILD]([A].[POLYGON_ID]) AS VARCHAR)
        WHEN '1' THEN 'No'
        ELSE 'SI'
      END [HAS_CHILD_MESSAGE]
     ,[DIPROCOM].[SWIFT_FN_GET_TASK_TYPE_BY_POLYGON]([A].[POLYGON_ID]) [TYPE_TASK]
     ,[DIPROCOM].[SWIFT_FUNC_GET_TASK_TYPE_DESCRIPTION_BY_POLYGON]([A].[POLYGON_ID]) [TYPE_TASK_DESCRIPTION]
     ,[W].[CODE_WAREHOUSE]
    ...
    FROM @POLYGON_AVG [A]
    INNER JOIN [DIPROCOM].[SWIFT_POLYGON] [P]
      ON (
      [A].[POLYGON_ID] = [P].[POLYGON_ID]
      )
    LEFT JOIN [DIPROCOM].[SWIFT_POLYGON] [PP]
      ON (
      [P].[POLYGON_ID_PARENT] = [PP].[POLYGON_ID]
      )
    LEFT JOIN [DIPROCOM].[SWIFT_WAREHOUSES] [W]
      ON (
    ...
    LEFT JOIN [DIPROCOM].[SWIFT_VIEW_ALL_SELLERS] [VS]
      ON (
