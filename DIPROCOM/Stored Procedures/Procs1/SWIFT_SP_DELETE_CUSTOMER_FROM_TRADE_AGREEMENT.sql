...
SELECT * FROM [PACASA].[SWIFT_TRADE_AGREEMENT_BY_CUSTOMER]
	
  EXEC [PACASA].[SWIFT_SP_DELETE_CUSTOMER_FROM_TRADE_AGREEMENT]
		@TRADE_AGREEMENT_ID = 1038
		,@CODE_CUSTOMER = 'C002'

SELECT * FROM [PACASA].[SWIFT_TRADE_AGREEMENT] WHERE [TRADE_AGREEMENT_ID] = 1

==================================================*/

CREATE PROCEDURE [PACASA].SWIFT_SP_DELETE_CUSTOMER_FROM_TRADE_AGREEMENT (@TRADE_AGREEMENT_ID INT
, @CODE_CUSTOMER VARCHAR(50))
AS
BEGIN
  BEGIN TRY

    DELETE FROM [PACASA].[SWIFT_TRADE_AGREEMENT_BY_CUSTOMER]
