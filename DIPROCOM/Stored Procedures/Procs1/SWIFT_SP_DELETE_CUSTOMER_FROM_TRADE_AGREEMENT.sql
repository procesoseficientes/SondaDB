...
SELECT * FROM [acsa].[SWIFT_TRADE_AGREEMENT_BY_CUSTOMER]
	
  EXEC [acsa].[SWIFT_SP_DELETE_CUSTOMER_FROM_TRADE_AGREEMENT]
		@TRADE_AGREEMENT_ID = 1038
		,@CODE_CUSTOMER = 'C002'

SELECT * FROM [acsa].[SWIFT_TRADE_AGREEMENT] WHERE [TRADE_AGREEMENT_ID] = 1

==================================================*/

CREATE PROCEDURE [acsa].SWIFT_SP_DELETE_CUSTOMER_FROM_TRADE_AGREEMENT (@TRADE_AGREEMENT_ID INT
, @CODE_CUSTOMER VARCHAR(50))
AS
BEGIN
  BEGIN TRY

    DELETE FROM [acsa].[SWIFT_TRADE_AGREEMENT_BY_CUSTOMER]
