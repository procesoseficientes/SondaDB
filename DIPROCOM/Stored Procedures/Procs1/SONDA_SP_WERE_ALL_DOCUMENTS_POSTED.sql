...
*/
-- =============================================
CREATE PROCEDURE [PACASA].[SONDA_SP_WERE_ALL_DOCUMENTS_POSTED] (@QUANTITY INT, @ROUTE_ID VARCHAR(50))
AS
BEGIN
  SET NOCOUNT ON;
  ...
  SELECT @COUNT_SALES_ORDER= COUNT(1) FROM [PACASA].SONDA_SALES_ORDER_HEADER ssoh
    WHERE
  ...
  SELECT @COUNT_INVOICES =  COUNT(1) FROM [PACASA].SONDA_POS_INVOICE_HEADER spih
