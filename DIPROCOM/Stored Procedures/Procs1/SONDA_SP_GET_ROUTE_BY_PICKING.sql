...
-- Ejemplo de Ejecucion:
				-- 
				EXEC [acsa].[SONDA_SP_GET_ROUTE_BY_PICKING]			
					
*/
-- =============================================

CREATE PROCEDURE [acsa].SONDA_SP_GET_ROUTE_BY_PICKING
...
BEGIN
  ...
  FROM [acsa].SWIFT_PICKING_HEADER A
  LEFT JOIN [acsa].SWIFT_VIEW_ALL_COSTUMER B
    ON (A.CODE_CLIENT = B.CODE_CUSTOMER)
  --LEFT JOIN [acsa].SWIFT_FREQUENCY_X_CUSTOMER FC ON (A.CODE_CLIENT = FC.CODE_CUSTOMER)
  --LEFT JOIN [acsa].SWIFT_FREQUENCY F ON (F.ID_FREQUENCY = FC.ID_FREQUENCY)
  LEFT JOIN [acsa].SWIFT_VIEW_ALL_SELLERS VS
    ON (A.CODE_SELLER = VS.SELLER_CODE)
  LEFT JOIN [acsa].SWIFT_VIEW_ALL_ROUTE VR
    ON (VR.CODE_ROUTE = A.CODE_ROUTE)
  LEFT JOIN [acsa].SWIFT_MANIFEST_DETAIL MD
  ...
  LEFT JOIN  [acsa].SWIFT_VEHICLE_X_USER svxu
    ON (svxu.[LOGIN] = u.[LOGIN])
  LEFT JOIN  [acsa].SWIFT_VEHICLES sv
