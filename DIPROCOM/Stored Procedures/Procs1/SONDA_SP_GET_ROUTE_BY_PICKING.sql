...
-- Ejemplo de Ejecucion:
				-- 
				EXEC [PACASA].[SONDA_SP_GET_ROUTE_BY_PICKING]			
					
*/
-- =============================================

CREATE PROCEDURE [PACASA].SONDA_SP_GET_ROUTE_BY_PICKING
...
BEGIN
  ...
  FROM [PACASA].SWIFT_PICKING_HEADER A
  LEFT JOIN [PACASA].SWIFT_VIEW_ALL_COSTUMER B
    ON (A.CODE_CLIENT = B.CODE_CUSTOMER)
  --LEFT JOIN [PACASA].SWIFT_FREQUENCY_X_CUSTOMER FC ON (A.CODE_CLIENT = FC.CODE_CUSTOMER)
  --LEFT JOIN [PACASA].SWIFT_FREQUENCY F ON (F.ID_FREQUENCY = FC.ID_FREQUENCY)
  LEFT JOIN [PACASA].SWIFT_VIEW_ALL_SELLERS VS
    ON (A.CODE_SELLER = VS.SELLER_CODE)
  LEFT JOIN [PACASA].SWIFT_VIEW_ALL_ROUTE VR
    ON (VR.CODE_ROUTE = A.CODE_ROUTE)
  LEFT JOIN [PACASA].SWIFT_MANIFEST_DETAIL MD
  ...
  LEFT JOIN  [PACASA].SWIFT_VEHICLE_X_USER svxu
    ON (svxu.[LOGIN] = u.[LOGIN])
  LEFT JOIN  [PACASA].SWIFT_VEHICLES sv
