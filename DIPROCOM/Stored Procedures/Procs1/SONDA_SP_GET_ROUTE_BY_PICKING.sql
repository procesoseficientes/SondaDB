...
-- Ejemplo de Ejecucion:
				-- 
				EXEC [DIPROCOM].[SONDA_SP_GET_ROUTE_BY_PICKING]			
					
*/
-- =============================================

CREATE PROCEDURE [DIPROCOM].SONDA_SP_GET_ROUTE_BY_PICKING
...
BEGIN
  ...
  FROM [DIPROCOM].SWIFT_PICKING_HEADER A
  LEFT JOIN [DIPROCOM].SWIFT_VIEW_ALL_COSTUMER B
    ON (A.CODE_CLIENT = B.CODE_CUSTOMER)
  --LEFT JOIN [DIPROCOM].SWIFT_FREQUENCY_X_CUSTOMER FC ON (A.CODE_CLIENT = FC.CODE_CUSTOMER)
  --LEFT JOIN [DIPROCOM].SWIFT_FREQUENCY F ON (F.ID_FREQUENCY = FC.ID_FREQUENCY)
  LEFT JOIN [DIPROCOM].SWIFT_VIEW_ALL_SELLERS VS
    ON (A.CODE_SELLER = VS.SELLER_CODE)
  LEFT JOIN [DIPROCOM].SWIFT_VIEW_ALL_ROUTE VR
    ON (VR.CODE_ROUTE = A.CODE_ROUTE)
  LEFT JOIN [DIPROCOM].SWIFT_MANIFEST_DETAIL MD
