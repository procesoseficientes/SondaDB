CREATE PROC [PACASA].[SWIFT_SP_GET_COUNT_HEADER_BY_ID]
@COUNT_ID VARCHAR(50)
AS
--DECLARE @COUNT_TYPE VARCHAR(50)

SELECT COUNT_TYPE, COUNT_NAME, A.COUNT_ASSIGNED_TO AS NAME_USER,COUNT_STATUS 
FROM [PACASA].SWIFT_CYCLE_COUNT_HEADER A, [PACASA].USERS B
...
FROM [PACASA].SWIFT_CYCLE_COUNT_DETAIL AS A
WHERE COUNT_ID = @COUNT_ID
--SET @COUNT_TYPE = (SELECT COUNT_TYPE
--FROM [PACASA].SWIFT_CYCLE_COUNT_HEADER
--WHERE  COUNT_ID = @COUNT_ID)

--EXEC [PACASA].SWIFT_SP_GET_COUNT_HEADER_BY_ID '53'

--SELECT * FROM [PACASA].SWIFT_CYCLE_COUNT_DETAIL WHERE
