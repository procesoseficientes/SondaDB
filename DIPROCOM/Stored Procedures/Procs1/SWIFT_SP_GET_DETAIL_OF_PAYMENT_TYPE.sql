...
-- Ejemplo de Ejecucion:
				EXEC [DIPROCOM].[SWIFT_SP_GET_DETAIL_OF_PAYMENT_TYPE]
				@PAYMENT_HEADER_ID = 20
*/
-- =============================================
CREATE PROCEDURE [DIPROCOM].[SWIFT_SP_GET_DETAIL_OF_PAYMENT_TYPE]
	(
		...
		SELECT
			...
			,(
				SELECT TOP 1
					[C].[MPC01]
				FROM
					[DIPROCOM].[SWIFT_CLASSIFICATION] AS [C]
				WHERE
					[C].[NAME_CLASSIFICATION] = [OIPTD].[BANK_NAME]
				) [BANK_CODE]
		FROM
			[DIPROCOM].[SONDA_PAYMENT_TYPE_DETAIL_FOR_OVERDUE_INVOICE_PAYMENT] AS [OIPTD]
		WHERE
