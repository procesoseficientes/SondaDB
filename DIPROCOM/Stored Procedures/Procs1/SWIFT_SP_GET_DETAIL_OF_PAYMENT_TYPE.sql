...
-- Ejemplo de Ejecucion:
				EXEC [PACASA].[SWIFT_SP_GET_DETAIL_OF_PAYMENT_TYPE]
				@PAYMENT_HEADER_ID = 20
*/
-- =============================================
CREATE PROCEDURE [PACASA].[SWIFT_SP_GET_DETAIL_OF_PAYMENT_TYPE]
	(
		...
		SELECT
			...
			,(
				SELECT TOP 1
					[C].[MPC01]
				FROM
					[PACASA].[SWIFT_CLASSIFICATION] AS [C]
				WHERE
					[C].[NAME_CLASSIFICATION] = [OIPTD].[BANK_NAME]
				) [BANK_CODE]
		FROM
			[PACASA].[SONDA_PAYMENT_TYPE_DETAIL_FOR_OVERDUE_INVOICE_PAYMENT] AS [OIPTD]
		WHERE
