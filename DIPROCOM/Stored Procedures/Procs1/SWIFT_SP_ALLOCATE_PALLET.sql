...
-- Ejemplo de Ejecucion:				
				--
EXECUTE  [PACASA].[SWIFT_SP_ALLOCATE_PALLET] 
   @TASK_ID = 7444
  ,@PALLET_ID = 1251  
  ,@CODE_LOCATION = '001'
...
*/
-- =============================================
CREATE PROCEDURE [PACASA].[SWIFT_SP_ALLOCATE_PALLET]
     @TASK_ID AS INT
	,@PALLET_ID AS INT
...
BEGIN TRY
	...
	SELECT @BATCHID = [BATCH_ID], @QTY_IN = CONVERT(FLOAT, [QTY]), @QTY_TX = CONVERT(NUMERIC(18,2), [QTY])  FROM [PACASA].[SWIFT_PALLET] WHERE [PALLET_ID] = @PALLET_ID 

	SELECT @CODE_SKU = [SKU]  FROM [PACASA].[SWIFT_BATCH]  WHERE [BATCH_ID] = @BATCHID  

	SELECT @DESCRIPTION_SKU = [DESCRIPTION_SKU], @TXN_SKU_BARCODE= [BARCODE_SKU] FROM [PACASA].[SWIFT_VIEW_ALL_SKU] WHERE [CODE_SKU] = @CODE_SKU

	SELECT @NAME_USER = [NAME_USER] FROM [PACASA].[USERS] WHERE [LOGIN] = @LAST_UPDATE_BY

	SELECT @TXN_COSTUMER_CODE = [COSTUMER_CODE], @TXN_COSTUMER_NAME = [COSTUMER_NAME] FROM [PACASA].[SWIFT_TASKS] WHERE [TASK_ID] =  @TASK_ID

	SELECT @CODE_WAREHOUSE = [CODE_WAREHOUSE] FROM [PACASA].[SWIFT_LOCATIONS] WHERE [CODE_LOCATION] = @CODE_LOCATION

	SELECT @RECEPTION_HEADER = RECEPTION_NUMBER FROM [PACASA].[SWIFT_TASKS] WHERE TASK_ID = @TASK_ID
	
	SELECT @VALIDATE_PALLET = [PACASA].[SWIFT_FUNC_VALIDATE_PALLET](@TASK_ID,@CODE_SKU, @PALLET_ID)
	...
	BEGIN
		...
		EXEC [PACASA].[SWIFT_SP_LOCATE_PALLET] @LAST_UPDATE_BY,@CODE_LOCATION ,@PALLET_ID,@CODE_WAREHOUSE		
		COMMIT
	END
	ELSE
	BEGIN		
