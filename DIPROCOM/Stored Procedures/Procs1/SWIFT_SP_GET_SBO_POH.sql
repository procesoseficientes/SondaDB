...
CREATE PROC [acsa].[SWIFT_SP_GET_SBO_POH]
...
SELECT  @SQL = 'SELECT    TOP (1) po.DocNum, '+@DOC_ENTRY+' AS DocEntry, po.CardCode AS CardCode, po.CardName AS CardName, ''N'' AS HandWritten, ISNULL(t.TXN_CREATED_STAMP, GETDATE()) AS DocDate, po.Comments, 
                      ...
                      AS UTipoPago, CAST(NULL AS varchar) AS UCuotas, CAST(NULL AS varchar) AS UTotalTarjeta, CAST(NULL AS varchar) AS UFechap, CAST(NULL AS varchar) AS UTrasladoOC from  [acsa].SWIFT_TXNS AS t INNER JOIN
