...
--Ejemplo de Ejecucion:
						exec [PACASA].[SWIFT_SP_GET_SCOUNTING_part] 
...
-- =============================================
CREATE PROCEDURE [PACASA].[SWIFT_SP_GET_SCOUNTING_Part]
	@TAGS VARCHAR(MAX)
	,@Type AS VARCHAR(20)
	,@NOT_TAGS AS INT
...
BEGIN
	...
	FROM [PACASA].[SWIFT_CUSTOMERS_NEW] CN
	...
	LEFT JOIN [PACASA].[SWIFT_TAG_X_CUSTOMER_NEW] TCN ON (CN.CODE_CUSTOMER = TCN.CUSTOMER)
	WHERE RU.LOGIN = @LOGIN
		...
		AND (@NOT_TAGS = 0 OR 0 < (SELECT COUNT(*) FROM [PACASA].SWIFT_TAG_X_CUSTOMER_NEW TC WHERE CN.CODE_CUSTOMER = TC.CUSTOMER))
END
