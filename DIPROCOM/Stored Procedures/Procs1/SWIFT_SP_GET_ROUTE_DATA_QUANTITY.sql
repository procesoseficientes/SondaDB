...
-- Ejemplo de Ejecucion:				
				EXECUTE [PACASA].[SWIFT_SP_GET_ROUTE_DATA_QUANTITY] @CODE_ROUTE = '001'
*/
-- =============================================
CREATE PROCEDURE [PACASA].[SWIFT_SP_GET_ROUTE_DATA_QUANTITY]
...
BEGIN
	...
	--
	SELECT @PRICE_LIST = [PACASA].[SWIFT_FN_GET_PRICE_LIST](DEFAULT)
	...
	INNER JOIN [PACASA].[SWIFT_VIEW_USERS] AS U 	ON (T.[ASSIGEND_TO] = U.[LOGIN])
	...
	-- ------------------------------
	 ...
	 FROM [PACASA].[SWIFT_FN_GET_CUSTUMER_FOR_ROUTE](@CODE_ROUTE) C
	...
	INNER JOIN [PACASA].SWIFT_VIEW_ALL_SKU S on (S.CODE_SKU =  I.SKU)
	INNER JOIN [PACASA].SWIFT_PRICE_LIST_BY_SKU P on (P.CODE_SKU = I.SKU)
	INNER JOIN [PACASA].USERS U ON (U.DEFAULT_WAREHOUSE = i.WAREHOUSE)
	WHERE u.SELLER_ROUTE = @CODE_ROUTE
	...
	FROM [PACASA].[SWIFT_VIEW_PRESALE_SKU] I
	INNER JOIN [PACASA].SWIFT_PRICE_LIST_BY_SKU p on (p.CODE_SKU = I.SKU)
	...
	FROM [PACASA].[SONDA_POS_RES_SAT] AS F
