...
-- Ejemplo de Ejecucion:				
				EXECUTE [DIPROCOM].[SWIFT_SP_GET_ROUTE_DATA_QUANTITY] @CODE_ROUTE = '001'
*/
-- =============================================
CREATE PROCEDURE [DIPROCOM].[SWIFT_SP_GET_ROUTE_DATA_QUANTITY]
...
BEGIN
	...
	--
	SELECT @PRICE_LIST = DIPROCOM.[SWIFT_FN_GET_PRICE_LIST](DEFAULT)
	...
	INNER JOIN [DIPROCOM].[SWIFT_VIEW_USERS] AS U 	ON (T.[ASSIGEND_TO] = U.[LOGIN])
	...
	-- ------------------------------
	 ...
	 FROM DIPROCOM.[SWIFT_FN_GET_CUSTUMER_FOR_ROUTE](@CODE_ROUTE) C
	...
	INNER JOIN DIPROCOM.SWIFT_VIEW_ALL_SKU S on (S.CODE_SKU =  I.SKU)
	INNER JOIN DIPROCOM.SWIFT_PRICE_LIST_BY_SKU P on (P.CODE_SKU = I.SKU)
	INNER JOIN DIPROCOM.USERS U ON (U.DEFAULT_WAREHOUSE = i.WAREHOUSE)
	WHERE u.SELLER_ROUTE = @CODE_ROUTE
	...
	FROM [DIPROCOM].[SWIFT_VIEW_PRESALE_SKU] I
	INNER JOIN SWIFT_EXPRESS.DIPROCOM.SWIFT_PRICE_LIST_BY_SKU p on (p.CODE_SKU = I.SKU)
	...
	FROM [DIPROCOM].[SONDA_POS_RES_SAT] AS F
	...
	FROM [DIPROCOM].[SWIFT_DOCUMENT_SEQUENCE] S
