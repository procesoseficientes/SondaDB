...
-- EJEMPLO DE EJECUCION: 
		EXEC [DIPROCOM].[SWIFT_SP_DELETE_POLYGON_BY_REGION]
			@POLYGON_ID = 4154
		--
		SELECT * FROM [DIPROCOM].[SWIFT_POLYGON] WHERE POLYGON_NAME = 'pacopaco'
		--
		SELECT * FROM [DIPROCOM].SWIFT_ROUTES WHERE NAME_ROUTE = 'pacopaco'
*/
-- =========================================================
CREATE PROCEDURE [DIPROCOM].SWIFT_SP_DELETE_POLYGON_BY_REGION (@POLYGON_ID INT
...
BEGIN
  ...
  BEGIN TRY
    IF (SELECT
          COUNT([P].[POLYGON_ID])
        FROM [DIPROCOM].[SWIFT_POLYGON] AS [P]
    ...
    BEGIN
      ...
      IF @POLYGON_TYPE = 'REGION' BEGIN
        ...
        DELETE [CGAP] 
        FROM [DIPROCOM].[SWIFT_CUSTOMER_GPS_ASSOCIATE_TO_POLYGON] [CGAP] 
        INNER JOIN  [DIPROCOM].[SWIFT_CUSTOMER_ASSOCIATE_TO_POLYGON] [CAP] ON (
      ...
      -- ------------------------------------------------------------------------------------
      DELETE FROM [DIPROCOM].[SWIFT_POLYGON_POINT]
      ...
      FROM [DIPROCOM].[SWIFT_TASK_BY_POLYGON]
      WHERE [POLYGON_ID] = @POLYGON_ID
      --
      DELETE [FC]
      ...
      -- ------------------------------------------------------------------------------------
      DELETE FROM [DIPROCOM].[SWIFT_POLYGON_BY_ROUTE]
      WHERE [POLYGON_ID] = @POLYGON_ID
      ...
      -- ------------------------------------------------------------------------------------
      DELETE FROM [DIPROCOM].[SWIFT_FREQUENCY_BY_POLYGON]
      WHERE [POLYGON_ID] = @POLYGON_ID

      -- ------------------------------------------------------------------------------------
      -- Eliminamos las frecuencia
      -- ------------------------------------------------------------------------------------
      DELETE FROM [DIPROCOM].[SWIFT_FREQUENCY]
      WHERE [POLYGON_ID] = @POLYGON_ID

      -- ------------------------------------------------------------------------------------
      -- Elimina los clientes asociados al poligono
      -- ------------------------------------------------------------------------------------
      DELETE FROM [DIPROCOM].[SWIFT_POLYGON_X_CUSTOMER]
      WHERE [POLYGON_ID] = @POLYGON_ID
      ...
      -- Elimina el poligono
      -- ------------------------------------------------------------------------------------
      DELETE FROM [DIPROCOM].[SWIFT_POLYGON]
      WHERE [POLYGON_ID] = @POLYGON_ID;      
