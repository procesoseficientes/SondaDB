...
-- Ejemplo de Ejecucion:
          ...
          EXECUTE @RC = [PACASA].SWIFT_SP_GET_ERP_POD @RECEPTION_HEADER
          GO
*/

CREATE PROC [PACASA].[SWIFT_SP_GET_ERP_POD] @RECEPTION_HEADER VARCHAR(50)
AS
  ...
  FROM [PACASA].SWIFT_TXN_X_DOC_ERP stxde
  INNER JOIN [PACASA].SWIFT_RECEPTION_HEADER srh
  ...
  FROM [PACASA].SWIFT_RECEPTION_DETAIL srd
  INNER JOIN [PACASA].SWIFT_TASKS st
...
SELECT o.DocNum , r.LineNum , r.ObjType,r.ItemCode,r.DocEntry, r.OpenQty, r.WhsCode WarehouseCode FROM BD_Prueba_Viscosa.DBO.POR1 r  INNER JOIN BD_Prueba_Viscosa.dbo.OPOR o
  ...
  INSERT INTO #TMP_SWIFT_POD
    ...
    INNER JOIN [PACASA].SWIFT_TXN_X_DOC_ERP stxde
