...
-- Ejemplo de Ejecucion:
				...
				EXEC [acsa].[SWIFT_SP_GENERATE_BONUS_LIST]
					@CODE_ROUTE = '44'
				--
				SELECT * FROM [acsa].SWIFT_BONUS_LIST WHERE CODE_ROUTE = '44'
*/
-- =============================================
CREATE PROCEDURE [acsa].[SWIFT_SP_GENERATE_BONUS_LIST] (
...
BEGIN
	...
	-- ------------------------------------------------------------------------------------
	IF (SELECT [acsa].[SWIFT_FN_VALIDATE_TRADE_AGREEMENT_BY_ROUTE](@CODE_ROUTE)) = 1
	BEGIN
		DECLARE @TRADE_AGREEMENT_ID INT = NULL
		--
		SELECT @TRADE_AGREEMENT_ID = [acsa].[SWIFT_FN_GET_TRADE_AGREEMENT_BY_ROUTE](@CODE_ROUTE)
		--
		INSERT INTO @BONUS_LIST
				([NAME_BONUS_LIST])
