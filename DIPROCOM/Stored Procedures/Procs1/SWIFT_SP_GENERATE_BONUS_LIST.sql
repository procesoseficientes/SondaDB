...
-- Ejemplo de Ejecucion:
				...
				EXEC [PACASA].[SWIFT_SP_GENERATE_BONUS_LIST]
					@CODE_ROUTE = '44'
				--
				SELECT * FROM [PACASA].SWIFT_BONUS_LIST WHERE CODE_ROUTE = '44'
*/
-- =============================================
CREATE PROCEDURE [PACASA].[SWIFT_SP_GENERATE_BONUS_LIST] (
...
BEGIN
	...
	-- ------------------------------------------------------------------------------------
	IF (SELECT [PACASA].[SWIFT_FN_VALIDATE_TRADE_AGREEMENT_BY_ROUTE](@CODE_ROUTE)) = 1
	BEGIN
		DECLARE @TRADE_AGREEMENT_ID INT = NULL
		--
		SELECT @TRADE_AGREEMENT_ID = [PACASA].[SWIFT_FN_GET_TRADE_AGREEMENT_BY_ROUTE](@CODE_ROUTE)
		--
		INSERT INTO @BONUS_LIST
				([NAME_BONUS_LIST])
