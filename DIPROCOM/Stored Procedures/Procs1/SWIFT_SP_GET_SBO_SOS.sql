...
CREATE PROC [DIPROCOM].[SWIFT_SP_GET_SBO_SOS]
@DOC_NUM VARCHAR(50), @DOC_ENTRY VARCHAR(50),@ITEM_CODE VARCHAR(50),@LINE_NUM VARCHAR(50),@TRANS_ID VARCHAR(50)
AS
...
FROM         (SELECT     t.TXN_ID, t.MANIFEST_SOURCE, t.SAP_REFERENCE, t.TASK_SOURCE_ID, t.TXN_TYPE, t.TXN_DESCRIPTION, t.TXN_CATEGORY, t.TXN_CREATED_STAMP, t.TXN_OPERATOR_ID, 
                       ...
                       FROM          [DIPROCOM].SWIFT_TXNS AS t ) AS t LEFT OUTER JOIN	
		...
		'' 
			...
			FROM          
				[BD_Prueba_Viscosa].dbo.RDR1 AS po  
			WHERE 
				(po.DocEntry = '+@DOC_ENTRY+')  
				AND (po.ItemCode = '''''+@ITEM_CODE+''''')
				AND (po.LineNum = '+@LINE_NUM+')
