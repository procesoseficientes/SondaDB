...
-- Ejemplo de Ejecucion:
				EXEC [PACASA].[SWIFT_SP_GET_COMPLETED_LIQUIDATION]
...
*/
-- =============================================
CREATE PROCEDURE [PACASA].[SWIFT_SP_GET_COMPLETED_LIQUIDATION](
...
BEGIN
	...
	--
	SELECT @DELIMETER = [PACASA].[SWIFT_FN_GET_PARAMETER]('DELIMITER','DEFAULT_DELIMITER')
	--
	DECLARE @ROUTE TABLE (
	...
	BEGIN
		INSERT INTO @ROUTE
				([CODE_ROUTE])
		SELECT [VALUE]
		FROM [PACASA].[SWIFT_FN_SPLIT_2](@ROUTES,@DELIMETER)
	END
	--
	SELECT
		[L].[LIQUIDATION_ID]
	...
	INNER JOIN [PACASA].[USERS] [U1] ON (
		[U1].[LOGIN] = [L].[LOGIN]
	)
	INNER JOIN [PACASA].[SWIFT_SELLER] [S] ON (
		[U1].[RELATED_SELLER] = [S].[SELLER_CODE]
	)
	INNER JOIN [PACASA].[USERS] [U2] ON (
		[U2].[LOGIN] = [L].[LOGIN]
	)
