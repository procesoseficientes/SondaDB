...
-- Ejemplo de Ejecucion:
				SELECT [DIPROCOM].[SWIFT_FN_VALIDATE_RESULT_SUM_PRODUCT](6184)
*/
-- =============================================


CREATE FUNCTION [DIPROCOM].[SWIFT_FN_VALIDATE_RESULT_SUM_PRODUCT]
(
	@TASK_ID INT 
)
RETURNS INT
...
BEGIN
	...
	FROM [DIPROCOM].[SWIFT_INVENTORY]  AS INV INNER JOIN [DIPROCOM].[SWIFT_PALLET] AS PAL  ON (INV.[PALLET_ID] = PAL.[PALLET_ID]) 
	WHERE PAL.TASK_ID = @TASK_ID
	GROUP BY PAL.TASK_ID


	SELECT @QUANT_GEN = SUM(DA.[SCANNED]) 
	FROM [DIPROCOM].[SWIFT_RECEPTION_DETAIL] AS DA INNER JOIN [DIPROCOM].[SWIFT_TASKS] AS TA  ON (TA.[RECEPTION_NUMBER] = DA.RECEPTION_HEADER) 
