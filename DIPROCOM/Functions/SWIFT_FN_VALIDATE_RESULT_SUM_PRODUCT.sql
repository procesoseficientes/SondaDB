...
-- Ejemplo de Ejecucion:
				SELECT [PACASA].[SWIFT_FN_VALIDATE_RESULT_SUM_PRODUCT](6184)
*/
-- =============================================


CREATE FUNCTION [PACASA].[SWIFT_FN_VALIDATE_RESULT_SUM_PRODUCT]
(
	@TASK_ID INT 
)
RETURNS INT
...
BEGIN
	...
	FROM [PACASA].[SWIFT_INVENTORY]  AS INV INNER JOIN [PACASA].[SWIFT_PALLET] AS PAL  ON (INV.[PALLET_ID] = PAL.[PALLET_ID]) 
	WHERE PAL.TASK_ID = @TASK_ID
	GROUP BY PAL.TASK_ID


	SELECT @QUANT_GEN = SUM(DA.[SCANNED]) 
	FROM [PACASA].[SWIFT_RECEPTION_DETAIL] AS DA INNER JOIN [PACASA].[SWIFT_TASKS] AS TA  ON (TA.[RECEPTION_NUMBER] = DA.RECEPTION_HEADER) 
