...
-- Ejemplo de Ejecucion:
				SELECT DIPROCOM.[SWIFT_FN_COUNT_PRODUCT_TASK] (8)
*/
-- =============================================


CREATE FUNCTION [DIPROCOM].[SWIFT_FN_COUNT_PRODUCT_TASK]
(
	@TASK_ID INT 
)
...
BEGIN
	...
	INNER JOIN [DIPROCOM].[SWIFT_BATCH] AS BT ON (PA.[BATCH_ID] = BT.[BATCH_ID]) 
	INNER JOIN [DIPROCOM].[SWIFT_TASKS] AS TS ON (PA.[TASK_ID] = TS.[TASK_ID]) 
	INNER JOIN [DIPROCOM].[SWIFT_VIEW_ALL_SKU] AS SU ON (BT.[SKU] = SU.[SKU]) 
	INNER JOIN [DIPROCOM].[SWIFT_RECEPTION_DETAIL] AS RD ON (RD.[CODE_SKU] = SU.[CODE_SKU]) 
	INNER JOIN [DIPROCOM].[SWIFT_INVENTORY] AS IV ON (IV.[SKU] = SU.[CODE_SKU]) 
