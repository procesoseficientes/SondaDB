...
-- Ejemplo de Ejecucion:
				SELECT [SONDA].[SWIFT_FN_COUNT_PRODUCT_TASK] (8)
*/
-- =============================================


CREATE FUNCTION [SONDA].[SWIFT_FN_COUNT_PRODUCT_TASK]
(
	@TASK_ID INT 
)
...
BEGIN
	...
	INNER JOIN [SONDA].[SWIFT_BATCH] AS BT ON (PA.[BATCH_ID] = BT.[BATCH_ID]) 
	INNER JOIN [SONDA].[SWIFT_TASKS] AS TS ON (PA.[TASK_ID] = TS.[TASK_ID]) 
	INNER JOIN [SONDA].[SWIFT_VIEW_ALL_SKU] AS SU ON (BT.[SKU] = SU.[SKU]) 
	INNER JOIN [SONDA].[SWIFT_RECEPTION_DETAIL] AS RD ON (RD.[CODE_SKU] = SU.[CODE_SKU]) 
	INNER JOIN [SONDA].[SWIFT_INVENTORY] AS IV ON (IV.[SKU] = SU.[CODE_SKU]) 
