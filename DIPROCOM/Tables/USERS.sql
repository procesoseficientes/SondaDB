CREATE TABLE [PACASA].[USERS] (
    ...
    CONSTRAINT [FK__USERS__DISTRIBUT__45ABAF15] FOREIGN KEY ([DISTRIBUTION_CENTER_ID]) REFERENCES [PACASA].[SWIFT_DISTRIBUTION_CENTER] ([DISTRIBUTION_CENTER_ID]),
    CONSTRAINT [FK_SWIFT_USER_ZONE_ID] FOREIGN KEY ([ZONE_ID]) REFERENCES [PACASA].[SWIFT_ZONE] ([ZONE_ID])
);


GO
CREATE NONCLUSTERED INDEX [IDX_CODE_ROUTE_DEVICE_ID_VALIDATION_TYPE_USERS]
    ON [PACASA].[USERS]([SELLER_ROUTE] ASC)
    INCLUDE([VALIDATION_TYPE], [DEVICE_ID]);


GO
CREATE NONCLUSTERED INDEX [IDX_SWIFT_USERS_USER_ROLE]
    ON [PACASA].[USERS]([CORRELATIVE] ASC)
    INCLUDE([USER_ROLE], [LOGIN]);


GO
CREATE NONCLUSTERED INDEX [IDX_USERS_LOGIN_DISTRIBUTION_CENTER_ID]
    ON [PACASA].[USERS]([LOGIN] ASC)
...
CREATE NONCLUSTERED INDEX [IN_USERS_RELATED_SELLER]
    ON [PACASA].[USERS]([RELATED_SELLER] ASC);
