...
-- Ejemplo de Ejecucion:
				...
				SELECT * FROM [DIPROCOM].[SWIFT_VIEW_ALL_SKU]
*/
-- =============================================
CREATE VIEW [DIPROCOM].[SWIFT_VIEW_ALL_SKU]
AS
SELECT
	[SKU]
  ...
  --0 AS ON_HAND,
	...
	,ERP.VAT_CODE
FROM
	[SWIFT_INTERFACES].[DIPROCOM].[ERP_VIEW_SKU] [ERP]
LEFT JOIN [DIPROCOM].[SWIFT_SKU_SALE_PACK_UNIT] [PU] ON ([ERP].[CODE_SKU] = [PU].[CODE_SKU])
LEFT JOIN [DIPROCOM].[SONDA_PACK_UNIT] [ERPSPU] ON [PU].[CODE_PACK_UNIT] = [ERPSPU].[CODE_PACK_UNIT]
WHERE
...
SELECT
  ...
  --ISNULL(B.ON_HAND,0) AS ON_HAND,
	...
	,NULL VAT_CODE
FROM
	[DIPROCOM].[SWIFT_SKU] AS [A]
INNER JOIN [DIPROCOM].[SWIFT_PARAMETER] [sp] ON (
											[sp].[GROUP_ID] = 'ERP_HARDCODE_VALUES'
											AND [sp].[PARAMETER_ID] = 'PRICE_LIST'
											)
LEFT JOIN [DIPROCOM].[SWIFT_SKU_SALE_PACK_UNIT] [SPU] ON ([A].[CODE_SKU] = [SPU].[CODE_SKU])
LEFT JOIN [DIPROCOM].[SONDA_PACK_UNIT] [SOPU] ON [SPU].[CODE_PACK_UNIT] = [SOPU].[CODE_PACK_UNIT];
...
Begin DesignProperties = 
   ...
   Begin DiagramPane = 
      ...
      End
      Begin Tables = 
         Begin Table = "SWIFT_SKU (DIPROCOM)"
            Begin Extent = 
               Top = 6
               Left = 38
               Bottom = 126
   ...
   End
End
', @level0type = N'SCHEMA', @level0name = N'DIPROCOM', @level1type = N'VIEW', @level1name = N'SWIFT_VIEW_ALL_SKU';


GO
EXECUTE sp_addextendedproperty @name = N'MS_DiagramPaneCount', @value = 1, @level0type = N'SCHEMA', @level0name = N'DIPROCOM', @level1type = N'VIEW', @level1name = N'SWIFT_VIEW_ALL_SKU';
