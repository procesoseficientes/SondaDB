...
-- Ejemplo de Ejecucion:
				...
				SELECT * FROM [acsa].[SWIFT_VIEW_ALL_SKU]
*/
-- =============================================
CREATE VIEW [acsa].[SWIFT_VIEW_ALL_SKU]
AS
SELECT
	[SKU]
  ...
  --0 AS ON_HAND,
	...
	,ERP.VAT_CODE
FROM
	[SWIFT_INTERFACES_QA].[acsa].[ERP_VIEW_SKU] [ERP]
LEFT JOIN [acsa].[SWIFT_SKU_SALE_PACK_UNIT] [PU] ON ([ERP].[CODE_SKU] = [PU].[CODE_SKU])
LEFT JOIN [acsa].[SONDA_PACK_UNIT] [ERPSPU] ON [PU].[CODE_PACK_UNIT] = [ERPSPU].[CODE_PACK_UNIT]
WHERE
