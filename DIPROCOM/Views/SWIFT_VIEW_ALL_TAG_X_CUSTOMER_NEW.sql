...
-- Ejemplo de Ejecucion:
        SELECT * FROM [DIPROCOM].SWIFT_VIEW_ALL_TAG_X_CUSTOMER_NEW
*/
-- =============================================
CREATE VIEW [DIPROCOM].[SWIFT_VIEW_ALL_TAG_X_CUSTOMER_NEW]
AS (
	SELECT
		[TAG_COLOR]
		,[CN].[CODE_CUSTOMER] [CUSTOMER] 
	FROM [DIPROCOM].[SONDA_TAG_X_CUSTOMER_NEW] [T]
		INNER JOIN [DIPROCOM].[SONDA_CUSTOMER_NEW] [CN] ON [CN].[CUSTOMER_ID] = [T].[CUSTOMER_ID]
	UNION ALL
	SELECT [TGN].[TAG_COLOR]
			,[TGN].[CUSTOMER] 
	FROM [DIPROCOM].[SWIFT_TAG_X_CUSTOMER] [TGN] 
)
