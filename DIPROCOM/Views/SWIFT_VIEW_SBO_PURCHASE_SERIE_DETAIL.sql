CREATE VIEW  [PACASA].[SWIFT_VIEW_SBO_PURCHASE_SERIE_DETAIL]  
AS 
...
FROM         (SELECT     t.TXN_ID, t.MANIFEST_SOURCE, t.SAP_REFERENCE, t.TASK_SOURCE_ID, t.TXN_TYPE, t.TXN_DESCRIPTION, t.TXN_CATEGORY, t.TXN_CREATED_STAMP, t.TXN_OPERATOR_ID, 
                       ...
                       FROM          [PACASA].SWIFT_TXNS AS t INNER JOIN
                                              [PACASA].SWIFT_TXNS_SERIES AS s ON t.TXN_ID = s.TXN_ID AND t.TXN_CODE_SKU = s.TXN_CODE_SKU) AS t LEFT OUTER JOIN
                      SWIFT_INTERFACES.[PACASA].[ERP_VIEW_PURCHASE_SERIE_DETAIL]  AS po ON t.TXN_CODE_SKU COLLATE SQL_Latin1_General_CP1_CI_AS = po.Item_Code AND t.SAP_REFERENCE = po.Doc_Entry
