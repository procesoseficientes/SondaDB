CREATE VIEW  [PACASA].[SWIFT_VIEW_SBO_PURCHASE_ORDER_HEADER] 
AS 
...
FROM         [PACASA].[SWIFT_TXNS] AS [t] INNER JOIN
                      [SWIFT_INTERFACES_QA].[PACASA].[ERP_VIEW_PURCHASE_ORDER_HEADER] AS [po] ON [po].[Doc_Entry] = [t].[SAP_REFERENCE]
