CREATE VIEW  [DIPROCOM].[SWIFT_VIEW_SBO_PURCHASE_ORDER_HEADER] 
AS 
SELECT     TOP (1) po.Doc_Entry AS Doc_Entry, po.Card_Code AS Card_Code, po.Card_Name AS Card_Name, 'N' AS Hand_Written, ISNULL(t.TXN_CREATED_STAMP, GETDATE()) AS Doc_Date, po.Comments, 
...
FROM         DIPROCOM.SWIFT_TXNS AS t INNER JOIN
                      SWIFT_INTERFACES.DIPROCOM.[ERP_VIEW_PURCHASE_ORDER_HEADER] AS po ON po.Doc_Entry = t.SAP_REFERENCE
