...
AS 
	SELECT * FROM OPENQUERY (DIPROCOM_SERVER,'
		...
		SELECT   
		    CAST(NULL AS varchar) AS SERIAL_NUMBER
			,RTRIM(RP.CODIGO_BODEGA) AS WAREHOUSE
			,RTRIM(RP.CODIGO_BODEGA) AS LOCATION
			,RTRIM(RP.CODIGO_PRODUCTO) AS SKU
			,RTRIM(RP.CODIGO_PRODUCTO) AS SKU_DESCRIPTION
			,SUM(ISNULL(RP.CANTIDAD,0))  AS CANTIDAD
			,NULL             AS BATCH_ID
			,GETDATE()        AS LAST_UPDATE
			,''BULK_DATA''    AS LAST_UPDATE_BY
			,9999             AS TXN_ID
			,0                AS IS_SCANNED
			,NULL             AS RELOCATED_DATE
			,RTRIM(RP.UM) COLLATE database_default AS CODE_PACK_UNIT_STOCK
		FROM [SONDA].[dbo].[REPLICA_BODEGA] RP
		GROUP BY CODIGO_BODEGA,CODIGO_PRODUCTO,UM,CANTIDAD
	')