...
-- Ejemplo de Ejecucion:
				SELECT * FROM [DIPROCOM].[SWIFT_VIEW_USERS] 
*/
-- =============================================


CREATE VIEW [DIPROCOM].[SWIFT_VIEW_USERS]
AS
SELECT
  A.[CORRELATIVE]
 ,A.[LOGIN]
...
FROM [DIPROCOM].[USERS] A
LEFT OUTER JOIN [DIPROCOM].[SWIFT_CLASSIFICATION] AS B
  ON A.[USER_TYPE] = B.NAME_CLASSIFICATION
LEFT OUTER JOIN [DIPROCOM].[SWIFT_VIEW_ALL_SELLERS] AS C
  ON A.RELATED_SELLER = C.SELLER_CODE
LEFT OUTER JOIN [DIPROCOM].[SWIFT_VIEW_WAREHOUSES] AS D
  ON A.DEFAULT_WAREHOUSE = D.CODE_WAREHOUSE
LEFT OUTER JOIN [DIPROCOM].[SWIFT_VIEW_WAREHOUSES] AS F
  ON A.PRESALE_WAREHOUSE = F.CODE_WAREHOUSE
LEFT OUTER JOIN [DIPROCOM].SWIFT_ROLE AS E
  ON A.USER_ROLE = E.ROLE_ID
LEFT JOIN DIPROCOM.SWIFT_ROUTES SR ON A.SELLER_ROUTE = SR.CODE_ROUTE
LEFT JOIN [DIPROCOM].[SWIFT_ZONE] [SZ] ON [A].[ZONE_ID] = [SZ].[ZONE_ID]
