...
-- Ejemplo de Ejecucion:
				SELECT * FROM [DIPROCOM].[ERP_VIEW_DOC_FOR_PICKING]
*/
-- =============================================
CREATE VIEW [DIPROCOM].[ERP_VIEW_DOC_FOR_PICKING]
AS
SELECT
  [SAP_REFERENCE]
...
'
  ...
  FROM bd_viscosa2008.dbo.ORDR o
  WHERE o.DocStatus = ''O''
  UNION ALL
  SELECT 
    o.DocNum SAP_REFERENCE
   ...
   ,NULL DOC_DATE
   ,0 TOTAL_AMOUNT
  FROM bd_viscosa2008.dbo.OWTQ o
  INNER JOIN bd_viscosa2008.dbo.OWHS o1
    ON o1.WhsCode = (SELECT TOP 1 w1.WhsCode FROM bd_viscosa2008.dbo.WTQ1 w1 where w1.DocEntry = o.DocEntry)
  WHERE o.DocStatus = ''O''
    ');