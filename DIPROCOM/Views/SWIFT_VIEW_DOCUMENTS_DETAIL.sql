...
-- Ejemplo de Ejecucion:
        SELECT * FROM [PACASA].[SWIFT_VIEW_DOCUMENTS_DETAIL]
*/
-- =============================================
CREATE VIEW [PACASA].[SWIFT_VIEW_DOCUMENTS_DETAIL]
AS
	SELECT 
		H.DOC_SERIE
	...
	INNER JOIN [PACASA].SONDA_SALES_ORDER_HEADER H ON (
		H.SALES_ORDER_ID = D.SALES_ORDER_ID
	)
	LEFT OUTER JOIN [PACASA].SWIFT_VIEW_ALL_SKU S ON (
		D.SKU = S.CODE_SKU COLLATE DATABASE_DEFAULT
	)
	...
	SELECT
		...
		,'INVOICE' AS [DOC_TYPE]
	FROM [PACASA].[SONDA_POS_INVOICE_DETAIL] AS D
	LEFT OUTER JOIN [PACASA].[SWIFT_VIEW_ALL_SKU] AS S ON (
		D.SKU = S.CODE_SKU COLLATE DATABASE_DEFAULT
	)
