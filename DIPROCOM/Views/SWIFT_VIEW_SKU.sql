...
-- Ejemplo de Ejecucion:
				SELECT * FROM [DIPROCOM].[SWIFT_VIEW_SKU]
*/
-- =============================================
CREATE VIEW [DIPROCOM].[SWIFT_VIEW_SKU]
AS
	SELECT
		-1 [SKU]
	...
	SELECT
		...
		,[DIPROCOM].[SWIFT_CLASSIFICATION].[NAME_CLASSIFICATION]
		,[DIPROCOM].[SWIFT_CLASSIFICATION].[VALUE_TEXT_CLASSIFICATION]
	...
	FROM [DIPROCOM].[SWIFT_SKU] AS [A]
	INNER JOIN [DIPROCOM].[SWIFT_FAMILY_SKU] AS [FU]
	ON	[FU].[CODE_FAMILY_SKU] = [A].[CODE_FAMILY_SKU]
	LEFT OUTER JOIN [DIPROCOM].[SWIFT_CLASSIFICATION]
	ON	[A].[CLASSIFICATION_SKU] = [DIPROCOM].[SWIFT_CLASSIFICATION].[CLASSIFICATION]
	LEFT OUTER JOIN [DIPROCOM].[SWIFT_VIEW_ALL_PROVIDERS] AS [B]
	ON	[A].[CODE_PROVIDER] COLLATE SQL_Latin1_General_CP1_CI_AS = [B].[PROVIDER]
	LEFT OUTER JOIN [DIPROCOM].[SWIFT_MEASURE_UNIT] AS [C]
	ON	[A].[MEASURE] = [C].[CODE]
	LEFT JOIN [DIPROCOM].[SWIFT_SKU_SALE_PACK_UNIT] [SPU]
	ON	([A].[CODE_SKU] = [SPU].[CODE_SKU])
	LEFT JOIN [DIPROCOM].[SONDA_PACK_UNIT] [SOPU]
