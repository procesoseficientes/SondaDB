...
-- Ejemplo de Ejecucion:
				SELECT * FROM [PACASA].[SWIFT_VIEW_SKU]
*/
-- =============================================
CREATE VIEW [PACASA].[SWIFT_VIEW_SKU]
AS
	SELECT
		-1 [SKU]
		...
		,0 [HANDLE_DIMENSION]
	FROM [SWIFT_INTERFACES_QA].[PACASA].[ERP_VIEW_SKU] [ERP]
	--INNER JOIN [PACASA].[SWIFT_PARAMETER] [sp]
	--ON	[LIST_NUM] = [sp].[VALUE]
	--	AND [sp].[GROUP_ID] = 'ERP_HARDCODE_VALUES'
	--	AND [sp].[PARAMETER_ID] = 'PRICE_LIST'
	LEFT JOIN [PACASA].[SWIFT_SKU_SALE_PACK_UNIT] [PU]
	ON	([ERP].[CODE_SKU] = [PU].[CODE_SKU])
	LEFT JOIN [PACASA].[SONDA_PACK_UNIT] [ERPSPU]
	...
	SELECT
		...
		,[PACASA].[SWIFT_CLASSIFICATION].[NAME_CLASSIFICATION]
		,[PACASA].[SWIFT_CLASSIFICATION].[VALUE_TEXT_CLASSIFICATION]
	...
	FROM [PACASA].[SWIFT_SKU] AS [A]
	INNER JOIN [PACASA].[SWIFT_FAMILY_SKU] AS [FU]
	ON	[FU].[CODE_FAMILY_SKU] = [A].[CODE_FAMILY_SKU]
	LEFT OUTER JOIN [PACASA].[SWIFT_CLASSIFICATION]
	ON	[A].[CLASSIFICATION_SKU] = [PACASA].[SWIFT_CLASSIFICATION].[CLASSIFICATION]
	LEFT OUTER JOIN [PACASA].[SWIFT_VIEW_ALL_PROVIDERS] AS [B]
	ON	[A].[CODE_PROVIDER] COLLATE SQL_Latin1_General_CP1_CI_AS = [B].[PROVIDER]
	LEFT OUTER JOIN [PACASA].[SWIFT_MEASURE_UNIT] AS [C]
	ON	[A].[MEASURE] = [C].[CODE]
	LEFT JOIN [PACASA].[SWIFT_SKU_SALE_PACK_UNIT] [SPU]
	ON	([A].[CODE_SKU] = [SPU].[CODE_SKU])
	LEFT JOIN [PACASA].[SONDA_PACK_UNIT] [SOPU]
...
Begin DesignProperties = 
   ...
   End
End
', @level0type = N'SCHEMA', @level0name = N'DIPROCOM', @level1type = N'VIEW', @level1name = N'SWIFT_VIEW_SKU';


GO
EXECUTE sp_addextendedproperty @name = N'MS_DiagramPaneCount', @value = 1, @level0type = N'SCHEMA', @level0name = N'DIPROCOM', @level1type = N'VIEW', @level1name = N'SWIFT_VIEW_SKU';
