...
CREATE  VIEW  [acsa].[SWIFT_VIEW_SBO_ORDER_SERIE_DETAIL]  
AS 
...
FROM         (SELECT     t.TXN_ID, t.MANIFEST_SOURCE, t.SAP_REFERENCE, t.TASK_SOURCE_ID, t.TXN_TYPE, t.TXN_DESCRIPTION, t.TXN_CATEGORY, t.TXN_CREATED_STAMP, t.TXN_OPERATOR_ID, 
                       ...
                       FROM          [acsa].SWIFT_TXNS AS t INNER JOIN
                                              [acsa].SWIFT_TXNS_SERIES AS s ON t.TXN_ID = s.TXN_ID AND t.TXN_CODE_SKU = s.TXN_CODE_SKU INNER JOIN
                                                  SWIFT_INTERFACES.[acsa].[ERP_VIEW_ORDER_SERIE_DETAIL] AS ss ON ss.MnfSerial = s.TXN_SERIE COLLATE SQL_Latin1_General_CP850_CI_AS AND 
