...
BEGIN
  BEGIN TRY
	...
	--						ELSE PIH.GPS_URL
	--						END
	--					FROM [SONDA].SONDA_POS_INVOICE_HEADER PIH
	--		LEFT JOIN [SONDA].SWIFT_VIEW_ALL_COSTUMER VAC ON PIH.CLIENT_ID = VAC.CODE_CUSTOMER
    ...
    BEGIN
	  ...
	  SELECT @ALTERNATIVEGPS = (
		SELECT TOP 1 PIH.GPS_URL FROM [SONDA].SONDA_POS_INVOICE_HEADER PIH 
			INNER JOIN @TASKS T 
	  ...
	  BEGIN
		SELECT @ALTERNATIVEGPS = (
				SELECT TOP 1 VAC.GPS FROM [SONDA].SWIFT_VIEW_ALL_COSTUMER VAC INNER JOIN @TASKS T ON VAC.CODE_CUSTOMER = T.CUSTOMER_CODE WHERE VAC.CODE_CUSTOMER = T.CUSTOMER_CODE
		)
	  END
	  ---------------------------------------------------------------------------------------------------------------------------------------------------------------
