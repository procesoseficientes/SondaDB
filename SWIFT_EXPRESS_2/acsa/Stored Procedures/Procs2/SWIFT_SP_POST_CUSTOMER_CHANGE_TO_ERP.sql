...
-- Description:    Inserta los Registros de Modificaciones de Clientes en la Base Intermedia de [SONDA]
...
BEGIN
	...
	BEGIN
		...
		BEGIN TRY
			...
			IF (SELECT COUNT(*) FROM [[SONDA]_SERVER].[SONDA].[dbo].[REPLICA_CLIENTE] WHERE [CODIGO_SONDA]=@CODE_CUSTOMER)>0
			BEGIN
				UPDATE [[SONDA]_SERVER].[SONDA].[dbo].[REPLICA_CLIENTE]
