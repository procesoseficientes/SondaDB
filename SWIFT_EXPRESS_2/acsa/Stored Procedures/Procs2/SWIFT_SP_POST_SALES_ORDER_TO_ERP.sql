...
-- EJEMPLO DE EJECUCION: 
		EXEC [SONDA].[SWIFT_SP_POST_SALES_ORDER_TO_ERP]

=========================================================*/
CREATE PROCEDURE [SONDA].[SWIFT_SP_POST_SALES_ORDER_TO_ERP]
AS
BEGIN
	SET NOCOUNT OFF;
	...
	BEGIN
		...
		BEGIN TRY
			...
			INSERT INTO [[SONDA]_SERVER].[SONDA].[dbo].[REPLICA_PEDIDO_ENC]
			(
				[NUMERO_DE_PEDIDO] ,
				[FECHA_PEDIDO] ,
				[CODIGO_DE_CLIENTE] ,
			...
			FROM [SONDA].[SONDA_SALES_ORDER_HEADER] [ssoh]
			INNER JOIN [SONDA].USERS US ON [ssoh].POSTED_BY = US.LOGIN
			left join [[SONDA]_SERVER].SONDA.dbo.vsVENDEDORES_PREVENTA vendedores on us.RELATED_SELLER=vendedores.codigo_vendedor
			WHERE [ssoh].[SALES_ORDER_ID] = @SALES_ORDER_ID

			INSERT INTO [[SONDA]_SERVER].SONDA.dbo.REPLICA_PEDIDO_DET
			( 
				[NUMERO_DE_PEDIDO] ,
				PRODUCT0 ,
				CODIGO_UNIDAD_VTA ,
			...
			SELECT
                ...
                [ssod].[CODE_PACK_UNIT] ,
                [ssod].QTY
			FROM [SONDA].[SONDA_SALES_ORDER_DETAIL] [ssod]
			WHERE [ssod].[SALES_ORDER_ID]=@SALES_ORDER_ID
				
			EXEC [SONDA].[SWIFT_SP-STATUS-SEND_SO_TO_SAP] @SALES_ORDER_ID = @SALES_ORDER_ID, -- int
			    @POSTED_RESPONSE = 'Proceso Exitoso', -- varchar(4000)
			    @ERP_REFERENCE = @SALES_ORDER_ID, -- varchar(256)
			    @OWNER = '[SONDA]', -- varchar(125)
			    @CUSTOMER_OWNER = '[SONDA]' -- varchar(125)
		...
		BEGIN CATCH
			...
			DELETE FROM [[SONDA]_SERVER].[SONDA].[dbo].[REPLICA_PEDIDO_DET]
			WHERE [NUMERO_DE_PEDIDO] = @SALES_ORDER_ID

			DELETE FROM [[SONDA]_SERVER].[SONDA].[dbo].[REPLICA_PEDIDO_ENC]
			WHERE [NUMERO_DE_PEDIDO] = @SALES_ORDER_ID

			EXEC [SONDA].[SWIFT_SP-STATUS-ERROR_SO_TO_SAP] @SALES_ORDER_ID = @SALES_ORDER_ID, -- int
