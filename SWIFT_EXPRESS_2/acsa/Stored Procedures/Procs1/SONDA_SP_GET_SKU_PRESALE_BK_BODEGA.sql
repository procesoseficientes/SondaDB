...
-- Ejemplo de Ejecucion:
				exec [SONDA].[SONDA_SP_GET_SKU_PRESALE] @WAREHOUSES = 'C002'
				  CODIGO BODEGA
...
*/
-- =============================================
CREATE PROCEDURE [SONDA].[SONDA_SP_GET_SKU_PRESALE_BK_BODEGA]
...
BEGIN
	...
	--
	--SELECT @ListaPrecios = [SONDA].[SWIFT_FN_GET_PRICE_LIST](DEFAULT)
	SELECT @ListaPrecios = [SONDA].[SWIFT_FN_GET_PRICE_LIST_BY_WH] (@WAREHOUSES)--(DEFAULT)
	--	
	SELECT 
		[WAREHOUSE]
		,I.[SKU]
	...
	LEFT JOIN [SONDA].SWIFT_VIEW_ALL_SKU S ON (I.SKU = S.CODE_SKU AND P.CODE_SKU = S.CODE_SKU)
