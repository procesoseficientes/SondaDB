...
*/
-- =============================================
CREATE PROCEDURE [SONDA].[SWIFT_SP_GET_GPS_ACTIVE_ROUTE] (@ASSIGNED_TO VARCHAR(50)
, @START_DATE DATE
, @END_DATE DATE)
AS
BEGIN
  ...
  INSERT INTO @SALES_ORDER_HEADER_TOTALS ([SALES_ORDER_ID], [TOTAL_AMOUNT], [QTY])
    SELECT
     ...
     ,CASE
      ...
      END AS [TOTAL_AMOUNT]
     ,SUM([SOD].[QTY]) AS [QTY]
    FROM [SONDA].[SONDA_SALES_ORDER_HEADER] [SOH] WITH (NOLOCK)
    INNER JOIN [SONDA].[SONDA_SALES_ORDER_DETAIL] [SOD] WITH (NOLOCK)
  ...
  INSERT INTO @TEMP_DOCUMENT_HEADER ([ID], [DOC_SERIAL], [ROUTE], [CLIENT], [CLIENT_ID], [ASSIGNED_TO], [TOTAL_AMOUNT], [TOTAL_AMOUNT_WHIT_DISCOUNT], [UNIDADES_VENDIDAS], [SIGNATURE], [IMAGE], [EXPECTED_GPS], [Latitude], [Longitude], [CREATED_DATESTAMP], [CDF_SERIE], [CDF_RESOLUCION], [STATUS], [CLOSED_ROUTE_DATETIME], [IMAGE_1], [IMAGE_2], [DOC_ID], [DOC_TYPE], [DOC_TYPE_DESCRIPTION], [SALES_ORDER_TYPE], [TASK_ID], [IS_POSTED_ERP], [ATTEMPTED_WITH_ERROR], [POSTED_RESPONSE], [POSTED_ERP], [POSTED_DATETIME], [LAST_UPDATE_IS_SENDING])
    ...
    INNER JOIN [SONDA].[SONDA_SALES_ORDER_HEADER] [SOH] WITH (NOLOCK)
  ...
  INSERT INTO @TEMP_DOCUMENT_HEADER ([ID], [DOC_SERIAL], [ROUTE], [CLIENT], [CLIENT_ID], [ASSIGNED_TO], [TOTAL_AMOUNT], [TOTAL_AMOUNT_WHIT_DISCOUNT], [UNIDADES_VENDIDAS], [SIGNATURE], [IMAGE], [EXPECTED_GPS], [Latitude], [Longitude], [CREATED_DATESTAMP], [CDF_SERIE], [CDF_RESOLUCION], [STATUS], [CLOSED_ROUTE_DATETIME], [IMAGE_1], [IMAGE_2], [DOC_ID], [DOC_TYPE], [DOC_TYPE_DESCRIPTION], [SALES_ORDER_TYPE], [TASK_ID], [IS_POSTED_ERP], [ATTEMPTED_WITH_ERROR], [POSTED_RESPONSE], [POSTED_ERP], [POSTED_DATETIME], [LAST_UPDATE_IS_SENDING], [TYPE_ACTION])
    SELECT
     ...
     ,'SALE' AS [TYPE_ACTION]
    FROM [SONDA].[SONDA_POS_INVOICE_HEADER] [IH] WITH (NOLOCK)
  ...
  SELECT DISTINCT
   ...
   ,(SELECT TOP 1
        SYMBOL_CURRENCY
      FROM [SONDA].SWIFT_CURRENCY
      WHERE IS_DEFAULT = 1)
    AS SYMBOL_CURRENCY
   ,CASE
      WHEN [TT].REASON IS NULL THEN 'NARANJA'
      ELSE (SELECT
            [SONDA].[SWIFT_FN_GET_KPI]([SONDA].SWIFT_CALCULATE_DISTANCE([TT].EXPECTED_GPS, [TT].POSTED_GPS, 'M'), @ASSIGNED_TO, 'SALES_DISTANCE', CASE
   ...
   ,(SELECT
    ...
    AS COLOR
   ,CASE
      WHEN [SONDA].SWIFT_CALCULATE_DISTANCE([TT].EXPECTED_GPS, [TT].POSTED_GPS, 'K') <= 0 THEN CONVERT(VARCHAR(50), 0) + 'km.'
      ELSE CASE
          WHEN [SONDA].SWIFT_CALCULATE_DISTANCE([TT].EXPECTED_GPS, [TT].POSTED_GPS, 'K') < 1 THEN CONVERT(VARCHAR(50), [SONDA].SWIFT_CALCULATE_DISTANCE([TT].EXPECTED_GPS, [TT].POSTED_GPS, 'M')) + 'm.'
          ELSE CONVERT(VARCHAR(50), [SONDA].SWIFT_CALCULATE_DISTANCE([TT].EXPECTED_GPS, [TT].POSTED_GPS, 'k')) + 'km.'
        END
    END AS DIFERENCE
