...
BEGIN
	...
	BEGIN
		...
		SET @BODY += '<p>En [SONDA] en el servidor ' + @@SERVERNAME + ' con la IP ' + CAST(CONNECTIONPROPERTY('local_net_address') AS VARCHAR) + ',' + CAST(CONNECTIONPROPERTY('local_tcp_port') AS VARCHAR) + ' se han encontrado las siguientes ordenes de venta incompletas:</p>'
		SET @BODY += '<table>'
		SET @BODY += '<tr>'
