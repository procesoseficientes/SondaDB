...
CREATE PROCEDURE [SONDA].[SONDA_SP_GET_TASKS_FOR_SALE_ROUTE_PLAN] @CODE_ROUTE VARCHAR(50)
AS
BEGIN
    SET NOCOUNT ON;
  ...
  FROM [SONDA].[SONDA_ROUTE_PLAN] [RP]
        ...
        LEFT JOIN [SONDA].[SWIFT_PRICE_LIST_BY_CUSTOMER_FOR_ROUTE] [LCR]
            ON [LCR].[CODE_CUSTOMER] = [C].[CODE_CUSTOMER]
        LEFT JOIN [$(SWIFT_INTERFACES)].[SONDA].[SWIFT_ERP_CUSTOMERS] [EC]
