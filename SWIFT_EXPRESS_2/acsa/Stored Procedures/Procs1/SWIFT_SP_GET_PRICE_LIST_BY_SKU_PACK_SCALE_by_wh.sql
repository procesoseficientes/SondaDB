...
-- Ejemplo de Ejecucion:
        EXEC [SONDA].[SWIFT_SP_GET_PRICE_LIST_BY_SKU_PACK_SCALE]
			@CODE_ROUTE = 'HUE0003@[SONDA]'
*/
-- =============================================
create PROCEDURE [SONDA].[SWIFT_SP_GET_PRICE_LIST_BY_SKU_PACK_SCALE_by_wh] (
	@CODE_ROUTE VARCHAR(50)
)
AS
BEGIN
		DECLARE @WAREHOUSES VARCHAR(50)
	SELECT @WAREHOUSES =PRESALE_WAREHOUSE FROM [SONDA].USERS
	...
	SELECT @PRICE_LIST_BY_WH = [SONDA].[SWIFT_FN_GET_PRICE_LIST_BY_WH](@WAREHOUSES)
	...
	SELECT DISTINCT
		...
		,PL.[HIGH_LIMIT]
		,PL.[PRICE]
	FROM [SONDA].[SWIFT_PRICE_LIST_BY_SKU_PACK_SCALE] AS PL
