...
-- Ejemplo de Ejecucion:
          ...
          EXECUTE @RC = [acsa].SWIFT_SP_GET_ERP_POD @RECEPTION_HEADER
          GO
*/

CREATE PROC [acsa].[SWIFT_SP_GET_ERP_POD] @RECEPTION_HEADER VARCHAR(50)
AS
  ...
  FROM [acsa].SWIFT_TXN_X_DOC_ERP stxde
  INNER JOIN [acsa].SWIFT_RECEPTION_HEADER srh
  ...
  FROM [acsa].SWIFT_RECEPTION_DETAIL srd
...
SELECT o.DocNum , r.LineNum , r.ObjType,r.ItemCode,r.DocEntry, r.OpenQty, r.WhsCode WarehouseCode FROM BD_Prueba_Viscosa.DBO.POR1 r  INNER JOIN BD_Prueba_Viscosa.dbo.OPOR o
  ...
  INSERT INTO #TMP_SWIFT_POD
    ...
    INNER JOIN [acsa].SWIFT_TXN_X_DOC_ERP stxde
