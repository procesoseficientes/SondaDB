...
/*
  EXEC [acsa].[SWIFT_SP_INSERT_CUSTOMER_CHANGE_TAG]
  	 @TAG_COLOR ='#123'
    ,@CUSTOMER = 1
	,@DEVICE_NETWORK_TYPE = '3G'
	,@IS_POSTED_OFFLINE = 0
	
    SELECT * FROM [acsa].SWIFT_TAG_X_CUSTOMER_CHANGE WHERE TAG_COLOR = '#123' AND CUSTOMER = 1
*/


-- =============================================

CREATE PROCEDURE [acsa].[SWIFT_SP_INSERT_CUSTOMER_CHANGE_TAG] 
    @TAG_COLOR VARCHAR (8)
    ,@CUSTOMER INT
...
BEGIN TRY
  ...
  INSERT INTO [acsa].SWIFT_TAG_X_CUSTOMER_CHANGE(
     TAG_COLOR
    ,CUSTOMER
	,[DEVICE_NETWORK_TYPE]
	,[IS_POSTED_OFFLINE]
