...
        --
        EXEC [acsa].[SWIFT_SP_GET_CUSTOMER_PERCENT_BY_POLYGON_TYPE]
			@POLYGON_TYPE = 'REGION'
		--
		EXEC [acsa].[SWIFT_SP_GET_CUSTOMER_PERCENT_BY_POLYGON_TYPE]
			@POLYGON_TYPE = 'SECTOR'
...
*/
-- =============================================
CREATE PROCEDURE [acsa].SWIFT_SP_GET_CUSTOMER_PERCENT_BY_POLYGON_TYPE (@POLYGON_TYPE VARCHAR(250)
...
BEGIN
  BEGIN TRY
    ...
    INSERT INTO @POLYGON
      ...
      LEFT JOIN [acsa].[SWIFT_POLYGON_BY_ROUTE] [PR]
        ON (
    ...
    SELECT
     ...
     ,[P].[COMMENT]
     ,[acsa].[SWIFT_FN_VALIDATE_POLYGON_HAS_CHILD]([A].[POLYGON_ID]) [HAS_CHILD]
     ,CASE CAST([acsa].[SWIFT_FN_VALIDATE_POLYGON_HAS_CHILD]([A].[POLYGON_ID]) AS VARCHAR)
        WHEN '1' THEN 'No'
        ELSE 'SI'
      END [HAS_CHILD_MESSAGE]
     ,[acsa].[SWIFT_FN_GET_TASK_TYPE_BY_POLYGON]([A].[POLYGON_ID]) [TYPE_TASK]
     ,[acsa].[SWIFT_FUNC_GET_TASK_TYPE_DESCRIPTION_BY_POLYGON]([A].[POLYGON_ID]) [TYPE_TASK_DESCRIPTION]
     ,[W].[CODE_WAREHOUSE]
    ...
    FROM @POLYGON_AVG [A]
    INNER JOIN [acsa].[SWIFT_POLYGON] [P]
      ON (
    ...
    LEFT JOIN [acsa].[SWIFT_WAREHOUSES] [W]
      ON (
    ...
    LEFT JOIN [acsa].[SWIFT_VIEW_ALL_SELLERS] [VS]
      ON (
