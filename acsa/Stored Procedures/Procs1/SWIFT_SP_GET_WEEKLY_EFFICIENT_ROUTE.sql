...
-- Ejemplo de Ejecucion:
				--
				EXEC [acsa].[SWIFT_SP_GET_WEEKLY_EFFICIENT_ROUTE]
					@STAR_DATE = '20160612'
					,@END_DATE = '20160618'
...
*/
-- =============================================
CREATE PROCEDURE [acsa].[SWIFT_SP_GET_WEEKLY_EFFICIENT_ROUTE] (
	@STAR_DATE DATETIME
	,@END_DATE DATETIME
...
BEGIN
	...
	/*FROM [acsa].[SWIFT_FN_SPLIT](@CODE_ROUTE,@DELIMITER) [S]
	INNER JOIN [acsa].[SWIFT_ROUTES] [R] ON (
	...
	SELECT
		...
		END [TOTAL_AMOUNT]
	INTO #SALES_ORDER
	FROM [acsa].[SONDA_SALES_ORDER_HEADER] [S]
	...
	SELECT
		...
		--END 
		,[S].[TOTAL_AMOUNT]
	FROM [acsa].[SONDA_POS_INVOICE_HEADER] [S]
	INNER JOIN [#ROUTE] [R] ON (
		[S].[POS_TERMINAL] = [R].[CODE_ROUTE] COLLATE DATABASE_DEFAULT
		AND CAST([S].[POSTED_DATETIME] AS DATE) = [R].[DATE]
	)
	...
	SELECT 
		...
		,[R].[CODE_ROUTE]
		,[R].[NAME_ROUTE]
		,CONVERT(DECIMAL(18,' + CAST(@DEFAULT_DISPLAY_DECIMALS AS VARCHAR) + '),[acsa].[SWIFT_FN_GET_DISPLAY_NUMBER]([R].[SUNDAY])) [SUNDAY]
		,CONVERT(DECIMAL(18,' + CAST(@DEFAULT_DISPLAY_DECIMALS AS VARCHAR) + '),[acsa].[SWIFT_FN_GET_DISPLAY_NUMBER]([R].[MONDAY])) [MONDAY]
		,CONVERT(DECIMAL(18,' + CAST(@DEFAULT_DISPLAY_DECIMALS AS VARCHAR) + '),[acsa].[SWIFT_FN_GET_DISPLAY_NUMBER]([R].[TUESDAY])) [TUESDAY]
		,CONVERT(DECIMAL(18,' + CAST(@DEFAULT_DISPLAY_DECIMALS AS VARCHAR) + '),[acsa].[SWIFT_FN_GET_DISPLAY_NUMBER]([R].[WEDNESDAY])) [WEDNESDAY]
		,CONVERT(DECIMAL(18,' + CAST(@DEFAULT_DISPLAY_DECIMALS AS VARCHAR) + '),[acsa].[SWIFT_FN_GET_DISPLAY_NUMBER]([R].[THURSDAY])) [THURSDAY]
		,CONVERT(DECIMAL(18,' + CAST(@DEFAULT_DISPLAY_DECIMALS AS VARCHAR) + '),[acsa].[SWIFT_FN_GET_DISPLAY_NUMBER]([R].[FRIDAY])) [FRIDAY]
		,CONVERT(DECIMAL(18,' + CAST(@DEFAULT_DISPLAY_DECIMALS AS VARCHAR) + '),[acsa].[SWIFT_FN_GET_DISPLAY_NUMBER]([R].[SATURDAY])) [SATURDAY]
		,CONVERT(DECIMAL(18,' + CAST(@DEFAULT_DISPLAY_DECIMALS AS VARCHAR) + '),[acsa].[SWIFT_FN_GET_DISPLAY_NUMBER](([R].[SUNDAY] + [R].[FRIDAY] + [R].[MONDAY] + [R].[SATURDAY] + [R].[THURSDAY] + [R].[TUESDAY] + [R].[WEDNESDAY]))) [TOTAL_AMOUNT] --,[acsa].[SWIFT_FN_GET_DISPLAY_NUMBER](([R].[SUNDAY] + [R].[FRIDAY] + [R].[MONDAY] + [R].[SATURDAY] + [R].[THURSDAY] + [R].[TUESDAY] + [R].[WEDNESDAY])) [TOTAL_AMOUNT]
	FROM [#RESULT] [R]
