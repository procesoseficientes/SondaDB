...
-- Ejemplo de Ejecucion:
				EXEC [acsa].[SONDA_SP_VALIDATE_INVOICE_IS_POSTED]
					@XML = '<?xml version="1.0"?>
<Data>
    <Invoices>
        <IdBo>417</IdBo>
...
*/
-- =============================================
CREATE PROCEDURE [acsa].[SONDA_SP_VALIDATE_INVOICE_IS_POSTED](
	@XML XML
	,@JSON VARCHAR(MAX)
)
AS
BEGIN
	...
	BEGIN
		...
		SELECT @INVOICE_XML = (
			...
			FOR XML PATH ('invoice'),ROOT('Data')
		)
		PRINT('3')
		--
		INSERT INTO @RESUTL_VALIDATION
		EXEC [acsa].[SONDA_SP_VALIDATED_IF_EXISTS_INVOICE] 
			...
			PRINT ' @DOC_RESOLUTION = ' +  '''' + @DOC_RESOLUTION + ''''
			PRINT ' @DOC_SERIE =  ' +   '''' + @DOC_SERIE + ''''
			PRINT ' @DOC_NUM = ' + '''' + CAST(@DOC_NUM AS VARCHAR) + ''''
			PRINT @POSTED_DATIME
			PRINT ' @DETAIL_QTY = ' +  '''' + CAST( @DETAIL_QTY AS VARCHAR) + ''''
			PRINT ' @DECREASE_INVENTORY = 1'
			PRINT ' @XML = ' + '''' + CAST(@INVOICE_XML AS VARCHAR(MAX)) + ''''
			PRINT ' @JSON = ' +	'''' +CAST(@JSON AS VARCHAR(MAX)) + ''''
