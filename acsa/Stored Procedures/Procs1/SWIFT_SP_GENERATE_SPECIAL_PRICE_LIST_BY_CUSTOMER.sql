...
-- Ejemplo de Ejecucion:
		-- 
		EXEC [acsa].[SWIFT_SP_GENERATE_SPECIAL_PRICE_LIST_BY_CUSTOMER]
		@CODE_ROUTE = '136'
*/
-- =============================================
CREATE PROCEDURE [acsa].[SWIFT_SP_GENERATE_SPECIAL_PRICE_LIST_BY_CUSTOMER]
	(
...
AS
	BEGIN
		...
		INNER JOIN [acsa].[SWIFT_SPECIAL_PRICE_LIST] [DL]
		ON	(
		...
		BEGIN
			...
			BEGIN
				...
				SELECT TOP 1
					@IS_ALREADY = 1
				FROM
					[acsa].[SWIFT_SPECIAL_PRICE_LIST_BY_CUSTOMER] [DLC]
				INNER JOIN [acsa].[SWIFT_SPECIAL_PRICE_LIST] [DL]
    ...
    --
		...
		INNER JOIN [acsa].[SWIFT_PROMO_SPECIAL_PRICE_LIST_BY_SCALE] [PDS]
