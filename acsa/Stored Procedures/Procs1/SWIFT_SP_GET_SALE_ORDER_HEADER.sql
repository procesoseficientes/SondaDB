...
-- Ejemplo de Ejecucion:
        ...
        GO
        --
        EXEC [acsa].[SWIFT_SP_GET_SALE_ORDER_HEADER]
			@idOrdenDeVenta = '5160'
*/
-- =============================================
CREATE PROCEDURE [acsa].[SWIFT_SP_GET_SALE_ORDER_HEADER] (
@idOrdenDeVenta INT
) AS
BEGIN
	SET NOCOUNT ON;
	...
	LEFT JOIN [acsa].[SWIFT_SELLER] [S] ON ([U].[RELATED_SELLER] = [S].[SELLER_CODE])
	LEFT JOIN [acsa].[SWIFT_SALES_OFFICE] [SO] ON ([SO].[SALES_OFFICE_ID] = [S].[SALES_OFFICE_ID])
	LEFT JOIN [acsa].[SWIFT_SALES_ORGANIZATION] [ORG] ON ([ORG].[SALES_ORGANIZATION_ID] = [SO].[SALES_ORGANIZATION_ID])
	WHERE
