...
-- Ejemplo de Ejecucion:
				EXEC [acsa].[SWIFT_SP_GENERATE_BONUS_MULTIPLE_BY_TRADE_AGREEMENT_FOR_DUPLICATE]
					@CODE_ROUTE = '44'
					,@ORDER = 1
*/
-- =============================================
CREATE PROCEDURE [acsa].[SWIFT_SP_GENERATE_BONUS_MULTIPLE_BY_TRADE_AGREEMENT_FOR_DUPLICATE] (@CODE_ROUTE VARCHAR(250)
, @ORDER INT)
AS
BEGIN
  SET NOCOUNT ON;
  ...
  /*IF @ORDER = 1
	BEGIN
		...
		-- ------------------------------------------------------------------------------------
		INSERT INTO [acsa].[SWIFT_BONUS_LIST]
				([NAME_BONUS_LIST])
		...
		-- ------------------------------------------------------------------------------------
		INSERT INTO [acsa].[SWIFT_BONUS_LIST_BY_CUSTOMER]
				(
					[BONUS_LIST_ID]
					,[CODE_CUSTOMER]
				)
  ...
  -- ------------------------------------------------------------------------------------
  INSERT INTO [acsa].[SWIFT_BONUS_LIST_BY_SKU_MULTIPLE] ([BONUS_LIST_ID]
  , [CODE_SKU]
  , [CODE_PACK_UNIT]
  , [MULTIPLE]
  , [CODE_SKU_BONUS]
  ...
  , [FREQUENCY])
    ...
    LEFT JOIN [acsa].[SWIFT_BONUS_LIST_BY_SKU_MULTIPLE] [BLBS]
      ON (
