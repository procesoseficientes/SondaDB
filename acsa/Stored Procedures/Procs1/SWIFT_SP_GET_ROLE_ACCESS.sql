...
*/
-- =============================================
CREATE PROCEDURE [acsa].[SWIFT_SP_GET_ROLE_ACCESS]
(@LOGIN_ID VARCHAR(50),@PRIVILEGE_ID VARCHAR(50))
AS
BEGIN

	SELECT COUNT(1) TIENE_ACCESO FROM [acsa].SWIFT_PRIVILEGES A
	INNER JOIN [acsa].SWIFT_PRIVILEGES_X_ROLE B
	ON A.ID=B.PRIVILEGE_ID
	INNER JOIN [acsa].USERS C
