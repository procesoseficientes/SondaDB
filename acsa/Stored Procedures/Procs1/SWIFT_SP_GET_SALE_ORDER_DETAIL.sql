...
EXECUTE @RC = [acsa].SWIFT_SP_GET_SALE_ORDER_DETAIL @SALES_ORDER_ID
GO

*/
-- =============================================  
CREATE PROCEDURE [acsa].[SWIFT_SP_GET_SALE_ORDER_DETAIL] (
	@SALES_ORDER_ID INT
	, @INTERFACE_OWNER VARCHAR(125) = 'acsa'
...
BEGIN
	...
	BEGIN	
		...
		FROM [acsa].[SONDA_SALES_ORDER_DETAIL] AS D
			INNER JOIN [acsa].SONDA_SALES_ORDER_HEADER ssoh ON (D.SALES_ORDER_ID = ssoh.SALES_ORDER_ID)
			INNER JOIN [acsa].SWIFT_VIEW_ALL_SKU svas ON(svas.CODE_SKU = D.SKU) 
			INNER JOIN [acsa].[SWIFT_SKU_INTERCOMPANY] [SKUI] ON [svas].[CODE_SKU] = [SKUI].[MASTER_ID] AND SKUI.[SOURCE]=@INTERFACE_OWNER
	...
	-- ------------------------------------------------------------------------------------
		...
		FROM [acsa].[SONDA_SALES_ORDER_DETAIL] AS D
			INNER JOIN [acsa].SONDA_SALES_ORDER_HEADER ssoh ON (D.SALES_ORDER_ID = ssoh.SALES_ORDER_ID)
			INNER JOIN [acsa].SWIFT_VIEW_ALL_SKU svas ON(svas.CODE_SKU = D.SKU) 
			INNER JOIN [acsa].[SWIFT_SKU_INTERCOMPANY] [SKUI] ON [svas].[CODE_SKU] = [SKUI].[MASTER_ID]
		WHERE D.SALES_ORDER_ID = @SALES_ORDER_ID
			AND SSOH.IS_READY_TO_SEND=1
			AND ISNULL([D].[IS_POSTED_ERP], 0) = 0
			AND (ISNULL([D].ATTEMPTED_WITH_ERROR,0) < CAST(@SHIPPING_ATTEMPTS AS INT))
