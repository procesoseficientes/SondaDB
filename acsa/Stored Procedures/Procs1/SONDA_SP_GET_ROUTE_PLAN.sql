...
-- Ejemplo de Ejecucion:
				exec [acsa].SONDA_SP_GET_ROUTE_PLAN @CODE_ROUTE = '101109'
  select * from [acsa].
*/
-- =============================================
CREATE PROCEDURE [acsa].[SONDA_SP_GET_ROUTE_PLAN] @CODE_ROUTE VARCHAR(50)
AS
BEGIN
  SET NOCOUNT ON;
  --
  ...
  FROM [acsa].[SONDA_ROUTE_PLAN] [RP]
    INNER JOIN [acsa].[SWIFT_VIEW_ALL_COSTUMER] [C] ON [C].[CODE_CUSTOMER] = [RP].[RELATED_CLIENT_CODE]
