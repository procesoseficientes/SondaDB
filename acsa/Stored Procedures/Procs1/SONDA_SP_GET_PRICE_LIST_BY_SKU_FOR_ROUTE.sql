...
-- EJEMPLO DE EJECUCION: 
		...
		EXEC [acsa].[SONDA_SP_GET_PRICE_LIST_BY_SKU_FOR_ROUTE]
			@CODE_ROUTE = '011202'

*/
-- =====================================================
CREATE PROCEDURE [acsa].[SONDA_SP_GET_PRICE_LIST_BY_SKU_FOR_ROUTE] (@CODE_ROUTE VARCHAR(50))
AS
BEGIN
	SET NOCOUNT ON;
	--
	...
	--
	SELECT 
		@DEFAULT_CODE_PRICE_LIST = [acsa].SWIFT_FN_GET_PARAMETER('ERP_HARDCODE_VALUES','PRICE_LIST')
		,@GENERATE_DEFAULT_PRICE_LIST = [acsa].SWIFT_FN_GET_PARAMETER('ERP_HARDCODE_VALUES','GENERATE_DEFAULT_PRICE_LIST')
	--
	SELECT DISTINCT
		splbc.CODE_PRICE_LIST
	INTO #PRICE_LIST
	FROM [acsa].[SONDA_ROUTE_PLAN] srp
	INNER JOIN [acsa].[SWIFT_PRICE_LIST_BY_CUSTOMER] splbc 
	...
	SELECT DISTINCT
		...
		,[PL].[CODE_PACK_UNIT]
		,[PL].[UM_ENTRY]
	FROM [acsa].[SWIFT_PRICE_LIST_BY_SKU] AS PL
