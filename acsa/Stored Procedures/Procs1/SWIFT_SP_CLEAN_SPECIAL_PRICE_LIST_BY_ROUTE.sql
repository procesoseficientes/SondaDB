...
-- Ejemplo de Ejecucion:
        EXEC [acsa].[SWIFT_SP_CLEAN_SPECIAL_PRICE_LIST_BY_ROUTE]
		@CODE_ROUTE = '136'
*/
-- =============================================
CREATE PROCEDURE [acsa].[SWIFT_SP_CLEAN_SPECIAL_PRICE_LIST_BY_ROUTE]
	(
...
AS
  ...
  --
		...
		FROM
			[acsa].[SWIFT_SPECIAL_PRICE_LIST] AS [SP]
		WHERE
		...
		DELETE
			[SPLBC]
		FROM
			[acsa].[SWIFT_SPECIAL_PRICE_LIST_BY_CUSTOMER] AS [SPLBC]
  ...
  --
		DELETE
			[SPBL]
		FROM
			[acsa].[SWIFT_SPECIAL_PRICE_LIST_BY_SCALE] AS [SPBL]
		INNER JOIN @SPECIAL_PRICE_LIST AS [SPL]
  ...
  --
		DELETE
			[SPL]
		FROM
			[acsa].[SWIFT_SPECIAL_PRICE_LIST] AS [SPL]
