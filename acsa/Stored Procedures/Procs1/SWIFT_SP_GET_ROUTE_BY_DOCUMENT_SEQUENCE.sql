...
-- TODO: Set parameter values here.
	...
	EXECUTE [acsa].[SWIFT_SP_GET_ROUTE_BY_DOCUMENT_SEQUENCE]
                @DOC_TYPE = 'DRAFT'
              ,@ASSIGNED_TO = 'RUDI@acsa'

				--				
*/
-- =============================================
CREATE PROCEDURE [acsa].[SWIFT_SP_GET_ROUTE_BY_DOCUMENT_SEQUENCE]
	 @DOC_TYPE VARCHAR(50)
...
BEGIN	
  SELECT 
		 VR.CODE_ROUTE
		,VR.NAME_ROUTE
	FROM [acsa].SWIFT_VIEW_ROUTES VR   
  LEFT JOIN [acsa].SWIFT_DOCUMENT_SEQUENCE PR ON (VR.CODE_ROUTE = PR.ASSIGNED_TO AND PR.DOC_TYPE = @DOC_TYPE)
