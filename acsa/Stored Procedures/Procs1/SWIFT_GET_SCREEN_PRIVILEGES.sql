...
-- Ejemplo de Ejecucion:
				EXEC [acsa].[SWIFT_GET_SCREEN_PRIVILEGES]
					@USER_LOGGED = 'gerente@acsa'
          ,@PARENT_PRIVILEGE_ID = 'btnBonusForPromo'
*/
-- =============================================

CREATE PROCEDURE [acsa].[SWIFT_GET_SCREEN_PRIVILEGES] (@USER_LOGGED VARCHAR(50)
...
AS
	...
	FROM
		[acsa].[USERS] [U]
	INNER JOIN [acsa].[SWIFT_ROLE] [R] ON ([R].[ROLE_ID] = [U].[USER_ROLE])
	INNER JOIN [acsa].[SWIFT_PRIVILEGES_X_ROLE] [PR] ON ([R].[ROLE_ID] = [PR].[ROLE_ID])
	INNER JOIN [acsa].[SWIFT_PRIVILEGES] [P] ON (
