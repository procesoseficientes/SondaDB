...
-- Ejemplo de Ejecucion:
		--
		EXEC [acsa].[SONDA_SP_INSERT_TRACEABILITY_CONSIGMENT]
			@CONSIGNMENT_ID = 26
			...
			,@POSTED_BY = 'RUDI@acsa'
			--
			SELECT * FROM [acsa].[SONDA_HISTORICAL_TRACEABILITY_CONSIGNMENT]
*/
-- =============================================
CREATE PROCEDURE [acsa].[SONDA_SP_INSERT_TRACEABILITY_CONSIGMENT]
(
	@CONSIGNMENT_ID INT
...
BEGIN
  ...
  SELECT @HANDLE_SERIAL = CASE WHEN @HANDLE_SERIAL IS NULL THEN 0 ELSE @HANDLE_SERIAL END
		...
		SELECT @EXIST = 1 FROM [acsa].[SONDA_HISTORICAL_TRACEABILITY_CONSIGNMENT] AS H
		...
		--
		IF(@EXIST = 0) BEGIN
			INSERT INTO [acsa].[SONDA_HISTORICAL_TRACEABILITY_CONSIGNMENT](
			[CONSIGNMENT_ID] 
			,[DOC_SERIE]
			,[DOC_NUM]
			,[CODE_SKU] 
