...
-- Ejemplo de Ejecucion:
				EXEC [acsa].[SWIFT_SP_ADD_MANUAL_FRECUENCY] 
					@TRADE_AGREEMENT_ID = 1
*/
-- =============================================
CREATE PROCEDURE [acsa].[SWIFT_SP_ADD_MANUAL_FRECUENCY] (@XML XML
...
BEGIN TRY
  ...
  -- ------------------------------------------------------
  MERGE [acsa].[SWIFT_ROUTE_BY_USER] AS [RU]
  USING @TABLE_SELLER [F]
  ...
  BEGIN
    ...
    FROM [acsa].[SWIFT_POLYGON_BY_ROUTE] AS [PBR]
    INNER JOIN [acsa].[SWIFT_FREQUENCY] AS [F]
    ...
    -- -----------------------------------------------
    INSERT INTO [acsa].[SWIFT_FREQUENCY] ([CODE_FREQUENCY]
    , [SUNDAY]
    , [MONDAY]
    , [TUESDAY]
    , [WEDNESDAY]
  ...
  BEGIN
    ...
    , [LAST_WEEK_VISITED])
      ...
      LEFT JOIN [acsa].[SWIFT_FREQUENCY_X_CUSTOMER] AS [FC]
