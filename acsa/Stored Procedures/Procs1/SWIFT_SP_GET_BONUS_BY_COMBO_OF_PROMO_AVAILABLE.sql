...
-- Ejemplo de Ejecucion:
			EXEC  [acsa].[SWIFT_SP_GET_BONUS_BY_COMBO_OF_PROMO_AVAILABLE] @PROMO_ID = 66
*/
-- =============================================
CREATE PROCEDURE [acsa].[SWIFT_SP_GET_BONUS_BY_COMBO_OF_PROMO_AVAILABLE] (@PROMO_ID INT = NULL)
AS
BEGIN
  SET NOCOUNT ON;
  --
  ...
  INNER JOIN [acsa].[SWIFT_SKU_BY_COMBO] [SC]
    ON ([C].[COMBO_ID] = [SC].[COMBO_ID])
  LEFT JOIN [acsa].[SWIFT_VIEW_COMBO_BY_PROMO] [VCBP]
