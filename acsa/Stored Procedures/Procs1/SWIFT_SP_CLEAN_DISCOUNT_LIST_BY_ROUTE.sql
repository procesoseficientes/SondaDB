...
-- Ejemplo de Ejecucion:
				-- 
				EXEC [acsa].[SWIFT_SP_CLEAN_DISCOUNT_LIST_BY_ROUTE]
					@CODE_ROUTE = '4'
*/
-- =============================================
CREATE PROCEDURE [acsa].[SWIFT_SP_CLEAN_DISCOUNT_LIST_BY_ROUTE] (@CODE_ROUTE VARCHAR(250)) WITH RECOMPILE
AS
BEGIN
  SET NOCOUNT ON;
  --
  ...
  INSERT INTO @DISCOUNT_LIST
    SELECT DISTINCT
      [D].[DISCOUNT_LIST_ID]
    FROM [acsa].[SWIFT_DISCOUNT_LIST] [D]
  ...
  -- ------------------------------------------------------------------------------------	
  DELETE [D]
    FROM [acsa].[SWIFT_DISCOUNT_LIST_BY_CUSTOMER] [D]
  ...
  --
  DELETE [D]
    FROM [acsa].[SWIFT_DISCOUNT_LIST_BY_SKU] [D]
    INNER JOIN @DISCOUNT_LIST [DL]
      ON (
      [D].[DISCOUNT_LIST_ID] = [DL].[DISCOUNT_LIST_ID]
      )
  ...
  --
  DELETE [D]
    FROM [acsa].[SWIFT_DISCOUNT_LIST_BY_GENERAL_AMOUNT] [D]
    INNER JOIN @DISCOUNT_LIST [DL]
      ON (
      [D].[DISCOUNT_LIST_ID] = [DL].[DISCOUNT_LIST_ID]
      )
  ...
  --
  DELETE [D]
    FROM [acsa].[SWIFT_DISCOUNT_LIST_BY_GENERAL_AMOUNT_AND_FAMILY] [D]
    INNER JOIN @DISCOUNT_LIST [DL]
      ON (
      [D].[DISCOUNT_LIST_ID] = [DL].[DISCOUNT_LIST_ID]
      )
  WHERE [DL].[DISCOUNT_LIST_ID] > 0
    --
  DELETE [D]
    FROM [acsa].[SWIFT_DISCOUNT_LIST_BY_PAYMENT_TYPE_AND_FAMILY] [D]
    INNER JOIN @DISCOUNT_LIST [DL]
      ON (
      [D].[DISCOUNT_LIST_ID] = [DL].[DISCOUNT_LIST_ID]
      )
  ...
  DELETE [D]
    FROM [acsa].[SWIFT_DISCOUNT_LIST_BY_FAMILY_AND_PAYMENT_TYPE_BY_CUSTOMER] AS [D]
        INNER JOIN [acsa].[SWIFT_VIEW_ALL_COSTUMER] AS [vac]
