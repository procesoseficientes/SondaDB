...
-- Ejemplo de Ejecucion:
				SELECT * FROM [acsa].[SONDA_SALES_ORDER_HEADER] WHERE [SALES_ORDER_ID] = 831
				--
				EXEC [acsa].[SWIFT_SP_AUTHORIZE_SALES_ORDER]
					@SALES_ORDER_ID = 831
					,@LOGIN = 'gerente@acsa'
				--
				SELECT * FROM [acsa].[SONDA_SALES_ORDER_HEADER] WHERE [SALES_ORDER_ID] = 831
*/
-- =============================================
CREATE PROCEDURE [acsa].[SWIFT_SP_AUTHORIZE_SALES_ORDER](
	@SALES_ORDER_ID INT
	,@LOGIN VARCHAR(50)
)
AS
