...
-- Ejemplo de Ejecucion:
        exec [acsa].[SWIFT_SP_GET_CUSTUMER_FOR_SCOUTING_TEST] @CODE_ROUTE = 'es000128'
*/
-- =============================================
CREATE PROCEDURE [acsa].[SWIFT_SP_GET_CUSTUMER_FOR_SCOUTING_TEST] (
...
--
	...
	SELECT DISTINCT
		...
		,[acsa].[SONDA_FN_GET_LAST_PURCHASE_FOR_CUSTOMER] ([C].[CODE_CUSTOMER]) AS LAST_PURCHASE
		,[C].[INVOICE_NAME]
	FROM [acsa].[SWIFT_VIEW_ALL_COSTUMER] [C]
	INNER JOIN [acsa].[USERS] [U] ON (
	      [C].[SELLER_DEFAULT_CODE] = [U].[RELATED_SELLER]
	)
	LEFT JOIN [acsa].[SWIFT_DISCOUNT_LIST_BY_CUSTOMER] [DLC] ON (
		[DLC].[CODE_CUSTOMER] = [C].[CODE_CUSTOMER]
	)
	LEFT JOIN [acsa].[SWIFT_BONUS_LIST_BY_CUSTOMER] [BLC] ON (
		[BLC].[CODE_CUSTOMER] = [C].[CODE_CUSTOMER]
	)
	LEFT JOIN [acsa].[SWIFT_PRICE_LIST_BY_CUSTOMER_FOR_ROUTE] [PLC] ON (
		[PLC].[CODE_CUSTOMER] = [C].[CODE_CUSTOMER]
		AND [PLC].[CODE_ROUTE] = @CODE_ROUTE
	)
	LEFT JOIN [acsa].[SWIFT_SKU_SALES_BY_MULTIPLE_LIST_BY_CUSTOMER] [SMC] ON (
	...
	SELECT DISTINCT
		...
		,[acsa].[SONDA_FN_GET_LAST_PURCHASE_FOR_CUSTOMER] ([VAC].[CODE_CUSTOMER]) AS LAST_PURCHASE
		,[VAC].[INVOICE_NAME]
	FROM [acsa].[SWIFT_VIEW_ALL_COSTUMER] [VAC]
	INNER JOIN [acsa].[SONDA_ROUTE_PLAN] [RP] ON (
