CREATE PROCEDURE [acsa].[SWIFT_GET_ALL_TASKS_TODAY]
AS
...
,A.[COMPLETED_STAMP]
FROM [acsa].SWIFT_TASKS AS A

--,[acsa].SWIFT_RECEPTION_HEADER AS B,[acsa].SWIFT_PICKING_HEADER AS C,[acsa].SWIFT_RECEPTION_DETAIL AS D,[acsa].SWIFT_PICKING_DETAIL AS E

LEFT OUTER JOIN
[acsa].SWIFT_RECEPTION_HEADER AS B ON A.RECEPTION_NUMBER = B.RECEPTION_HEADER 
LEFT OUTER JOIN
[acsa].SWIFT_PICKING_HEADER AS C ON A.PICKING_NUMBER = C.PICKING_HEADER
LEFT OUTER JOIN
[acsa].SWIFT_RECEPTION_DETAIL AS D ON A.RECEPTION_NUMBER = D.RECEPTION_HEADER
LEFT OUTER JOIN
[acsa].SWIFT_PICKING_DETAIL AS E ON A.PICKING_NUMBER = E.PICKING_HEADER
LEFT OUTER JOIN
[acsa].[SWIFT_CYCLE_COUNT_HEADER] AS F ON A.COUNT_ID = F.COUNT_ID
LEFT OUTER JOIN
[acsa].[SWIFT_CYCLE_COUNT_DETAIL] AS G ON A.COUNT_ID = G.COUNT_ID
