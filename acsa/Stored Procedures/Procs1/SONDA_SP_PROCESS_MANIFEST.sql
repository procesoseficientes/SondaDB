...
-- Ejemplo de Ejecucion:
				EXEC [acsa].[SONDA_SP_PROCESS_MANIFEST]
				@MANIFES_HEADER_ID = 1108
				, @LOGIN_ID = 'ADOLFO@acsa'
				, @CURRENT_GPS_USER = '14.64986000,-90.53980000'
*/
-- =============================================
CREATE PROCEDURE [acsa].[SONDA_SP_PROCESS_MANIFEST](
...
BEGIN
	...
	BEGIN TRY
		...
		-- -----------------------------------------------------------------------------------------
		EXEC [acsa].[SONDA_SP_VALIDATE_IF_EXIST_3PL_MANIFEST] @MANIFEST_HEADER_ID = @MANIFES_HEADER_ID,@LOGIN_ID = @LOGIN_ID -- int

		-- -----------------------------------------------------------------------------------------
		-- Se manda a validar el ESTADO del manifiesto escaneado
		-- -----------------------------------------------------------------------------------------
		EXEC [acsa].[SWIFT_SP_VALIDATE_MANIFEST_3PL] @MANIFEST_HEADER_ID = @MANIFES_HEADER_ID -- int

		-- -----------------------------------------------------------------------------------------
		-- Se manda a generar el listado de tareas del manifiesto si la validacion fue exitosa
		-- -----------------------------------------------------------------------------------------
		EXEC [acsa].[SWIFT_SP_GENERATE_DELIVERY_PLAN_FROM_MANIFEST_3PL] 
