...
*/
-- =============================================
CREATE PROCEDURE [acsa].SWIFT_SP_GET_DETAIL_SALES_ORDER_FOR_PICKING(
...
BEGIN
  ...
  INTO #SALES_ORDER
  FROM [acsa].[SWIFT_FN_SPLIT](@SALES_ORDERS_IDS, @DELIMITER) S
  ...
  INNER JOIN [acsa].SONDA_SALES_ORDER_HEADER SO ON(
    SO.SALES_ORDER_ID = SOT.SALES_ORDER_ID
  )
  INNER JOIN [acsa].SWIFT_VIEW_ALL_SKU VS ON (
    VS.CODE_SKU = SOD.SKU
  )
  INNER JOIN [acsa].SWIFT_VIEW_ALL_COSTUMER VC ON(
