...
-- Ejemplo de Ejecucion:
				EXEC [acsa].[SWIFT_SP_GENERATE_SKU_SALES_BY_MULTIPLE_BY_TRADE_AGREEMENT_FOR_DUPLICATE]
					@CODE_ROUTE = '44'
					,@ORDER = 1
*/
-- =============================================
CREATE PROCEDURE [acsa].[SWIFT_SP_GENERATE_SKU_SALES_BY_MULTIPLE_BY_TRADE_AGREEMENT_FOR_DUPLICATE] (@CODE_ROUTE VARCHAR(50)
, @ORDER INT)
AS
BEGIN
  SET NOCOUNT ON;
  ...
  BEGIN
    ...
    -- ------------------------------------------------------------------------------------
    INSERT INTO [acsa].[SWIFT_SKU_SALES_BY_MULTIPLE_LIST_BY_CUSTOMER] ([SALES_BY_MULTIPLE_LIST_ID]
    , [CODE_CUSTOMER])
      SELECT DISTINCT
        [SM].[SALES_BY_MULTIPLE_LIST_ID]
       ,[TA].[CODE_CUSTOMER]
      FROM @TRADE_AGREEMENT [TA]
      INNER JOIN [acsa].[SWIFT_SKU_SALES_BY_MULTIPLE_LIST] [SM]
        ON (
  ...
  -- ------------------------------------------------------------------------------------
  INSERT INTO [acsa].[SWIFT_SKU_SALES_BY_MULTIPLE_LIST_BY_SKU] ([SALES_BY_MULTIPLE_LIST_ID]
  , [CODE_SKU]
  , [CODE_PACK_UNIT]
  , [MULTIPLE]
  , [PROMO_ID]
  ...
  , [FREQUENCY])
    ...
    INNER JOIN [acsa].[SWIFT_PROMO_SKU_SALES_BY_MULTIPLE] [TAS]
      ON (
    ...
    LEFT JOIN [acsa].[SWIFT_SKU_SALES_BY_MULTIPLE_LIST_BY_SKU] [SMLS]
      ON (
