CREATE PROCEDURE [acsa].[SWIFT_GET_INCOME_PANEL_RECEPTION]
@INCOMEHEADER INT
AS
SELECT DISTINCT
	...
	, D.TASK_ID
FROM
	[acsa].SWIFT_RECEPTION_HEADER AS A
	left JOIN [acsa].SWIFT_VIEW_ALL_PROVIDERS B on(A.CODE_PROVIDER = B.CODE_PROVIDER)
	left JOIN [acsa].SWIFT_VIEW_ALL_COSTUMER C on(A.CODE_PROVIDER = C.CODE_CUSTOMER)
	left JOIN [acsa].SWIFT_TASKS D on(A.RECEPTION_HEADER = D.RECEPTION_NUMBER)
...
z.SCANNED 
FROM [acsa].SWIFT_RECEPTION_DETAIL Z
