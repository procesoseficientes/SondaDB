...
Ejemplo de Ejecución:
	...
	EXEC [acsa].[SWIFT_SP_GET_CUSTOMER_NOT_IN_TRADE_AGREEMENT] @TRADE_AGREEMENT_ID = 30 , @CODE_CUSTOMER = 'V1'

==================================================*/

CREATE PROCEDURE [acsa].[SWIFT_SP_GET_CUSTOMER_NOT_IN_TRADE_AGREEMENT] (
...
BEGIN
	SELECT
		...
		,CASE ISNULL([TA].[TRADE_AGREEMENT_ID], 1)
			...
			END AS [ASOCIADO]
	FROM
		[acsa].[SWIFT_VIEW_ALL_COSTUMER] AS [VAC]
	LEFT JOIN [acsa].[SWIFT_TRADE_AGREEMENT_BY_CUSTOMER] AS [TA] ON ([VAC].[CODE_CUSTOMER] = [TA].[CODE_CUSTOMER])
	LEFT JOIN [acsa].[SWIFT_SELLER] [svas] ON ([svas].[SELLER_CODE] = [VAC].[SELLER_DEFAULT_CODE])
	LEFT JOIN [acsa].[SWIFT_CHANNEL_X_CUSTOMER] [scxc] ON ([VAC].[CODE_CUSTOMER] = [scxc].[CODE_CUSTOMER])
