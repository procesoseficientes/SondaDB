...
-- Ejemplo de Ejecucion:
				EXEC [acsa].[SWIFT_SP_GET_PORTFOLIO_SKU_AVAILABLE]  @CODE_PORTFOLIO = 5
*/
-- =============================================
CREATE PROC [acsa].SWIFT_SP_GET_PORTFOLIO_SKU_AVAILABLE
	@CODE_PORTFOLIO VARCHAR(50)
AS
	SELECT *
	FROM [acsa].SWIFT_VIEW_ALL_SKU S
	WHERE NOT EXISTS(SELECT 1
					FROM [acsa].SWIFT_VIEW_PORTFOLIO_BY_SKU PS
