...
-- Ejemplo de Ejecucion:
				EXEC [acsa].[SWIFT_SP_GENERATE_BONUS_COMBO_BY_CHANNEL_FOR_DUPLICATE]
					@CODE_ROUTE = '44'
					,@ORDER = 2
*/
-- =============================================
CREATE PROCEDURE [acsa].[SWIFT_SP_GENERATE_BONUS_COMBO_BY_CHANNEL_FOR_DUPLICATE] (@CODE_ROUTE VARCHAR(250)
, @ORDER INT)
AS
BEGIN
  SET NOCOUNT ON;
  ...
  , [FREQUENCY]  
    ...
    LEFT JOIN [acsa].[SWIFT_BONUS_LIST_BY_COMBO] [BLC]
      ON (
  ...
  , [IS_MULTIPLE])
    ...
    LEFT JOIN [acsa].[SWIFT_BONUS_LIST_BY_COMBO_SKU] [BLC]
      ON (
      [BLC].[BONUS_LIST_ID] = [BL].[BONUS_LIST_ID]
      AND [BLC].[COMBO_ID] = [TAR].[COMBO_ID]
