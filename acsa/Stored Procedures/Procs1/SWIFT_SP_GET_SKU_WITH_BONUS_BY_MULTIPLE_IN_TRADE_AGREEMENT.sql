...
-- Ejemplo de Ejecucion:
				EXEC [acsa].[SWIFT_SP_GET_SKU_WITH_BONUS_BY_MULTIPLE_IN_TRADE_AGREEMENT]
					@TRADE_AGREEMENT_ID = 20
*/
-- =============================================
CREATE PROCEDURE [acsa].SWIFT_SP_GET_SKU_WITH_BONUS_BY_MULTIPLE_IN_TRADE_AGREEMENT(
...
BEGIN
	...
	SELECT
		...
		,[M].[BONUS_QTY]
	FROM [acsa].[SWIFT_TRADE_AGREEMENT_BONUS_BY_MULTIPLE] [M]
	INNER JOIN [acsa].[SWIFT_VIEW_ALL_SKU] [S1] ON (
		[S1].[CODE_SKU] = [M].[CODE_SKU]
	)
	INNER JOIN [acsa].[SWIFT_VIEW_ALL_SKU] [S2] ON (
