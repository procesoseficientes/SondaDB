...
-- EJEMPLO DE EJECUCION: 
		EXEC [acsa].[SWIFT_SP_GET_USER_PRESALE_INFO]
			@LOGIN = 'gerente@acsa'

*/		
-- =============================================
CREATE  PROCEDURE [acsa].[SWIFT_SP_GET_USER_PRESALE_INFO]
		@LOGIN VARCHAR(50)
AS
BEGIN
	SET NOCOUNT ON;
	...
	SELECT
		...
		,CASE
			WHEN ((CONVERT(NUMERIC(18,6),[acsa].[SWIFT_FN_GET_PARAMETER]('SALES_ORDER', 'DOC_SEQUENCE_ALERT')) * [D].[DOC_TO]) / 100) >= ([D].[DOC_TO] - [D].[CURRENT_DOC])
				THEN 1
			ELSE 0
		END [HAVE_ALERT]
	FROM [acsa].[USERS] [U]
	LEFT JOIN [acsa].[SWIFT_DOCUMENT_SEQUENCE] [D] ON (
