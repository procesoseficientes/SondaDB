...
-- Ejemplo de Ejecucion:
				-- 
  SELECT * FROM [acsa].[SONDA_SALES_ORDER_LOG_EXISTS]
...
<Data>
    <salesOrder>
        ...
        <Status>0</Status>
        <PostedBy>Alberto@acsa</PostedBy>
        ...
        <IsParent>1</IsParent>
        <ReferenceId>Alberto@acsa2017/03/16 08:43:55-2</ReferenceId>
    ...
    <dbuser>Uacsa</dbuser>
    <dbuserpass>acsaServer1237710</dbuserpass>
    <battery>23</battery>
    <routeid>6</routeid>
...
*/
-- =============================================
CREATE PROCEDURE [acsa].SONDA_SP_VALIDATE_SALE_ORDER_IS_POSTED (@SALES_ORDER_ID INT
...
BEGIN
  ...
  BEGIN TRY
    ...
    FROM [acsa].[SONDA_SALES_ORDER_HEADER] [H] /*WITH(ROWLOCK,XLOCK,HOLDLOCK)*/
    ...
    FROM [acsa].[SONDA_SALES_ORDER_HEADER] [H] /*WITH(ROWLOCK,XLOCK,HOLDLOCK)*/
    INNER JOIN [acsa].[SONDA_SALES_ORDER_DETAIL] [D]
      ON (
    ...
    BEGIN
      ...
      SET DOC_NUM = NEXT VALUE FOR [acsa].SALES_ORDER_NEGATIVE_SEQUENCE
      ...
      SELECT
       ...
       ,@INSERT = 1
      --
      INSERT INTO [acsa].[SONDA_SALES_ORDER_LOG_EXISTS] ([LOG_DATETIME]
      , [EXISTS_SALES_ORDER]
      , [DOC_SERIE]
      , [DOC_NUM]
      , [CODE_ROUTE]
