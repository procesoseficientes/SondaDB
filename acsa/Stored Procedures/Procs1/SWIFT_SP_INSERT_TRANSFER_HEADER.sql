...
CREATE PROC [acsa].[SWIFT_SP_INSERT_TRANSFER_HEADER]
...
AS
	BEGIN TRY
		DECLARE
			@return_value INT
			,@pID NUMERIC(18 ,0);
		EXEC @return_value = [acsa].[SWIFT_SP_GET_NEXT_SEQUENCE] @SEQUENCE_NAME = N'TRANSFER' ,
			@pResult = @pID OUTPUT;
			
		INSERT	INTO [acsa].[SWIFT_TRANSFER_HEADER]
				(
					[TRANSFER_ID]
					,[SELLER_CODE]
					,[SELLER_ROUTE]
