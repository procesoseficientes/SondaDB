...
-- Ejemplo de Ejecucion:
				-- 
				EXEC [acsa].[SONDA_SP_GENERATE_BONUS_FROM_TRADE_AGREEMENT]
					@CODE_ROUTE = 'RUDI@acsa'
*/
-- =============================================
CREATE PROCEDURE [acsa].[SONDA_SP_GENERATE_BONUS_FROM_TRADE_AGREEMENT]
(
...
BEGIN
	...
	-- ------------------------------------------------------------------------------------	
	DELETE BL
	FROM [acsa].SWIFT_BONUS_LIST_BY_CUSTOMER BL
	INNER JOIN #BONUS B ON (
		BL.BONUS_LIST_ID = B.BONUS_LIST_ID
	)
	--
	DELETE BL 
	FROM [acsa].SWIFT_BONUS_LIST_BY_SKU BL
	INNER JOIN #BONUS B ON (
		BL.BONUS_LIST_ID = B.BONUS_LIST_ID
	)
	--
	...
	INNER JOIN [acsa].SWIFT_TRADE_AGREEMENT_BY_CUSTOMER TAC ON(
		C.CODE_CUSTOMER = TAC.CODE_CUSTOMER
	)
	INNER JOIN [acsa].SWIFT_TRADE_AGREEMENT TA ON (
	...
	SELECT
		@CODE_ROUTE + '|' + TA.CODE_TRADE_AGREEMENT AS NAME_BONUS_LIST
	FROM [acsa].SWIFT_VIEW_ALL_COSTUMER C
	INNER JOIN [acsa].SWIFT_CHANNEL_X_CUSTOMER CC ON(
		C.CODE_CUSTOMER = CC.CODE_CUSTOMER
	)
	INNER JOIN [acsa].SWIFT_TRADE_AGREEMENT_BY_CHANNEL TAC ON (
	...
	INNER JOIN [acsa].SWIFT_BONUS_LIST DL ON (
	...
	-- ------------------------------------------------------------------------------------
	INSERT INTO [acsa].SWIFT_BONUS_LIST_BY_SKU
	SELECT DISTINCT
		BL.BONUS_LIST_ID
		,TAB.CODE_SKU
		,TAB.PACK_UNIT
		...
		,TAB.PACK_UNIT_BONUS
	FROM #TRADE_AGREEMENT_BY_CHANNEL TA
	INNER JOIN [acsa].SWIFT_TRADE_AGREEMENT_BONUS TAB ON (
	...
	INNER JOIN [acsa].SONDA_PACK_UNIT PU1 ON (
		TAB.PACK_UNIT = PU1.PACK_UNIT
	)
	INNER JOIN [acsa].SONDA_PACK_UNIT PU2 ON (
