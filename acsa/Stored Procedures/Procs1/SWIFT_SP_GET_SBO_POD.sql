...
(
	...
	from [acsa].SWIFT_RECEPTION_HEADER h
		inner join [acsa].SWIFT_RECEPTION_DETAIL d
		on  h.RECEPTION_HEADER = d.RECEPTION_HEADER 
		inner join (
			select top 1  t.TASK_SOURCE_id TXN_ID, t.TXN_TYPE, t.TXN_IS_POSTED_ERP, tsk.RECEPTION_NUMBER  from [acsa].SWIFT_TXNS t 
				inner join [acsa].SWIFT_TASKS  tsk 
