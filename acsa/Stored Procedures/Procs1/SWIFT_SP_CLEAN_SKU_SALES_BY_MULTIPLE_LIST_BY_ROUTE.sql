...
*/
-- =============================================
CREATE PROCEDURE [acsa].[SWIFT_SP_CLEAN_SKU_SALES_BY_MULTIPLE_LIST_BY_ROUTE] (@CODE_ROUTE VARCHAR(50))
AS
BEGIN
  SET NOCOUNT ON;
  --
  ...
  INSERT INTO @SALES_BY_MULTIPLE
    ...
    FROM [acsa].[SWIFT_SKU_SALES_BY_MULTIPLE_LIST] [S]
  ...
  -- ------------------------------------------------------------------------------------	
  DELETE [S]
    FROM [acsa].[SWIFT_SKU_SALES_BY_MULTIPLE_LIST_BY_CUSTOMER] [S]
  ...
  --
  DELETE [S]
    FROM [acsa].[SWIFT_SKU_SALES_BY_MULTIPLE_LIST_BY_SKU] [S]
    INNER JOIN @SALES_BY_MULTIPLE [SL]
      ON (
      [S].[SALES_BY_MULTIPLE_LIST_ID] = [SL].[SALES_BY_MULTIPLE_LIST_ID]
      )
