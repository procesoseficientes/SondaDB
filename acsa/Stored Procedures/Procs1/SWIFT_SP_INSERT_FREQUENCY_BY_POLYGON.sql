...
-- Ejemplo de Ejecucion:
				EXEC [acsa].[SWIFT_SP_INSERT_FREQUENCY_BY_POLYGON]
					...
					,@LAST_UPDATED_BY = 'generente@acsa'
					,@CODE_ROUTE = 'RUDI@acsa'
					,@TYPE_TASK = 'PRESALE'
					,@POLYGON_ID = 66
				-- 
				SELECT * FROM [acsa].[SWIFT_FREQUENCY] WHERE CODE_ROUTE= 'RUDI@acsa' AND TYPE_TASK = 'PRESALE'
*/
-- =============================================
CREATE PROCEDURE [acsa].[SWIFT_SP_INSERT_FREQUENCY_BY_POLYGON] (
	@SUNDAY AS INT
	,@MONDAY AS INT
	,@TUESDAY AS INT
	,@WEDNESDAY AS INT
...
BEGIN
	...
	BEGIN TRY
		...
		-- ------------------------------------------------------------------------------------
		DELETE FROM [acsa].[SWIFT_FREQUENCY_BY_POLYGON] WHERE [POLYGON_ID] = @POLYGON_ID
		--
		INSERT INTO [acsa].[SWIFT_FREQUENCY_BY_POLYGON]
				([POLYGON_ID] 
				,[ID_FREQUENCY])
		SELECT DISTINCT
			@POLYGON_ID
