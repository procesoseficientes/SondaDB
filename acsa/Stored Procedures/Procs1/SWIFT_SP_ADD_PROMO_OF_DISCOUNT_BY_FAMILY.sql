...
*/
-- =============================================
CREATE PROCEDURE [acsa].[SWIFT_SP_ADD_PROMO_OF_DISCOUNT_BY_FAMILY] (@PROMO_ID INT
, @XML XML
, @LOGIN_ID VARCHAR)
AS
BEGIN
  BEGIN TRY
    ...
    INSERT INTO @TABLE_RESULT ([Resultado], [Mensaje])
      ...
      FROM [acsa].[SWIFT_PROMO_DISCOUNT_BY_FAMILY] AS [DF]
    ...
    IF NOT EXISTS (SELECT TOP 1
        ...
        FROM @TABLE_RESULT)
    BEGIN
      INSERT INTO [acsa].[SWIFT_PROMO_DISCOUNT_BY_FAMILY] ([PROMO_ID]
      , [CODE_FAMILY_SKU]
      , [LOW_AMOUNT]
      , [HIGH_AMOUNT]
      , [DISCOUNT]
