...
-- Ejemplo de Ejecucion:				
				EXECUTE [acsa].[SWIFT_SP_GET_ROUTE_DATA_QUANTITY] @CODE_ROUTE = '001'
*/
-- =============================================
CREATE PROCEDURE [acsa].[SWIFT_SP_GET_ROUTE_DATA_QUANTITY]
...
BEGIN
	...
	--
	SELECT @PRICE_LIST = [acsa].[SWIFT_FN_GET_PRICE_LIST](DEFAULT)
	...
	INNER JOIN [acsa].[SWIFT_VIEW_USERS] AS U 	ON (T.[ASSIGEND_TO] = U.[LOGIN])
	...
	-- ------------------------------
	 ...
	 FROM [acsa].[SWIFT_FN_GET_CUSTUMER_FOR_ROUTE](@CODE_ROUTE) C
	...
	INNER JOIN [acsa].SWIFT_VIEW_ALL_SKU S on (S.CODE_SKU =  I.SKU)
	INNER JOIN [acsa].SWIFT_PRICE_LIST_BY_SKU P on (P.CODE_SKU = I.SKU)
	INNER JOIN [acsa].USERS U ON (U.DEFAULT_WAREHOUSE = i.WAREHOUSE)
	WHERE u.SELLER_ROUTE = @CODE_ROUTE
	...
	FROM [acsa].[SONDA_POS_RES_SAT] AS F
