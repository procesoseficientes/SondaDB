...
-- Description:    Inserta los Registros de Modificaciones de Clientes en la Base Intermedia de acsa
			
-- EJEMPLO DE EJECUCION: 
		EXEC [acsa].[SWIFT_SP_POST_CUSTOMER_CHANGE_TO_ERP]

=========================================================*/
CREATE PROCEDURE [acsa].[SWIFT_SP_POST_CUSTOMER_CHANGE_TO_ERP]
AS
BEGIN
	SET NOCOUNT OFF;
	...
	BEGIN
		...
		BEGIN TRY
			...
			IF (SELECT COUNT(*) FROM [acsa_SERVER].[acsa].[dbo].[REPLICA_CLIENTE] WHERE [CODIGO_SONDA]=@CODE_CUSTOMER)>0
			BEGIN
				UPDATE [acsa_SERVER].[acsa].[dbo].[REPLICA_CLIENTE]
			...
			EXEC [acsa].[SWIFT_SP_SET_STATUS_SEND_CUSTOMER_CHANGE_TO_ERP] @CUSTOMER = @CUSTOMER, -- int
