...
BEGIN
	...
	BEGIN
		...
		BEGIN TRY
			...
			INSERT INTO [acsa_SERVER].[acsa].[dbo].[REPLICA_PEDIDO_ENC]
			(
				[NUMERO_DE_PEDIDO] ,
				[FECHA_PEDIDO] ,
				[CODIGO_DE_CLIENTE] ,
			...
			FROM [acsa].[SONDA_SALES_ORDER_HEADER] [ssoh]
			INNER JOIN [acsa].USERS US ON [ssoh].POSTED_BY = US.LOGIN
			left join [acsa_SERVER].SONDA.dbo.vsVENDEDORES_PREVENTA vendedores on us.RELATED_SELLER=vendedores.codigo_vendedor
			WHERE [ssoh].[SALES_ORDER_ID] = @SALES_ORDER_ID

			INSERT INTO [acsa_SERVER].SONDA.dbo.REPLICA_PEDIDO_DET
			( 
				[NUMERO_DE_PEDIDO] ,
				PRODUCT0 ,
				CODIGO_UNIDAD_VTA ,
			...
			EXEC [acsa].[SWIFT_SP-STATUS-SEND_SO_TO_SAP] @SALES_ORDER_ID = @SALES_ORDER_ID, -- int
			    ...
			    @OWNER = 'acsa', -- varchar(125)
			    @CUSTOMER_OWNER = 'acsa' -- varchar(125)
		...
		BEGIN CATCH
			...
			DELETE FROM [acsa_SERVER].[acsa].[dbo].[REPLICA_PEDIDO_DET]
			WHERE [NUMERO_DE_PEDIDO] = @SALES_ORDER_ID

			DELETE FROM [acsa_SERVER].[acsa].[dbo].[REPLICA_PEDIDO_ENC]
			WHERE [NUMERO_DE_PEDIDO] = @SALES_ORDER_ID
