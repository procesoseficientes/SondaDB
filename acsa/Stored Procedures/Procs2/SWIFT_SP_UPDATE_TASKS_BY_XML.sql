...
-- Ejemplo de Ejecucion:
				EXEC [acsa].[SWIFT_SP_UPDATE_TASKS_BY_XML]
				@XML = '
...
*/
-- =============================================
CREATE PROCEDURE [acsa].[SWIFT_SP_UPDATE_TASKS_BY_XML] (@XML XML)
AS
BEGIN
  BEGIN TRY
    DECLARE @ID INT
	...
	--						ELSE PIH.GPS_URL
	--						END
	--					FROM [acsa].SONDA_POS_INVOICE_HEADER PIH
	--		LEFT JOIN [acsa].SWIFT_VIEW_ALL_COSTUMER VAC ON PIH.CLIENT_ID = VAC.CODE_CUSTOMER
    ...
    BEGIN
	  ...
	  SELECT @ALTERNATIVEGPS = (
		SELECT TOP 1 PIH.GPS_URL FROM [acsa].SONDA_POS_INVOICE_HEADER PIH 
			INNER JOIN @TASKS T 
	  ...
	  BEGIN
		SELECT @ALTERNATIVEGPS = (
				SELECT TOP 1 VAC.GPS FROM [acsa].SWIFT_VIEW_ALL_COSTUMER VAC INNER JOIN @TASKS T ON VAC.CODE_CUSTOMER = T.CUSTOMER_CODE WHERE VAC.CODE_CUSTOMER = T.CUSTOMER_CODE
		)
	  END
	  ---------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    BEGIN CATCH
      ...
      EXEC [acsa].[SONDA_SP_INSERT_SONDA_SERVER_ERROR_LOG] @CODE_ROUTE = NULL
                                                           ,@LOGIN = NULL
