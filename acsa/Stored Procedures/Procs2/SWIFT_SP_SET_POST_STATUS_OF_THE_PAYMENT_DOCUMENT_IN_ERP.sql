...
-- Ejemplo de Ejecucion:
				EXEC [acsa].[SWIFT_SP_SET_POST_STATUS_OF_THE_PAYMENT_DOCUMENT_IN_ERP]
					@PAYMENT_HEADER_ID = 20
					,@IS_POSTED_ERP = 1
					,@POSTING_RESPONSE = 'Proceso Exitoso'
					,@ERP_REFERENCE = '123456'
				-- 
				SELECT * FROM [acsa].[SONDA_OVERDUE_INVOICE_PAYMENT_HEADER]
*/
-- =============================================
CREATE PROCEDURE [acsa].[SWIFT_SP_SET_POST_STATUS_OF_THE_PAYMENT_DOCUMENT_IN_ERP]
	(
...
AS
	BEGIN
		BEGIN TRY
			...
			BEGIN
				UPDATE
					[acsa].[SONDA_OVERDUE_INVOICE_PAYMENT_HEADER]
				SET	
