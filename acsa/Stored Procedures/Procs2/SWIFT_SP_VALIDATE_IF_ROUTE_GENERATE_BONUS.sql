...
-- Ejemplo de Ejecucion:
			EXEC  [acsa].[SWIFT_SP_VALIDATE_IF_ROUTE_GENERATE_DISCOUNT] @CODE_ROUTE = '136'
*/
-- =============================================
CREATE PROCEDURE [acsa].[SWIFT_SP_VALIDATE_IF_ROUTE_GENERATE_BONUS] (@CODE_ROUTE VARCHAR(50), @GENERATE_BONUS INT OUTPUT)
AS
BEGIN
  SET NOCOUNT ON;
  --
  ...
  FROM [acsa].[SWIFT_TRADE_AGREEMENT_WATERMARK] [TAW]
  ...
  FROM [acsa].[SWIFT_PROMO_BONUS_BY_GENERAL_AMOUNT] [BBGA]
  ...
  FROM [acsa].[SWIFT_PROMO_BONUS_BY_SCALE] [BBS]
  ...
  FROM [acsa].[SWIFT_PROMO_BONUS_BY_MULTIPLE] [BBM]
  ...
  FROM [acsa].[SWIFT_PROMO_BY_COMBO_PROMO_RULE] [PCPR]
  ...
  FROM [acsa].[SWIFT_PROMO_SKU_BY_PROMO_RULE] [PSPR]
  ...
  FROM [acsa].[SWIFT_SKU_BY_COMBO] [SC]
  ...
  IF NOT EXISTS (SELECT
        WATER_MARK
      FROM [acsa].[SWIFT_TRADE_AGREEMENT_WATERMARK]
      WHERE [CODE_ROUTE] = @CODE_ROUTE)
  BEGIN

    INSERT INTO [acsa].[SWIFT_TRADE_AGREEMENT_WATERMARK] ([WATER_MARK], [CODE_ROUTE], [TIMES_REQUIRE], [LAST_REQUIRE], [MARKED_TABLE])
      VALUES (@LAST_WATER_MARK, @CODE_ROUTE, 1, GETDATE(), @MARKED_TABLE);

  END
  ELSE
  BEGIN
    UPDATE [acsa].[SWIFT_TRADE_AGREEMENT_WATERMARK]
