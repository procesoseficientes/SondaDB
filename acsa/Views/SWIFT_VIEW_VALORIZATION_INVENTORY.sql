...
CREATE VIEW [acsa].[SWIFT_VIEW_VALORIZATION_INVENTORY]
AS
--SELECT distinct
--	A.INVENTORY, 
...
--	LEFT OUTER JOIN
--    [acsa].SWIFT_VIEW_ALL_SKU AS C ON A.SKU = C.CODE_SKU
--    LEFT OUTER JOIN
--    [acsa].SWIFT_BATCHES AS D ON A.BATCH_ID = D.BATCH
--WHERE A.ON_HAND > 0
SELECT DISTINCT
...
FROM 
	[acsa].SWIFT_INVENTORY AS A
	LEFT OUTER JOIN    [acsa].SWIFT_VIEW_ALL_SKU AS C ON A.SKU = C.CODE_SKU
    LEFT OUTER JOIN    [acsa].SWIFT_BATCH AS D ON A.BATCH_ID = D.BATCH_ID
WHERE A.ON_HAND > 0
GROUP BY 
  A.SERIAL_NUMBER
, A.WAREHOUSE
